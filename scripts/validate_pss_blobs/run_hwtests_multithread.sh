...
mvToolsVersion="21.11.0-internal"
generateScript="/home/mpedus/git/dldt_workspace/pss_1_0/kmb/mtl-pss-1.0/src/vpux_translate_utils/src/hwtest/generate_hw_testcases.py"
vpuxTranslateBin="/home/mpedus/git/dldt_workspace/pss_1_0/dldt/master/bin/intel64/Release/vpux-translate"
inferenceManagerDemoFolder="/home/mpedus/git/firmware.vpu.iot/application/demo/InferenceManagerDemo"
moviSimBin="/home/mpedus/WORK/Tools/Releases/General/${mvToolsVersion}/linux64/bin/moviSim"
FPGAhostName="iirfpga011.ir.intel.com"
  ...
  if [ $infer = true ]; then
    ...
    if [ -e "${hwtests}/${config}/vpuip.blob" ]; then
      ...
      if [ $device = "movisim" ]; then
        ...
        sleep 5
        timeout 7m ${moviSimBin} -cv:3700xx -nodasm -q -tcpip:${port} > ${config_dir}/movisim.log &
        sleep 10
        movisim_pid=$!
        timeout 6m make CONFIG_FILE=.config_sim_3720xx run srvPort=${port} MV_TOOLS_VERSION=${mvToolsVersion} > ${config_dir}/imd.log 2>&1
