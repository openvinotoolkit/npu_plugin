...
SmallVector<int64_t> vpux::alignShape(ArrayRef<int64_t> shape, Optional<ArrayRef<int64_t>> alignment) {
    ...
    if (!alignment.hasValue())
        return alignedShape;
    std::transform(shape.begin(), shape.end(), alignment.getValue().begin(), alignedShape.begin(),
                   [](int64_t shapeVal, int64_t alignmentValue) {
                       return alignVal(shapeVal, alignmentValue);
                   });
    return alignedShape;
}
