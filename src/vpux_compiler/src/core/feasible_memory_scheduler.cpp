...
bool FeasibleMemoryScheduler::isComputeOpWithSomeActiveInputs(operationIdxType opIdx) {
    ...
    auto usedBuffs = getUsedBuffersByOperation(opIdx);
...
SmallVector<mlir::Value> FeasibleMemoryScheduler::getUsedBuffersByOperation(operationIdxType opIdx) {
    // retrieve all buffers used by the op
    auto op = _depsInfo.getExecuteOpAtIndex(opIdx);
...
bool FeasibleMemoryScheduler::isReadyComputeOperationSchedulable(operationIdxType opIdx) {
    ...
    auto usedBuffers = getUsedBuffersByOperation(opIdx);
