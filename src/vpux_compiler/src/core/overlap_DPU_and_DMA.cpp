...
SmallVector<OverlapDMAandDPU::DataOpOptimalCycles> OverlapDMAandDPU::FreeInterval::frontCandidatesOfSize(size_t size) {
    ...
    auto front = prefetchOps_.begin();
    while (front != prefetchOps_.end()) {
        auto currLevel = front->level_;
        if (candidateSize < size) {
            lastLevel = currLevel;
        } else if (candidateSize >= size && lastLevel != currLevel) {
            // add all candidates from the same level
            break;
        }
    ...
    // candidates on previous levels which are not sorted to preserve order
    SmallVector<DataOpOptimalCycles> concreteCandidates;
    // candidates on last level which can be choosen selectively based on best-fit
    SmallVector<DataOpOptimalCycles*> sortCandidates;
    candidateSize = 0;
    for (auto candidate : candidates) {
        // sort only last level
        if (candidate->level_ < lastLevel) {
            concreteCandidates.push_back(*candidate);
            candidateSize += candidate->cycleCost();
        } else {
            sortCandidates.push_back(candidate);
        }
    }

    size_t targetSizeForSort = size - candidateSize;
    // select best fit from sortCandidates to exceed targetSizeForSort

    // sort based on cycle size
    llvm::sort(sortCandidates.begin(), sortCandidates.end(),
               [](const DataOpOptimalCycles* val1, const DataOpOptimalCycles* val2) {
                   return (val1->cycleEnd_ - val1->cycleBegin_) < (val2->cycleEnd_ - val2->cycleBegin_);
               });

    size_t currentSize = 0;
    mlir::DenseSet<size_t> selected;
    auto sortFront = sortCandidates.begin();
    auto sortEnd = sortCandidates.begin();

    // add from front
    while (currentSize <= targetSizeForSort && sortFront != sortCandidates.end()) {
        auto candidate = *sortFront;
        currentSize += candidate->cycleCost();
        selected.insert(candidate->opIdx_);
        ++sortFront;
    }
    // remove from front
    while (currentSize > targetSizeForSort && sortEnd != sortCandidates.end()) {
        auto candidate = *sortEnd;
        auto candidateGap = currentSize - candidate->cycleCost();
        if (candidateGap >= targetSizeForSort) {
            currentSize = candidateGap;
            selected.erase(candidate->opIdx_);
        }
        ++sortEnd;
    }

    for (auto sortCandidate : sortCandidates) {
        if (selected.find(sortCandidate->opIdx_) != selected.end()) {
            concreteCandidates.push_back(*sortCandidate);
        }
    }

    size_t sizeBefore = prefetchOps_.size();
    for (auto concreteCandidate : concreteCandidates) {
        llvm::erase_value(prefetchOps_, concreteCandidate);
    }
    VPUX_THROW_UNLESS(sizeBefore == prefetchOps_.size() + concreteCandidates.size(),
                      "Candidates not removed correctly");
...
void OverlapDMAandDPU::ActivationSpill::overlapActivationCopyIn(CycleInterval activationCopyInCycles,
    ...
    if (thisIntervalActivationSpill) {
        ...
        // find the read
        while (first->opIdx_ != activationCopyInCycles.opIdx_ && first != reads_.end()) {
            ++first;
        }
...
void OverlapDMAandDPU::PrefetchPipeline::populatePrefetchEdges(scheduleWithPrefetch& prefetchSchedule) {
    ...
    for (auto& prefetchInterval : prefetchIntervals_) {
        ...
        for (auto& DPU : DPUCycles) {
            ...
            bool activationSpill = DPU.opIdx_ == DPUCycles.back().opIdx_;
...
bool OverlapDMAandDPU::PrefetchPipeline::shiftToPreviousInterval(PrefetchInterval* currentInterval,
    ...
    // try to find interval with enough total size to store the DMA
    while (newTargetInterval->freeIntervalTotalSize(dataOp.cycleEnd_) < dataOp.cycleCost() &&
           newTargetInterval != prefetchIntervals_.begin()) {
        --newTargetInterval;
    }
    ...
    if (newTargetInterval->canOpFitInInterval(dataOp)) {
        ...
        if (newTargetInterval->canOpFitInInterval(dataOp)) {
            ...
            // stall next DPU start cycle
            _log.nest(5).trace("not enough free cycles after shift for DMA {0}", dataOp.opIdx_);
            return false;
        }
    }
...
void OverlapDMAandDPU::generatePrefetchEdgesFromOverlap(scheduleWithPrefetch& prefetchSchedule) {
    ...
    if (op->executor == VPU::ExecutorKind::SHAVE_UPA) {
        ++op;  // skip input UPA op
    }
    ...
    // STEP 2: create optimal DPU pipeline
    ...
    _log.trace("Finding Optimal DPU pipeline");

    // create a schedule of only DPU and activation copy outs
    // find free DMA intervals
