...
ExecutorStallCycles vpux::getExecutorStallRegions(SmallVector<ScheduledOpInfo>& scheduledOps) {
    ExecutorStallCycles executorStalls;
    DenseMap<VPU::ExecutorKind, size_t> executorCycles;

    for (auto& schedOp : scheduledOps) {
        if (schedOp.cycleBegin_ > executorCycles[schedOp.executor]) {
            // stall on executor from
            auto executorStall = std::make_pair(executorCycles[schedOp.executor], schedOp.cycleBegin_);
            executorStalls[schedOp.executor].push_back(executorStall);
        }
        executorCycles[schedOp.executor] = schedOp.cycleEnd_;
    }

    return executorStalls;
}

StallCycles vpux::getStallsOnAllExecutorPipelines(SmallVector<ScheduledOpInfo>& scheduledOps) {
    ...
    auto minStallSizeExecutor =
            std::min_element(executorStalls.begin(), executorStalls.end(),
                             [](const llvm::detail::DenseMapPair<VPU::ExecutorKind, StallCycles>& stall1,
                                const llvm::detail::DenseMapPair<VPU::ExecutorKind, StallCycles>& stall2) {
...
void vpux::verifyDependenciesPreservedInCycles(AsyncDepsInfo& depsInfo, SmallVector<ScheduledOpInfo>& scheduledOps) {
    for (auto& schedOp : scheduledOps) {
...
void vpux::printScheduleStatistics(mlir::FuncOp& netFunc, AsyncDepsInfo& depsInfo, Logger log,
    ...
    log.trace("Total Possible Improvement = {0} %", (100.0 * totalPossibleImprovement / (pipelineEndCycle != 0 ? pipelineEndCycle : 1)));
...
void vpux::createTracingJSON(mlir::FuncOp& netFunc, StringRef fileName) {
    ...
    out_stream << "{\"traceEvents\":[" << 0 << std::endl;
