...
void CycleBasedBarrierScheduler::optimizeDependency() {
    ...
    SmallVector<VPURT::TaskOp> allTasksSortedByCycleStart;
    for (auto taskList : _orderedTasksByCycleStart) {
        allTasksSortedByCycleStart.insert(allTasksSortedByCycleStart.end(), taskList.second.begin(),
                                          taskList.second.end());
    }
...
void CycleBasedBarrierScheduler::updateDependency() {
    ...
    for (size_t ind = 0; ind < _configureBarrierOpWaitTask.size(); ind++) {
        ...
        size_t firstStart = *(_orderedTimeStamp.rbegin());
...
void CycleBasedBarrierScheduler::updateCycleStartTimeDueToNativeExecutorDependency(
    ...
    if (itr != orderedTasksByCycleStart.end()) {
        ...
        if (nextTaskStart < newStartCycle) {
            _log.trace("Update start cycle from {0} to {1} for consumer task {2}", nextTaskStart, newStartCycle,
                       nextTaskID);
            updateCycleStartTime(_orderedTasks[nextTaskID], newStartCycle);
        }
    }
}
...
void CycleBasedBarrierScheduler::updateCycleStartTime(vpux::VPURT::TaskOp taskOp, size_t newStartCycle) {
    auto taskId = getUniqueID(taskOp.getOperation());
    auto previousStartCycle = _operationStartCycle[taskId];
    VPUX_THROW_UNLESS(newStartCycle > previousStartCycle, "newStartCycle {0} is smaller than oldStartCycle {1}");
    ...
    _orderedTimeStamp.insert(newEndCycle);
    ...
    for (auto consumer : _taskConsumerMapOriginal[taskOp.getOperation()]) {
        ...
        if (consumerStart < newEndCycle) {
            _log.trace("Update start cycle from {0} to {1} for consumer task {2}", consumerStart, newEndCycle,
                       consumerId);
            updateCycleStartTime(_orderedTasks[consumerId], newEndCycle);
        }
    }

    // update cycle by implicit dependency between tasks of same executor
    auto executorKind = taskOp.getExecutorKind();
    updateCycleStartTimeDueToNativeExecutorDependency(taskOp, newEndCycle, _orderedTasksByCycleStart[executorKind]);
}
...
bool CycleBasedBarrierScheduler::performCycleBasedSchedulingTaskLoop() {
    ...
    _currentTime = 0;
    size_t indNce, indDma, indUpa, indAct, indM2i;
    indNce = indDma = indUpa = indAct = indM2i = 0;
    auto nextTimeStamp = _orderedTimeStamp.begin();
    // keeps looping until the index for each executor reaches the end, thus all tasks have been scheduled then
    while (indNce < _orderedTasksByCycleStart[VPU::ExecutorKind::NCE].size() ||
           indDma < _orderedTasksByCycleStart[VPU::ExecutorKind::DMA_NN].size() ||
           indUpa < _orderedTasksByCycleStart[VPU::ExecutorKind::SHAVE_UPA].size() ||
           indAct < _orderedTasksByCycleStart[VPU::ExecutorKind::SHAVE_ACT].size() ||
           indM2i < _orderedTasksByCycleStart[VPU::ExecutorKind::M2I].size()) {
        _log.trace("Current time is {0}", _currentTime);
        _schedulingTasksInEachLoop.clear();
        // NCE task
        if (indNce < _orderedTasksByCycleStart[VPU::ExecutorKind::NCE].size()) {
            vpux::VPURT::TaskOp taskOp = _orderedTasksByCycleStart[VPU::ExecutorKind::NCE][indNce];
        ...
        // DMA task
        if (indDma < _orderedTasksByCycleStart[VPU::ExecutorKind::DMA_NN].size()) {
            vpux::VPURT::TaskOp taskOp = _orderedTasksByCycleStart[VPU::ExecutorKind::DMA_NN][indDma];
            if (attemptToScheduleTaskWithAvailableBarrier(taskOp)) {
                indDma++;
            }
        }
        ...
        // SHAVE UPA task
        if (indUpa < _orderedTasksByCycleStart[VPU::ExecutorKind::SHAVE_UPA].size()) {
            vpux::VPURT::TaskOp taskOp = _orderedTasksByCycleStart[VPU::ExecutorKind::SHAVE_UPA][indUpa];
            if (attemptToScheduleTaskWithAvailableBarrier(taskOp)) {
                indUpa++;
            }
        }
        ...
        // SHAVE ACT task
        if (indAct < _orderedTasksByCycleStart[VPU::ExecutorKind::SHAVE_ACT].size()) {
            vpux::VPURT::TaskOp taskOp = _orderedTasksByCycleStart[VPU::ExecutorKind::SHAVE_ACT][indAct];
            if (attemptToScheduleTaskWithAvailableBarrier(taskOp)) {
                indAct++;
            }
        }
        ...
        // M2I task
        if (indM2i < _orderedTasksByCycleStart[VPU::ExecutorKind::M2I].size()) {
            vpux::VPURT::TaskOp taskOp = _orderedTasksByCycleStart[VPU::ExecutorKind::M2I][indM2i];
            if (attemptToScheduleTaskWithAvailableBarrier(taskOp)) {
                indM2i++;
            }
        }
        ...
        if (nextTimeStamp != _orderedTimeStamp.end()) {
            // move up the schedule time to the next time stamp
            _currentTime = *nextTimeStamp;
            nextTimeStamp++;
...
size_t CycleBasedBarrierScheduler::countProducerTasksToBarrier(VPURT::TaskOp op) {
    ...
    if (op.getExecutorKind() == VPU::ExecutorKind::DMA_NN || op.getExecutorKind() == VPU::ExecutorKind::SHAVE_UPA ||
        op.getExecutorKind() == VPU::ExecutorKind::SHAVE_ACT || op.getExecutorKind() == VPU::ExecutorKind::M2I) {
        return 1;
    }
...
void CycleBasedBarrierScheduler::removeRedundantBarriers(bool optimizeIRDependency) {
    if (_enableDMAOptimization) {
        ...
        // 1) if a barrier only has DMAs as its producer, remove all DMAs from its consumers
        // 2) if a barrier only has DMAs as its consumer, remove all DMAs from its producers
        for (size_t ind = 0; ind < _configureBarrierOpWaitTask.size(); ind++) {
            auto& producers = _configureBarrierOpWaitTask[ind];
            auto& consumers = _configureBarrierOpUpdateTask[ind];
...
bool CycleBasedBarrierScheduler::doesPathExist(int64_t a, int64_t b, bool checkConsumer) {
    auto findPath = [&](int64_t task1, int64_t task2) {
        ...
        if (checkConsumer && _orderedTasks[task1].getExecutorKind() == _orderedTasks[task2].getExecutorKind() &&
        ...
        if (_enableDMAOptimization && _orderedTasks[task1].getExecutorKind() == VPU::ExecutorKind::DMA_NN) {
            auto itr = std::find(_orderedTasksByCycleStart[VPU::ExecutorKind::DMA_NN].begin(),
                                 _orderedTasksByCycleStart[VPU::ExecutorKind::DMA_NN].end(), _orderedTasks[task1]);
            VPUX_THROW_UNLESS(itr != _orderedTasksByCycleStart[VPU::ExecutorKind::DMA_NN].end(),
                              "task {0} is not found in list", task1);
            itr++;
            if (itr != _orderedTasksByCycleStart[VPU::ExecutorKind::DMA_NN].end()) {
                auto nextTaskID = getUniqueID((*itr).getOperation());
...
void CycleBasedBarrierScheduler::getPerTaskStartAndEndCycleTime() {
    ...
    _func.walk([&](VPURT::TaskOp taskOp) {
        ...
        _orderedTasksByCycleStart[taskOp.getExecutorKind()].push_back(taskOp);
        ...
        if (taskOp->hasAttr(cycleEnd)) {
            ...
            _orderedTimeStamp.insert(_operationEndCycle[opId]);
        } else {
