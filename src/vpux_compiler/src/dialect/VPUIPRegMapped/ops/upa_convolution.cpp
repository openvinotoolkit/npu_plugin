...
mlir::LogicalResult vpux::VPUIPRegMapped::verifyOp(ConvolutionUPAOp op) {
...
void vpux::VPUIPRegMapped::ConvolutionUPAOp::build(mlir::OpBuilder& builder, mlir::OperationState& state,
                                                   mlir::Value input, mlir::Value filter, mlir::Value bias,
                                                   mlir::Value output, mlir::ArrayAttr strides,
...
void vpux::VPUIPRegMapped::ConvolutionUPAOp::inferLayoutInfo(mlir::Operation* origOp, IE::LayerLayoutInfo& info) {
...
// vpux::VPUIPRegMapped::ConvolutionUPAOp::serialize(VPUIPRegMapped::BlobWriter& writer) {
void vpux::VPUIPRegMapped::ConvolutionUPAOp::serialize(std::vector<char>& buffer) {
    /*
    ...
    const auto dilations = VPUIPRegMapped::BlobWriter::createOrder3(this->dilations());
...
/*
mlir::Operation* vpux::VPUIPRegMapped::BlobReader::parseConvolution(mlir::OpBuilder& builder,
    ...
    if (const auto* params = task->softLayerParams_as_ConvolutionParams()) {
        ...
        return builder.create<VPUIPRegMapped::ConvolutionUPAOp>(
                mlir::UnknownLoc::get(_ctx), inputs[0], inputs[1], inputs.size() == 3 ? inputs[2] : nullptr, outputs[0],
                strides, dilations, padsBegin, padsEnd, params->group());
