...
void vpux::VPUIPRegMapped::RegionYoloUPAOp::build(mlir::OpBuilder& builder, mlir::OperationState& state,
                                                  mlir::Value input, mlir::Value output, mlir::IntegerAttr coords,
                                                  mlir::IntegerAttr classes, mlir::IntegerAttr regions,
                                                  mlir::BoolAttr do_softmax, mlir::ArrayAttr mask) {
    build(builder, state, input, output, coords, classes, regions, do_softmax, mask, mlir::ValueRange{},
          mlir::ValueRange{}, nullptr, nullptr);
}

// VPUIPRegMapped::BlobWriter::SpecificTask vpux::VPUIPRegMapped::RegionYoloUPAOp::serialize(VPUIPRegMapped::BlobWriter&
// writer) {
void vpux::VPUIPRegMapped::RegionYoloUPAOp::serialize(std::vector<char>& buffer) {
    /*
    VPUIPRegMapped::BlobWriter::Vector<int32_t> serializedMask;
    if (mask().hasValue()) {
...
void vpux::VPUIPRegMapped::RegionYoloUPAOp::inferLayoutInfo(mlir::Operation* origOp, IE::LayerLayoutInfo& info) {
    ...
    if (regionYoloOp.do_softmax()) {
        ...
        IERT::inferLayoutInfoSameInOutSpecificDimsOrder(
                info, {DimsOrder::NCHW, DimsOrder::NHWC});  // 2021_10_14: after source repo rebase
    }
}
