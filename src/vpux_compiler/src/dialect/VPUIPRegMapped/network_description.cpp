...
/*
// Alex: Note: We have in schema/memoryManagement_generated.h types such as: DType_FP16 = 3
...
vpux::VPUIPRegMapped::NetworkDescription::NetworkDescription(std::vector<char> blob)
    ...
    VPUX_THROW_UNLESS(MVCNN::VerifyGraphFileBuffer(verifier), "Got invalid VPUIPRegMapped blob");
    ...
    VPUX_THROW_UNLESS(!_networkOutputs.empty(), "VPUIPRegMapped blob does not contain network outputs");
    VPUX_THROW_UNLESS(!_deviceOutputs.empty(), "VPUIPRegMapped blob does not contain device outputs");
    */
}
