...
void deserializePreprocessInfo(vpux::ELF::PreprocessingInfo* preprocessInfo, uint32_t count,
...
std::vector<OVRawNode> deserializeOVNodes(vpux::ELF::OVNode* OVNode, uint32_t count, const bool isResult) {
    // Check for the existence of a field in a blob. In older versions of the blob, this field may not exist
    if (count == 0) {
        return {};
    }
...
vpux::VPUIPRegMapped::NetworkDescription::NetworkDescription(std::vector<char> blob)
    ...
    elf::Reader<elf::ELF_Bitness::Elf64> reader(binaryNetworkPtr, 0);

    vpux::ELF::NetworkMetadata* metadata = nullptr;
    ...
    for (size_t secIndex = 0; secIndex < reader.getSectionsNum(); secIndex++) {
        ...
        if (secHeader->sh_type == static_cast<elf::Elf_Word>(vpux::ELF::SectionTypeAttr::VPU_SHT_NETDESC)) {
            metadata = const_cast<vpux::ELF::NetworkMetadata*>(section.getData<vpux::ELF::NetworkMetadata>());
            break;
        }
    }
