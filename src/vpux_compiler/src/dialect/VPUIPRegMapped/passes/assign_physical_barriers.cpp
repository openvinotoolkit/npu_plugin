...
BarrierAllocation::BarrierAllocation(mlir::Operation* op, int64_t numBarriers, Logger log) {
    ...
    const auto callback = [&](VPUIPRegMapped::DeclareVirtualBarrierOp virtOp) {
...
class VirtualBarrierRewrite final : public mlir::OpRewritePattern<VPUIPRegMapped::DeclareVirtualBarrierOp> {
public:
    VirtualBarrierRewrite(mlir::MLIRContext* ctx, const BarrierAllocation& allocInfo, Logger log)
            : mlir::OpRewritePattern<VPUIPRegMapped::DeclareVirtualBarrierOp>(ctx), _allocInfo(allocInfo), _log(log) {
    }

public:
    mlir::LogicalResult matchAndRewrite(VPUIPRegMapped::DeclareVirtualBarrierOp origOp,
...
mlir::LogicalResult VirtualBarrierRewrite::matchAndRewrite(VPUIPRegMapped::DeclareVirtualBarrierOp origOp,
    ...
    rewriter.replaceOpWithNewOp<VPUIPRegMapped::ConfigureBarrierOp>(origOp, barrierID);
    return mlir::success();
}
...
class AssignPhysicalBarriersPass final : public VPUIPRegMapped::AssignPhysicalBarriersBase<AssignPhysicalBarriersPass> {
public:
...
void AssignPhysicalBarriersPass::safeRunOnFunc() {
    ...
    const auto nceAttr =
            VPUIPRegMapped::PhysicalProcessorAttr::get(&ctx, VPUIPRegMapped::PhysicalProcessor::NCE_Cluster);
    ...
    target.addIllegalOp<VPUIPRegMapped::DeclareVirtualBarrierOp>();
...
std::unique_ptr<mlir::Pass> vpux::VPUIPRegMapped::createAssignPhysicalBarriersPass(Logger log) {
