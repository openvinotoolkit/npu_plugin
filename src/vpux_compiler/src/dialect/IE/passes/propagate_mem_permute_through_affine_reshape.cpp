...
bool splitAxesAreModifiedIntegratedly(const SmallVector<SmallVector<int64_t>>& dimMapping, ArrayRef<int64_t> permAxis,
    ...
    for (auto splitAxes : splitAxesVec) {
        ...
        // 2.Split axes have single non-trivial memDim, it's allowed to break split axes.
...
mlir::AffineMap calculateNewPermutation(SmallVector<SmallVector<int64_t>> dimMapping, ArrayRef<int64_t> origPermVec,
    ...
    // Delete duplicated axes
    // e.g. Convert[0, 3, 3, 1, 2] to [0, 3, 1, 2]
    SmallVector<unsigned> newPermVec;
    unsigned dupValue = 0;

    for (size_t idx = 0; idx < inAxesVec.size(); idx++) {
        // Regardless of the value of the first element in inAxesVec, it cannot be a duplicate element and should
        // always be pushed into newPermVec.
        if (idx == 0 || inAxesVec[idx] != dupValue) {
            ...
            dupValue = checked_cast<unsigned>(inAxesVec[idx]);
        }
    }
...
//
// PropagateMemPermuteThroughAffineReshapePass
//

class PropagateMemPermuteThroughAffineReshapePass final :
        public IE::PropagateMemPermuteThroughAffineReshapeBase<PropagateMemPermuteThroughAffineReshapePass> {
public:
    explicit PropagateMemPermuteThroughAffineReshapePass(Logger log): _log(log) {
...
void PropagateMemPermuteThroughAffineReshapePass::safeRunOnFunc() {
    auto& ctx = getContext();
...
std::unique_ptr<mlir::Pass> vpux::IE::createPropagateMemPermuteThroughAffineReshapePass(Logger log) {
    return std::make_unique<PropagateMemPermuteThroughAffineReshapePass>(log);
}
