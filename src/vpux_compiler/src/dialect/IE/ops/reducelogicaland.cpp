...
mlir::LogicalResult vpux::IE::ReduceLogicalAndOp::inferReturnTypeComponents(
    ...
    IE::xSumOpAdaptor reduceLogicalAnd(operands, attrs);
    if (mlir::failed(reduceLogicalAnd.verify(loc))) {
        return mlir::failure();
    }

    const auto input = reduceLogicalAnd.input();
    const auto keepDims = reduceLogicalAnd.keep_dims() != nullptr;
    auto axes = IE::constInputToData(loc, reduceLogicalAnd.axes()).getValue();
