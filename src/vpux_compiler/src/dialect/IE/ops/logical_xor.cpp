...
mlir::LogicalResult vpux::IE::LogicalXorOp::inferReturnTypeComponents(
    ...
    IE::LogicalXorOpAdaptor logicalxor(operands, attrs);
    if (mlir::failed(logicalxor.verify(loc))) {
        return mlir::failure();
    }

    const auto in1Type = logicalxor.input1().getType().cast<mlir::ShapedType>();
    const auto in2Type = logicalxor.input2().getType().cast<mlir::ShapedType>();

    const auto outShapeRes = IE::broadcastEltwiseShape(in1Type.getShape(), in2Type.getShape(),
                                                       logicalxor.auto_broadcast().getValue(), loc);
...
EMU::BlobWriter::SpecificTask vpux::IE::LogicalXorOp::serialize(EMU::BlobWriter& writer) {
