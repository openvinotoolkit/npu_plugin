...
class FtNormalizeAttributes final : public mlir::OpRewritePattern<IE::DFTOp> {
public:
    using mlir::OpRewritePattern<IE::DFTOp>::OpRewritePattern;

public:
    mlir::LogicalResult matchAndRewrite(IE::DFTOp op, mlir::PatternRewriter& rewriter) const final;
};

mlir::LogicalResult FtNormalizeAttributes::matchAndRewrite(IE::DFTOp op, mlir::PatternRewriter& rewriter) const {
    if (!op.input()) {
        return mlir::failure();
    }
    SmallVector<int64_t> axes = dftOpsNormalizeAxes(op.input(), parseIntArrayAttr<int64_t>(op.axes_attr()));
    ...
    rewriter.replaceOpWithNewOp<IE::DFTOp>(op, op.input(), axesAttr, op.signal_size_attr());
