...
mlir::LogicalResult vpux::IE::SelectOp::inferReturnTypeComponents(
    ...
    //     //IE::broadcastEltwiseShape(in2Type.getShape(), in3Type.getShape(), select.auto_broadcast().getValue(), loc);
    //     //inferredReturnShapes.emplace_back(in1Type.getShape(), in1Type.getElementType());

    //auto boolType = mlir::BoolAttr::get(ctx, select.input1());

    // if (mlir::succeeded(outShapeRes)) {
    //     inferredReturnShapes.emplace_back(outShapeRes.getValue(), in2Type.getElementType());
    //     //inferredReturnShapes.emplace_back(outShapeRes.getValue(), boolType );
    // }
