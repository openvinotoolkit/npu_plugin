...
std::unique_ptr<ngraph::Node> vpux::IE::SquaredDifferenceOp::toNgraph(ngraph::OutputVector &outputs)
{
    const ngraph::op::AutoBroadcastType autoBroadCastType = exportBroadcastType(auto_broadcast());

    return std::make_unique<opset_latest::SquaredDifference>(outputs.at(0), outputs.at(1),
