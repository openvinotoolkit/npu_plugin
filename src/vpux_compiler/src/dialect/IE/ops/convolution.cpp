...
SmallVector<int64_t> vpux::IE::ConvolutionOp::getMaxNumTiles() {
...
void vpux::IE::ConvolutionOp::adjustAttrs(ArrayRef<TileInfo> inputTiles, mlir::OpBuilder& builder) {
...
SmallVector<int64_t> vpux::IE::GroupConvolutionOp::getMaxNumTiles() {
    return vpux::IE::getMaxNumTiles(getOperation());
}
...
void vpux::IE::GroupConvolutionOp::adjustAttrs(ArrayRef<TileInfo> inputTiles, mlir::OpBuilder& builder) {
    ...
    const auto groupsNewAttr = getIntAttr(builder, groups);

    auto* op = getOperation();
    op->setAttr(groupsAttrName(), groupsNewAttr);
}
