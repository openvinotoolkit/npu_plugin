...
void SqueezeBiasShapePass::safeRunOnFunc() {
    ...
    patterns.insert<BiasShapeConverter<VPU::ConvolutionOp>>(&ctx, _log);
    patterns.insert<BiasShapeConverter<VPU::GroupConvolutionOp>>(&ctx, _log);
    patterns.insert<BiasShapeConverter<VPU::FullyConnectedOp>>(&ctx, _log);
