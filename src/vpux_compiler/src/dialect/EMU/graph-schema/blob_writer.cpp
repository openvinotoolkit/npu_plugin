...
EMU::BlobWriter::Task EMU::BlobWriter::createTask(mlir::Operation* op) {
...
EMU::BlobWriter::SpecificTask EMU::BlobWriter::createUPALayerTask(mlir::Operation* op,
...
EMU::BlobWriter::TensorReference EMU::BlobWriter::createTensor(StringRef name, NDTypeInterface type,
...
EMU::BlobWriter::TensorReference EMU::BlobWriter::replaceTensor(mlir::Value val, StringRef name, NDTypeInterface type,
                                                                ArrayRef<int64_t> mult, ArrayRef<int64_t> shift,
                                                                int64_t postShift, ArrayRef<uint8_t> zeroPoints,
                                                                VPURT::BufferSection locale,
                                                                const uint32_t localeIndex) {
    VPUX_THROW_UNLESS(_tensors.count(val) == 1, "Value {0} was not serialized", val);

    const auto off = createTensor(name, type, mult, shift, postShift, zeroPoints, locale, localeIndex);
...
EMU::BlobWriter::TensorReference EMU::BlobWriter::createTensor(mlir::Value val, StringRef name,
                                                               VPURT::BufferSection locale,
                                                               const uint32_t localeIndex) {
...
EMU::BlobWriter::TensorReference EMU::BlobWriter::getTensor(mlir::Value val) const {
    const auto it = _tensors.find(val);
    VPUX_THROW_UNLESS(it != _tensors.end(), "Value {0} wasn't serialized yet", val);
    return it->second;
}

EMU::BlobWriter::Vector<uint32_t> EMU::BlobWriter::createDims(ShapeRef shape) {
    return createVector(shape | transformed([](int64_t val) {
                            return checked_cast<uint32_t>(val);
                        }));
}

EMU::BlobWriter::Vector<uint32_t> EMU::BlobWriter::createDims(NDTypeInterface type) {
    return createDims(type.getShape());
}

EMU::BlobWriter::Vector<float> EMU::BlobWriter::createStrides(StridesRef strides, Bit elemSize) {
    Strides temp;
...
EMU::BlobWriter::Vector<float> EMU::BlobWriter::createStrides(NDTypeInterface type) {
...
EMU::BlobWriter::BinaryData EMU::BlobWriter::createBinaryData(ArrayRef<uint64_t> content, mlir::ShapedType type) {
