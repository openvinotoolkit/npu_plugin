...
private:
    ...
    void assignCyclesToExecOps(AsyncDepsInfo& depsInfo,
...
void FeasibleAllocationPass::assignCyclesToExecOps(
        AsyncDepsInfo& depsInfo, SmallVector<FeasibleMemoryScheduler::ScheduledOpInfo>& scheduledOps) {
...
void FeasibleAllocationPass::safeRunOnModule() {
    ...
    if (!prefetchSchedule.empty()) {
        FeasibleMemoryScheduler schedulerWithPrefetch(_memKind, prefetchLiveRangeInfo, depsInfo, aliasesInfo, _log,
                                                      prefetchScan, arch, costModel, nceClusterCount);
    ...
    // Execution is linear - based on cycles so dependencies for all executors can be inserted
