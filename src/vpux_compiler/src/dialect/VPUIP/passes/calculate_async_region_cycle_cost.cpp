        ...
        if (asyncExec->hasAttr(VPUIP::VPUIPDialect::getExecutorAttrName())) {
            ...
            if (executor.getLeafNameAttr() ==
                ...
                cycleCost = calculateUPACycleCost(asyncExec);
                       ...
                       VPU::ExecutorKindAttr::get(asyncExec->getContext(), VPU::ExecutorKind::SHAVE_ACT)) {
                // calculate UPA cycles
                cycleCost = calculateUPACycleCost(asyncExec);
            } else if (executor.getLeafNameAttr() ==
                       VPU::ExecutorKindAttr::get(asyncExec->getContext(), VPU::ExecutorKind::SHAVE_NN)) {
                // calculate UPA cycles
                cycleCost = calculateUPACycleCost(asyncExec);
            } else if (executor.getLeafNameAttr() ==
                       VPU::ExecutorKindAttr::get(asyncExec->getContext(), VPU::ExecutorKind::DMA_NN)) {
                // calculate DMA cycles
                cycleCost = calculateDMACycleCost(asyncExec, arch, costModel);
            } else if (executor.getLeafNameAttr() ==
                       VPU::ExecutorKindAttr::get(asyncExec->getContext(), VPU::ExecutorKind::NCE)) {
                // DPU have cost calculated during workload generation, retrieve cost
                cycleCost = retrieveDPUCycleCost(asyncExec);
            } else if (executor.getLeafNameAttr() ==
                       VPU::ExecutorKindAttr::get(asyncExec->getContext(), VPU::ExecutorKind::DPU)) {
                // DPU have cost calculated during workload generation, retrieve cost
                cycleCost = retrieveDPUCycleCost(asyncExec);
            }
        }
