...
void DPUProfilingPass::safeRunOnModule() {
    ...
    clusterSchedulers[1] = std::unique_ptr<BaseClusterBufferScheduler>(
            new SingleClusterScheduler(profilingWorkloadSize, builder, ctx, memKind, netFunc));
    ...
    netFunc.walk([&](VPUIP::NCEClusterTaskOp nceClusterTaskOp) {
        ...
        if (clusterSchedulers.count(numClusters) == 0) {
            clusterSchedulers[numClusters] = std::unique_ptr<BaseClusterBufferScheduler>(
                    new MultiClusterScheduler(numClusters, profilingWorkloadSize, builder, ctx, memKind, netFunc));
        }
    ...
    auto concatview = builder.create<VPUIP::ConcatViewOp>(
            mlir::NameLoc::get(mlir::Identifier::get("dpuDDRProfiling", ctx)), concatResults, profilingResult);
