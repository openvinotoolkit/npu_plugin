...
unsigned int vpux::VPUIP::swizzle_addr(unsigned int addr, unsigned char key) {
...
template <class T>
void vpux::VPUIP::swizzled_img(T* input_array, T* output_array, const unsigned char key, const unsigned int size) {
    for (unsigned int addr = 0; addr < size; addr++) {
        output_array[vpux::VPUIP::swizzle_addr(addr, key)] = input_array[addr];
    }
}

template <class T>
int vpux::VPUIP::swizzled_size(T*, const unsigned char key, const unsigned int size) {
    unsigned int max_size = 0;
    for (unsigned int addr = 0; addr < size; addr++) {
        max_size = std::max(max_size, vpux::VPUIP::swizzle_addr(addr, key));
    }
    // Align to 64 bytes
