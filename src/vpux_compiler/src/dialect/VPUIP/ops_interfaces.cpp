...
ptrdiff_t getLastMemRefPosition(mlir::ValueRange vals) {
    return std::find_if(
                   ...
                   [](mlir::Value val) {
                       return !val.getType()
                                       .isa<mlir::MemRefType, VPUIP::SparseBufferType, VPUIP::DistributedBufferType>();
                   }) -
           vals.begin();
}
...
void vpux::VPUIP::inferLayoutInfoSameInOutDimsOrder(IE::LayerLayoutInfo& info) {
...
void vpux::VPUIP::inferLayoutInfoSameInOutSpecificDimsOrder(IE::LayerLayoutInfo& info,
                                                            ArrayRef<DimsOrder> supportedLayouts) {
    const auto filter = [](size_t ind) {
        return ind != 0;
    };
