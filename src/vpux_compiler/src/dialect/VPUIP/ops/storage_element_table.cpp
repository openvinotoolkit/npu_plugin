...
mlir::LogicalResult vpux::VPUIP::StorageElementTableOp::inferReturnTypes(
    ...
    const auto outType = getMemRefType(ShapeRef(shape), getInt32Type(ctx), DimsOrder::NCHW, /*memSpace=*/nullptr);
    inferredReturnTypes.push_back(outType);

    return mlir::success();
}

mlir::LogicalResult vpux::VPUIP::verifyOp(vpux::VPUIP::StorageElementTableOp setOp) {
    using namespace VPU::NCESparsity;
    if (!setOp.base_ptrs().hasValue()) {
        return mlir::success();
    }

    const auto basePtrs = parseIntArrayAttr<int32_t>(setOp.base_ptrs().getValue());
