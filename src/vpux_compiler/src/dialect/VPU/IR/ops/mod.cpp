...
mlir::LogicalResult vpux::VPU::ModOp::inferReturnTypes(mlir::MLIRContext* ctx, std::optional<mlir::Location> optLoc,
    ...
    VPU::ModOpAdaptor mod(operands, attrs);
    if (mlir::failed(mod.verify(loc))) {
        return mlir::failure();
    }

    const auto in1Type = mod.getInput1().getType().cast<vpux::NDTypeInterface>();
    const auto in2Type = mod.getInput2().getType().cast<vpux::NDTypeInterface>();
