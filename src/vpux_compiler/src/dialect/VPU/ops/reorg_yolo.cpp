...
mlir::LogicalResult vpux::VPU::ReorgYoloOp::inferReturnTypes(mlir::MLIRContext* ctx,
    ...
    if (inType.getShape().raw()[2] % reorgYolo.stride().getInt() != 0) {
        return errorAt(loc, "Input H should be divisible by stride.");
    }
    if (inType.getShape().raw()[3] % reorgYolo.stride().getInt() != 0) {
        return errorAt(loc, "Input W should be divisible by stride.");
    }
    if (inType.getShape().raw()[1] < reorgYolo.stride().getInt() * reorgYolo.stride().getInt()) {
    ...
    for (size_t i = 2; i < inType.getShape().size(); i++) {
        outputShape.push_back(inType.getShape().raw()[i] / reorgYolo.stride().getInt());
