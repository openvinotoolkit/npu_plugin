...
vpux::InputTiling vpux::VPU::GRUSequenceOp::backInferTileInfo(const vpux::TileInfo& outputTileY, vpux::Logger) {
    const auto origInputShape = getShape(input_data());
    const auto origInitialHiddenStateShape = getShape(initial_hidden_state());
    const auto origWShape = getShape(weights());
    const auto origRShape = getShape(recurrence_weights());
    const auto origBShape = getShape(biases());
...
void vpux::VPU::GRUSequenceOp::adjustAttrs(const TilingInfo& inputTiling, const TileInfo& outputYTile) {
    ...
    auto origSeqLength = seq_lengthAttr().dyn_cast_or_null<mlir::IntegerAttr>().getValue().getSExtValue();
    ...
    if (origSeqLength != tiledSeqLength) {
        ...
        this->seq_lengthAttr(newSeqLength);
    }
    ...
    auto origDirection = directionAttr().getValue();
    if (origDirection == IE::RNNSequenceDirection::BIDIRECTIONAL && outputYTile.shape[Dim(1)] == 1 &&
        ...
        this->directionAttr(newDirectionAttr);
    }
    if (origDirection == IE::RNNSequenceDirection::BIDIRECTIONAL && outputYTile.shape[Dim(1)] == 1 &&
...
mlir::LogicalResult vpux::VPU::GRUSequenceOp::applyTileStrategyGRUSequence(VPU::TilingBuilderOpInterface origOp,
    ...
    auto origDirection = gruSequenceOp.directionAttr().getValue();
    ...
    auto opConcatY =
            rewriter.create<VPU::ConcatOp>(origOp->getLoc(), origOp->getResult(0).getType(),
                                           mlir::ValueRange(resultTileYVals), makeArrayRef(resultTileYOffsets));
    opsConcat.push_back(opConcatY.output());
    auto opConcatHo =
            rewriter.create<VPU::ConcatOp>(origOp->getLoc(), origOp->getResult(1).getType(),
                                           mlir::ValueRange(resultTileHoVals), makeArrayRef(resultTileHoOffsets));
    opsConcat.push_back(opConcatHo.output());
