...
mlir::LogicalResult vpux::VPU::AdaptiveAvgPoolOp::inferReturnTypes(
    ...
    VPU::AdaptiveAvgPoolOpAdaptor adaptiveAvgPool(operands, attrs);
    if (mlir::failed(adaptiveAvgPool.verify(loc))) {
        return mlir::failure();
    }

    const auto inputType = adaptiveAvgPool.input().getType().cast<vpux::NDTypeInterface>();
...
EMU::BlobWriter::SpecificTask vpux::VPU::AdaptiveAvgPoolOp::serialize(EMU::BlobWriter& writer) {
