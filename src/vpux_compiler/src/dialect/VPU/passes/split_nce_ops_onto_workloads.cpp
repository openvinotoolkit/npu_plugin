...
mlir::LogicalResult GenericNCERewrite<ConcreteOp>::matchAndRewrite(ConcreteOp origOp,
    ...
    auto isZTilingSupported = mpeMode == VPU::MPEMode::VECTOR;
    if (auto convOp = mlir::dyn_cast<VPU::NCEConvolutionOp>(origOp.getOperation())) {
        const auto inOrder = DimsOrder::fromValue(convOp.input());
        const auto isCMajor = inOrder == DimsOrder::NCHW;
        isZTilingSupported |= !isCMajor;
    }

    rewriter.updateRootInPlace(origOp, [&]() {
        addDPUTasks(rewriter, origOp, pads, outputShape, _numDPU, mpeMode, isZTilingSupported);
    });
...
mlir::LogicalResult EltwiseNCERewrite::matchAndRewrite(VPU::NCEEltwiseOp origOp,
    ...
    rewriter.updateRootInPlace(origOp, [&]() {
        addDPUTasks(rewriter, origOp, pads, outputShape, _numDPU, mpeMode, false);
    });
