...
SmallVector<VPU::DistributedTypeInterface> CompressConvolutionStrategy::getDistributedTensorType(
        VPU::ClusteredOpInterface clusteredOp, VPU::MultiClusterStrategy strategy) const {
    auto origOp = mlir::dyn_cast<NCECompressConvolutionOp>(clusteredOp.getOperation());
    VPUX_THROW_UNLESS(origOp != nullptr, "Got non VPU::NCECompressConvolutionOp operation {0}", clusteredOp->getName());
...
bool CompressConvolutionStrategy::isOperationSplitOverHeightCompatible(VPU::ClusteredOpInterface nceOp,
    ...
    auto origOp = mlir::dyn_cast<NCECompressConvolutionOp>(nceOp.getOperation());
    VPUX_THROW_UNLESS(origOp != nullptr, "Got non VPU::NCECompressConvolutionOp operation {0}", nceOp->getName());
