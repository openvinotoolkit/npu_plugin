...
mlir::LogicalResult EliminateConcat::matchAndRewrite(VPU::ConcatOp origOp, mlir::PatternRewriter& rewriter) const {
    if (!origOp.static_offsets().has_value()) {
        return mlir::failure();
    }

    const auto concatOffsets = parseIntArrayOfArrayAttr<int64_t>(origOp.static_offsets().value());
    ...
    const auto allUsersSliceSubTensors = llvm::all_of(origOp->getUsers(), [&](auto userOp) {
        ...
        auto sliceOffset = parseIntArrayAttr<int64_t>(maybeSliceOp.static_offsets());
        const auto sliceOutShape = getShape(maybeSliceOp.result()).raw();

        for (const auto& p : zip(origOp.inputs(), concatOffsets)) {
    ...
    for (const auto& keyValue : newSliceOffsetsInputMap) {
        ...
        rewriter.replaceOpWithNewOp<VPU::SliceOp>(origSlice, origSlice.result().getType(), sliceInput,
                                                  getIntArrayAttr(getContext(), sliceOffset), origSlice.static_sizes());
    }
