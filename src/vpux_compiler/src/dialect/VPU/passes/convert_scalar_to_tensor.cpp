...
mlir::LogicalResult ConvertScalarToTensorPass::GatherScalarConverter::matchAndRewrite(
    ...
    auto newOp = rewriter.create<VPU::GatherOp>(origOp.getLoc(), origOp.input(), newIndices.output(), nullptr,
                                                origOp.axis_valueAttr(), origOp.batch_dims());

    const auto origShapeAttr = getIntArrayAttr(origOp->getContext(), getShape(origOp.output()));
    auto reshapeOp = rewriter.create<VPU::ReshapeOp>(origOp.getLoc(), origOp.getType(), newOp.output(), nullptr,
...
void ConvertScalarToTensorPass::safeRunOnFunc() {
    ...
    patterns.add<GatherScalarConverter>(&ctx, _log);
    patterns.add<TopKScalarConverter>(&ctx, _log);
