...
void vpux::Const::ReverseAttr::print(mlir::DialectAsmPrinter& printer) const {
    printer << getMnemonic() << "<";
...
mlir::Attribute vpux::Const::ReverseAttr::parse(mlir::DialectAsmParser& parser, mlir::Type) {
...
Const::Content reverseImpl(const Const::Content& input, NDTypeInterface outputType, int64_t axis) {
    ...
    SmallVector<StorageType> reversedVals(input.getValues<StorageType>());
