...
mlir::LogicalResult ChannelMajorConvolutionRewrite::matchAndRewrite(
        IERT::ConvolutionOp origOp, mlir::PatternRewriter& rewriter) const {
            ...
            auto allocOutOp = rewriter.create<mlir::memref::AllocOp>(origOp->getLoc());
            rewriter.create<IERT::ConvolutionOp>(origOp->getLoc(), origOp.input(), origOp.filter(), origOp.bias(), allocOutOp,
