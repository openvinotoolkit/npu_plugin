...
public:
    ...
    _log(log), _userDimsOrder(userDimsOrder) {
            ...
            for (auto& op : func.getOps())
                if (auto convOp = mlir::dyn_cast<vpux::IERT::ConvolutionOp>(op)) {
                    ...
                    if((IC == 3) && (width%16 == 0) && _userDimsOrder == DimsOrder::NCHW)
                    {
                         ...
                         convOp->setAttr("ChannelMajorCompitable", getIntAttr(module.getContext(), 1));
                    }
                    else
                         convOp->setAttr("ChannelMajorCompitable", getIntAttr(module.getContext(), 0));
                }
        }
    }
}
