    ...
    */
    ...
    for (auto user : concat.output().getUsers()) {
        if (mlir::isa<IERT::SubViewOp>(user)) {
            for (auto subUser : user->getUsers()) {
                ...
                const auto dstMemory = VPUIP::getPhysicalMemory(copyOut.output().getType().cast<mlir::MemRefType>());
                // If failed to obtain, or the destination is not CMX
                if (mlir::failed(dstMemory) || dstMemory.getValue() != VPUIP::PhysicalMemory::CMX_NN) {
                    return mlir::failure();
                }
    ...
    for (size_t idx = 0; idx < copyOutOps.size(); idx++) {
        if (!copyOutSubViews.empty()) {
            ...
            copyOutOps[idx].output().replaceAllUsesWith(newOutSubView);
            copyOutOps[idx].output_buff().replaceAllUsesWith(newOutSubView);
        } else {
