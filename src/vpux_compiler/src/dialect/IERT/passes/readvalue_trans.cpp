...
class ReadValueTransPass final : public IERT::ReadValueTransBase<ReadValueTransPass> {
public:
    explicit ReadValueTransPass(IERT::AttrCreateFunc memSpaceCb, Logger log): _memSpaceCb(std::move(memSpaceCb)) {
...
std::unique_ptr<mlir::Pass> vpux::IERT::createReadValueTransPass(AttrCreateFunc memSpaceCb, Logger log) {
    return std::make_unique<ReadValueTransPass>(std::move(memSpaceCb), log);
}
