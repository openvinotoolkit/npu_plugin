...
mlir::LogicalResult vpux::IERT::ViewOp::inferReturnTypes(mlir::MLIRContext* ctx, mlir::Optional<mlir::Location> optLoc,
    ...
    /* Force output memsapce to be the same as input
       view like op can not change it */
    auto origMemSpace = origType.getMemorySpace();
    auto newTypeWithMemSpace = changeMemSpace(newType.cast<mlir::MemRefType>(), origMemSpace);

    inferredTypes.push_back(newTypeWithMemSpace);
