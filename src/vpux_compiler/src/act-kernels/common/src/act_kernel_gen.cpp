...
static void compileAndLinkSHAVE(
    ...
    SmallString<128> srcNamePath = unitDesc.codePath[0];
...
static void compileAndLinkManagementKernelSHAVE(
        const movitools::MoviCompileParams& params,
        const CompilationUnitDesc & unitDesc,
        std::vector<uint8_t>& textBinary, std::vector<uint8_t>& dataBinary) {
    ...
    for (int i = 0; (i < MaxExtraDefines) && !unitDesc.defines[i].empty(); ++i) {
        extraOptions += StringRef(" -D");
        extraOptions += unitDesc.defines[i];
    }
    for (int i = 0; (i < MaxExtraIncludePaths) && !unitDesc.includePaths[i].empty(); ++i) {
        SmallString<128> inc(vpuip2Dir);
        sys::path::append(inc, unitDesc.includePaths[i]);
        extraOptions += StringRef(" -I");
        extraOptions += inc;
    }

    for (int i = 0; (i < MaxCompilationUnitSources) && !unitDesc.codePath[i].empty(); ++i) {
        SmallString<128> srcNamePath = unitDesc.codePath[i];
...

        {
            ...
            std::cout << "# srcPath      = \"" << std::string(srcPath)      << "\"" << std::endl;
            std::cout << "# objPath      = \"" << std::string(objPath)      << "\"" << std::endl;
            std::cout << "# mvToolsDir   = \"" << std::string(mvToolsDir)   << "\"" << std::endl;
            std::cout << "# incPath      = \"" << std::string(incPath)      << "\"" << std::endl;
            std::cout << "# extraOptions = \"" << std::string(extraOptions) << "\"" << std::endl;
#endif
...
ActKernelDesc compileManagementKernelForACTShave(const CompilationUnitDesc & unitDesc,
    ...
    std::vector<uint8_t> textBinary;
    std::vector<uint8_t> dataBinary;
    compileAndLinkManagementKernelSHAVE(params, unitDesc, textBinary, dataBinary);
