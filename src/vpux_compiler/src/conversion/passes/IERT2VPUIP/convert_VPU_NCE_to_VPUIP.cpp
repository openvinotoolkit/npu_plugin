...
VPUIP::NCEClusterTaskOp createNCEClusterTask(
        mlir::ConversionPatternRewriter& rewriter, mlir::Location loc, mlir::Value input, mlir::Value weights,
        mlir::Value weightsTable, mlir::Value instructionListTable, mlir::Value activationWindow,
        ArrayRef<mlir::Value> outputBuffs, vpux::VPUIP::NCETaskType taskType, mlir::ArrayAttr kernelSizeAttr,
        mlir::ArrayAttr kernelStridesAttr, vpux::VPU::PaddingAttr kernelPaddingAttr,
        mlir::IntegerAttr activationWindowChannelLengthAttr, mlir::UnitAttr isContinuedAttr = nullptr,
        mlir::IntegerAttr cmSpPatternAttr = nullptr, mlir::UnitAttr isSegmentedAttr = nullptr,
        mlir::IntegerAttr outChannelOffsetAttr = nullptr) {
    ...
    return rewriter.create<VPUIP::NCEClusterTaskOp>(
            ...
            kernelStridesAttr, kernelPaddingAttr, activationWindowChannelLengthAttr, isContinuedAttr, cmSpPatternAttr,
            isSegmentedAttr, outChannelOffsetAttr);
}
...
mlir::LogicalResult ConvRewriter::matchAndRewrite(VPU::NCEConvolutionOp origOp, OpAdaptor newArgs,
    ...
    auto nceOp = createNCEClusterTask(
            ...
            newArgs.instructionListTable(), newArgs.activationWindow(), outputBuffers, taskType, kernelSizeAttr,
    ...
    if (nceOp.output_sparsity_map() != nullptr) {
        auto groupedOp = rewriter.create<VPUIP::GroupSparseBufferOp>(origOp.getLoc(), nceOp.output(),
                                                                     nceOp.output_sparsity_map());
        rewriter.replaceOp(origOp, groupedOp.output());
    } else {
...
mlir::LogicalResult MaxPoolRewriter::matchAndRewrite(VPU::NCEMaxPoolOp origOp, OpAdaptor newArgs,
    ...
    const auto outputBuffers = allocateBuffers(_log, origOp.getLoc(), rewriter, *typeConverter, {origOp.output()},
                                               /*individualBuffers=*/true);
    VPUX_THROW_UNLESS(outputBuffers.size() == 1, "Expected only one output buffer, got {0}", outputBuffers.size());
    const auto outputBuffer = outputBuffers.front();
    ...
    auto nceOp = createNCEClusterTask(
            ...
            /*instruction_list_table=*/nullptr, newArgs.activationWindow(), outputBuffer, VPUIP::NCETaskType::MAXPOOL,
...
mlir::LogicalResult AveragePoolRewriter::matchAndRewrite(VPU::NCEAveragePoolOp origOp, OpAdaptor newArgs,
    ...
    auto nceOp = createNCEClusterTask(rewriter, origOp->getLoc(), newArgs.input(), /*weights=*/nullptr,
                                      ...
                                      /*instruction_list_table=*/nullptr, /*activation_window=*/nullptr, outputBuffer,
...
mlir::LogicalResult DepthwiseConvRewriter::matchAndRewrite(VPU::NCEDepthConvolutionOp origOp, OpAdaptor newArgs,
    ...
    auto nceOp = createNCEClusterTask(
            rewriter, origOp->getLoc(), newArgs.input(), newArgs.filter(), newArgs.weightsTable(),
            newArgs.instructionListTable(), newArgs.activationWindow(), outputBuffer, VPUIP::NCETaskType::DWCONV,
