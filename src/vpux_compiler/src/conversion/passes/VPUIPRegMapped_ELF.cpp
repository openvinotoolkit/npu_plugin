...
int SIZEOF_SERIALIZED_STRUCT = 80;
...
private:
    ...
    mlir::Value createSectionOp(mlir::FuncOp func, mlir::MLIRContext* ctx, std::string secNameStr,
...
mlir::Value Convert2VPUIPRegMappedAndELFPass::createSectionOp(mlir::FuncOp func, mlir::MLIRContext* ctx,
                                                              ...
                                                              vpux::ELF::SectionTypeAttr secType,
    ...
    for (mlir::Operation& op : blkFunc.getOperations()) {
        ...
        if (opCasted) {
            if (isNNDMAOp) {
                ...
                // "\n"; llvm::dbgs() << "createSectionOp(): NNDMAOpOutput[numNNDMAOps] = " <<
...
void Convert2VPUIPRegMappedAndELFPass::safeRunOnModule() {
    ...
    SIZEOF_SERIALIZED_STRUCT = sizeof(host_parsing::DmaWrapper);
    ...
    // llvm::dbgs() << "safeRunOnFunc(): SIZEOF_SERIALIZED_STRUCT = " << SIZEOF_SERIALIZED_STRUCT << "\n";
    ...
    for (mlir::Operation& op : moduleOp) {
        if (vpux::IE::CNNNetworkOp cnnOp = llvm::dyn_cast<vpux::IE::CNNNetworkOp>(op)) {
            ...
            createSectionOp<vpux::Const::DeclareOp>(funcOp, ctx, ".data.Weights_ct",
                                                    vpux::ELF::SectionTypeAttr::SHT_PROGBITS,
                                                    vpux::ELF::SectionFlagsAttr::SHF_ALLOC);
