...
//
// ConvertVPUIPRegMapped2ELFPass
//

class ConvertVPUIPRegMapped2ELFPass final : public ConvertVPUIPRegMapped2ELFBase<ConvertVPUIPRegMapped2ELFPass> {
public:
    explicit ConvertVPUIPRegMapped2ELFPass(Logger log): _log(log) {
...
private:
    ...
    mlir::SmallVector<mlir::Value> createDMASections(mlir::FuncOp& func, mlir::MLIRContext* ctx,
    ...
    void createDMARelocs(mlir::FuncOp& func, mlir::MLIRContext* ctx, mlir::SmallVector<int64_t>& dmaCount,
    ...
    mlir::Value getNextDMATask(mlir::Value& currentTask) {
        for (auto taskUser : currentTask.getUsers()) {
            if (auto nextDMAOp = mlir::dyn_cast_or_null<VPUIPRegMapped::NNDMAOp>(taskUser)) {
                auto currentDMAOp = mlir::cast<VPUIPRegMapped::NNDMAOp>(currentTask.getDefiningOp());
...
//   from func (a FuncOp).
template <typename DerivedOpType, typename CreateSectionOpType>
mlir::Value ConvertVPUIPRegMapped2ELFPass::createSection(mlir::FuncOp func, mlir::MLIRContext* ctx,
...
mlir::SmallVector<mlir::Value> ConvertVPUIPRegMapped2ELFPass::createDMASections(
        mlir::FuncOp& func, mlir::MLIRContext* ctx, mlir::SmallVector<int64_t>& dmaCount,
        mlir::Operation::operand_range dmaTasks) {
    ...
    vpux::ELF::SectionType sectionType = vpux::ELF::SectionType::get(ctx);
    size_t opAlignmentRequirements = VPUIPRegMapped::NNDMAOp::getAlignmentRequirements();
    ...
    for (auto listHead : dmaTasks) {
        ...
        for (auto dmaTaskIdx = 0; dmaTaskIdx < listElemCount; ++dmaTaskIdx) {
            auto nndmaOp = mlir::cast<VPUIPRegMapped::NNDMAOp>(listHead.getDefiningOp());
...
template <>
mlir::Value ConvertVPUIPRegMapped2ELFPass::createSection<Const::DeclareOp, ELF::CreateSectionOp>(
...
mlir::Value ConvertVPUIPRegMapped2ELFPass::createBuffersSecAndSymtab(mlir::FuncOp func, mlir::MLIRContext* ctx) {
...
mlir::Value ConvertVPUIPRegMapped2ELFPass::createCMXMappingSymtab(mlir::FuncOp funcOp, mlir::MLIRContext* ctx) {
...
void ConvertVPUIPRegMapped2ELFPass::createNetworkIOSymtab(mlir::FuncOp func, mlir::MLIRContext* ctx,
...
void ConvertVPUIPRegMapped2ELFPass::createKernelParamsRelocs(mlir::FuncOp func) {
...
mlir::Value ConvertVPUIPRegMapped2ELFPass::lookupELFSymbol(mlir::Value& symtabValue, mlir::Value& sym_input_value) {
...
void ConvertVPUIPRegMapped2ELFPass::createActKernelRelocs(mlir::FuncOp func) {
    mlir::OpBuilder builderFunc(&(func.getBody().front().back()));
...
void ConvertVPUIPRegMapped2ELFPass::setupActKernelRtConfigs(mlir::FuncOp func, mlir::ModuleOp moduleOp,
    ...
    if (runtimeKernelFunction) {
        ...
        auto actShvRtOp = builderFunc.create<VPUIPRegMapped::ActShaveRtOp>(
...
void ConvertVPUIPRegMapped2ELFPass::createDPURelocs(mlir::FuncOp func) {
    ...
    for (auto invariant : invariants) {
        ...
        if (bufferSection.getValue() == VPURT::BufferSection::CMX_NN) {
            ...
            if (!invariant.is_segmented().getValueOr(false) || invariant.task_type() == VPUIP::NCETaskType::ELTWISE) {
                auto secIdx = bufferMemSpace.getIndex().getValueOr(0);
        ...
        if (auto weights = invariant.weights()) {
            ...
            if (opType != VPUIP::NCETaskType::ELTWISE) {
                ...
                auto actSecIdx =
                        invariant.input().getType().cast<vpux::NDTypeInterface>().getMemSpace().getIndex().getValueOr(
                                0);
        ...
        if (!invariant.is_continued()) {
            ...
            auto minOutIt =
                    std::min_element(outputBuffs.begin(), outputBuffs.end(), [](mlir::Value lhs, mlir::Value rhs) {
                        auto lhsSliceIdx =
                                lhs.getType().cast<vpux::NDTypeInterface>().getMemSpace().getIndex().getValueOr(0);
                        auto rhsSliceIdx =
                                rhs.getType().cast<vpux::NDTypeInterface>().getMemSpace().getIndex().getValueOr(0);
        ...
        for (auto child : children) {
            ...
            int64_t clusterIdx = bufferMemSpace.getIndex().getValueOr(0);
...
void ConvertVPUIPRegMapped2ELFPass::createDMARelocs(mlir::FuncOp& funcOp, mlir::MLIRContext* ctx,
                                                    mlir::SmallVector<int64_t>& dmaCount,
                                                    mlir::Operation::operand_range dmaTasks,
    ...
    for (auto listHead : dmaTasks) {
        ...
        for (auto dmaIdx = 0; dmaIdx < listElemCount; ++dmaIdx) {
            auto dmaOp = mlir::dyn_cast_or_null<VPUIPRegMapped::NNDMAOp>(listHead.getDefiningOp());
            // input addr
...
void ConvertVPUIPRegMapped2ELFPass::safeRunOnModule() {
    mlir::MLIRContext* ctx = &(getContext());
    mlir::FuncOp funcOp;
    mlir::ModuleOp moduleOp = getOperation();

    _log.trace("ConvertVPUIPRegMapped2ELFPass::safeRunOnFunc(): START\n {0}\n", moduleOp);
    ...
    mlir::OpBuilder builderFunc(&(funcOp.getBody().front().back()));
    auto mappedInferenceOps = funcOp.getOps<VPUIPRegMapped::MappedInferenceOp>();
    VPUX_THROW_UNLESS(!mappedInferenceOps.empty(), "MappedInferenceOp could not be located.");
    auto mappedInferenceOp = *(mappedInferenceOps.begin());
    ...
    _log.trace("ConvertVPUIPRegMapped2ELFPass, after sections creation:\n {0} \n", moduleOp);
    ...
    _log.trace("ConvertVPUIPRegMapped2ELFPass, after symtabs creation:\n {0} \n", moduleOp);
    ...
    _log.trace("ConvertVPUIPRegMapped2ELFPass, after DMA Relocs creation:\n {0} \n", moduleOp);

    createKernelParamsRelocs(funcOp);
    createActKernelRelocs(funcOp);
    setupActKernelRtConfigs(funcOp, moduleOp, ctx);
    _log.trace("ConvertVPUIPRegMapped2ELFPass, after Shave Relocs creation:\n {0} \n", moduleOp);

    createDPURelocs(funcOp);
    _log.trace("ConvertVPUIPRegMapped2ELFPass, after ActKernel Relocs creation:\n {0} \n", moduleOp);
    ...
    _log.trace("Convert2VPUIPRegMappsedAndELFPass::safeRunOnFunc(): FINISH\n {0}\n", moduleOp);
}
}  // namespace

//
// createConvertVPUIPRegMapped2ELFPass
//

std::unique_ptr<mlir::Pass> vpux::createConvertVPUIPRegMapped2ELFPass(Logger log) {
    return std::make_unique<ConvertVPUIPRegMapped2ELFPass>(log);
}
