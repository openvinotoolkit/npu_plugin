...
A PXA op will be added to the IERT dialect that represents launching a SHAVE ‘kernel’ written in PXA which will use the same typing as existing IERT ops.
...
If a Tile op is to target he DPU (i.e. convolutions), it will be recognized via a stenciling pass and converted to the appropriate IE Op (i.e. ConvolutionOp)
...
IERT.PXA ops will run the existing EDSL -> SHAVE lowering pipeline to produce a binary, and then convert the IERT.PXA op into a VPUIP.EdslLayer op.
