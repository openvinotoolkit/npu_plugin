...
```
operation ::= `IERT.ExtractParam` `(` $inputs `:` type($inputs) `,` $idx `)`
              attr-dict
              `->` type(results)
```
...
Interfaces: InferTypeOpInterface, LayerOpInterface, MemoryEffectsOpInterface, MultiViewOpInterface
...
### `IERT.DeformablePSROIPooling` (vpux::IERT::DeformablePSROIPoolingOp)

InferenceEngine run-time DeformablePSROIPooling layer


Syntax:

```
operation ::= `IERT.DeformablePSROIPooling` attr-dict
...
| Operand | Description |
| :-----: | ----------- |
| `input_score_maps` | memref of floating-point values
| `input_rois` | memref of floating-point values
| `input_transformations` | memref of floating-point values
...
### `IERT.EmbeddingBagOffsetsSum` (vpux::IERT::EmbeddingBagOffsetsSumOp)

InferenceEngine run-time EmbeddingBagOffsetsSum layer


Syntax:

```
operation ::= `IERT.EmbeddingBagOffsetsSum` attr-dict
              `inputs` `(` $input `:` type($input) `)`
              `outputs` `(` $output_buff `:` type($output_buff) `)`
              `->` type(results)
```
...
InferenceEngine run-time EmbeddingSegmentsSum layer


Syntax:

```
operation ::= `IERT.EmbeddingSegmentsSum` attr-dict
...
### `IERT.GRUCell` (vpux::IERT::GRUCellOp)

InferenceEngine run-time GRUCell layer


Syntax:

```
operation ::= `IERT.GRUCell` attr-dict
...
| Operand | Description |
| :-----: | ----------- |
| `input_data` | memref of 16-bit float or 32-bit float values
| `initial_hidden_state` | memref of 16-bit float or 32-bit float values
| `weights` | memref of 16-bit float or 32-bit float values
| `recurrence_weights` | memref of 16-bit float or 32-bit float values
| `biases` | memref of 16-bit float or 32-bit float values
| `output_hidden_state_buff` | memref of 16-bit float or 32-bit float values

#### Results:

| Result | Description |
| :----: | ----------- |
| `output_hidden_state` | memref of 16-bit float or 32-bit float values

### `IERT.GRUSequence` (vpux::IERT::GRUSequenceOp)

InferenceEngine run-time GRUSequence layer


Syntax:

```
operation ::= `IERT.GRUSequence` attr-dict
              `inputs` `(` $input_data `:` type($input_data) `,` $initial_hidden_state `:` type($initial_hidden_state) `,` $weights `:` type($weights)
              `,` $recurrence_weights `:` type($recurrence_weights) `,` $biases `:` type($biases) `)`
...
| `recurrence_weights` | memref of 16-bit float or 32-bit float values
| `biases` | memref of 16-bit float or 32-bit float values
| `middle_hidden_state_buff` | memref of 16-bit float or 32-bit float values
| `output_hidden_state_buff` | memref of 16-bit float or 32-bit float values
...
| Result | Description |
| :----: | ----------- |
| `middle_hidden_state` | memref of 16-bit float or 32-bit float values
| `output_hidden_state` | memref of 16-bit float or 32-bit float values
...
### `IERT.GatherND` (vpux::IERT::GatherNDOp)

InferenceEngine run-time GatherND layer


Syntax:

```
operation ::= `IERT.GatherND` attr-dict
              `inputs` `(` $input `:` type($input) `,` $indices `:` type($indices) `)`
              `outputs` `(` $output_buff `:` type($output_buff) `)`
              `->` type(results)
```
...
### `IERT.GridSample` (vpux::IERT::GridSampleOp)

InferenceEngine run-time GridSample layer
...
| :-----: | ----------- |
| `input` | memref of any type values
| `grid` | memref of any type values
| `output_buff` | memref of any type values
...
### `IERT.NonMaxSuppression` (vpux::IERT::NonMaxSuppressionOp)

InferenceEngine run-time NonMaxSuppression layer


Syntax:

```
operation ::= `IERT.NonMaxSuppression` attr-dict
              `inputs` `(`
...
Interfaces: InferTypeOpInterface, LayerOpInterface, MemoryEffectsOpInterface
...
| Operand | Description |
| :-----: | ----------- |
| `in_box_coords` | memref of 16-bit float or 32-bit float values
| `in_box_scores` | memref of 16-bit float or 32-bit float values
| `out_selected_indices_buff` | memref of 32-bit signed integer values
| `out_selected_scores_buff` | memref of 16-bit float or 32-bit float values
...
| `out_selected_scores` | memref of 16-bit float or 32-bit float values
...
### `IERT.PermuteQuantize` (vpux::IERT::PermuteQuantizeOp)

InferenceEngine run-time PermuteQuantize layer


Syntax:

```
operation ::= `IERT.PermuteQuantize` attr-dict
              `inputs` `(` $input `:` type($input) `)`
              `outputs` `(` $output_buff `:` type($output_buff) `)`
              `->` type(results)
```
...
Traits: AttrSizedOperandSegments, SameOperandsElementType
...
### `IERT.Tan` (vpux::IERT::TanOp)

InferenceEngine run-time Tan layer


Syntax:

```
operation ::= `IERT.Tan` attr-dict
              `inputs` `(` $input `:` type($input) `)`
              `outputs` `(` $output_buff `:` type($output_buff) `)`
              `->` type(results)
```
...
```
operation ::= `IERT.SpecialCast` `(` $inputs `:` type($inputs) `)`
              attr-dict
              `->` type(results)
```
