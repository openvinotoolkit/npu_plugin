...
## LayerWithPermuteInterface (`IE_LayerWithPermuteInterface`)

Interface for operations that support hardware output permutations
### Methods:
#### `isSupportedPermutation`

```c++
bool isSupportedPermutation(mlir::Operation*permuteOp);
```
...
```c++
void inferLayoutInfo(vpux::IE::LayerLayoutInfo&info, const bool seOpsEnabled);
```
