...
#include "vpux/compiler/core/list_scheduler_traits.hpp"
...
public:
    typedef vpux::interval_traits<T> traits;
    typedef typename traits::unit_t unit_t;
    typedef typename traits::interval_t interval_t;
    typedef vpux::Interval_Utils<interval_t> interval_utils_t;
    typedef vpux::Disjoint_Interval_Set<unit_t, interval_t> interval_tree_t;
...
protected:
    bool overlaps(const query_iterator_t& qitr, unit_t ibegin, unit_t iend) const {
        return interval_utils_t::intersects(qitr.interval_begin(), qitr.interval_end(), ibegin, iend);
    }

    template <typename OutputFunctor = noop_functor_t>
    size_t process_next_interval(const interval_t& curr_interval, OutputFunctor& output = OutputFunctor()) {
        size_t edge_count = 0UL;
        ...
        while ((qitr != qitr_end) && overlaps(qitr, curr_rem_beg, curr_rem_end)) {  // foreach overlap //
            interval_t qinterval = *qitr;

            assert((curr_rem_beg <= curr_rem_end) && (curr_beg <= curr_rem_beg) && (curr_rem_end <= curr_end));
            ...
            rcount = interval_utils_t::interval_xor(qbeg, qend, curr_rem_beg, curr_rem_end, result_beg, result_end);
            ...
            if (!rcount) {
                ...
                for (size_t r = 0; r < rcount; r++) {
                    if (interval_utils_t::is_subset(result_beg[r], result_end[r], curr_rem_beg, curr_rem_end)) {
                        result_val[r] = &curr_interval;
                    } else {
                        assert(interval_utils_t::is_subset(result_beg[r], result_end[r], qbeg, qend));
                ...
                if (result_end[next_xor_interval_index] < inter_beg) {
                    ...
                    interval_tree_.insert(result_beg[next_xor_interval_index], result_end[next_xor_interval_index],
        ...
        if (curr_rem_beg <= curr_rem_end) {
            ...
            const bool status = interval_tree_.insert(curr_rem_beg, curr_rem_end, curr_interval);
            std::ignore = status;  // mateusz
            assert(status);
        }
    ...
    void merge_abutting_intervals(const interval_t& curr_interval) {
        ...
        if ((qitr == qitr_end) || !(*qitr == curr_interval)) {
            return;
        }
        ...
        ++qitr;
        while ((qitr != qitr_end) && ((*qitr == curr_interval) && ((prev_right_end + 1) == qitr.interval_begin()))) {
        ...
        if (prev_right_end > qitr_start.interval_end()) {
            interval_tree_.erase(qitr_start.interval_begin(), qitr_start.interval_end());
...
// control edge.
template <typename OpDag, typename OpSelector, typename SchedulerTraits = vpux::scheduler_traits<OpDag>>
...
public:
    ...
    typedef vpux::Control_Edge_Generator<scheduled_op_t> memory_overlap_control_edge_generation_algo_t;
    typedef std::unordered_map<operation_t, scheduled_op_t> original_schedule_map_t;
    ...
    typedef std::back_insert_iterator<overlap_edge_list_t> overlap_edge_list_back_insert_iterator_t;
    ...
    struct memory_control_edge_extractor_t {
        ...
        memory_control_edge_extractor_t(BackInsertIterator itr): back_insert_iterator_(itr), medge_() {
        }

        void operator()(const scheduled_op_t& source, const scheduled_op_t& sink) {
    ...
    void generate(const dag_t& input_dag, ScheduledOpIterator sbegin, ScheduledOpIterator send,
                  BackInsertIterator output) {
        static_assert(std::is_same<typename ScheduledOpIterator::value_type, scheduled_op_t>::value,
                      "Invalid ScheduledOpIterator");
        ...
        memory_control_edge_extractor_t<overlap_edge_list_back_insert_iterator_t> extractor(
                std::back_inserter(overlap_edges));
        ...
        for (auto eitr = overlap_edges.begin(); eitr != overlap_edges.end(); ++eitr) {
            ...
            if (!is_selectable_op(input_dag, source_op) || !is_selectable_op(input_dag, sink_op)) {
                continue;
            }

            medge.source_ = source_op;
            medge.sink_ = sink_op;
            *output = medge;  // (a) //
            ...
            for (; cop_itr != cop_itr_end; ++cop_itr) {
                ...
                if (itr == original_schedule.end()) {
                    continue;
                }
                schedule_time_t child_time = (itr->second).schedule_time_;

                if (!((child_time > source_time) && (child_time < sink_time))) {
                    continue;
                }

                if (!is_selectable_op(input_dag, child_op)) {
                    continue;
                }
...
private:
    bool does_this_op_use_resources(const dag_t& dag, const operation_t& op) const {
