...
struct op_resource_state_t {
    op_resource_state_t(size_t n = 0UL, size_t m = 0UL)
        ...
        Logger::global().error("Initializing op_resource_state in Barrier_Schedule_Generator with barrier count {0} slots_per_barrie {1}", barrier_count_, slots_per_barrier_);
    }
    ...
    void init(const op_resource_state_t& other) {
        ...
        bool schedule_operation(const operation_t& op, resource_t& demand) {
            ...
            if (barrier_map_.find(op) != barrier_map_.end()) {
                Logger::global().error("Could not find operation  {0} in the barrier map" , getUniqueID(const_cast<mlir::Operation*>(op)), demand);
                return false;
            }
            Logger::global().error("Inserting operation  {0}, in barrier_map_ with demand {1}" , getUniqueID(const_cast<mlir::Operation*>(op)), demand);
