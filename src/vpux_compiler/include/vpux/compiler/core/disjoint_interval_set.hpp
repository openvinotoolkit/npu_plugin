...
struct Interval_Utils {
    ...
    static bool intersects(const unit_t& abeg, const unit_t& aend, const unit_t& bbeg, const unit_t& bend) {
    ...
    static bool interval_intersection(const interval_t& a, const interval_t& b, interval_t& output) {
        unit_t abeg = traits::interval_begin(a);
        unit_t aend = traits::interval_end(a);
    ...
    static bool interval_intersection(const interval_t& a, const interval_t& b, unit_t& obeg, unit_t& oend) {
        unit_t abeg = traits::interval_begin(a);
        unit_t aend = traits::interval_end(a);
    ...
    static bool interval_overlap_union(const interval_t& a, const interval_t& b, interval_t& result) {
        if (!intersects(a, b)) {
            return false;
        }
    ...
    static size_t interval_xor(const interval_t& a, const interval_t& b, BackInsertIterator out_itr) {
        size_t ret_size = 0UL;
        if (!intersects(a, b)) {
            *out_itr = a;
            ++out_itr;
    ...
    static size_t interval_xor(const interval_t& a, const interval_t& b, unit_t (&out_begin)[2UL],
                               unit_t (&out_end)[2UL]) {
        unit_t abeg = traits::interval_begin(a);
        unit_t aend = traits::interval_end(a);
    ...
    static size_t interval_xor(const unit_t& abeg, const unit_t& aend, const unit_t& bbeg, const unit_t& bend,
...
public:
    ...
    //            ----[*]---
    //                 x
    enum orient_e { LEFT_END = 0, RIGHT_END = 1 };  // enum orient_e //
    ...
    struct end_point_t {
        ...
        bool is_left_end() const {
            return orient_ == LEFT_END;
        }
        bool is_right_end() const {
            return orient_ == RIGHT_END;
        }
    ...
    typedef typename end_point_tree_t::const_iterator const_end_point_iterator_t;

    // Invariant: itr_begin_ should always point to
    class interval_iterator_t {
    public:
        interval_iterator_t(const_end_point_iterator_t begin, const_end_point_iterator_t end)
        ...
        const element_t& operator*() const {
            return itr_begin_->second;
        }

        // Precondition: itr_begin_ != itr_end_ //
        unit_t interval_begin() const {
            return (itr_begin_->first).x_;
        }

        // Precondition: itr_begin_ != itr_end_ //
        unit_t interval_end() const {
            const_end_point_iterator_t itr_next = itr_begin_;
            ++itr_next;
    ...
    public:
        free_interval_iterator_t(const_end_point_iterator_t begin, const_end_point_iterator_t end,
        ...
        free_interval_iterator_t(): itr_begin_(), itr_end_(), prev_right_end_(std::numeric_limits<unit_t>::max()) {
        }
        ...
        unit_t interval_begin() const {
            return prev_right_end_;
        }
        unit_t interval_end() const {
            return (itr_begin_ == itr_end_) ? std::numeric_limits<unit_t>::max() : (itr_begin_->first).x_;
        }
    ...
    bool insert(const unit_t& ibeg, const unit_t& iend, const element_t& e) {
        ...
        if (!is_interval_disjoint(lkey, rkey, itr)) {
            return false;
        }
    ...
    bool erase(const unit_t& ibeg, const unit_t& iend) {
        ...
        if ((litr == tree_.end()) || (ritr == tree_.end())) {
            return false;
        }
    ...
    interval_iterator_t query(const unit_t& ibeg, const unit_t& iend) const {
        const_end_point_iterator_t litr = tree_.lower_bound(end_point_t(ibeg, LEFT_END));
        ...
        const_end_point_iterator_t ritr = tree_.lower_bound(end_point_t(iend, RIGHT_END));
    ...
    static bool do_intervals_overlap(unit_t abeg, unit_t aend, unit_t bbeg, unit_t bend) {
        return std::max(abeg, bbeg) <= std::min(aend, bend);
    }
...
private:
    ...
    bool is_interval_disjoint(const end_point_t& lkey, const end_point_t& rkey,
        ...
        if (lkey_lower_bound == tree_.end()) {
            return true;
        }
        const end_point_t& lkey_lb = lkey_lower_bound->first;
        return (lkey_lb.is_left_end() && (lkey.x_ < lkey_lb.x_) && (rkey.x_ < lkey_lb.x_));
    }
