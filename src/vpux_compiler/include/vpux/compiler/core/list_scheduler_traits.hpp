...
struct scheduler_traits {
    ...
    static const_operation_iterator_t outgoing_operations_begin(const dag_t&, const operation_t&);
    static const_operation_iterator_t outgoing_operations_end(const dag_t&, const operation_t&);

    // Given v \in V , iterator over { u | (u, v) \in E }
    static const_operation_iterator_t incoming_operations_begin(const dag_t&, const operation_t&);
    static const_operation_iterator_t incoming_operations_end(const dag_t&, const operation_t&);
    ...
    static bool is_pseudo_input_edge(const dag_t&, const operation_t& a, const operation_t& b);
    ...
    static void initialize_resource_upper_bound(const resource_t& upper_bound, resource_state_t&);

    static void initialize_resource_state(const resource_state_t& start_state, resource_state_t&);

    static bool is_empty_demand(const resource_t& demand);
    static bool is_resource_available(const resource_t& demand, const resource_state_t&);

    template <typename DemandIterator>
    static bool are_resources_available(DemandIterator ditr, DemandIterator ditr_end, const resource_state_t&);

    // Precondition: is_resource_available(demand) = true.
    // Invariant: makes an update in the resource usage of using the operations//
    static bool schedule_operation(const operation_t&, resource_t demand, resource_state_t&);

    static bool schedule_operation(const operation_t& op, resource_t demand, resource_state_t& rstate,
