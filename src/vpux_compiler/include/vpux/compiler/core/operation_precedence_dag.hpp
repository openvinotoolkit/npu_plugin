...
struct Scheduled_Op {
    ...
    Scheduled_Op()
            : op_(),
              schedule_time_(std::numeric_limits<size_t>::max()),
              schedule_end_time_(),
              cmx_address_start_(),
              cmx_address_end_(),
              op_type_() {
    ...
    Scheduled_Op(operation_t op, size_t t, size_t start, size_t end, op_type_e op_type = op_type_e::ORIGINAL_OP)
            : op_(op),
              schedule_time_(t),
              schedule_end_time_(),
              cmx_address_start_(start),
              cmx_address_end_(end),
              op_type_(op_type) {
    }

    Scheduled_Op(const Scheduled_Op& o)
            : op_(o.op_),
    ...
    bool operator==(const Scheduled_Op& o) const {
        return (op_ == o.op_) && (schedule_time_ == o.schedule_time_) && (cmx_address_start_ == o.cmx_address_start_) &&
               (cmx_address_end_ == o.cmx_address_end_);
    }
    ...
    const char* op_type_name() const {
        if (op_type_ == op_type_e::SPILLED_READ_OP) {
            return "SPILLED READ";
        }
        if (op_type_ == op_type_e::SPILLED_WRITE_OP) {
            return "SPILLED WRITE";
        }
        return "ORIGINAL";
    }
