...
public:
    explicit FeasibleMemorySchedulerSpilling(mlir::FuncOp& netFunc, mlir::Attribute& memSpace,
                                             ...
                                             LinearScan<mlir::Value, LinearScanHandler>& scan, Logger& log);
...
private:
    ...
    void updateSpillWriteReadUsers(mlir::async::ExecuteOp opThatWasSpilled, mlir::async::ExecuteOp spillWriteExecOp,
...
private:
    ...
    // op
    llvm::SmallVector<std::pair<FeasibleMemoryScheduler::operationIdxType, mlir::async::ExecuteOp>>
            _opIdAndSpillWritePairs;
};
