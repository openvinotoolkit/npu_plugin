...
public:
    TensorReference createTensor(StringRef name, mlir::ShapedType type, ArrayRef<uint16_t> mult,
                                 ...
                                 VPURT::BufferSection locale = VPURT::BufferSection::DDR, const uint32_t localeIndex = 0);
    TensorReference createTensor(StringRef name, mlir::ShapedType type,
                                 VPURT::BufferSection locale = VPURT::BufferSection::DDR, const uint32_t localeIndex = 0);
    TensorReference createTensor(mlir::Value val, StringRef name, VPURT::BufferSection locale = VPURT::BufferSection::DDR,
