...
typedef struct {
    ...
    union {
        HglDma2DAttributes attr2d;    // Attributes that apply for 2D jobs (i.e. striding)
        HglDmaBarrierCfg barriers1d;  // Barrier mask configurations for 1D jobs
    };
    HglDmaBarrierCfg barriers;  // Barrier mask configurations for 2D jobs
    // The descriptor must be aligned to 64 byte boundary
    // This is needed for L2 cache line alignment
} HglDmaDescriptor;

typedef HglDmaDescriptor ALIGN_DMA(64) vpu_dma_descriptor_t;
