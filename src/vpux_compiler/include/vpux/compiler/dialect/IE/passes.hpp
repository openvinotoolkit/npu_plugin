...
std::unique_ptr<mlir::Pass> createAdjustLayoutsPass(const bool seOpsEnabled = false, Logger log = Logger::global());
std::unique_ptr<mlir::Pass> createOptimizeReordersPass(const bool seOpsEnabled = false, Logger log = Logger::global());
...
std::unique_ptr<mlir::Pass> createConvertStridedSlice2DWConvPass(Logger log = Logger::global());
...
std::unique_ptr<mlir::Pass> createConvertGroupDeconvToGroupConvPass(Logger log = Logger::global());
...
std::unique_ptr<mlir::Pass> createFuseQuantizedOpsPass(const bool seOpsEnabled = false, Logger log = Logger::global());
