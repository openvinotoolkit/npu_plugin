...
private:
    ...
    void updateCycleStartTimeDueToNativeExecutorDependency(vpux::VPURT::TaskOp task, size_t newStartCycle,
                                                           SmallVector<vpux::VPURT::TaskOp>& orderedTasksByCycleStart);
    void updateCycleStartTime(vpux::VPURT::TaskOp taskOp, size_t newStartCycle);
    ...
    // initialized with the start and end times from memory scheduler.
    ...
    std::map<VPU::ExecutorKind, SmallVector<VPURT::TaskOp>> _orderedTasksByCycleStart;
