...
void mv::RuntimeModel::serialize(const std::string& filename)
{
    ...
    if (filename.find("/") == 0)
        throw ArgumentError("mv::Runtime", "file:location", "invalid", "Cannot write to root. Check configuration");
    
    // Check we're not trying to write to a higher directory, eg, ../path/to/blob
    if (filename.find("..") != std::string::npos)
        throw ArgumentError("mv::Runtime", "file:location", "invalid", "Cannot write to higher directory. Check configuration");
    ...
    {   
        ...
        {   // folder structure does not exist - create
            Logger::log(mv::Logger::MessageType::Debug, "RuntimeModel", "Not found. Creating " + path);
            ...
            while ((pos = path.find("/")) != std::string::npos) { 
                ...
                all_dir += this_dir + "/";
                Logger::log(mv::Logger::MessageType::Debug, "RuntimeModel", "Making:  " + all_dir);
                mkdir(all_dir.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
                path.erase(0, pos+1);
            }
        }
        else if( info.st_mode & S_IFDIR ) //folder exists
            Logger::log(mv::Logger::MessageType::Debug, "RuntimeModel", "Output location found:  " + path);
        else // not a directory - unknown error (probably a symbolic link)
            throw ArgumentError("mv::Runtime", "file:location", "invalid", "Cannot write to output location. Check configuration.");
    }
