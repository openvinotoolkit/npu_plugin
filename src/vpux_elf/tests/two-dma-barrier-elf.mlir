...
func @main(%arg0: memref<1x1x2x1000xf16>, %arg1: memref<1x1x2x1000xf16>) -> memref<1x1x2x1000xf16> {
    ...
    %mappedInference = VPUIPRegMapped.MappedInference
                            dmas(%dma0 : !VPUIPRegMapped.Index<0>) // we can verify with the MappedInferenceOp Verifier: if we have dmaCount(>1) we need to have dmaTasks() (reverse applies also), else error; the same for the other ...
