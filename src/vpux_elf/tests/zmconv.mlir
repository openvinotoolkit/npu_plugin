...
module @mainModule attributes {VPUIP.arch = "MTL", VPUIP.compilationMode = "ReferenceHW"}  {
  ...
  func private @zmajor_conv_f16_f16_f16(%arg0: memref<1x16x16x16xf16, #NHWC, "ProgrammableInput">, %arg1: memref<1x64x16x16xf16, #NHWC, "ProgrammableOutput">) -> memref<1x64x16x16xf16, #NHWC, "ProgrammableOutput"> {
    ...
    %0 = VPURT.DeclareBuffer "VPU_CMX_NN" [0] <41984> -> memref<64x16x1x1xf16, #NHWC, "VPU_CMX_NN">
    %1 = VPURT.DeclareBuffer "VPU_CMX_NN" [0] <32768> -> memref<1x16x16x16xf16, #NHWC, "VPU_CMX_NN">
    %2 = VPURT.DeclareBuffer "VPU_CMX_NN" [0] <0> -> memref<1x64x16x16xf16, #NHWC, "VPU_CMX_NN">
    ...
    %3 = VPURT.DeclareBuffer "VPU_CMX_NN" [0] <40960> -> memref<64x1x1x4xsi32, #NHWC, "VPU_CMX_NN">
    ...
    VPURT.Task {isTrailingSWLayer = false} updates(%4 : !VPURT.Barrier) op :  {
        %6 = VPUIP.NNDMA {port = 0 : i64, set_crit = false, set_ord = true} inputs(%arg0 : memref<1x16x16x16xf16, #NHWC, "ProgrammableInput">) outputs(%1 : memref<1x16x16x16xf16, #NHWC, "VPU_CMX_NN">) -> memref<1x16x16x16xf16, #NHWC, "VPU_CMX_NN">
    }
    VPURT.Task {isTrailingSWLayer = false} updates(%4 : !VPURT.Barrier) op :  {
        %6 = VPUIP.NNDMA {port = 0 : i64, set_crit = false, set_ord = true} inputs(%cst : memref<64x16x1x1xf16, #NHWC, "GraphFile">) outputs(%0 : memref<64x16x1x1xf16, #NHWC, "VPU_CMX_NN">) -> memref<64x16x1x1xf16, #NHWC, "VPU_CMX_NN">
    }
    VPURT.Task {isTrailingSWLayer = false} updates(%4 : !VPURT.Barrier) op :  {
        %6 = VPUIP.NNDMA {port = 0 : i64, set_crit = false, set_ord = true} inputs(%cst_0 : memref<64x1x1x4xsi32, #NHWC, "GraphFile">) outputs(%3 : memref<64x1x1x4xsi32, #NHWC, "VPU_CMX_NN">) -> memref<64x1x1x4xsi32, #NHWC, "VPU_CMX_NN">
    }
    VPURT.Task {isTrailingSWLayer = false} waits(%5 : !VPURT.Barrier) op :  {
        %6 = VPUIP.NNDMA {port = 0 : i64, set_crit = false, set_ord = true} inputs(%2 : memref<1x64x16x16xf16, #NHWC, "VPU_CMX_NN">) outputs(%arg1 : memref<1x64x16x16xf16, #NHWC, "ProgrammableOutput">) -> memref<1x64x16x16xf16, #NHWC, "ProgrammableOutput">
    }
    VPURT.Task {isTrailingSWLayer = false} waits(%4 : !VPURT.Barrier) updates(%5 : !VPURT.Barrier) op :  {
        %6 = VPUIP.NCEClusterTask {kernel_padding = [0, 0, 0, 0], kernel_size = [1, 1], kernel_strides = [1, 1], task_type = "CONV"} input(%1 : memref<1x16x16x16xf16, #NHWC, "VPU_CMX_NN">) weights(%0 : memref<64x16x1x1xf16, #NHWC, "VPU_CMX_NN">) weight_table(%3 : memref<64x1x1x4xsi32, #NHWC, "VPU_CMX_NN">) parent_input(%1 : memref<1x16x16x16xf16, #NHWC, "VPU_CMX_NN">) parent_output(%2 : memref<1x64x16x16xf16, #NHWC, "VPU_CMX_NN">) outputs(%2 : memref<1x64x16x16xf16, #NHWC, "VPU_CMX_NN">) -> memref<1x64x16x16xf16, #NHWC, "VPU_CMX_NN"> variants :  {
