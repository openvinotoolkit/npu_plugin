...
func @main(%arg0: memref<1x1x1x1000xf16>, %arg1: memref<1x1x1x1000xf16>) -> memref<1x1x1x1000xf16> {
    ...
    %secDWCT = ELF.CreateSection secFlags(SHF_ALLOC) {secName = ".data.Weights_ct", secType = "SHT_PROGBITS", secInfo = 1, secAddrAlign = 4} -> !ELF.Section
      {
        ELF.PutAnyOpInSection %ctDcl : memref<1x3x1x1xf16>
      }
    ...
    %sec0 = ELF.CreateSection secFlags(SHF_EXECINSTR) {secName = ".text.BarrierConfigs", secType = "SHT_PROGBITS", secInfo = 1, secAddrAlign = 4} -> !ELF.Section
      {
        ELF.PutAnyOpInSection %3 : !VPURT.Barrier
        ELF.PutAnyOpInSection %5 : !VPURT.Barrier
     }
    ...
    %sec1 = ELF.CreateSection secFlags("SHF_ALLOC|SHF_EXECINSTR") {secName = ".text.dmaTasks", secType = "SHT_PROGBITS", secInfo = 1, secAddrAlign = 4} -> !ELF.Section
      {
          ELF.PutAnyOpInSection %2 : memref<1x1x1x1000xf16, "CMX_NN">
          ELF.PutAnyOpInSection %4 : memref<1x1x1x1000xf16, "CMX_NN">
          ELF.PutAnyOpInSection %6 : memref<1x1x1x1000xf16>
      }



    %symSecDW = ELF.Symbol %secDW : !ELF.Section
    %symSecDWCT = ELF.Symbol %secDWCT : !ELF.Section
    %symSec0 = ELF.Symbol %sec0 : !ELF.Section
    %symSec1 = ELF.Symbol %sec1 : !ELF.Section
    //
    %symCtDcl = ELF.Symbol %ctDcl : memref<1x3x1x1xf16>
    //
    %symArg0 = ELF.Symbol %arg0 : memref<1x1x1x1000xf16>
    %symArg1 = ELF.Symbol %arg1 : memref<1x1x1x1000xf16>
    //
    %sym0 = ELF.Symbol %0 : memref<1x1x1x1000xf16, "CMX_NN">
    %sym1 = ELF.Symbol %1 : memref<1x1x1x1000xf16, "CMX_NN">
    %sym2 = ELF.Symbol %2 : memref<1x1x1x1000xf16, "CMX_NN">
    %sym4 = ELF.Symbol %4 : memref<1x1x1x1000xf16, "CMX_NN">


    // These symbols are used for 1 relocation per inference run (JIT relocation)
    %InputSymTableSection = ELF.CreateSymbolTableSection secName(".input.symbolTableSection") secFlags(SHF_USERINPUT) -> !ELF.Section
    {
        ELF.PutAnyOpInSection %symArg0 : !ELF.Symbol
    }

    %OutputSymTableSection = ELF.CreateSymbolTableSection secName(".output.symbolTableSection") secFlags(SHF_USEROUTPUT) -> !ELF.Section
    {
    ...
    %InputRelocSection = ELF.CreateRelocationSection secName(".rela.input") sourceSymbolTableSection(%InputSymTableSection) targetSection(%sec1) secFlags(SHF_USERINPUT) -> !ELF.Section
    {
    ...
    %OutputRelocSection = ELF.CreateRelocationSection secName(".rela.output") sourceSymbolTableSection(%OutputSymTableSection) targetSection(%sec1) secFlags(SHF_USEROUTPUT) -> !ELF.Section
    {
