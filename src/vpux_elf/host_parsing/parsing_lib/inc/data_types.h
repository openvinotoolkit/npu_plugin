...
struct Relocation {
    MemoryLocation location; // TODO how are we specifying the sections to link up?
    uint32_t offset;         // Offset from the start of the struct (DMA descriptor, registers, etc)
    uint32_t size;           // Size of data to patch
};
