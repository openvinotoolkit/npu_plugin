...
public:
    std::map<std::string, std::string> GetMap() const { return layerParam; }
...
CustomLayerParserNGraph::CustomLayerParserNGraph(
    std::shared_ptr<ngraph::Node>& node, std::vector<mv::Data::TensorIterator> inputs)
...
std::vector<uint8_t> CustomLayerParserNGraph::resolveKernelArguments(
...
std::vector<mv::TensorInfo> CustomLayerParserNGraph::resolveStageOutputs(
            ...
            if (cnnParam != _layerParam.end()) {
                ...
                if (binding.type == CustomParamType::Int) {
                    ...
                    VPU_THROW_UNLESS(val.hasValue(),
                        ...
                        binding.argName, _node->description(), _node->get_friendly_name(), binding.irSource);
