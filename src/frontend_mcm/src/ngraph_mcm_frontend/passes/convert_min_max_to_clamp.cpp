...
bool ConvertMinMaxToClamp::run_on_node(std::shared_ptr<ngraph::Node> node) {
    ...
    if (min_node != nullptr) {
        ...
        if (pre_max_node != nullptr) {
            auto max_constant = std::dynamic_pointer_cast<ngraph::op::v0::Constant>(
                    pre_max_node->input_value(1).get_node_shared_ptr());
            auto min_constant =
            ...
            if ((max_constant != nullptr) && (min_constant != nullptr)) {
                ...
                if ((max_value.size() == 1) && (min_value.size() == 1)) {
                    const auto clamp = std::make_shared<ngraph::op::v0::Clamp>(pre_max_node->input_value(0),
        ...
        if (pre_min_node != nullptr) {
            ...
            auto min_constant = std::dynamic_pointer_cast<ngraph::op::v0::Constant>(
                    pre_min_node->input_value(1).get_node_shared_ptr());
            if ((max_constant != nullptr) && (min_constant != nullptr)) {
                auto max_value = max_constant->cast_vector<double>();
                auto min_value = min_constant->cast_vector<double>();
                if ((max_value.size() == 1) && (min_value.size() == 1)) {
                    const auto clamp = std::make_shared<ngraph::op::v0::Clamp>(pre_min_node->input_value(0),
                                                                               max_value[0], min_value[0]);
