...
static bool inputsHasSameScales(const std::vector<InferenceEngine::CNNLayerPtr>& inputs, const size_t& maxValues,
...
bool needAlignZeroPoints(std::vector<T> lowValues, std::vector<T> highValues, const float levels) {
    auto firstZP = QuantizationHelpers::calculateZeroPoint(highValues[0], lowValues[0], levels,
                                                           InferenceEngine::Precision::U8);
    for (size_t i = 1; i < lowValues.size(); i++) {
        auto zp = QuantizationHelpers::calculateZeroPoint(highValues[i], lowValues[i], levels,
                                                          InferenceEngine::Precision::U8);
        if (firstZP != zp) {
            return true;
        }
...
bool isInputLayoutSupported(const ie::Layout& inputLayout) {
    const std::set<ie::Layout> supportedInLayouts = {ie::Layout::NHWC, ie::Layout::NCHW, ie::Layout::CHW,
                                                     ie::Layout::NC, ie::Layout::C};
...
bool isOutputLayoutSupported(const ie::Layout& outputLayout) {
    std::set<ie::Layout> supportedOutLayouts = {ie::Layout::NHWC, ie::Layout::NCHW, ie::Layout::CHW, ie::Layout::NC,
                                                ie::Layout::C};
...
void FrontEndMcm::parseInputData() {
    ...
    for (const auto& inputInfo : _parsedNetwork.networkInputs) {
        ...
        auto mvInput = _modelMcm.input(netInput->name(), inputShape, precisionToDType(inputPrecision), mcmInputOrder,
                                       networkInput);
        mvInput->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseOutputData() {
    ...
    for (const auto& outputInfo : _parsedNetwork.networkOutputs) {
        ...
        // S#34832
        auto mvOutput = _modelMcm.output(outputInfo.first + "REMOVE_ME", lastLayerOut->getMcmNode(), outputType, true);
...
void FrontEndMcm::parseConvolution(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    if (isDepthWiseConv) {
        ...
        mvConv = _modelMcm.depthwiseConv(convLayer->name, input->getMcmNode(), mvWeights,
        ...
        mvConv = _modelMcm.conv(convLayer->name, input->getMcmNode(), mvWeights,
                                {static_cast<uint16_t>(kernelStrideX), static_cast<uint16_t>(kernelStrideY)},
                                {static_cast<uint16_t>(padLeft), static_cast<uint16_t>(padRight),
                                 static_cast<uint16_t>(padTop), static_cast<uint16_t>(padBottom)},
                                static_cast<unsigned>(dilationX), static_cast<unsigned>(groupSize));
    }
    mvConv->setQuantParams(initialQuantParams());

    //  Need quantize bias, this logic provide by MCM team, need check
    if (inputs.size() == 3) {
        mvConvOnly = mvConv;
        auto mvBiases = inputs[2]->getMcmNode();
        mvConv = _modelMcm.bias(convLayer->name + ":bias", mvConvOnly, mvBiases);
        mvConv->setQuantParams(initialQuantParams());
        _logger->debug("'%s' layer '%s': Bias part (%s) added to mcmModel", convLayer->type, convLayer->name,
                       mvConv->getName());
    }
...
void FrontEndMcm::parsePooling(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    cvtPaddingsFromCeilToFloorMode(input->origData()->getDims().at(3), outDesc.getDims().at(3), kernelSizeX,
                                   kernelStrideX, padLeft, padRight);
    cvtPaddingsFromCeilToFloorMode(input->origData()->getDims().at(2), outDesc.getDims().at(2), kernelSizeY,
                                   kernelStrideY, padTop, padBottom);

    mv::Data::TensorIterator mvPooling;
    if (poolType == ie::PoolingLayer::AVG) {
        mvPooling = _modelMcm.averagePool(poolLayer->name, inputs[0]->getMcmNode(),
        ...
        mvPooling = _modelMcm.maxPool(poolLayer->name, inputs[0]->getMcmNode(),
                                      {static_cast<uint16_t>(kernelSizeX), static_cast<uint16_t>(kernelSizeY)},
                                      {static_cast<uint16_t>(kernelStrideX), static_cast<uint16_t>(kernelStrideY)},
                                      {static_cast<uint16_t>(padLeft), static_cast<uint16_t>(padRight),
                                       static_cast<uint16_t>(padTop), static_cast<uint16_t>(padBottom)},
    ...
    mvPooling->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseFullyConnected(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto mvWeights = inputs[1]->getMcmNode();
    auto mvFullyConnected = _modelMcm.fullyConnected(FClayer->name, input->getMcmNode(), mvWeights);
    mvFullyConnected->setQuantParams(initialQuantParams());

    if (inputs.size() == 3) {
        auto mvBiases = inputs[2]->getMcmNode();
        mvFullyConnected = _modelMcm.bias(FClayer->name + ":bias", mvFullyConnected, mvBiases);
        mvFullyConnected->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseReLU(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    if (std::fabs(negativeSlope) < std::numeric_limits<float>::epsilon()) {
        mvRelu = _modelMcm.relu(reluLayer->name, inputs[0]->getMcmNode());
    } else {
        mvRelu = _modelMcm.leakyRelu(reluLayer->name, inputs[0]->getMcmNode(), negativeSlope);
    }
    mvRelu->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseSoftMax(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto mvSoftmax = _modelMcm.softmax(softMaxLayer->name, inputs[0]->getMcmNode(), mcmAxis);
    mvSoftmax->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseScaleImpl(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs,
                                 std::vector<double>& weights, ie::Blob::Ptr biases) {
    logParsingStartHelper(_logger, layer, inputs);

    auto input = inputs[0];

    mv::Shape weightsShape = {weights.size()};
    auto mvWeights = _modelMcm.constant("", weights, weightsShape, mv::DType("Float32"), mv::Order::getColMajorID(1));
    mvWeights->setQuantParams(initialQuantParams());

    auto scale = _modelMcm.scale(layer->name, input->getMcmNode(), mvWeights);
    scale->setQuantParams(initialQuantParams());
    ...
    if (biases != nullptr) {
        ...
        auto shiftData =
                _modelMcm.constant("", biasData, shiftShape, mv::DType("Float32"), mv::Order::getColMajorID(1));
        shiftData->setQuantParams(initialQuantParams());
        // TODO: return logging
        scaleShift = _modelMcm.bias(layer->name + ":bias", scale, shiftData);
        scaleShift->setQuantParams(initialQuantParams());
    }
...
void FrontEndMcm::parsePermute(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto mvPerm = _modelMcm.permute(layer->name, inputs[0]->getMcmNode(), mv::Order(newOrder));
    mvPerm->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseEltwise(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    case ie::EltwiseLayer::eOperation::Sub:
        mvEltwise = _modelMcm.eltwise(eltwiseLayer->name, mvInputs, "Subtract");
        break;
    case ie::EltwiseLayer::eOperation::Sum:
        mvEltwise = _modelMcm.eltwise(eltwiseLayer->name, mvInputs, "Add");
        break;
    ...
    mvEltwise->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseBias(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    if (inputs.size() == 1) {
        ...
        mvBias = _modelMcm.bias(layer->name, input->getMcmNode(), mvBiasValues);
        mvBias->setQuantParams(initialQuantParams());
    } else if (inputs.size() == 2) {
        logParsingStartHelper(_logger, layer, inputs);

        auto input = inputs[0];
        auto input1 = inputs[1];
        mvBias = _modelMcm.bias(layer->name, input->getMcmNode(), input1->getMcmNode());
        mvBias->setQuantParams(initialQuantParams());
    } else {
...
void FrontEndMcm::parseClamp(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto mvClampMin = _modelMcm.minimum(clampLayer->name + "clamp-min", inputs[0]->getMcmNode(), clampLayer->max_value);
    mvClampMin->setQuantParams(initialQuantParams());
    auto mvClampMax = _modelMcm.maximum(clampLayer->name + "clamp-max", mvClampMin, clampLayer->min_value);
    mvClampMax->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseReshape(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto mvReshape = _modelMcm.reshape(layer->name, inputs[0]->getMcmNode(), newShape);
    mvReshape->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseConcat(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto mvConcat = _modelMcm.concat(concatLayer->name, concatInputs, mcmAxis);
    ...
    if (getInputTo(concatLayer->outData[0]).empty()) {
        mvConcat = _modelMcm.maxPool(concatLayer->name + "_maxpool", mvConcat, {1, 1}, {1, 1}, {0, 0, 0, 0}, true);
    }
    // end of workaround

    mvConcat->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseRegionYolo(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto region = _modelMcm.regionYolo(layer->name, inputs[0]->getMcmNode(), coords, classes, do_softmax, num, mask);
    region->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseReorgYolo(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto reorg = _modelMcm.reorgYolo(layer->name, inputs[0]->getMcmNode(), stride);
    reorg->setQuantParams(initialQuantParams());
...
bool isInteger(ie::Precision iePrecision) {
    static std::set<ie::Precision> integer_precision{ie::Precision::I8, ie::Precision::U8, ie::Precision::I32,
                                                     ie::Precision::I64};
...
void FrontEndMcm::parseConst(const InferenceEngine::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    if (isInteger(blobPrecision)) {
        ...
        auto constMCM = _modelMcm.constantInt(layer->name, constData, mcmShape,
                                              precisionToDType(constBlob->getTensorDesc().getPrecision()),
                                              mv::Order::getColMajorID(mcmShape.ndims()));
        constMCM->setQuantParams(initialQuantParams());
        bindOutput(constMCM, layer->outData[0]);
    } else {
        std::vector<double> constData = packBlobToVector<double>(constBlob, constBlob->size());
        auto constMCM = _modelMcm.constant(
                layer->name, constData, mcmShape, precisionToDType(Precision(Precision::ePrecision::FP32)),
...
void FrontEndMcm::parseGather(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    if (inputs[1]->getMcmNode()->getDType() == precisionToDType(Precision::I64)) {
        ...
        auto constMCM = _modelMcm.constantInt(layer->name + "_indices_i32", indices_new,
                                              inputs[1]->getMcmNode()->getShape(), precisionToDType(Precision::I32),
                                              mv::Order::getColMajorID(inputs[1]->getMcmNode()->getShape().ndims()));
        constMCM->setQuantParams(initialQuantParams());
        ...
        auto mvGather = _modelMcm.gather(layer->name, inputs[0]->getMcmNode(), constMCM, axis);
        mvGather->setDType(mv::DType("Float16"));
        mvGather->setQuantParams(initialQuantParams());
        bindOutput(mvGather, layer->outData[0]);
        _logger->debug(FINISH_PARSING_STR, mvGather->getName());
    } else {
        auto mvGather = _modelMcm.gather(layer->name, inputs[0]->getMcmNode(), inputs[1]->getMcmNode(), axis);
        mvGather->setDType(mv::DType("Float16"));
        mvGather->setQuantParams(initialQuantParams());
        bindOutput(mvGather, layer->outData[0]);
        _logger->debug(FINISH_PARSING_STR, mvGather->getName());
...
void FrontEndMcm::parseFakeQuantize(const InferenceEngine::CNNLayerPtr& layer, const vpu::McmNodeVector& inputs) {
    ...
    auto fakeQuantize =
            _modelMcm.fakeQuantize(layer->name, inputs[0]->getMcmNode(), inputs[1]->getMcmNode(),
...
void FrontEndMcm::parseTopK(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto topK = _modelMcm.topK(layer->name, inputs[0]->getMcmNode(), sort, mode, k, axis);
    topK->setQuantParams(initialQuantParams());
    bindOutput(topK, layer->outData[0]);
    auto topKOp = _modelMcm.getSourceOp(topK);
    if (topKOp->outputSlots() > 1)
        bindOutput(topKOp->getOutputTensor(1), layer->outData[1]);
...
void FrontEndMcm::parsePower(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    if (powerLayer->power == -1) {
        ...
        auto reciprocal_result = _modelMcm.reciprocal(layer->name, inputs[0]->getMcmNode());
...
void FrontEndMcm::parseDetectionOutput(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    mvDetectionOutput = _modelMcm.detectionOutput(
            layer->name, detectionInputs, num_classes, keep_top_k, nms_threshold, background_label_id, top_k,
    ...
    mvDetectionOutput->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseSigmoid(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto mvSigmoid = _modelMcm.sigmoid(layer->name, inputs[0]->getMcmNode());
    mvSigmoid->setQuantParams(inputQuantParams);
...
void FrontEndMcm::parseTanH(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto mvTanh = _modelMcm.tanh(layer->name, inputs[0]->getMcmNode());
    mvTanh->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseDeconvolution(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    if (isDepthWiseConv) {
        ...
        auto mvWeightsValues = _modelMcm.constant("", weightsData, mcmShape, precisionToDType(Precision::FP32),
                                                  mv::Order::getZMajorID(mcmShape.ndims()));
        ...
        mvDeconv = _modelMcm.deconv(deconvLayer->name, input->getMcmNode(), mvWeightsValues,
                                    {static_cast<uint16_t>(kernelStrideX), static_cast<uint16_t>(kernelStrideY)},
                                    {static_cast<uint16_t>(padLeft), static_cast<uint16_t>(padRight),
                                     static_cast<uint16_t>(padTop), static_cast<uint16_t>(padBottom)},
                                    static_cast<unsigned>(dilationX), static_cast<unsigned>(groupSize), true);
        mvDeconv->setQuantParams(initialQuantParams());
    } else {
        auto weights = layer->blobs["weights"];
        auto weightsData = packBlobToVector<double>(weights, weights->size());

        mv::Shape mcmShape = {static_cast<uint64_t>(kernelSizeY), static_cast<uint64_t>(kernelSizeX), inputGroupSize,
                              outputGroupSize};
        ...
        auto mvWeightsValues =
                _modelMcm.constant("", weightsDataReorder, mcmShape,
                                   precisionToDType(Precision(Precision::ePrecision::FP32)), mv::Order("NCHW"));
    ...
    if (layer->blobs.count("biases")) {
        ...
        auto mvBiases =
                _modelMcm.constant("", biasData, mcmShape, precisionToDType(Precision(Precision::ePrecision::FP32)),
                                   mv::Order::getColMajorID(mcmShape.ndims()));

        mvDeconvOnly = mvDeconv;
        mvDeconv = _modelMcm.bias(deconvLayer->name + ":bias", mvDeconvOnly, mvBiases);
        mvDeconv->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseELU(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    mv::Data::TensorIterator mvElu = _modelMcm.elu(layer->name, inputs[0]->getMcmNode(), alpha);
...
void FrontEndMcm::parseCrop(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    if (layer->CheckParamPresence("axis") && layer->CheckParamPresence("offset") &&
        ...
        mvSlice = _modelMcm.slice(layer->name, input->getMcmNode(), mvOffsets, outShape);
        mvSlice->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseNormalize(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto mvWeightsValues =
            _modelMcm.constant("", weightsData, weightsShape, mv::DType(precisionToDType(Precision::ePrecision::FP32)),
                               mv::Order::getZMajorID(4));

    mv::Data::TensorIterator mvNormalize;

    mvNormalize = _modelMcm.normalize(layer->name, inputs[0]->getMcmNode(), mvWeightsValues, eps, across_spatial,
                                      channel_shared);
    mvNormalize->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseInterp(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto mvInterp = _modelMcm.interp(layer->name, inputs[0]->getMcmNode(), factor, pad_begin, pad_end, height, width,
                                     align_corners);
    mvInterp->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseProposal(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto proposal =
            _modelMcm.proposal("", proposal_ins, scale, ratio, base_size, pre_nms_topn, post_nms_topn, nms_thresh,
                               feat_stride, min_size, pre_nms_thresh, clip_before_nms, clip_after_nms, normalize,
                               box_size_scale, box_coordinate_scale, framework, for_deformable);
...
void FrontEndMcm::parseROIPooling(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto roipool = _modelMcm.rOIPooling(layer->name, roi_inputs, pooled_w, pooled_h, spatial_scale, roi_pooling_method,
                                        num_rois);
    roipool->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parsePSROIPooling(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto psroi = _modelMcm.pSROIPooling(layer->name, psroi_ins, output_dim, group_size, spatial_scale, pooled_h,
                                        pooled_w, spatial_bins_x, spatial_bins_y, mode);
    psroi->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseResample(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto resample_result = _modelMcm.resample(layer->name, inputs[0]->getMcmNode(), interpolationMap.at(interpolation),
                                              antialias, output_shape);
    resample_result->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parsePriorBox(const ie::CNNLayerPtr& layer, const McmNodeVector&) {
    ...
    KmbPlugin::utils::priorBoxParam param(offset, step, min_sizes, max_sizes, flip, clip, scale_all_sizes, fixed_sizes,
                                          fixed_ratios, densitys, src_aspect_ratios, src_variance, data_dims,
                                          image_dims, out_dims);

    auto boxes = KmbPlugin::utils::computePriorbox(param);
    auto priorbox = _modelMcm.constant(layer->name + "_const", boxes, {boxes.size() / 2, 2, 1, 1}, mv::DType("Float64"),
                                       mv::Order("NHWC"));
    priorbox->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parsePriorBoxClustered(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    KmbPlugin::utils::priorBoxClusteredParam param{offset,
                                                   clip,
                                                   step_w,
                                                   step_h,
                                                   layer_width,
    ...
    priorboxClustered->setQuantParams(initialQuantParams());
...
void FrontEndMcm::parseSplit(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    if (splitLayer == nullptr) {
        THROW_IE_EXCEPTION << "parseSplit: can't cast layer to ie::SplitLayer";
    }
...
std::vector<CustomLayer::Ptr> FrontEndMcm::getSuitableCustomLayers(const std::vector<CustomLayer::Ptr>& customLayers,
...
CustomLayer::Ptr FrontEndMcm::findMatchingCustomLayer(const std::vector<CustomLayer::Ptr>& customLayers,
...
void FrontEndMcm::parseExp(const ie::CNNLayerPtr& layer, const McmNodeVector& inputs) {
    ...
    auto exp_result = _modelMcm.exp(layer->name, inputs[0]->getMcmNode());
    exp_result->setQuantParams(initialQuantParams());
