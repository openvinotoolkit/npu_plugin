...
#include <cassert>
#include <cstring>
#include <custom_layer/ShaveElfMetadata.hpp>
#include <iostream>
#include <string>
#include <vector>
...
struct md_parser_t {
  md_parser_t(const uint8_t *data, size_t data_size,
    ...
    , strtab_size(strtab_size) {
        (void)data_size;
        (void)strtab_size;
        assert(hdr->version == md_version_1_0);
    }
    ...
    const md_kernel_descriptor_t* get_kernel(const std::string& name) const {
        const int kernel_id = get_kernel_id(name);
        if (kernel_id == -1) {
            return nullptr;
        }
    ...
    //
    bool kernel_has_variant(const md_kernel_descriptor_t* kernel, md_kernel_variant_type_t variant) const {
        const auto& v = kernel->variant[variant];
        return v.name != md_invalid_index && v.func != md_invalid_index;
    }
    ...
    const char* get_name(const md_kernel_argument_t* arg) const {
        if (arg->name == -1u) return strtab;
        return strtab + arg->name;
    }

    // Evaluate an arbitary expression
    //
    uint32_t evaluate_expr(const md_expr_t* expr, const uint32_t local_size[3], const uint32_t global_size[3],
        const uint32_t* param, uint32_t param_count) const;

    const char* get_string_table() const { return strtab; }
