...
static void disableCMconvOpFcn(const mv::pass::PassEntry& pass, mv::ComputationModel& model, mv::TargetDescriptor&, mv::Element&, mv::Element&);
...
void disableCMconvOpFcn(const mv::pass::PassEntry& pass, mv::ComputationModel& model, mv::TargetDescriptor&, mv::Element&, mv::Element&)
{
    ...
    if(hasFQ)
    {
        for (auto& convOp : convOps)
        {
            ...
            {
                convOp->set<bool>("enableCMconv", false);
            }
        }
        ...
        if (sinkOperators[0]->getOpType() != "Scale")
        {
            ...
            if (nextSinkOperators[0]->getOpType() == "Conv"){
                nextSinkOperators[0]->set<bool>("enableCMconv", false);
            }
        }
    }
