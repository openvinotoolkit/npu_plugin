    ...
    namespace graphOptimizer
    {
        ...
        public:
            ...
            size_t alignedWeightsSize(const mv::Data::TensorIterator tensorToSize, const Shape& streamConfig, const Attribute& clustering){
                ...
                if(clustering.toString() == "SplitOverK")
                {
                                            ...
                                            bool outputActivationSparsity, bool weightsSparsity, const Shape& streamConfig, bool fakeSparsity)
            {
                ...
                if(opType == "Conv" || opType == "DepthwiseConv")
                {
                    ...
                    if (opType == "Conv")
                    {
                        weightSize += alignedWeightsSize(op.getInputTensor(1),{1,1,streamConfig["C"],streamConfig["K"],1}, clustering);
                    }
                    else{
                ...
                for( const auto spilling : spillingPool)
                {
                    for( const auto clustering : clusteringStrategyPool)
                    {
                        ...
                        for(const auto k : streamsOverK)
                        {
                            ...
                            {
                                maxSplitOverH = getStreamsOverH(op,clustering,iAS,outputActivationSparsity,weightsSparsity,{1,1,1,k,n},fakeSparsity);
