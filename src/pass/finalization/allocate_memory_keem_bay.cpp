...
void allocateInputOutputTensorsKeemBay(const mv::pass::PassEntry& pass, mv::ComputationModel& model, mv::TargetDescriptor&, mv::Element&, mv::json::Object&)
{
    ...
    auto inTensor = inputOp->getOutputTensor();

    for (auto inItr = inTensor.begin(); inItr < inTensor.end(); inItr++)
        if (!(*inItr)->isPopulated() && (!(*inItr)->hasAttr("allocators")))
            dm.allocateTensor("ProgrammableInput", stageIt, (*inItr));

    auto outputOp = om.getOutput();
    auto outTensor = outputOp->getInputTensor();
    for (auto outItr = outTensor.begin(); outItr < outTensor.end(); outItr++)
        if (!(*outItr)->isPopulated() && (! (*outItr)->hasAttr("allocators")))
            dm.allocateTensor("ProgrammableOutput", stageIt, (*outItr));
