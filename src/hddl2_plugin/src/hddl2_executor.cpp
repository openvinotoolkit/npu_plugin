...
void HDDL2Executor::loadGraphToDevice() {
    std::unordered_map<std::string, std::string> hddlUnite_config = {};
    const auto csram_size = _config.getCSRAMSize();
    if (csram_size) {
        hddlUnite_config.insert(std::make_pair("CSRAM_SIZE", std::to_string(csram_size)));
    }

    if (_context == nullptr) {
        _uniteGraphPtr = std::make_shared<vpu::HDDL2Plugin::HddlUniteGraph>(
            _network, _config.device_id(), hddlUnite_config, _config.logLevel());
    } else {
        _uniteGraphPtr = std::make_shared<vpu::HDDL2Plugin::HddlUniteGraph>(
            _network, _context, hddlUnite_config, _config.logLevel());
    }
}
