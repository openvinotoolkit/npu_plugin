...
public:
    ...
    std::shared_ptr<vpux::IDevice> getDeviceToUse(const std::string& specificName = "") const;
    std::shared_ptr<vpux::IDevice> getDeviceFromContext(const InferenceEngine::RemoteContext::Ptr& context) const;
...
private:
    ...
    std::vector<std::shared_ptr<vpux::IEngineBackend>> _backends;
};
