...
class StageNode :
    ...
    VPU_MODEL_ATTRIBUTE_RANGE(StageInputVector, inputEdges)
    VPU_MODEL_ATTRIBUTE_RANGE(StageOutputVector, outputEdges)

    VPU_MODEL_ATTRIBUTE_RANGE(StageTempBufferVector, tempBufferEdges)

    VPU_MODEL_ATTRIBUTE(InjectedStage, parentStageEdge, nullptr)
    VPU_MODEL_ATTRIBUTE_RANGE(InjectedStageList, injectedStageEdges)
    ...
    //
    // Model/SubGraph access
    //

    VPU_MODEL_ATTRIBUTE(ModelHandle, model, nullptr)
    VPU_MODEL_ATTRIBUTE(SubGraphHandle, subGraph, nullptr)

    //
    // Comparison operators (by name and by index)
    //
...
public:
    ...
    //
    // Cache for faster Graph traversal
    //

private:
    using OrderedSet = std::set<Stage, StageNameCmp>;
    using RevOrderedSet = std::set<Stage, StageNameRevCmp>;

    // It holds the number of separate edges per each prev/next stage
    using OrderedMap = std::map<Stage, int, StageNameCmp>;
    using RevOrderedMap = std::map<Stage, int, StageNameRevCmp>;

    struct Cache final {
        RevOrderedMap prevStages;
        RevOrderedMap nextStages;
    };
...
private:
    ...
    struct StageOrderMapAccess final {
        inline const Stage& operator()(const OrderedMap::value_type& p) const {
            return p.first;
        }
    };
...
public:
    ...
    inline auto prevStages() const -> decltype(mapRange<StageOrderMapAccess>(contRange(_cache.prevStages))) {
        return mapRange<StageOrderMapAccess>(contRange(_cache.prevStages));
    }
    inline auto nextStages() const -> decltype(mapRange<StageOrderMapAccess>(contRange(_cache.nextStages))) {
        return mapRange<StageOrderMapAccess>(contRange(_cache.nextStages));
    }
...
protected:
    inline StageNode() :
            ...
            _stridesInfo(this),
            _batchInfo(this),
            _posInSubGraphCache(this),
            _posInSubGraphBfsTempList(this) {
    }
...
private:
    StagePtrList::iterator _ptrPosInSubGraph;
    IntrusivePtrListNode<StageNode> _posInSubGraphCache;
    IntrusivePtrListNode<StageNode> _posInSubGraphBfsTempList;
