...
private:
    ...
    void throwWhenUnsupported(const std::string func, uint32_t since) {
        if (_driverExtVersion < since) {
            OPENVINO_THROW("L0 extension function ", func, " is only available with driver version ",
                           ZE_MAJOR_VERSION(since), ".", ZE_MINOR_VERSION(since), " or later");
        }
    }

public:
    ze_graph_dditable_ext_decorator(ze_graph_dditable_ext_last_t* impl, uint32_t driverExtVersion)
            : _impl(impl), _driverExtVersion(driverExtVersion) {
        // version 1.0
    ...
    // version 1.5
    ze_result_t ZE_APICALL pfnCreate2(ze_context_handle_t hContext, ze_device_handle_t hDevice,
                                      const ze_graph_desc_2_t* desc, ze_graph_handle_t* phGraph) {
        throwWhenUnsupported("pfnCreate2", ZE_GRAPH_EXT_VERSION_1_5);
        return _impl->pfnCreate2(hContext, hDevice, desc, phGraph);
    }

    ze_result_t ZE_APICALL pfnQueryNetworkCreate2(ze_context_handle_t hContext, ze_device_handle_t hDevice,
    ...
    ze_result_t ZE_APICALL pfnQueryContextMemory(ze_context_handle_t hContext, ze_graph_memory_query_type_t type,
