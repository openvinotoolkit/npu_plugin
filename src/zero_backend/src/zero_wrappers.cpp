...
EventPool::~EventPool() {
    ...
    if (ZE_RESULT_SUCCESS != result) {
        _log.error("zeEventPoolDestroy failed {0:X+}", uint64_t(result));
    }
}
...
Event::~Event() {
    ...
    if (ZE_RESULT_SUCCESS != result) {
        _log.error("zeEventDestroy failed {0:X+}", uint64_t(result));
    }
}
...
CommandList::~CommandList() {
    ...
    if (ZE_RESULT_SUCCESS != result) {
        _log.error("zeCommandListDestroy failed {0:X+}", uint64_t(result));
    }
}
...
CommandQueue::~CommandQueue() {
    ...
    if (ZE_RESULT_SUCCESS != result) {
        _log.error("zeCommandQueueDestroy failed {0:X+}", uint64_t(result));
    }
}
...
Fence::~Fence() {
    ...
    if (ZE_RESULT_SUCCESS != result) {
        _log.error("zeFenceDestroy failed {0:X+}", uint64_t(result));
    }
}
