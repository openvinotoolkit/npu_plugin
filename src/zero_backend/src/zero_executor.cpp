...
void prepareInputForInference(const IE::Blob::Ptr& userInput, const IE::TensorDesc& deviceTensorDesc, void* destData,
                              const bool surfaceFPQuantization, std::shared_ptr<vpu::Logger>& logger) {
    ...
    vpux::QuantizeParameters quantizeParams = vpux::QuantizeParameters(surfaceFPQuantization);
    ...
    if (!isPrecisionMatched) {
        ...
        if (!isLayoutMatched) {
            expectedInput = toPrecision(IE::as<IE::MemoryBlob>(expectedInput), devicePrecision, quantizeParams);
            ...
            toPrecision(IE::as<IE::MemoryBlob>(expectedInput), devicePrecision, quantizeParams, nullptr, destData);
        }
...
void ZeroExecutor::push(const IE::BlobMap& inputs) {
    ...
    const auto quantParams = _networkDesc->getQuantParamsInfo();
    ...
    for (const auto& inferInput : inputs) {
        ...
        const auto surfaceFPQuantization = quantParams.at(name);
        ...
        if (!isRepackingRequired(input->getTensorDesc(), deviceInput->getTensorDesc())) {
            ...
            prepareInputForInference(input, deviceInput->getTensorDesc(), hostMem.data(), surfaceFPQuantization,
                                     _logger);
        }
    }
