...
ConvertInstanceNormToMVN::ConvertInstanceNormToMVN() {
    ...
    ngraph::matcher_pass_callback matcherPassCallback = [=](ngraph::pattern::Matcher& matcher) {
        ...
        auto const_0_5_node = std::dynamic_pointer_cast<ngraph::opset6::Constant>(patternToOutput.at(const_0_5).get_node_shared_ptr());
        auto constEpsNode = std::dynamic_pointer_cast<ngraph::opset6::Constant>(patternToOutput.at(eps).get_node_shared_ptr());
        auto constGammaNode = std::dynamic_pointer_cast<ngraph::opset6::Constant>(patternToOutput.at(gamma).get_node_shared_ptr());
        auto constBetaNode = std::dynamic_pointer_cast<ngraph::opset6::Constant>(patternToOutput.at(beta).get_node_shared_ptr());
        ...
        float epsValue = 0.0;
        bool validConstValues = op::util::has_constant_value<float>(const_0_5_node, -0.5) && op::util::get_single_value(constEpsNode, epsValue);
        if (!validConstValues) {
            return false;
        }

        auto mean1AxisNode = std::dynamic_pointer_cast<ngraph::opset6::Constant>(patternToOutput.at(mean1Axis).get_node_shared_ptr());
        auto mean2AxisNode = std::dynamic_pointer_cast<ngraph::opset6::Constant>(patternToOutput.at(mean2Axis).get_node_shared_ptr());
        ...
        // The reduction axes are 1,2 hence a transpose pair is required to shift the axes to 2,3 as MVN can only work with across_channels true or false
        // Reduction axes are determined by across channles flag
        // across_channels = true reduction axes are 1, 2, 3 
        // across channels = false reduction axes 2, 3 
        const std::vector<int64_t> transposeArg0Weights = {0, 3, 2, 1};
        auto transposeArg0Const = std::make_shared<ngraph::op::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{transposeArg0Weights.size()}, transposeArg0Weights.data());
        auto transposeArg0Node = std::make_shared<ngraph::op::v1::Transpose>(xOutput.get_node_shared_ptr(), transposeArg0Const->output(0));

        auto mvn = std::make_shared<ngraph::op::v0::MVN>(transposeArg0Node, /*across_channels*/false, /*normalize_variance*/true, epsValue);

        const std::vector<int64_t> transposeArg1Weights = {0, 3, 2, 1};
        auto transposeArg1Const = std::make_shared<ngraph::op::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{transposeArg1Weights.size()}, transposeArg1Weights.data());
        ...
        ngraph::NodeVector nodes_to_copy_info(
            {   
                patternToOutput.at(mean1).get_node_shared_ptr(),
                patternToOutput.at(sqDiff).get_node_shared_ptr(),
                patternToOutput.at(addEps).get_node_shared_ptr(),

                patternToOutput.at(power).get_node_shared_ptr(),
                patternToOutput.at(mul1).get_node_shared_ptr(),
                
                patternToOutput.at(mulLeft).get_node_shared_ptr(),
                patternToOutput.at(mulRight).get_node_shared_ptr(),

                patternToOutput.at(sub).get_node_shared_ptr(),
                patternToOutput.at(add).get_node_shared_ptr()
            });  

        ngraph::copy_runtime_info(nodes_to_copy_info,{ transposeArg0Const, transposeArg0Node, mvn, transposeArg1Const, transposeArg1Node, constGammaNode, mulGamma, constBetaNode, addBeta });
