...
std::shared_ptr<ngraph::Node> clone_up_fq_node(std::shared_ptr<ngraph::Node> fq_node, const ngraph::Output<ngraph::Node> &output, int &copy_num) {
    copy_num++;
    ...
    const_in_low->set_friendly_name(fq_node->input_value(1).get_node_shared_ptr()->get_friendly_name() + "_copy_" + std::to_string(copy_num));
    const_in_high->set_friendly_name(fq_node->input_value(2).get_node_shared_ptr()->get_friendly_name() + "_copy_" + std::to_string(copy_num));
    auto clone_fq_node = fq_node->clone_with_new_inputs({output, const_in_low, const_in_high, const_in_low, const_in_high});
    clone_fq_node->set_friendly_name(fq_node->get_friendly_name() + "_copy_" + std::to_string(copy_num));
    return clone_fq_node;
}

std::shared_ptr<ngraph::Node> clone_down_fq_node(std::shared_ptr<ngraph::Node> fq_node, const ngraph::Output<ngraph::Node> &output, int &copy_num) {
    copy_num++;
    ...
    const_out_low->set_friendly_name(fq_node->input_value(3).get_node_shared_ptr()->get_friendly_name() + "_copy_" + std::to_string(copy_num));
    const_out_high->set_friendly_name(fq_node->input_value(4).get_node_shared_ptr()->get_friendly_name() + "_copy_" + std::to_string(copy_num));
    auto clone_fq_node = fq_node->clone_with_new_inputs({output, const_out_low, const_out_high, const_out_low, const_out_high});
    clone_fq_node->set_friendly_name(fq_node->get_friendly_name() + "_copy_" + std::to_string(copy_num));
    return clone_fq_node;
}
