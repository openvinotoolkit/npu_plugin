...
bool is_fq_agnostic(const std::shared_ptr<ngraph::Node>& node) {
    return (std::dynamic_pointer_cast<ngraph::op::v1::StridedSlice>(node) != nullptr ||
            ...
            //  std::dynamic_pointer_cast<ngraph::op::v1::GroupConvolution>(node->output(0).get_target_inputs().begin()->get_node()->shared_from_this())
            //  == nullptr) ||
...
void replace_node_if_changed(const std::shared_ptr<ngraph::op::v0::Constant>& node, const std::vector<double>& data,
    ...
    if (node->get_element_type() == ngraph::element::f32) {
        new_node = std::make_shared<ngraph::op::v0::Constant>(node->get_element_type(), node->get_shape(),
                                                              new_node->cast_vector<float>().data());
    }
    if (node->get_element_type() == ngraph::element::f16) {
        new_node = std::make_shared<ngraph::op::v0::Constant>(node->get_element_type(), node->get_shape(),
                                                              new_node->cast_vector<ngraph::float16>().data());
    }
...
bool replace_node_if_changed(const std::shared_ptr<ngraph::op::v0::Constant>& node, const ngraph::element::Type_t type,
    ...
    auto new_node = std::make_shared<ngraph::op::v0::Constant>(
            type, data.size() == 1 ? ngraph::Shape(0) : ngraph::Shape({data.size(), 1, 1, 1}), data.data());
    if (node->get_friendly_name().find(name_postfix) == std::string::npos)
        new_node->set_friendly_name(node->get_friendly_name() + name_postfix);
    else
        new_node->set_friendly_name(node->get_friendly_name());
...
std::vector<int64_t> quantizeData(const ngraph::Shape& outShape, const ngraph::element::Type outElemType,
    ...
    ngraph::runtime::reference::autobroadcast_binop(srcInds.data(), scalesInds.data(), out.data(), srcShape,
                                                    scalesShape, broadcast_spec,
                                                    [&](size_t srcInd, size_t scaleInd) -> int64_t {
...
bool all_fqs_are_equal(std::vector<std::shared_ptr<ngraph::Node>>& fqs) {
    auto get_fq_param = [](int input_num, std::shared_ptr<ngraph::Node> fq_node) -> std::vector<float> {
        ...
        auto fq_param = std::dynamic_pointer_cast<ngraph::op::v0::Constant>(
                fq_node->input_value(input_num).get_node_shared_ptr());
        if (fq_param == nullptr) {
    ...
    return std::all_of(fqs.begin() + 1, fqs.end(), [&](const std::shared_ptr<ngraph::Node> fq) {
        bool is_same = true;
