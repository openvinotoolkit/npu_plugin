...
mlir::OwningModuleRef importHWTEST(llvm::StringRef sourceJson, mlir::MLIRContext* ctx) {
    ...
    bool isMultiClusteringSOH = jsonDesc.getCaseType() == nb::CaseType::MultiClusteringSOH;
    ...
    } else if (isMultiClusteringSOH) {
        hwtest::buildMultiClusteringSOHTest(jsonDesc, module, builder, log, input_type, weightType(), output_type);
