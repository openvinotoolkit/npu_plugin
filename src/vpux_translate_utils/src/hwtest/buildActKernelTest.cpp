...
void buildActKernelTest(const nb::TestCaseJsonDescriptor& testDesc, mlir::ModuleOp module, mlir::OpBuilder builder,
    ...
    SmallVector<mlir::Type, 2> inputTensorTypes{getTensorType(ioShape, builder.getF16Type(), DimsOrder::NHWC),
                                                getTensorType(ioShape, builder.getF16Type(), DimsOrder::NHWC)};
    SmallVector<mlir::Type, 2> outputTensorTypes{getTensorType(ioShape, builder.getF16Type(), DimsOrder::NHWC)};
    ...
    // Run the kernel.
    auto globalRef = builder.getSymbolRefAttr(globalOp);
