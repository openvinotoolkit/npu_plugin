...
//            (barrier)
//            |       |
//        ... (loop with conv ops, ACTshave ops and barriers)
//            |       |
//         (conv)    (ACTshave)
//            |       |
//            (barrier)
...
void buildRaceConditionDPUACTTest(const nb::TestCaseJsonDescriptor& testDesc, mlir::ModuleOp module,
    ...
    VPUX_THROW_UNLESS(!inputShape.empty(), "buildRaceConditionDPUACTTest: Got empty inputShape");
    VPUX_THROW_UNLESS(!outputShape.empty(), "buildRaceConditionDPUACTTest: Got empty outputShape");
    VPUX_THROW_UNLESS(!weightsShape.empty(), "buildRaceConditionDPUACTTest: Got empty weightsShape");
    VPUX_THROW_UNLESS(!weightsTableShape.empty(), "buildRaceConditionDPUACTTest: Got empty weightsTableShape");
    ...
    inputCMXVec.push_back(inputCMX);
    ...
    for (std::size_t i = 1; i < iterationCount; ++i) {
        ...
        auto nceTask_0 = VPURT::wrapIntoTaskOp<VPUIP::NCEClusterTaskOp>(
                ...
                inputCMX.buffer(), outputCMX_0.buffer(), outputCMX_0.buffer(), vpux::VPUIP::NCETaskType::CONV,
        ...
        // ActShave
        buildActShaveTask(testDesc, module, functionBuilder, log, makeArrayRef(inputTypes), inputCMXVec, outputCMX_1,
