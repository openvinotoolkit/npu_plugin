...
mlir::DenseElementsAttr generateWeights(ArrayRef<int64_t> wt_shape, mlir::Type dtype, mlir::MLIRContext* ctx,
    ...
    if (auto qtype = dtype.dyn_cast<mlir::quant::QuantizedType>()) {
        ...
        if (qtype.getFlags() & mlir::quant::QuantizationFlags::Signed) {
            wtData_ddr_valueType = mlir::RankedTensorType::get(wt_shape, getSInt8Type(ctx));
        } else {
    ...
    std::ifstream wt_stream{weight_file_name, std::ios::in | std::ios::binary};
    if (!wt_stream) {
        std::cerr << "Warning: Unable to open weight data file " << weight_file_name << '\n';
    }

    if (dtype.isSignedInteger(4)) {
        std::cerr << "Warning : Int4 is being read and stored as Int8\n";
        std::vector<int8_t> wt_vec = readFile<int8_t>(wt_stream);
        wt_vec.resize(wt_vec.size() * 2, 0);
        ...
        std::vector<uint8_t> wt_vec = readFile<uint8_t>(wt_stream);
        wt_vec.resize(wt_vec.size() * 2, 0);
        auto wt_data_values = makeArrayRef<uint8_t>(wt_vec);
        wt_data_vals = mlir::DenseElementsAttr::get(wtData_ddr_valueType, wt_data_values);
        return wt_data_vals;
    } else if (dtype.isSignedInteger(8)) {
        std::vector<int8_t> wt_vec;
        wt_vec.resize(vecSize, 0);
        if (wt_stream) {
            wt_stream.read(reinterpret_cast<char*>(wt_vec.data()), vecSize);
        }
        auto wt_data_values = makeArrayRef<int8_t>(wt_vec);
        wt_data_vals = mlir::DenseElementsAttr::get(wtData_ddr_valueType, wt_data_values);
        return wt_data_vals;
    } else if (dtype.isInteger(8)) {
        std::vector<uint8_t> wt_vec;
        wt_vec.resize(vecSize, 0);
        if (wt_stream) {
            wt_stream.read(reinterpret_cast<char*>(wt_vec.data()), vecSize);
        }
        auto wt_data_values = makeArrayRef(wt_vec);
        wt_data_vals = mlir::DenseElementsAttr::get(wtData_ddr_valueType, wt_data_values);
        return wt_data_vals;
    } else if (dtype.isF16()) {
        std::vector<float16> wt_vec;
        wt_vec.reserve(vecSize);
        if (wt_stream) {
            wt_stream.read((char*)&wt_vec[0], (vecSize * sizeof(float16)));
        } else {
            wt_vec.resize(vecSize, 0.);
        }
        std::vector<float16> wt_vec_actual;
        wt_vec_actual.reserve(vecSize);
        for (size_t i = 0; i < vecSize; ++i) {
            wt_vec_actual.push_back(static_cast<float16>(wt_vec[i]));
        }
        auto wt_data_values = makeArrayRef<float16>(wt_vec_actual);
        wt_data_vals = mlir::DenseElementsAttr::get(wtData_ddr_valueType, wt_data_values);
        return wt_data_vals;
    } else if (dtype.isF32()) {
        std::vector<float_t> wt_vec;
        wt_vec.reserve(vecSize);
        if (wt_stream) {
            wt_vec.insert(wt_vec.begin(), std::istream_iterator<float_t>(wt_stream), std::istream_iterator<float_t>());
        } else {
            wt_vec.resize(vecSize, 0.);
        }
        ...
        std::vector<bfloat16> wt_vec;
        wt_vec.reserve(vecSize);
        if (wt_stream) {
            wt_stream.read((char*)&wt_vec[0], (vecSize * sizeof(bfloat16)));
        } else {
            wt_vec.resize(vecSize, 0.);
        }
        std::vector<bfloat16> wt_vec_actual;
        wt_vec_actual.reserve(vecSize);
        for (size_t i = 0; i < vecSize; ++i) {
            wt_vec_actual.push_back(static_cast<bfloat16>(wt_vec[i]));
        }
        auto wt_data_values = makeArrayRef<bfloat16>(wt_vec_actual);
        wt_data_vals = mlir::DenseElementsAttr::get(wtData_ddr_valueType, wt_data_values);
        return wt_data_vals;
    } else {
...
void buildCNNOp(mlir::OpBuilder& builder, llvm::StringRef mainFuncName, llvm::ArrayRef<mlir::Type> inputs,
    ...
    for (auto input : enumerate(inputs)) {
        ...
        const auto userTypeAttr = mlir::TypeAttr::get(input.value());
    ...
    for (auto output : enumerate(outputs)) {
        ...
        const auto userTypeAttr = mlir::TypeAttr::get(output.value());
