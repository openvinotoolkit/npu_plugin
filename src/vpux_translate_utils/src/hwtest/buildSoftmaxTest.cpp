...
void buildSoftmax(const nb::TestCaseJsonDescriptor& testDesc, mlir::ModuleOp module, mlir::OpBuilder builder,
    ...
    auto softmax_op = testDesc.getSoftMaxLayer();
    ...
    const int64_t axis = softmax_op.axis;
    auto kernelInfo = IERT::KernelInfo{SmallVector<mlir::Attribute>{getIntAttr(ctx, axis)},
                                       {"singleShaveSoftmax"},
    ...
    auto func = builder.create<mlir::FuncOp>(builder.getUnknownLoc(),
                                             llvm::formatv("softmax_{0}_{1}", inputType, outputType).str(), funcType,
    ...
    auto inputcmx_type = getMemRefType(VPURT::BufferSection::CMX_NN, inShape, inputType, DimsOrder::NCHW);
    ...
    auto outputcmx_type = getMemRefType(VPURT::BufferSection::CMX_NN, outShape, outputType, DimsOrder::NCHW);
