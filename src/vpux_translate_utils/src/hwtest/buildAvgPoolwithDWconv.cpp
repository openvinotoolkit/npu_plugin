...
void buildAvgpoolWithDwConv(const nb::TestCaseJsonDescriptor& testDesc, mlir::ModuleOp module, mlir::OpBuilder builder,
    ...
    //    Shape(sparsity_shape)); auto activationWindow_ddr_type = mlir::MemRefType::get(
    //            makeArrayRef(sparsity_shape), sparsity_type, activationWindowAffineMaps,
    ...
    //    auto activationwindow_totalsize_bytes = activationwindow_totalsize * sparsity_type.getIntOrFloatBitWidth() /
    //    8;
    ...
    const auto sparsity_type = getUInt8Type(builder.getContext());
    ...
    auto act_window_ddr_memreftype = getMemRefType(funcbuilder, VPUIP::MemoryLocation::GraphFile, sparsity_shape,
                                                   sparsity_type, act_window_maps);
    ...
    auto act_window_ddr =
            funcbuilder.create<Const::DeclareOp>(builder.getUnknownLoc(), act_window_ddr_memreftype,
                                                 Const::ContentAttr::get(sparsityAttr).reorder(DimsOrder::NHWC));
    auto act_window_cmx = createDeclareTensorOp(funcbuilder, VPUIP::MemoryLocation::VPU_CMX_NN, sparsity_shape,
                                                sparsity_type, act_window_maps, 0, ACTIVATIONWINDOW_CMX_OFFSET);

    funcbuilder.create<VPUIP::NNDMAOp>(builder.getUnknownLoc(), getConstResult(act_window_ddr),
                                       getTensorResult(act_window_cmx), mlir::ValueRange(),
                                       mlir::ValueRange(barrier0.barrier()), false);
    ...
    const std::vector<int32_t> wtTbl_data_values_vec = vpux::VPUIP::NCESparsity::getWeightsTable(
            inputType, outputType, WEIGHTS_CMX_OFFSET, weights_set_nbytes, ACTIVATIONWINDOW_CMX_OFFSET,
            VPUIP::ArchKind::MTL, out_shape[1], weightsType); /*generateWeightsTablesValuesWithSparsity(
testDesc, inputcmx_type, outputcmx_type, wtData_cmx_type, act_window_cmx.getType(),
ACTIVATIONWINDOW_CMX_OFFSET, wtTbl_data_shape, WEIGHTS_CMX_OFFSET);*/
    ...
    auto nceTask = funcbuilder.create<VPUIP::NCEClusterTaskOp>(
            ...
            act_window_cmx.getOperation()->getResult(0), parent_inputcmx.getOperation()->getResult(0),
