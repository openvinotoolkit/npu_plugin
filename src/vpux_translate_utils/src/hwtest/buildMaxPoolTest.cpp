...
void buildMaxPool(const nb::TestCaseJsonDescriptor& testDesc, mlir::ModuleOp module, mlir::OpBuilder builder,
    ...
    const auto bitPatternSize =
            VPUIP::NCESparsity::getBitPatternSize(makeArrayRef(filter_size), stride_vec[0], uderlyingInputType, in_shape[1]);
    mlir::IntegerAttr actChannelLength = funcbuilder.getI32IntegerAttr(checked_cast<int32_t>(bitPatternSize));

    const auto fakeSparsity = VPUIP::NCESparsity::getFakeSparsity(VPUIP::NCETaskType::MAXPOOL, makeArrayRef(filter_size), stride_vec[0],
                                                                  uderlyingInputType, in_shape[1], in_shape[1]);
