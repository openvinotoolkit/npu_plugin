...
void buildContinuedConv(const nb::TestCaseJsonDescriptor& testDesc, mlir::ModuleOp module, mlir::OpBuilder builder,
    ...
    const llvm::SmallVector<std::int64_t> weightsShape{weights.shape[0], weights.shape[1], weights.shape[2], weights.shape[3]};

    const auto streamsOverC = 2;
    const llvm::SmallVector<std::int64_t> inputPartialShape({inputShape[0], inputShape[1]/streamsOverC, inputShape[2], inputShape[3]});
    const llvm::SmallVector<std::int64_t> weightsPartialShape({weightsShape[0], weightsShape[1]/streamsOverC, weightsShape[2], weightsShape[3]});
    ...
    const auto WEIGHTS_PARTIAL_0_CMX_OFFSET = WEIGHTSTABLE_1_CMX_OFFSET + 4 * weightsTableShape[0] * weightsTableShape[3];
    const auto WEIGHTS_PARTIAL_1_CMX_OFFSET = WEIGHTS_PARTIAL_0_CMX_OFFSET + totalTensorSize(weightsPartialShape, weightsType);
    ...
    const auto getMemRef = [&builder](const llvm::SmallVector<std::int64_t>& shape, mlir::Type type, vpux::VPUIP::MemoryLocation location) {
    ...
    auto function = builder.create<mlir::FuncOp>(
        ...
        llvm::formatv("continued_conv_{0}_{1}_{2}", inputType, weightsType, outputType).str(),
        funcType, builder.getStringAttr("private")
    );

    auto functionBuilder = mlir::OpBuilder::atBlockBegin(function.addEntryBlock(), builder.getListener());

    const auto getCMXTensor = [&builder, &functionBuilder, getMemRef](const llvm::SmallVector<std::int64_t>& shape, mlir::Type type, std::size_t offset) {
        const auto CMXType = getMemRef(shape, type, vpux::VPUIP::MemoryLocation::VPU_CMX_NN);
        return functionBuilder.create<vpux::VPUIP::DeclareTensorOp>(builder.getUnknownLoc(), CMXType, vpux::VPUIP::MemoryLocation::VPU_CMX_NN, 0, offset);
    };

    const auto getMACAccTensor = [&builder, &functionBuilder, getMemRef](const llvm::SmallVector<std::int64_t>& shape, mlir::Type type, std::size_t offset) {
        const auto MACAccType = getMemRef(shape, type, vpux::VPUIP::MemoryLocation::MAC_Accumulators);
        return functionBuilder.create<vpux::VPUIP::DeclareTensorOp>(builder.getUnknownLoc(), MACAccType, vpux::VPUIP::MemoryLocation::MAC_Accumulators, 0, offset);
    };
    ...
    // Weights partial 0
    const auto weightsPartialParamType = getMemRef(weightsPartialShape, weightsType, vpux::VPUIP::MemoryLocation::VPU_CMX_NN);
    auto weightsPartial0Values = splitWeightsOverC(weightsValues, weightsShape, weightsType, builder.getContext(), /*startC*/0, /*endC*/weightsPartialShape[1]);
    ...
    auto weightsPartial0DDR = functionBuilder.create<Const::DeclareOp>(builder.getUnknownLoc(), weightsPartialParamType, weightsPartial0Attribute.reorder(DimsOrder::NHWC));

    // Weights partial 1
    auto weightsPartial1Values = splitWeightsOverC(weightsValues, weightsShape, weightsType, builder.getContext(), /*startC*/weightsPartialShape[1], /*endC*/2*weightsPartialShape[1]);
    ...
    auto weightsPartial1DDR = functionBuilder.create<Const::DeclareOp>(builder.getUnknownLoc(), weightsPartialParamType, weightsPartial1Attribute.reorder(DimsOrder::NHWC));
    ...
    const auto weightsTable0 = vpux::VPUIP::NCESparsity::getWeightsTable(
        ...
        static_cast<std::int32_t>(WEIGHTS_PARTIAL_0_CMX_OFFSET),
        static_cast<std::int32_t>(weightsPartialShape[1] * weightsPartialShape[2] * weightsPartialShape[3] * getElemTypeSize(weightsType).count() / 8),
        static_cast<std::int32_t>(16777215),
        vpux::VPUIP::ArchKind::MTL,
        inputType,
        weightsType,
        outputType
    );
    ...
    const auto weightsTable0Values = mlir::DenseElementsAttr::get(weightsTableDDRType, llvm::makeArrayRef<std::int32_t>(weightsTable0));
    auto weightsTable0DDR = functionBuilder.create<vpux::Const::DeclareOp>(builder.getUnknownLoc(), weightsTableDDRMemRef, vpux::Const::ContentAttr::get(weightsTable0Values).reorder(vpux::DimsOrder::NHWC));
    ...
    const auto weightsTable1 = vpux::VPUIP::NCESparsity::getWeightsTable(
        outputShape[1],
        [](std::int64_t) { return 0.0f; },
        static_cast<std::int32_t>(WEIGHTS_PARTIAL_1_CMX_OFFSET),
        static_cast<std::int32_t>(weightsPartialShape[1] * weightsPartialShape[2] * weightsPartialShape[3] * getElemTypeSize(weightsType).count() / 8),
        static_cast<std::int32_t>(16777215),
        vpux::VPUIP::ArchKind::MTL,
        inputType,
    ...
    const auto weightsTable1Values = mlir::DenseElementsAttr::get(weightsTableDDRType, llvm::makeArrayRef<std::int32_t>(weightsTable1));
    auto weightsTable1DDR = functionBuilder.create<vpux::Const::DeclareOp>(builder.getUnknownLoc(), weightsTableDDRMemRef, vpux::Const::ContentAttr::get(weightsTable1Values).reorder(vpux::DimsOrder::NHWC));
    ...
    // Input DMAs
    functionBuilder.create<NNDMAOp>(builder.getUnknownLoc(), functionInput, inputCMX.getOperation()->getResult(0), mlir::ValueRange(), barriers[0], false);
    functionBuilder.create<NNDMAOp>(builder.getUnknownLoc(), weightsPartial0DDR.getOperation()->getResult(0), weightsPartial0CMX.getOperation()->getResult(0), mlir::ValueRange(), barriers[0], false);
    functionBuilder.create<NNDMAOp>(builder.getUnknownLoc(), weightsPartial1DDR.getOperation()->getResult(0), weightsPartial1CMX.getOperation()->getResult(0), mlir::ValueRange(), barriers[0], false);
    functionBuilder.create<NNDMAOp>(builder.getUnknownLoc(), weightsTable0DDR.getOperation()->getResult(0), weightsTable0CMX.getOperation()->getResult(0), mlir::ValueRange(), barriers[0], false);
    functionBuilder.create<NNDMAOp>(builder.getUnknownLoc(), weightsTable1DDR.getOperation()->getResult(0), weightsTable1CMX.getOperation()->getResult(0), mlir::ValueRange(), barriers[0], false);
    ...
    auto nceTask_0 = functionBuilder.create<NCEClusterTaskOp>(
        builder.getUnknownLoc(),
        inputPartial0CMX.memory(),
        weightsPartial0CMX.memory(),
        weightsTable0CMX.memory(),
    ...
    const auto end = getIntArrayAttr(builder.getContext(), std::vector<std::int64_t>{outputShape[3] - 1, outputShape[2] - 1, outputShape[1] - 1});
    const auto pad = vpux::VPUIP::PaddingAttr::get(
        vpux::getIntAttr(builder, paddings[PAD_NCETASK_LEFT]),
    ...
    auto nceTask_1 = functionBuilder.create<NCEClusterTaskOp>(
        builder.getUnknownLoc(),
        inputPartial1CMX.memory(),
        weightsPartial1CMX.memory(),
        weightsTable1CMX.memory(),
        /*activation_window=*/nullptr,
        inputPartial1CMX.memory(),
        output1CMX.memory(),
        output1CMX.memory(),
    ...
    functionBuilder.create<NNDMAOp>(builder.getUnknownLoc(), output1CMX.getOperation()->getResult(0), functionOutput, barriers[2], mlir::ValueRange(), false);
    ...
    pm.addPass(vpux::VPUIP::createSetCompileParamsPass(vpux::VPUIP::ArchKind::MTL, vpux::VPUIP::CompilationMode::ReferenceHW, None, log));

    VPUX_THROW_UNLESS(mlir::succeeded(pm.run(module)), "Compilation failed");

    buildCNNOp(builder, function.getName(), {getTensorType(inputShape, inputType, vpux::DimsOrder::NHWC)}, {getTensorType(outputShape, outputType, vpux::DimsOrder::NHWC)});
}
