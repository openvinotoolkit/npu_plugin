...
__kernel void reduction_mean(const __global half*  restrict src,
                            ...
                            __local half* restrict src_line)
{
    __global float* restrict mean = buffer;
    __global float* restrict variance = buffer + get_global_size(1) * get_global_size(2);
...
__kernel void mvn_scale(const __global half * restrict src,
                          ...
                          __local half * restrict dst_line)
{
    __global float* restrict mean_part = buffer;
    __global float* restrict power_mean = buffer + get_global_size(1) * get_global_size(2);
