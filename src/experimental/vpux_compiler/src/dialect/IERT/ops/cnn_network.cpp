...
mlir::LogicalResult vpux::IERT::CNNNetworkOp::verifySymbolUses(mlir::SymbolTableCollection& symbolTable) {
    ...
    auto inputsInfo = to_vector<1>(this->inputsInfo().getOps<IERT::DataInfoOp>());
    auto outputsInfo = to_vector<1>(this->outputsInfo().getOps<IERT::DataInfoOp>());

    auto netFuncType = netFunc.getType();
    ...
    if (netFuncType.getNumResults() != 0) {
        return printTo(emitError(), "'{0}' entryPoint '@{1}' can't have results, got '{2}'", getOperationName(),
                       entryPoint(), netFuncType.getNumResults());
    }

    for (const auto& p : zip(netFuncType.getInputs(), concat<IERT::DataInfoOp>(inputsInfo, outputsInfo)) | indexed) {
        const auto runtimeType = std::get<0>(p.value()).dyn_cast<mlir::MemRefType>();

        if (runtimeType == nullptr) {
            return printTo(emitError(), "'{0}' entryPoint '@{1}' input #{2} is not a 'MemRefType'", getOperationName(),
                           entryPoint(), p.index());
        }
        ...
        if ((checked_cast<unsigned>(runtimeType.getRank()) != userLayout.getNumDims()) &&
            ...
            return printTo(emitError(),
                           ...
                           getOperationName(), entryPoint(), p.index(),
                           p.index() < inputsInfo.size() ? "input" : "output", userLayout);
        }
    }
...
DimsOrder vpux::IERT::DataInfoOp::getDimsOrder() {
