...
mlir::LogicalResult vpux::IE::ProposalOp::inferReturnTypeComponents(
    ...
    auto inType = proposal.class_probs().getType().cast<mlir::RankedTensorType>();

    // out shape must be [batch_size * post_nms_topn, 5]
    std::vector<int64_t> outShape{inType.getShape().front() * proposal.proposal_attrs().postNmsTopN().getInt(), 5};
