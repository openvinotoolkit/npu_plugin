...
mlir::LogicalResult vpux::IE::LRNOp::inferReturnTypeComponents(
    ...
    auto inType = lrn.input().getType().cast<mlir::RankedTensorType>();
    inferredReturnShapes.emplace_back(inType.getShape(), inType.getElementType());

    return mlir::success();
}

SmallVector<mlir::Value, 4> vpux::IE::LRNOp::getInputs() {
    return {input()};
}

SmallVector<mlir::Value, 1> vpux::IE::LRNOp::getOutputs() {
    return {output()};
}
