...
KmbNNFlicExecutor::KmbNNFlicExecutor(const vpux::NetworkDescription::Ptr& networkDescription,
...
KmbNNFlicExecutor::~KmbNNFlicExecutor() { deallocateGraph(); }
...
void KmbNNFlicExecutor::allocateGraph(const std::vector<char>& graphFileContent) {
    if (!_config.useKmbExecutor()) {
        return;
    }
...
Blob::Ptr KmbNNFlicExecutor::prepareInputForInference(
...
void KmbNNFlicExecutor::push(const InferenceEngine::BlobMap& /*inputs*/, const vpux::PreprocMap& /*preProcMap*/) {
    THROW_IE_EXCEPTION << NOT_IMPLEMENTED_str;
}

void KmbNNFlicExecutor::push(const InferenceEngine::BlobMap& inputs) {
    if (!_config.useKmbExecutor()) {
        return;
    }
    ...
    _logger->info("KmbNNFlicExecutor::push started");
    ...
    *_inferenceId = 1;
    ...
    _logger->info("KmbNNFlicExecutor::push finished");
#else
    UNUSED(inputs);
#endif
}

uint32_t KmbNNFlicExecutor::extractPhysAddrForInference(const BlobMap& inputs) {
    uint32_t physAddr = 0;
    if (inputs.size() == 1) {
...
void KmbNNFlicExecutor::pull(InferenceEngine::BlobMap& outputs) {
    if (!_config.useKmbExecutor()) {
        return;
    }
    ...
    _logger->info("KmbNNFlicExecutor::pull started");
    ...
    _logger->info("KmbNNFlicExecutor::pull finished");
#else
    UNUSED(outputs);
#endif
}

BlobMap KmbNNFlicExecutor::extractOutputsFromPhysAddr(uint32_t physAddr) {
...
void KmbNNFlicExecutor::repackDeviceOutputsToNetworkOutputs(
...
void KmbNNFlicExecutor::setup(const InferenceEngine::ParamMap&) { THROW_IE_EXCEPTION << "Not implemented"; }

bool KmbNNFlicExecutor::isPreProcessingSupported(const InferenceEngine::PreProcessInfo&) const { return false; }

std::map<std::string, InferenceEngine::InferenceEngineProfileInfo> KmbNNFlicExecutor::getLayerStatistics() {
    THROW_IE_EXCEPTION << "Not implemented";
    return std::map<std::string, InferenceEngine::InferenceEngineProfileInfo>();
}

InferenceEngine::Parameter KmbNNFlicExecutor::getParameter(const std::string&) const {
