...
namespace mv
{
    ...
    namespace op_spacetodepth
    {
            ...
            std::string& errMsg) -> std::pair<bool, std::size_t>
        {
            ...
            {
                errMsg = "Attempt to set unsupported SpaceToDepth mode: " + mode
    ...
    namespace op {
        MV_REGISTER_OP(SpaceToDepth)
        .setInputs({"data"})
        .setOutputs({"output"})
        .setOptionalArg<std::string>("mode", op_spacetodepth::defaultMode)
        .setArg<uint32_t>("block_size")
        .setInputCheck(op_spacetodepth::inputCheckFcn)
        .setOutputDef(op_spacetodepth::outputDefFcn)
