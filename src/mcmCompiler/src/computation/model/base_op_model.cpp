...
mv::Data::OpListIterator mv::BaseOpModel::cloneOp(
    mv::Data::OpListIterator original_op) {
  //TODO(vamsikku): throw and exception if this is not an implicit op //
  std::string name = original_op->getName() + "_cloned";
  std::string opType = original_op->getOpType();
...
bool mv::BaseOpModel::pathSplit(Data::OpListIterator u, Data::OpListIterator v) {
  ...
  // chains are not split so need cleanup //
  return path_splitter.split(u, v, all_op_selector_t());
}

bool mv::BaseOpModel::getImplicitPath(
    Data::OpListIterator u, Data::OpListIterator v,
...
bool mv::BaseOpModel::pathSplit(mv::Data::OpListIterator u,
      ...
      const NodeSelector& node_selector) {
  typedef mv::Path_Splitter<mv::BaseOpModel, mv::OpModel_Path_Update_Traits>
      path_splitter_t;

  //STEP-0: collect all the original parents //
  std::set<std::string> original_parent_ops;
  for (auto vitr=vbegin; vitr!=vend; ++vitr) {
    for (auto pitr=(*vitr).leftmostParent(); pitr!=this->opEnd(); ++pitr) {
      original_parent_ops.insert( pitr->getName() );
    }
  }

  //STEP-1: do path splitting //
  path_splitter_t path_splitter(*this);
  bool dont_split_chains = false;
  bool erase_only_cloned_edge_at_end = true;
  bool some_paths_split = false;
  for (auto vitr=vbegin; vitr!=vend; ++vitr) {
    some_paths_split |= path_splitter.split(u, (*vitr), dont_split_chains,
        node_selector, erase_only_cloned_edge_at_end);
  }

  //STEP-2: clean up model by removing ops which got disconnected
  //due to pathsplit
  if (some_paths_split) {
    // do cleanup //
    cleanUpAfterPathSplit(original_parent_ops.begin(),
          original_parent_ops.end());
  }
  return some_paths_split;
}
...
bool mv::BaseOpModel::pathSplit(mv::Data::OpListIterator u,
  ...
  return pathSplit(u, vbegin, vend, all_op_selector_t());
}

template<typename OpSubsetIterator>
bool mv::BaseOpModel::pathSplitImplicit(mv::Data::OpListIterator u,
      OpSubsetIterator vbegin, OpSubsetIterator vend) {
  typedef mv::Path_Splitter<mv::BaseOpModel, mv::OpModel_Path_Update_Traits>
      path_splitter_t;
  typedef typename path_splitter_t::traits::implicit_op_selector_t
...
//NOTE: if you want additional container iterator just add a specialization here
typedef typename std::list<mv::Data::OpListIterator>::iterator list_iterator_t;
template
bool mv::BaseOpModel::pathSplit< list_iterator_t >(mv::Data::OpListIterator u,
    list_iterator_t, list_iterator_t);

typedef typename std::vector<mv::Data::OpListIterator>::iterator
  vector_iterator_t;
template
bool mv::BaseOpModel::pathSplit< vector_iterator_t >(mv::Data::OpListIterator u,
    vector_iterator_t, vector_iterator_t);

template
bool mv::BaseOpModel::pathSplitImplicit< list_iterator_t >(
    mv::Data::OpListIterator u, list_iterator_t, list_iterator_t);

template
bool mv::BaseOpModel::pathSplitImplicit< vector_iterator_t >(
    mv::Data::OpListIterator u, vector_iterator_t, vector_iterator_t);
...
void mv::BaseOpModel::addNetworkOutput(mv::Data::OpListIterator outputOp)
{
    if (!outputOp)
        throw ArgumentError(*this, "baseOpModel", "invalid", "Input argument is null");
