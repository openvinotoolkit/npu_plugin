...
static const size_t minWeightsPerClusterPerChainConstant = 66560;  // This value was derived from emperical testing
...
MV_REGISTER_PASS(ValidateStreamingOverKForChainPipelining)
        ...
        .setDescription("This pass evaluates and re-calculates streams over K to enable better performance when "
                        "combined with chained pipelining in the scheduler");
}
}  // namespace mv

size_t perClusterWeightsSize(mv::ComputationModel& model, mv::Op& op, const mv::Attribute& clustering,
                             bool weightsSparsity, const mv::Shape& streamConfig, size_t nClusters) {
    auto div = [](unsigned x, unsigned y) -> unsigned {
        return (x + y - 1) / y;
    };

    auto globalParams = model.getGlobalConfigParams();
    auto enableChannelMajorConv = globalParams->get<bool>("enable_channel_major_conv");
    size_t inputSize = 0;
    size_t outputSize = 0;
    size_t weightSize = 0;
    ...
    if (clusterStrategy == "SplitOverK") {
        alignedSplittedChannels = mv::round_up(alignedSplittedChannels / nClusters, 16);
    }
    ...
    if (opType == "Conv" || opType == "DepthwiseConv") {
        ...
        if (opType == "Conv") {
            ...
            if (clusterStrategy == "SplitOverK")
                weightSize = div(weightSize, nClusters);
        }
...
std::vector<mv::Attribute> getStrategiesfromCompilationDescriptor(
        std::unordered_map<std::string, StrategySet>& strategies, mv::Data::OpListIterator opIt, std::string strategy) {
...
std::map<size_t, size_t> getMinWeightsPerClusterSizePerChain(std::list<subgraph_t>& chainSubgraphs,
                ...
                for (auto layerNameStrategy : streamingStrategyList) {
                    ...
                    if (nodeName == op->getName()) {
                        ...
                        if (isKStreaming && mcStrategy == "SplitOverK" && !isHStreaming) {
                            ...
                            weightsPerCluster =
                                    perClusterWeightsSize(model, *oitr, clustering, weightsSparsity,
        ...
        for (auto& op : chain_subgraph.dpu_chain_) {
            ...
            if (opIt->getOpType() == "Conv") {
                ...
                maxpossibleStreams = floor(alignedFullOutputChannels /
                                           minOutputChannels[graphOptimizerMultiClusterStrategy.get<std::string>()]);
                ...
                if (isKStreaming && graphOptimizerMultiClusterStrategy.get<std::string>() == "SplitOverK" &&
                    ...
                    if (minWeightsPerClusterPerChain[chainID] > 0)
                        fullWeightsSizeOptimalKStreaming = fullWeightsSizeForOpandOptimalKStreaming(
                                ...
                                minWeightsPerClusterPerChain[chainID], isKStreaming,
                                graphOptimizerStreamingStrategy[3].get<int>("K"), nClusters);
...
void validateStreamingOverKForChainPipeliningFnc(const mv::pass::PassEntry& pass, mv::ComputationModel& model,
    ...
    if (mv::isDebugFilesEnabled()) {
        fptr = fopen("weight_streaming.txt", "w");
        if (nullptr == fptr) {
            throw mv::RuntimeError("Streaming", "Cannot open file for writing");
        }
    }
    ...
    auto minWeightsPerClusterPerChain =
    ...
    if (!minWeightsPerClusterPerChain.empty()) {
        ...
        saveNewStreamingStrategiesToJson(pass, newStreamingStrategies,
                                         "Streaming_activations_and_weights_performance_strategies");
    }
}
