...
//                 if (op.getName() == "yolov2/darknet19_model/conv14/bn/FusedBatchNorm/variance/Fused_Add_") {
//                         op.set<unsigned>("optimalNumberOfKStreams", 8);
//                     }
        ...
        public:
           ...
           // First, just enough to fit in cmx. Second, enough to enable pipelining.
            ...
            double transitionCost(Op& parentOp,Op& childOp,StrategySet& parent,StrategySet& child)
            {
                ...
                double schedulabilityMultiplier = 1.0;

                auto kStreams = childStreamShape["K"];

                auto compPerStream = ((double) cFullComp / kStreams);
                auto prefetch = std::min(pFullComp, cWeightDma);
                ...
                auto pre_cost = cWeightDma - prefetch + cFullComp;
                ...
                {
                    cost = prepipe_cost;
                    schedulabilityMultiplier = isSchedulerFriendly(childOp, child) ? 0.85 : 0.9;
                }
                else if(pipelineable)
                {
                    cost = pipe_cost;
                ...
                cost = (cost + heuristics) * schedulabilityMultiplier;
