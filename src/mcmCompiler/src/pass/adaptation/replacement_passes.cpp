...
namespace mv
{
    ...
    namespace pass
    {
        ...
        .setDescription(
            "Handles compatibility for input/output order by adding permute op if needed. Also takes into consideration if C-major convs are present."
        );
    }
        ...
        for(const auto& outputTensor : inTens)
        {
            ...
            if(inputOrder != mv::Order("NCHW"))
            {
                ...
                if(!flowsToAdd.empty())
                {
                    ...
                    auto outputTensorQuant = outputTensor->getQuantParams();
                    if(!outputTensorQuant.isEmpty())
