...
std::tuple<int,int, int, int, int> getGlobalCompilationDescriptorConf(const mv::pass::PassEntry& pass, mv::ComputationModel& model) {
    ...
    if (globalParams->hasAttr("VPUX30XXChannelPadding"))
        pad = globalParams->get<int>("VPUX30XXhannelPadding");
    ...
    for (auto opIt = om.opBegin(); opIt != om.opEnd(); ++opIt)
    {
        if (opIt->getOpType() == "DPUTask")
        {
            ...
            if((inputDType != outputDType) && outputDType != mv::DType("Int32") &&
                opIt->isHardwarizable() && !opIt->isEltwiseTypeOp() &&
            ...
            for(clusterNumber = 0; clusterNumber < nClusters; clusterNumber++)
            {
                ...
                for(auto nWorkloads : nWorkloadsSplitPool)
                {
                    ...
                    for (std::string algorithm : algorithms)
                    {
                        ...
                        if (algorithm == "Z-Tiling" && !opIt->isEltwiseTypeOp()
