...
do
    ...
    if [[ "$pipe_start" == "comp" ]] || [[ $proc -ne 0 ]]  ; then
        ...
        cp "${TENSOR_TMP_DIR}/VPUX30XX.blob" "${TEST_CASE_DIR}/"
        ...
        sed -i 's/VPUX30XX.blob/mcm.blob/g' "${TEST_CASE_DIR}/Makefile"
    ...
    if [[ "$pipe_start" == "app" ]] || [[ $proc -ne 0 ]]  ; then
        ...
        #make run MV_SOC_REV=ma2490 GRAPH_BIN=VPUX30XX.blob GRAPH_BIN_PATH=."
