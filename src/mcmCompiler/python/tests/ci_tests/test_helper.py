...
def compile_graphFile(model, nClusters, nDPU, strategyFile, cmx=4*918, emulator=True, cpp=False):
    ...
    graphFile = out_folder+"VPUX30XX.blob"   # Ideally returned from Fathom
...
def execute_network(gf):
    ...
    generated_files = ["VPUX30XX.blob", "input.dat", "NCE2Task_network_out.bin", "output.dat", "expected_result_sim.dat"]
    ...
    print("Copy:", gf, "to", APP_ROOT + "VPUX30XX.blob")
    copyfile(gf, APP_ROOT + "VPUX30XX.blob")
    ...
    command = "make run -j MV_SOC_REV=ma2490 MV_TOOLS_VERSION="+mvToolsV+" GRAPH_BIN=VPUX30XX.blob GRAPH_BIN_PATH=. "+sPort + sIP
