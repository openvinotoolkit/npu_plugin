...
typedef enum { WRITE_TO_BUFFER, WRITE_TO_FILE } huffmanOutputDataRouting_t;

typedef enum { READ_FROM_BUFFER, READ_FROM_FILE } huffmanInputDataRouting_t;
...
struct Symbol {
    ...
    explicit Symbol(const char cSymbol, int occurrencesNum)
            : symbol(std::string(1, cSymbol)), occurrences(occurrencesNum) {
    }
     ...
     * @param [in] occurrencesNum - Number of occurrences of the symbol.
     */
    explicit Symbol(const std::string& sSymbol, int occurrencesNum): symbol(sSymbol), occurrences(occurrencesNum) {
    }
...
struct HuffmanTuple_t {
     ...
     *                               -1 if the node is a leaf.
     */
    explicit HuffmanTuple_t(char cSymbol, int occurrencesNum, int nodeIndex, int leftSonNode, int rightSonNode) {
        *this = HuffmanTuple_t(std::string(1, cSymbol), occurrencesNum, nodeIndex, leftSonNode, rightSonNode);
    }
     ...
     *                               -1 if the node is a leaf.
     */
    explicit HuffmanTuple_t(const std::string& sSymbol, int occurrencesNum, int nodeIndex, int leftSonNode,
                            int rightSonNode)
            : symbol(sSymbol),
    ...
    bool operator==(const HuffmanTuple_t& b) const {
        return (symbol == b.symbol && occurrences == b.occurrences && index == b.index && leftSon == b.leftSon &&
                rightSon == b.rightSon);
    }
...
private:
    ...
    std::priority_queue<HuffmanTuple_t, std::vector<HuffmanTuple_t>, std::greater<HuffmanTuple_t>> heap;
    ...
    void constructOneSidedTree(std::vector<int>& leaves, HuffmanTuple_t& node, int level, int value);
...
public:
     ...
     */
    int writeEncodedDataToFile(const void* data, int length, const std::string& filename, bool bypass, bool statsOnly);
     ...
     * return               - 0 on success, negative value otherwise.
     */
    int writeEncodedData(const void* data, int length, const std::string& outputDataFileName,
     ...
     * return              - 0 on success, negative value otherwise.
     */
    int readEncodedDataFromFile(const std::string& srcFile, const std::string& dstFile);
     ...
     */
    uint32_t readEncodedData(const std::string& srcFile, const std::string& dstFile, uint8_t* inputDataBuffer,
                             const uint32_t& inputBufferLength, uint8_t* outputDataBuffer,
                             huffmanInputDataRouting_t inputDataRouting, huffmanOutputDataRouting_t outputDataRouting);
