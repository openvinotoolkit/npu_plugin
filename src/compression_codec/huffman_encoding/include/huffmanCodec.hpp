...
class huffmanCodec {
public:
    huffmanCodec(uint32_t pBitsPerSym = 8, uint32_t pMaxEncSyms = 16, uint32_t pVerbosity = 0,
                 uint32_t pBlockSizeBytes = 4096, bool pStatsOnly = false, bool pBypass = false,
    ...
    uint32_t huffmanCodecCompressArray(unsigned char* inputData, int inputDataLength, unsigned char* outputData,
                                       int /*outputDataLength*/) {
        return this->huffmanCodecCompressArray((uint32_t&)inputDataLength, inputData, outputData);
    }

    uint32_t huffmanCodecDecompressArray(unsigned char* inputData, int inputDataLength, unsigned char* outputData,
                                         int /*outputDataLength*/) {
        return this->huffmanCodecDecompressArray((uint32_t&)inputDataLength, inputData, outputData);
    }

    uint32_t huffmanCodecCompressArray(uint32_t& inputDataLength, uint8_t* inputData, uint8_t* outputData);

    uint32_t huffmanCodecDecompressArray(uint32_t& inputDataLength, uint8_t* inputData, uint8_t* outputData);
...
private:
    ...
    vector<char> getInputDataWithExclusions(const uint32_t& inputDataLength, const uint8_t* inputData,
