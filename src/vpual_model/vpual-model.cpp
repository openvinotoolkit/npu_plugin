...
public:
    ...
    void sendRequest(const void* data, uint32_t size) {
        ...
        if (msgType == VpualCmdHeader_t::DECODER_CREATE) {
            ...
            if (currentState == GraphManagerMethod::NN_GRAPH_ALLOCATE &&
                ...
                const BlobHandle_t* blobHandle =
                    reinterpret_cast<const BlobHandle_t*>(reinterpret_cast<const char*>(data) + sizeof(uint32_t));
    ...
    uint32_t getCurrentState() { return currentState; }
            ...
            *stubID = decoders.size();
            ...
            case GraphManagerMethod::NN_GRAPH_CHECK_AVAILABLE: {
                ...
                *status = GraphStatus::No_GraphId_Found;
                logger->debug(
                    "Send response GraphStatus::No_GraphId_Found on GraphManagerMethod::NN_GRAPH_CHECK_AVAILABLE");
                break;
            }
            default:
