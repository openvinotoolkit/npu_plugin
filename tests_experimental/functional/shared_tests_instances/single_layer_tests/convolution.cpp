...
const std::vector<InferenceEngine::SizeVector> targetReshapeShapes = {
    {2, 3, 10, 10, 10}, {1, 3, 12, 12, 12}, {2, 3, 12, 13, 14}};

/* ============= 2D Convolution Reshape ============= */
const auto convReshape = ::testing::Combine(::testing::Values(InferenceEngine::SizeVector({3, 3, 3})),
    ::testing::Values(InferenceEngine::SizeVector({1, 1, 1})), ::testing::Values(std::vector<ptrdiff_t>({0, 0, 0})),
    ::testing::Values(std::vector<ptrdiff_t>({0, 0, 0})), ::testing::Values(InferenceEngine::SizeVector({1, 1, 1})),
    ::testing::Values(5), ::testing::ValuesIn(padTypes));

const auto convReshapefullParamsProduct = ::testing::Combine(convReshape,
    ::testing::Values(InferenceEngine::Precision::FP32), ::testing::Values(InferenceEngine::Precision::FP32),
    ::testing::Values(InferenceEngine::SizeVector({1, 3, 10, 10, 10})), ::testing::ValuesIn(targetReshapeShapes),
    ::testing::Values(CommonTestUtils::DEVICE_KEEMBAY));

INSTANTIATE_TEST_CASE_P(
    Reshape, ConvolutionLayerTest, convReshapefullParamsProduct, ConvolutionLayerTest::getTestCaseName);

const auto convReshapeNegativefullParamsProduct = ::testing::Combine(convReshape,
    ::testing::Values(InferenceEngine::Precision::FP32), ::testing::Values(InferenceEngine::Precision::FP32),
...
INSTANTIATE_TEST_CASE_P(ReshapeNegative, NegativeReshapeConvolutionSingleLayerTest,
    convReshapeNegativefullParamsProduct, ConvolutionLayerTest::getTestCaseName);
