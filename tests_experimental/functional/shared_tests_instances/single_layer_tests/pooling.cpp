...
using namespace CommonTestUtils::NGraphFuncBuilders;
...
const std::vector<InferenceEngine::SizeVector> strides = {{1, 1},
                                                          {1, 2}};
const std::vector<InferenceEngine::SizeVector> padBegins = {{0, 0},
                                                            {0, 2}};
const std::vector<InferenceEngine::SizeVector> padEnds = {{0, 0},
                                                          {0, 2}};
...
const auto maxPool_ExplicitPad_FloorRounding_Params = ::testing::Combine(
        ::testing::Values(Pooling::PoolingTypes::MAX),
...
const auto avgPoolExplicitPadCeilRoundingParams = ::testing::Combine(
        ::testing::Values(Pooling::PoolingTypes::AVG),
        ::testing::ValuesIn(kernels),
        // TODO: Non 1 strides fails in ngraph reference implementation with error "The end corner is out of bounds at axis 3" thrown in the test body.
        ::testing::Values(InferenceEngine::SizeVector({1, 1})),
...
const auto allPools_ValidPad_Params = ::testing::Combine(
        ::testing::Values(Pooling::PoolingTypes::MAX, Pooling::PoolingTypes::AVG),
        ::testing::ValuesIn(kernels),
        ::testing::ValuesIn(strides),
        ::testing::Values(InferenceEngine::SizeVector({0, 0})),
        ::testing::Values(InferenceEngine::SizeVector({0, 0})),
...
poolSpecificParams defaulParams(Pooling::PoolingTypes::MAX, {3, 3}, {1, 1}, {0, 0}, {0, 0},
                                ngraph::op::RoundingType::FLOOR, ngraph::op::PadType::EXPLICIT,
                                false);

INSTANTIATE_TEST_CASE_P(Reshape, PoolingLayerTest,
                        ::testing::Combine(
                                ::testing::Values(defaulParams),
                                ::testing::Values(InferenceEngine::Precision::FP32),
                                ::testing::Values(InferenceEngine::Precision::FP32),
                                ::testing::Values(InferenceEngine::SizeVector({1, 3, 10, 10})),
                                ::testing::ValuesIn({
                                                            InferenceEngine::SizeVector({2, 3, 10, 10}),
                                                            InferenceEngine::SizeVector({1, 3, 10, 10}),
                                                            InferenceEngine::SizeVector({1, 3, 30, 30}),
                                                            InferenceEngine::SizeVector({2, 3, 30, 30}),
                                                            InferenceEngine::SizeVector({10, 10, 30, 30}),
                                                    }),
                                ::testing::Values(true, false),
                                ::testing::Values("KMB")
                        ),
