...
int main()
{
    ...
    // Define tensors
    std::vector<half> bboxPredData = mv::utils::generateSequence<double> (14*14*48*1);
    std::vector<half> imageInfoData({224.0,224.0,1.0});
    std::vector<half> scaleData({0.25,0.5,1.0,2.0});
    std::vector<half> ratioData({0.5,1.0,2.0});

    auto cls_pred = om.input({14,14,24,1}, mv::DType("Float16"), mv::Order::getZMajorID(4), {{0},{1.0},{-inf},{inf}}, "cls_pred0");
    auto bbox_pred = om.constant(bboxPredData, {14,14,48,1}, mv::DType("Float16"), mv::Order::getZMajorID(4), {{0},{1.0},{-inf},{inf}}, "bbox_pred0");
    auto im_info = om.constant(imageInfoData, {1,1,1,imageInfoData.size()}, mv::DType("Float16"), mv::Order::getZMajorID(4), {{0},{1.0},{-inf},{inf}}, "im_info0");
    auto scale = om.constant(scaleData, {1,1,1,scaleData.size()}, mv::DType("Float16"), mv::Order::getZMajorID(4), {{0},{1.0},{-inf},{inf}}, "scale0");
    auto ratio = om.constant(ratioData, {1,1,1,ratioData.size()}, mv::DType("Float16"), mv::Order::getZMajorID(4), {{0},{1.0},{-inf},{inf}}, "ratio0");
