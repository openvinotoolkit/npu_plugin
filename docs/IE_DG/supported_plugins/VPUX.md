...
| `PERFORMANCE_HINT`           | `LATENCY`/`THROUGHPUT`                                                                       |none              |Sets number of clusters and throughput streams if not manually set. `LATENCY`: 4 clusters, 3 streams; `THROUGHPUT`: 1 cluster, 6 streams|
