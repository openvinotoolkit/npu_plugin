...
int main(int argc, char* argv[]) {
    ...
    if (argc != 2) {
        std::cout << "Example usage is ./act-kernels-elf-poc <path-to-act-kernel-elf>" << '\n';
        return 1;
    }
    ...
    dmaTask.output.location.memLocation = vpux::VPUIP::MemoryLocation::VPU_DDR_Heap;
    dmaTask.output.location.locationIndex = 0;
    dmaTask.output.offset = 0;

    dmaTask2.input.location.memLocation = vpux::VPUIP::MemoryLocation::VPU_DDR_Heap;
    ...
    blobSerializer.write("nn_sw_blob.elf");
