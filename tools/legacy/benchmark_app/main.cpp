...
T getMedianValue(const std::vector<T>& vec) {
    ...
    return (sortedVec.size() % 2 != 0)
                   ...
                   : (sortedVec[sortedVec.size() / 2ULL] + sortedVec[sortedVec.size() / 2ULL - 1ULL]) /
                             static_cast<T>(2.0);
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (!FLAGS_report_type.empty()) {
            statistics = std::make_shared<StatisticsReport>(
            ...
            auto setThroughputStreams = [&]() {
                ...
                if (device_nstreams.count(device)) {
                    ...
                    auto supported_config_keys =
                            ie.GetMetric(device, METRIC_KEY(SUPPORTED_CONFIG_KEYS)).as<std::vector<std::string>>();
        ...
        if (FLAGS_load_from_file && !isNetworkCompiled) {
            ...
            app_inputs_info =
                    getInputsInfo<InputInfo::CPtr>(FLAGS_shape, FLAGS_layout, FLAGS_b, exeNetwork.GetInputsInfo());
            if (batchSize == 0) {
                batchSize = 1;
            }
        }
