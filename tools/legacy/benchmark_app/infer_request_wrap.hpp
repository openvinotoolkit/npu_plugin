...
public:
    ...
    explicit InferReqWrap(InferenceEngine::ExecutableNetwork& net, size_t id, Cb&& callbackQueue)
            : _request(net.CreateInferRequest()), _id(id), _callbackQueue(std::forward<Cb>(callbackQueue)) {
...
public:
    InferRequestsQueue(InferenceEngine::ExecutableNetwork& net, size_t nireq) {
        for (size_t id = 0; id < nireq; id++) {
            requests.push_back(std::make_shared<InferReqWrap>(net, id,
                                                              std::bind(&InferRequestsQueue::putIdleRequest, this,
    ...
    InferRequestsQueue(const InferRequestsQueue&) = delete;
    ...
    InferRequestsQueue& operator=(const InferRequestsQueue&) = delete;
