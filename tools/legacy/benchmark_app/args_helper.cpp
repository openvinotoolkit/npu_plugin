    ...
    try {
        ...
        if (dir_ptr == nullptr) {
            if (err == ENOTDIR) {
                ...
                slog::warn << "Directory " << arg << " cannot be opened! Error: " << strerror(errno) << slog::endl;
            }
            return;
        }

        struct dirent* entry_ptr;
        while (nullptr != (entry_ptr = readdir(dir_ptr))) {
            std::string fileName = entry_ptr->d_name;
            if (fileName == "." || fileName == "..")
                continue;
            files.push_back(arg + "/" + entry_ptr->d_name);
        }
        closedir(dir_ptr);
    } catch (...) {
        if (dir_ptr != nullptr) {
...
InferenceEngine::Precision getPrecision(const std::string& value) {
    static const supported_precisions_t supported_precisions = {
            ...
            {"BF16", InferenceEngine::Precision::BF16}, {"U64", InferenceEngine::Precision::U64},
            {"I64", InferenceEngine::Precision::I64},   {"U32", InferenceEngine::Precision::U32},
            {"I32", InferenceEngine::Precision::I32},   {"U16", InferenceEngine::Precision::U16},
            {"I16", InferenceEngine::Precision::I16},   {"U8", InferenceEngine::Precision::U8},
            {"I8", InferenceEngine::Precision::I8},     {"BOOL", InferenceEngine::Precision::BOOL},
    };
