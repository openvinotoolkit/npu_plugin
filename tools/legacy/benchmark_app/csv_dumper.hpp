...
public:
    ...
    explicit CsvDumper(bool enabled = true, const std::string& name = ""): canDump(enabled) {
        ...
        // cppreference points that std::basic_ios::rdbuf(...),
        // std::basic_ios::clear(...) and std::basic_ios::setstate(...),
        // used inside the std::basic_fstream::open(...), may throw some
        // exceptions.
        try {
            ...
            slog::warn << "Unexpected condition: exception is thrown during opening a file!" << slog::endl;
            canDump = false;
        }
    }
