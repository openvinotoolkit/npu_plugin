...
uint32_t deviceDefaultDeviceDurationInSeconds(const std::string& device) {
    ...
    if (duration == 0) {
        const auto unknownDeviceIt =
                find_if(deviceDefaultDurationInSeconds.begin(), deviceDefaultDurationInSeconds.end(),
...
std::map<std::string, std::string> parseNStreamsValuePerDevice(const std::vector<std::string>& devices,
    ...
    for (auto& device_value_string : device_value_strings) {
        ...
        if (device_value_vec.size() == 2) {
            ...
            if (it != devices.end()) {
                result[std::move(device_name)] = std::move(nstreams);
            } else {
                throw std::logic_error("Can't set nstreams value " + nstreams + " for device '" + device_name +
