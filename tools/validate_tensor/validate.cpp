...
double queryBlob(std::string pathBlob)
{
    std::ifstream infile;
    ...
    double realScale = graphFile_.header->net_output[0]->quant_real_scale[0];
    return realScale;
}
...
int validate(std::string blobPath, std::string expectedPath, std::string actualPath)
{
    ...
    if (dtype.compare("U8")==0)
    {
        ...
        // float qScale = j["header"]["net_output"][0]["quant_real_scale"][0].get<float>();
        double qScale = queryBlob(blobPath);
