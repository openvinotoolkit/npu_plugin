...
int main(int argc, char** argv)
{
    ...
    std::vector<vpux::profiling_task_info> taskProfiling;
    vpux::getTaskProfilingInfo(blob_bin.data(), blob_bin.size(), output_bin.data(), output_bin.size(), 
        taskProfiling, vpux::profiling_task_type::ALL);

    for (auto& task : taskProfiling ) {
        std::string exec_type_str;
        switch (task.exec_type) {
            case vpux::profiling_task_info::exec_type_t::DMA:
                exec_type_str = "DMA";
                std::cout << "Task(" << exec_type_str << "): " << std::setw(50) << task.name << "\tTime: " << (float)task.duration_ns/1000 << std::endl;
                break;
            case vpux::profiling_task_info::exec_type_t::DPU:
                exec_type_str = "DPU";
                std::cout << "Task(" << exec_type_str << "): " << std::setw(50) << task.name << "\tTime: " << (float)task.duration_ns/1000 << std::endl;
                break;
            case vpux::profiling_task_info::exec_type_t::SW:
                exec_type_str = "SW";
    ...
    std::cout << "TotalTime: " << total_time/1000 << "us" << std::endl;
    return 0;
}
