...
bool SortBBoxAscend(const NormalizedBBox& bbox1, const NormalizedBBox& bbox2) { return bbox1.score < bbox2.score; }

bool SortBBoxDescend(const NormalizedBBox& bbox1, const NormalizedBBox& bbox2) { return bbox1.score > bbox2.score; }

bool SortBBoxIndexPairDescend(
    const std::pair<NormalizedBBox, int>& bbox1, const std::pair<NormalizedBBox, int>& bbox2) {
    return SortBBoxDescend(bbox1.first, bbox2.first);
}

template <typename T>
bool SortDetectionResultPairAscend(
    const std::pair<DetectionResult, T>& pair1, const std::pair<DetectionResult, T>& pair2) {
    return pair1.first.mScore < pair2.first.mScore;
}

// Explicit initialization.
template <typename T>
bool SortDetectionResultPairDescend(
    const std::pair<DetectionResult, T>& pair1, const std::pair<DetectionResult, T>& pair2) {
    return pair1.first.mScore > pair2.first.mScore;
}
...
// Explicit initialization.
template bool SortScorePairAscend(const std::pair<float, int>& pair1, const std::pair<float, int>& pair2);
template bool SortScorePairAscend(
    const std::pair<float, std::pair<int, int>>& pair1, const std::pair<float, std::pair<int, int>>& pair2);
...
template <typename T>
bool SortScorePairDescendStable(const std::pair<float, T>& pair1, const std::pair<float, T>& pair2) {
...
// Explicit initialization.
template bool SortDetectionResultPairDescend(
    const std::pair<DetectionResult, int>& pair1, const std::pair<DetectionResult, int>& pair2);
...
template bool SortScorePairDescend(const std::pair<float, int>& pair1, const std::pair<float, int>& pair2);
template bool SortScorePairDescend(
    const std::pair<float, std::pair<int, int>>& pair1, const std::pair<float, std::pair<int, int>>& pair2);

template bool SortScorePairDescendStable(const std::pair<float, int>& pair1, const std::pair<float, int>& pair2);
template bool SortScorePairDescendStable(
    const std::pair<float, std::pair<int, int>>& pair1, const std::pair<float, std::pair<int, int>>& pair2);
...
// Explicit initialization.
template void IntersectBBox(const BBox<float>& bbox1, const BBox<float>& bbox2, BBox<float>* intersect_bbox);
template void IntersectBBox(const BBox<double>& bbox1, const BBox<double>& bbox2, BBox<double>* intersect_bbox);
...
template <typename Dtype>
void ScaleBBox(const BBox<Dtype>& bbox, const int height, const int width, BBox<Dtype>* scale_bbox) {
    scale_bbox->set_xmin(bbox.xmin() * width);
    scale_bbox->set_ymin(bbox.ymin() * height);
    scale_bbox->set_xmax(bbox.xmax() * width);
    scale_bbox->set_ymax(bbox.ymax() * height);
...
// Explicit initialization.
template void ScaleBBox(const BBox<float>& bbox, const int height, const int width, BBox<float>* scale_bbox);
template void ScaleBBox(const BBox<double>& bbox, const int height, const int width, BBox<double>* scale_bbox);
...
bool ProjectBBox_GetRatio(
    const NormalizedBBox& src_bbox, const NormalizedBBox& bbox, NormalizedBBox* proj_bbox, float& ratio) {
    if (bbox.xmin() >= src_bbox.xmax() || bbox.xmax() <= src_bbox.xmin() || bbox.ymin() >= src_bbox.ymax() ||
        bbox.ymax() <= src_bbox.ymin() || bbox.xmax() <= bbox.xmin() || bbox.ymax() <= bbox.ymin()) {
        return false;
    }
...
template <typename Dtype, typename BBoxType>
Dtype JaccardOverlap_impl(const BBoxType& bbox1, const BBoxType& bbox2, const bool normalized) {
...
template <typename Dtype>
Dtype JaccardOverlap(const BBox<Dtype>& bbox1, const BBox<Dtype>& bbox2, const bool normalized) {
    return JaccardOverlap_impl<Dtype, BBox<Dtype>>(bbox1, bbox2, normalized);
}

// Explicit initialization.
template float JaccardOverlap(const BBox<float>& bbox1, const BBox<float>& bbox2, const bool normalized);
template double JaccardOverlap(const BBox<double>& bbox1, const BBox<double>& bbox2, const bool normalized);
...
void DecodeBBox(const NormalizedBBox& prior_bbox, const float* prior_variance, const CodeType code_type,
...
void DecodeBBoxes(const std::vector<NormalizedBBox>& prior_bboxes, const std::vector<float>& prior_variances,
    const CodeType code_type, const bool variance_encoded_in_target, const std::vector<NormalizedBBox>& bboxes,
    ...
    for (int i = 0; i < num_bboxes; ++i) {
        ...
        DecodeBBox(
            prior_bboxes[i], &prior_variances[i * 4], code_type, variance_encoded_in_target, bboxes[i], &decode_bbox);
        (*decode_bboxes)[i] = decode_bbox;
    }
}

void DecodeBBoxesAll(const std::vector<LabelBBox>& all_loc_preds, const std::vector<NormalizedBBox>& prior_bboxes,
    const std::vector<float>& prior_variances, const int num, const bool share_location, const int num_loc_classes,
    const int background_label_id, const CodeType code_type, const bool variance_encoded_in_target,
    ...
    for (int i = 0; i < num; ++i) {
        ...
        for (int c = 0; c < num_loc_classes; ++c) {
            ...
            DecodeBBoxes(prior_bboxes, prior_variances, code_type, variance_encoded_in_target, label_loc_preds,
                &(decode_bboxes[label]));
        }
    }
}
...
void GetLocPredictions(const Dtype* loc_data, const int num, const int num_preds_per_class, const int num_loc_classes,
    ...
    for (int i = 0; i < num; ++i) {
        ...
        if (share_location) {
            label_bbox[-1] =
                BBoxArrayWrapper<Dtype>(&bbox_data[i * num_preds_per_class * num_loc_classes], num_preds_per_class);
        } else {
            for (int c = 0; c < num_loc_classes; ++c) {
                label_bbox[c] = BBoxArrayWrapper<Dtype>(
                    &bbox_data[(i * num_loc_classes + c) * num_preds_per_class], num_preds_per_class);
            }
        }
    }
}
...
template void GetLocPredictions(const float* loc_data, const int num, const int num_preds_per_class,
    const int num_loc_classes, const bool share_location, std::vector<LabelBBox>* loc_preds);
...
template void GetLocPredictions(const float* loc_data, const int num, const int num_preds_per_class,
    ...
    std::vector<std::map<int, BBoxArrayWrapper<float>>>* loc_preds);
template void GetLocPredictions(const double* loc_data, const int num, const int num_preds_per_class,
    const int num_loc_classes, const bool share_location,
    std::vector<std::map<int, BBoxArrayWrapper<double>>>* loc_preds);
...
// Explicit initialization.
template void GetOrientPredictions(const float* orient_data, const int num, std::vector<float>* orient_preds);
template void GetOrientPredictions(const double* orient_data, const int num, std::vector<float>* orient_preds);

template <typename Dtype>
void GetOrientationScores(const Dtype* orient_data, const int num, const int num_priors, const int num_orient_classes,
...
// Explicit initialization.
template void GetOrientationScores(const float* orient_data, const int num, const int num_priors,
    const int num_orient_classes, std::vector<std::vector<std::vector<float>>>* orient_preds);
template void GetOrientationScores(const double* orient_data, const int num, const int num_priors,
    const int num_orient_classes, std::vector<std::vector<std::vector<float>>>* orient_preds);

template void GetOrientationScores(const float* orient_data, const int num, const int num_priors,
    const int num_orient_classes, std::vector<std::vector<ArrayWrapper<float>>>* orient_preds);
template void GetOrientationScores(const double* orient_data, const int num, const int num_priors,
    const int num_orient_classes, std::vector<std::vector<ArrayWrapper<double>>>* orient_preds);
...
template void GetConfidenceScores(const float* conf_data, const int num, const int num_preds_per_class,
    const int num_classes, std::vector<std::map<int, std::vector<float>>>* conf_preds);
template void GetConfidenceScores(const double* conf_data, const int num, const int num_preds_per_class,
    const int num_classes, std::vector<std::map<int, std::vector<double>>>* conf_preds);
template void GetConfidenceScores(const float* conf_data, const int num, const int num_preds_per_class,
    const int num_classes, const bool class_major, std::vector<std::map<int, std::vector<float>>>* conf_preds);
template void GetConfidenceScores(const double* conf_data, const int num, const int num_preds_per_class,
    const int num_classes, const bool class_major, std::vector<std::map<int, std::vector<double>>>* conf_preds);

template <typename Dtype>
void GetMaxConfidenceScores(const Dtype* conf_data, const int num, const int num_preds_per_class, const int num_classes,
    const int background_label_id, const ConfLossType loss_type, std::vector<std::vector<float>>* all_max_scores) {
...
template <typename Dtype>
void GetPriorBBoxes(const Dtype* prior_data, const int num_priors, std::vector<NormalizedBBox>& prior_bboxes,
...
// Explicit initialization.
template void GetPriorBBoxes(const float* prior_data, const int num_priors, std::vector<NormalizedBBox>& prior_bboxes,
    std::vector<float>& prior_variances);
template void GetPriorBBoxes(const double* prior_data, const int num_priors, std::vector<NormalizedBBox>& prior_bboxes,
    std::vector<float>& prior_variances);

template <typename Dtype>
void GetTopKScoreIndex(const std::vector<Dtype>& scores, const std::vector<int>& indices, const int top_k,
    ...
    std::partial_sort_copy(indices.begin(), indices.end(), sorted_indices.begin(), sorted_indices.end(),
...
// Explicit initialization.
template void GetTopKScoreIndex(const std::vector<float>& scores, const std::vector<int>& indices, const int top_k,
    std::vector<std::pair<float, int>>* score_index_vec);
template void GetTopKScoreIndex(const std::vector<double>& scores, const std::vector<int>& indices, const int top_k,
    std::vector<std::pair<double, int>>* score_index_vec);

template <typename Dtype>
void GetMaxScoreIndex(const std::vector<Dtype>& scores, const float threshold, const int top_k,
    std::vector<std::pair<Dtype, int>>* score_index_vec) {
...
// Explicit initialization.
template void GetMaxScoreIndex(const std::vector<float>& scores, const float threshold, const int top_k,
    std::vector<std::pair<float, int>>* score_index_vec);
template void GetMaxScoreIndex(const std::vector<double>& scores, const float threshold, const int top_k,
    std::vector<std::pair<double, int>>* score_index_vec);

void ApplyNMS(const std::vector<NormalizedBBox>& bboxes, const std::vector<float>& scores, const float threshold,
    const int top_k, const bool reuse_overlaps, std::map<int, std::map<int, float>>* overlaps,
    ...
    while (score_index_vec.size() != 0) {
        ...
        for (std::vector<std::pair<float, int>>::iterator it = score_index_vec.begin(); it != score_index_vec.end();) {
            ...
            if (reuse_overlaps) {
                if (overlaps->find(best_idx) != overlaps->end() &&
                    overlaps->find(best_idx)->second.find(cur_idx) != (*overlaps)[best_idx].end()) {
                    // Use the computed overlap.
                    cur_overlap = (*overlaps)[best_idx][cur_idx];
                } else if (overlaps->find(cur_idx) != overlaps->end() &&
                           overlaps->find(cur_idx)->second.find(best_idx) != (*overlaps)[cur_idx].end()) {
                    // Use the computed overlap.
...
void ApplyNMSFast_impl(const ArrayOfBBox& bboxes, const std::vector<Dtype>& scores, const float score_threshold,
...
template <typename Dtype>
void ApplyNMSFast(const std::vector<NormalizedBBox>& bboxes, const std::vector<Dtype>& scores,
    const float score_threshold, const float nms_threshold, const int top_k, std::vector<int>* indices) {
    ApplyNMSFast_impl<Dtype, std::vector<NormalizedBBox>>(
        bboxes, scores, score_threshold, nms_threshold, top_k, indices);
}

template <typename Dtype>
void ApplyNMSFast(const BBoxArrayWrapper<Dtype>& bboxes, const std::vector<Dtype>& scores, const float score_threshold,
    const float nms_threshold, const int top_k, std::vector<int>* indices) {
    ApplyNMSFast_impl<Dtype, BBoxArrayWrapper<Dtype>>(bboxes, scores, score_threshold, nms_threshold, top_k, indices);
}

// Explicit initialization.
template void ApplyNMSFast(const std::vector<NormalizedBBox>& bboxes, const std::vector<float>& scores,
    const float score_threshold, const float nms_threshold, const int top_k, std::vector<int>* indices);

template void ApplyNMSFast(const std::vector<NormalizedBBox>& bboxes, const std::vector<double>& scores,
    const float score_threshold, const float nms_threshold, const int top_k, std::vector<int>* indices);

template void ApplyNMSFast(const BBoxArrayWrapper<float>& bboxes, const std::vector<float>& scores,
    const float score_threshold, const float nms_threshold, const int top_k, std::vector<int>* indices);

template void ApplyNMSFast(const BBoxArrayWrapper<double>& bboxes, const std::vector<double>& scores,
    const float score_threshold, const float nms_threshold, const int top_k, std::vector<int>* indices);
