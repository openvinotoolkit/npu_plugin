...
void print_buffer_HWC_fp16(InferenceEngine::ie_fp16* src_data, int32_t IW, int32_t IH, int32_t IC, const char* tname,
                           int32_t iw0 = 0, int32_t iw1 = -1, int32_t ih0 = 0, int32_t ih1 = -1, int32_t ic0 = 0,
                           int32_t ic1 = -1);
...
protected:
    ...
    void AddLayer(const std::string& layer_type, const std::map<std::string, std::string>* params, size_t weights_size,
                  size_t biases_size, calcWeights fillWeights, const IN_OUT_desc& inDim, const IN_OUT_desc& outDim,
                  func&& cl);
    template <typename func>
    ...
    bool Infer();
    bool GenerateNetAndInfer(const CNNNetwork& network, bool useHWOpt = false, bool runRefGraph = true,
                             int version = 2);
    ...
    InferenceEngine::Blob::Ptr GenReferenceOutput() {
...
protected:
    struct ReferenceSequence {
        ...
        void Add(const IN_OUT_desc& inDim, const IN_OUT_desc& outDim, calcWeights /*fillWeights*/, size_t weights_size,
            ...
            auto f = std::bind(std::forward<func>(cl), obj.input, obj.output, obj.weights, obj.weights_size,
                               obj.bias_size, params);
            obj.callback = [f] {
                f();
            };
        ...
        void genInputOutput(callbackObject& obj, const IN_OUT_desc& inDim, const IN_OUT_desc& outDim) {
            ...
            if (callbacks.empty()) {
                ...
                obj.input = InferenceEngine::make_shared_blob<uint16_t>(
                        {InferenceEngine::Precision::FP16, newW, inputLayout});
                obj.input->allocate();
            } else {
    ...
    void doNetworkInit(const std::string& layer_type, std::map<std::string, std::string>* params = nullptr,
                       int weights_size = 0, int biases_size = 0,
...
void vpuLayersTests::AddLayer(const std::string& layer_type, const ParamsStruct* params, size_t weights_size,
                              size_t biases_size, calcWeights fillWeights, const IN_OUT_desc& inDim,
...
inline bool vpuLayersTests::GenerateNetAndInfer(const CNNNetwork& network, bool useHWOpt, bool runRefGraph,
                                                int version) {
