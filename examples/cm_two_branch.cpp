...
int main()
{
    ...
    auto conv1WeightsIt = om.constant(conv1WeightsData, mv::Shape(3, 3, 3, 8), mv::DType::Float, mv::Order::ColumnMajor);
    auto conv1It = om.conv2D(input, conv1WeightsIt, {2, 2}, {1, 1, 1, 1});
    auto pool1It = om.maxpool2D(conv1It, {3, 3}, {2, 2}, {1, 1, 1, 1});
    auto conv2WeightsIt = om.constant(conv2WeightsData, mv::Shape(3, 3, 3, 8), mv::DType::Float, mv::Order::ColumnMajor);
    ...
    auto conv3WeightsIt = om.constant(conv3WeightsData, mv::Shape(5, 5, 16, 32), mv::DType::Float, mv::Order::ColumnMajor);
    auto conv3It = om.conv2D(concat1It, conv3WeightsIt, {2, 2}, {2, 2, 2, 2});
    auto pool3It = om.maxpool2D(conv3It, {5, 5}, {3, 3}, {2, 2, 2, 2});
    auto conv4WeightsIt = om.constant(conv4WeightsData, mv::Shape(6, 6, 32, 64), mv::DType::Float, mv::Order::ColumnMajor);
