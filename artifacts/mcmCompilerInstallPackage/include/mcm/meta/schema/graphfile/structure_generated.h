...
struct SourceStructureT : public flatbuffers::NativeTable {
  ...
  std::vector<std::unique_ptr<GraphNodeT>> nodes;
...
inline void SourceStructure::UnPackTo(SourceStructureT *_o, const flatbuffers::resolver_function_t *_resolver) const {
  (void)_o;
  (void)_resolver;
  { auto _e = nodes(); if (_e) { _o->nodes.resize(_e->size()); for (flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { _o->nodes[_i] = std::unique_ptr<GraphNodeT>(_e->Get(_i)->UnPack(_resolver)); } } };
...
inline flatbuffers::Offset<SourceStructure> CreateSourceStructure(flatbuffers::FlatBufferBuilder &_fbb, const SourceStructureT *_o, const flatbuffers::rehasher_function_t *_rehasher) {
  ...
  auto _nodes = _fbb.CreateVector<flatbuffers::Offset<GraphNode>> (_o->nodes.size(), [](size_t i, _VectorArgs *__va) { return CreateGraphNode(*__va->__fbb, __va->__o->nodes[i].get(), __va->__rehasher); }, &_va );
