...
enum ipc_device_status {
    IPC_DEV_OFF = 0, // device is off
    IPC_DEV_BUSY,    // device is busy and not available
    IPC_DEV_READY,   // device is available for use
    IPC_DEV_ERROR,   // device HW failure is detected
    IPC_DEV_STOPPING // device is stopping
};
...
enum xlink_error xlink_open_channel(struct xlink_handle *handle, uint16_t chan, enum xlink_opmode mode,
                                    uint32_t data_size, uint32_t timeout);

enum xlink_error xlink_data_available_event(struct xlink_handle *handle, uint16_t chan,
                                            xlink_event data_available_event);

enum xlink_error xlink_data_consumed_event(struct xlink_handle *handle, uint16_t chan, xlink_event data_consumed_event);

enum xlink_error xlink_close_channel(struct xlink_handle *handle, uint16_t chan);

enum xlink_error xlink_write_data(struct xlink_handle *handle, uint16_t chan, uint8_t const *message, uint32_t size);

enum xlink_error xlink_read_data(struct xlink_handle *handle, uint16_t chan, uint8_t **message, uint32_t *size);

enum xlink_error xlink_write_control_data(struct xlink_handle *handle, uint16_t chan, uint8_t const *message,
                                          uint32_t size);

enum xlink_error xlink_write_volatile(struct xlink_handle *handle, uint16_t chan, uint8_t const *message,
                                      uint32_t size);

enum xlink_error xlink_read_data_to_buffer(struct xlink_handle *handle, uint16_t chan, uint8_t *const message,
                                           uint32_t *size);

enum xlink_error xlink_release_data(struct xlink_handle *handle, uint16_t chan, uint8_t *const data_addr);

enum xlink_error xlink_disconnect(struct xlink_handle *handle);

enum xlink_error xlink_get_device_list(uint32_t *sw_device_id_list, uint32_t *num_devices);

enum xlink_error xlink_get_device_name(struct xlink_handle *handle, char *name, size_t name_size);

enum xlink_error xlink_get_device_status(struct xlink_handle *handle, uint32_t *device_status);

enum xlink_error xlink_boot_device(struct xlink_handle *handle, const char *binary_name);

enum xlink_error xlink_reset_device(struct xlink_handle *handle);

enum xlink_error xlink_set_device_mode(struct xlink_handle *handle, enum xlink_device_power_mode power_mode);

enum xlink_error xlink_get_device_mode(struct xlink_handle *handle, enum xlink_device_power_mode *power_mode);

enum xlink_error xlink_register_device_event(struct xlink_handle *handle, uint32_t *event_list, uint32_t num_events,
                                             int (*)(uint32_t, uint32_t));

enum xlink_error xlink_unregister_device_event(struct xlink_handle *handle, uint32_t *event_list, uint32_t num_events);
