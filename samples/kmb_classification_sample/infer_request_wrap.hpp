...
public:
	...
	InferenceEngine::PreProcessInfo getPreProcess(const std::string &name) {
	    return _request.GetPreProcess(name);
	}
	...
	void setBlob(const std::string &name, const InferenceEngine::Blob::Ptr &data, const InferenceEngine::PreProcessInfo &preproc) {
        _request.SetBlob(name, data, preproc);
    }
