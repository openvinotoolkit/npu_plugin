...
TEST(binary_data, dtype)
{
    ...
    mv::BinaryData bdata("Float16");
    bdata.setFp16(data);
    bdata.setType("Float32");
    ASSERT_EQ(bdata.getType(), "Float32");
...
TEST(binary_data, assignment)
{
    ...
    for (unsigned i = 0; i < data.size(); ++i)
         ASSERT_EQ(data[i], ret[i]);
    ASSERT_EQ(bdata_test.getType(), "Int32");

    //Assignment to an empty BD with same type
    mv::BinaryData bdata32_test("Int32");
