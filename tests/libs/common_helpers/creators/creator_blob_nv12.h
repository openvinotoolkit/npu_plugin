...
// If data != nullptr, use pre-allocated single memory area, else create 2 different blobs
InferenceEngine::NV12Blob::Ptr createFromFile(const std::string& filePath, const std::size_t width,
                                              const std::size_t height, uint8_t* data = nullptr);

void descriptorsFromFrameSize(const size_t width, const size_t height, InferenceEngine::TensorDesc& uvDesc,
                              InferenceEngine::TensorDesc& yDesc);

inline InferenceEngine::NV12Blob::Ptr createBlob(const std::size_t width, const std::size_t height, uint8_t* data) {
...
inline InferenceEngine::NV12Blob::Ptr createBlob(const InferenceEngine::TensorDesc& tensorDesc, uint8_t* data) {
    if (tensorDesc.getLayout() != InferenceEngine::NCHW && tensorDesc.getLayout() != InferenceEngine::NHWC) {
        IE_THROW() << "Only NCHW / NHWC Layout supported in nv12 blob creator!. Input: " << tensorDesc.getLayout();
    }
...
inline InferenceEngine::NV12Blob::Ptr createFromFile(const std::string& filePath, const std::size_t width,
                                                     const std::size_t height, uint8_t* data) {
