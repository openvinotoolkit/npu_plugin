...
std::vector<DimsOrder> getOrders() {
    return std::vector<DimsOrder>{vpux::DimsOrder::C,
                                  ...
                                  vpux::DimsOrder::NHWC,
                                  vpux::DimsOrder::NHCW,
                                  vpux::DimsOrder::NCDHW,
                                  vpux::DimsOrder::NDHWC};
}

std::vector<std::pair<DimsOrder, size_t>> getOrders2Dims() {
    return std::vector<std::pair<DimsOrder, size_t>>{
            std::make_pair(vpux::DimsOrder::C, 1u),
            std::make_pair(vpux::DimsOrder::NC, 2u),
            std::make_pair(vpux::DimsOrder::CHW, 3u),
            std::make_pair(vpux::DimsOrder::HWC, 3u),
            std::make_pair(vpux::DimsOrder::HCW, 3u),
            std::make_pair(vpux::DimsOrder::NCHW, 4u),
            std::make_pair(vpux::DimsOrder::NHWC, 4u),
            std::make_pair(vpux::DimsOrder::NHCW, 4u),
            std::make_pair(vpux::DimsOrder::NCDHW, 5u),
            std::make_pair(vpux::DimsOrder::NDHWC, 5u)};
}
...
std::vector<std::pair<std::vector<Dim>, DimsOrder>> getPerm2Order() {
    return std::vector<std::pair<std::vector<Dim>, DimsOrder>>{
            ...
            std::make_pair(std::vector<Dim>({Dim(2), Dim(1), Dim(0)}),
                           DimsOrder::CHW),
            std::make_pair(std::vector<Dim>({Dim(0), Dim(2), Dim(1)}),
                           DimsOrder::HWC),
            std::make_pair(std::vector<Dim>({Dim(2), Dim(0), Dim(1)}),
                           DimsOrder::HCW),
            std::make_pair(std::vector<Dim>({Dim(3), Dim(2), Dim(1), Dim(0)}),
                           DimsOrder::NCHW),
            std::make_pair(std::vector<Dim>({Dim(1), Dim(3), Dim(2), Dim(0)}),
                           DimsOrder::NHWC),
            std::make_pair(std::vector<Dim>({Dim(3), Dim(1), Dim(2), Dim(0)}),
                           DimsOrder::NHCW),
            std::make_pair(
                    std::vector<Dim>({Dim(4), Dim(3), Dim(2), Dim(1), Dim(0)}),
                    DimsOrder::NCDHW),
            std::make_pair(
                    std::vector<Dim>({Dim(1), Dim(4), Dim(3), Dim(2), Dim(0)}),
                    DimsOrder::NDHWC)};
}
std::vector<std::pair<DimsOrder::StorageType, DimsOrder>> getCode2Order() {
    return std::vector<std::pair<DimsOrder::StorageType, DimsOrder>>{
            std::make_pair(0x1, DimsOrder::C),
            std::make_pair(0x12, DimsOrder::NC),
            std::make_pair(0x123, DimsOrder::CHW),
            std::make_pair(0x231, DimsOrder::HWC),
            std::make_pair(0x213, DimsOrder::HCW),
            std::make_pair(0x1234, DimsOrder::NCHW),
            std::make_pair(0x1342, DimsOrder::NHWC),
            std::make_pair(0x1324, DimsOrder::NHCW),
            std::make_pair(0x12345, DimsOrder::NCDHW),
...
TEST(DimsOrderTest, ValidateNumDimsTest) {
    ...
    std::for_each(orders2dims.begin(),
                  orders2dims.end(),
                  [](const std::pair<DimsOrder, size_t>& order2dim) {
...
TEST(DimsOrderTest, ValidatePermutationTest) {
    ...
    std::for_each(validPermutation.begin(),
                  validPermutation.end(),
                  [](const DimArr& perm) {
    ...
    std::for_each(invalidPermutation.begin(),
                  invalidPermutation.end(),
                  [](const DimArr& perm) {
...
TEST(DimsOrderTest, getCodeFromNumDimsTest) {
    std::vector<DimsOrder> defOrders{DimsOrder::C,
                                     DimsOrder::NC,
                                     DimsOrder::CHW,
                                     DimsOrder::NCHW,
                                     DimsOrder::NCDHW};
...
TEST(DimsOrderTest, getCodeFromPermutationTest) {
    ...
    std::for_each(
            ...
            [](const std::pair<std::vector<Dim>, DimsOrder>& perm2order) {
                EXPECT_EQ(DimsOrder::getCodeFromPermutation(perm2order.first),
                          perm2order.second.code());
            });
}
...
TEST(DimsOrderTest, fromCodeTest) {
    ...
    std::for_each(
            ...
            [](const std::pair<DimsOrder::StorageType, DimsOrder>& code2order) {
                EXPECT_EQ(DimsOrder::fromCode(code2order.first),
                          code2order.second);
            });
}
...
TEST(DimsOrderTest, fromNumDimsTest) {
    std::vector<std::pair<size_t, DimsOrder>> numDims2DimsOrderTable{
            std::make_pair(1, DimsOrder::C),
            std::make_pair(2, DimsOrder::NC),
            std::make_pair(3, DimsOrder::CHW),
            std::make_pair(4, DimsOrder::NCHW),
            std::make_pair(5, DimsOrder::NCDHW)};

    std::for_each(numDims2DimsOrderTable.begin(),
                  numDims2DimsOrderTable.end(),
                  [](const std::pair<size_t, DimsOrder>& numDims2order) {
                      EXPECT_EQ(DimsOrder::fromNumDims(numDims2order.first),
                                numDims2order.second);
                  });
}
...
TEST(DimsOrderTest, FromPermutationTest) {
    ...
    std::for_each(perms2orders.begin(),
                  perms2orders.end(),
                  [](const std::pair<std::vector<Dim>, DimsOrder>& perm2order) {
                      EXPECT_EQ(DimsOrder::fromPermutation(perm2order.first),
                                perm2order.second);
                  });
}
...
TEST(DimsOrderTest, dimPosTest) {
    ...
    std::for_each(orders2dims.begin(),
                  ...
                  [](const std::pair<DimsOrder, size_t>& order2dim) {
                      for (size_t i = 0; i < MAX_NUM_DIMS; ++i) {
                          if (i < order2dim.second) {
                              EXPECT_EQ(order2dim.first.dimPos(
                                                order2dim.first.dimAt(i)),
                                        int32_t(i));
                          } else {
...
TEST(DimsOrderTest, toPermutationTest) {
    ...
    std::for_each(perms2orders.begin(),
                  perms2orders.end(),
                  [](const std::pair<std::vector<Dim>, DimsOrder>& perm2order) {
                      EXPECT_EQ(DimArr(perm2order.first.begin(),
                                       perm2order.first.end()),
                                perm2order.second.toPermutation());
                  });
}
...
TEST(DimsOrderTest, fromIETest) {
    ...
    EXPECT_EQ(DimsOrder::NCHW,
              DimsOrder::fromIE(InferenceEngine::Layout::NCHW));
    EXPECT_EQ(DimsOrder::NHWC,
              DimsOrder::fromIE(InferenceEngine::Layout::NHWC));
    EXPECT_EQ(DimsOrder::NCDHW,
              DimsOrder::fromIE(InferenceEngine::Layout::NCDHW));
    EXPECT_EQ(DimsOrder::NDHWC,
              DimsOrder::fromIE(InferenceEngine::Layout::NDHWC));
