...
std::vector<std::pair<StrideReqKind, int64_t>> getValidAttrs() {
    return std::vector<std::pair<StrideReqKind, int64_t>>{
            std::make_pair(StrideReqKind::Aligned, 2),
            std::make_pair(StrideReqKind::Aligned, 4),
            std::make_pair(StrideReqKind::Aligned, 8),
            std::make_pair(StrideReqKind::Compact, 0),
            std::make_pair(StrideReqKind::Fixed, 1),
            std::make_pair(StrideReqKind::Fixed, 2),
...
std::vector<std::pair<StrideReqKind, int64_t>> getInvalidAttrs() {
    return std::vector<std::pair<StrideReqKind, int64_t>>{
            std::make_pair(StrideReqKind::Aligned, -2),
            std::make_pair(StrideReqKind::Aligned, 0),
            std::make_pair(StrideReqKind::Aligned, 3),
            std::make_pair(StrideReqKind::Compact, -1),
            std::make_pair(StrideReqKind::Compact, 1),
            std::make_pair(StrideReqKind::Compact, 2),
            std::make_pair(StrideReqKind::Fixed, -1),
            std::make_pair(StrideReqKind::Fixed, 0),
    };
}
...
std::vector<std::tuple<MemShape, size_t, MemStrides>> getShapes2Strides() {
    return std::vector<std::tuple<MemShape, size_t, MemStrides>>{
            ...
            std::make_tuple(MemShape({2, 3, 4, 5}),
                            1u,
                            MemStrides({1, 2, 6, 24})),
            std::make_tuple(MemShape({2, 3, 4, 5}),
                            4u,
                            MemStrides({4, 8, 24, 96})),
    };
}

std::vector<std::tuple<MemShape, size_t, MemStrides>>
        getIncorrectShapes2Strides() {
    return std::vector<std::tuple<MemShape, size_t, MemStrides>>{
            ...
            std::make_tuple(MemShape({2, 3, 4, 5}),
                            4u,
                            MemStrides({3, 8, 24, 96})),
            std::make_tuple(MemShape({2, 3, 4, 5}),
                            4u,
                            MemStrides({4, 9, 24, 96})),
            std::make_tuple(MemShape({2, 3, 4, 5}),
                            4u,
                            MemStrides({4, 8, 25, 96})),
            std::make_tuple(MemShape({2, 3, 4, 5}),
                            4u,
                            MemStrides({4, 8, 24, 97})),
    };
}

}  //  namespace
TEST(EnumTraitsTest, EnumTraitsConvertionTest) {
    std::vector<std::pair<StrideReqKind, std::string>> enum2StrView{
            std::make_pair(StrideReqKind::Compact, "Compact"),
            std::make_pair(StrideReqKind::Aligned, "Aligned"),
            std::make_pair(StrideReqKind::Fixed, "Fixed")};

    std::for_each(enum2StrView.begin(),
                  enum2StrView.end(),
                  [](const std::pair<StrideReqKind, std::string>& enum2str) {
                      EXPECT_EQ(EnumTraits<StrideReqKind>::getEnumValueName(
                                        enum2str.first),
                                enum2str.second);
                      EXPECT_EQ(enum2str.first,
                                EnumTraits<StrideReqKind>::parseEnumValue(
                                        enum2str.second));
                  });
}
...
TEST(DimStrideReqTest, verifyAttrsTest) {
    ...
    std::for_each(validAttrs.begin(),
                  validAttrs.end(),
                  [](const std::pair<StrideReqKind, int64_t>& attr) {
                      EXPECT_NO_THROW(DimStrideReq::verifyAttrs(attr.first,
                                                                attr.second));
                  });

    // check invalid cases
    auto invalidAttrs = getInvalidAttrs();
    std::for_each(invalidAttrs.begin(),
                  invalidAttrs.end(),
                  [](const std::pair<StrideReqKind, int64_t>& attr) {
                      EXPECT_ANY_THROW(DimStrideReq::verifyAttrs(attr.first,
                                                                 attr.second));
                  });
}
    ...
    std::for_each(
            ...
            [](const std::pair<StrideReqKind, int64_t>& attr) {
                for (size_t dim = 0; dim < MAX_NUM_DIMS; ++dim) {
                    EXPECT_EQ(DimStrideReq(MemDim(dim), attr.first, attr.second)
                                      .memDim(),
                              MemDim(dim));
                }
            });
...
TEST(StrideReqRefTest, addRemoveTest) {
    ...
    std::for_each(validAttrs.begin(),
                  ...
                  [](const std::pair<StrideReqKind, int64_t>& attr) {
                      ...
                      for (size_t dim = 0; dim < MAX_NUM_DIMS; ++dim) {
                          EXPECT_NO_THROW(
                                  strideReq.add(DimStrideReq(MemDim(dim),
                                                             attr.first,
                                                             attr.second)));
                      }
                      for (size_t dim = 0; dim < MAX_NUM_DIMS; ++dim) {
...
TEST(StrideReqRefTest, checkStridesTest) {
    ...
    std::for_each(
            shapes2strides.begin(),
            shapes2strides.end(),
            [](const std::tuple<MemShape, size_t, MemStrides>& shape2stride) {
                auto stridesReq =
                        StrideReqs::compact(std::get<0>(shape2stride).size());

                EXPECT_TRUE(stridesReq.checkStrides(std::get<2>(shape2stride),
                                                    std::get<1>(shape2stride),
                                                    std::get<0>(shape2stride)));
            });
    // check strides for incorrect cases
    auto shapes2stridesIncorrect = getIncorrectShapes2Strides();
    std::for_each(
            shapes2stridesIncorrect.begin(),
            shapes2stridesIncorrect.end(),
            [](const std::tuple<MemShape, size_t, MemStrides>& shape2stride) {
                auto stridesReq =
                        StrideReqs::compact(std::get<0>(shape2stride).size());

                EXPECT_FALSE(
                        stridesReq.checkStrides(std::get<2>(shape2stride),
                                                std::get<1>(shape2stride),
                                                std::get<0>(shape2stride)));
            });
}
...
TEST(StrideReqRefTest, calcStridesTest) {
    ...
    std::for_each(
            ...
            [](const std::tuple<MemShape, size_t, MemStrides>& shape2stride) {
                ...
                stridesReq.calcStrides(memStrides,
                                       std::get<1>(shape2stride),
                                       std::get<0>(shape2stride));

                EXPECT_EQ(stridesReq.calcStrides(std::get<1>(shape2stride),
                                                 std::get<0>(shape2stride)),
