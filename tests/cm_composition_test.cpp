...
TEST(computation_model, minimal_functional_composition)
{
    ...
    auto convIt = cm.conv2D(inIt, weights, 4, 4, 1, 1);
...
TEST(computation_model, failure_sanity)
{
    ...
    mv::ComputationModel cm(mv::Logger::VerboseLevel::VerboseSilent);

    auto inIt = cm.input(mv::Shape(1, 32, 32, 3), mv::DType::Float, mv::Order::NWHC);
    auto outIt = cm.output(inIt);

    ASSERT_TRUE(cm.addAttr(inIt, "customAttr", mv::Attribute(mv::AttrType::IntegerType, 10)));
    ASSERT_FALSE(cm.addAttr(inIt, "customAttr", mv::Attribute(mv::AttrType::IntegerType, 10)));
