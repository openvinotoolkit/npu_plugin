#include "include/mcm/compiler/compilation_unit.hpp"
#include <iostream>
#include <fstream>

int main()
{

    mv::CompilationUnit unit("EltwiseFP16");
    mv::OpModel& om = unit.model();

    auto inputShape = mv::Shape({16,16,32,1});

    // Quantize zeropoints & scales
    auto in_qp0 = mv::QuantizationParams({{0}, {0.003921568627451}, {},{}});
    auto in_qp1 = mv::QuantizationParams({{0}, {0.003921568627451 * 2}, {},{}});

    // Input
    auto input0 = om.input(inputShape, mv::DType("UInt8"), mv::Order::getZMajorID(4), {{0},{0.003921568627451},{},{}}, "input0");

    // Identity MaxPools
    auto identity_maxPool0 = om.maxPool(input0, {1,1}, {1,1}, {0,0,0,0}, true, "", "floor", mv::DType("UInt8"), in_qp0, "identity_maxpool0");
    auto identity_maxPool1 = om.maxPool(input0, {1,1}, {1,1}, {0,0,0,0}, true, "", "floor", mv::DType("UInt8"), in_qp1, "identity_maxpool1");

    // Calculate appropriate output quantParams
    auto out_zp = 0;
    auto out_scale = std::min(in_qp0.getScale()[0], in_qp0.getScale()[1]);
    auto out_qp = mv::QuantizationParams({{out_zp}, {out_scale}, {},{}});

    // Eltwise in FP16
    std::vector<mv::Data::TensorIterator> inputs;
    inputs.push_back(identity_maxPool0);
    inputs.push_back(identity_maxPool1);
    auto eltwiseType = "Add";
    auto eltwise0 = om.eltwiseFP16(inputs, eltwiseType, mv::DType("Float16"), out_qp, "eltwise0");

    // Re-quantize output
    auto requantize0 = om.quantize(eltwise0, mv::DType("UInt8"), out_qp, "requantize0");

    // Output
    auto output0 = om.output(requantize0);

    std::string compDescPath = mv::utils::projectRootPath() + "/config/compilation/release_kmb_MC-Prefetch1.json";
    unit.loadCompilationDescriptor(compDescPath);
    unit.loadTargetDescriptor(mv::Target::ma2490);
    unit.initialize();
    unit.run();

}
