...
class KmbPatternTests : public KmbLayerTestBase, public testing::WithParamInterface<BatchNotEqualOneParams> {};
...
TEST_P(KmbPatternTests, BatchNotEqualToOneInTheMiddle) {
    ...
    registerBlobGenerator("input", input_desc, [&](const TensorDesc& desc) {
        return vpux::makeSplatBlob(desc, 1.0f);
    });

    registerBlobGenerator("shape0", shape_desc0, [&](const TensorDesc& desc) {
    ...
    registerBlobGenerator("shape1", shape_desc1, [&](const TensorDesc& desc) {
        auto blob = make_blob_with_precision(desc);
        blob->allocate();
    ...
    const auto netBuidler = [&](TestNetwork& testNet) {
        testNet.setUserInput("input", input_desc.getPrecision(), input_desc.getLayout())
...
const std::vector<BatchNotEqualOneParams> params = {BatchNotEqualOneParams()
                                                            .in_dims({1, 2, 4, 4})
                                                            .shape0({2, 4, 4, 1})
                                                            .shape1({1, 4, 2, 4})
