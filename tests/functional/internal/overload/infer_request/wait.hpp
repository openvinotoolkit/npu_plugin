...
using InferRequestWaitTestsVpux = BehaviorTestsUtils::InferRequestTestsVpux;

TEST_P(InferRequestWaitTestsVpux, CorrectOneAsyncInferWithGetInOutWithInfWait) {
    // Create InferRequest
...
TEST_P(InferRequestWaitTestsVpux, canStartAsyncInferWithGetInOutWithStatusOnlyWait) {
    // Create InferRequest
    InferenceEngine::InferRequest req;
    InferenceEngine::Blob::Ptr blob =
            FuncTestUtils::createAndFillBlob(cnnNet.getOutputsInfo().begin()->second->getTensorDesc());
...
// Plugin correct infer request with allocating input and result BlobMaps inside plugin
TEST_P(InferRequestWaitTestsVpux, FailedAsyncInferWithNegativeTimeForWait) {
    // Create InferRequest
    InferenceEngine::InferRequest req;
    InferenceEngine::Blob::Ptr blob =
            FuncTestUtils::createAndFillBlob(cnnNet.getOutputsInfo().begin()->second->getTensorDesc());
...
TEST_P(InferRequestWaitTestsVpux, canWaitWithotStartAsync) {
    // Create InferRequest
...
TEST_P(InferRequestWaitTestsVpux, returnDeviceBusyOnSetBlobAfterAsyncInfer) {
...
TEST_P(InferRequestWaitTestsVpux, returnDeviceBusyOnGetBlobAfterAsyncInfer) {
    // Create InferRequest
    auto req = execNet.CreateInferRequest();
    auto outputBlob = req.GetBlob(cnnNet.getInputsInfo().begin()->first);
    InferenceEngine::StatusCode sts;
