...
void checkInOutRank(int inputRank, int indexRank, int batchDims) {
    if (inputRank != 4) {
        throw LayerTestsUtils::VpuSkipTestException("Gather only supports 4D input shape, inRank = " +
                                                    std::to_string(inputRank));
    }

    auto outRank = inputRank + indexRank - 1 - batchDims;
    if (outRank != 4) {
        throw LayerTestsUtils::VpuSkipTestException("Gather only supports 4D output shape, outRank = " +
