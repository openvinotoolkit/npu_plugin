...
const auto selectTestParams_highDims = ::testing::Combine(
        ::testing::ValuesIn(shapesHighDims),
        ::testing::ValuesIn(inputPrecision),
        ::testing::Values(ngraph::op::AutoBroadcastSpec::NONE),
        ::testing::Values(LayerTestsUtils::testPlatformTargetDevice)
);

INSTANTIATE_TEST_CASE_P(smoke_select_common, KmbSelectLayerTest_MLIR, selectTestParams, KmbSelectLayerTest::getTestCaseName);

// Blob generated from VPUIP_2 does not generate the expected result when the 4th dimension's is not 1
// [Track number: E#26954]
INSTANTIATE_TEST_CASE_P(DISABLED_smoke_select_highDims, KmbSelectLayerTest_MLIR, selectTestParams_highDims, KmbSelectLayerTest::getTestCaseName);
}  // namespace
