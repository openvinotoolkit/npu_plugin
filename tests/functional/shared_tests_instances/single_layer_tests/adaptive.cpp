...
INSTANTIATE_TEST_CASE_P(smoke_TestsAdaPool3D, KmbAdaPoolLayerTest, AdaPool3DCases,
...
INSTANTIATE_TEST_CASE_P(smoke_TestsAdaPool4D, KmbAdaPoolLayerTest, AdaPool4DCases,
                        KmbAdaPoolLayerTest::getTestCaseName);

const auto AdaPool5DCases =
        ::testing::Combine(
                ::testing::ValuesIn(std::vector<std::vector<size_t>>{
                                {1, 17, 4, 5, 4},
                                {1, 1, 3, 2, 3}}),                                        // inputShape
                ::testing::ValuesIn(std::vector<std::vector<int>>{{1, 1, 1}, {3, 5, 3}}), // pooledSpatialShape
                ::testing::ValuesIn(std::vector<std::string>{"max", "avg"}),              // mode
                ::testing::ValuesIn(netPrecisions),                                       // precision
                ::testing::Values(LayerTestsUtils::testPlatformTargetDevice));            // device

INSTANTIATE_TEST_CASE_P(smoke_TestsAdaPool5D, KmbAdaPoolLayerTest, AdaPool5DCases,
                        KmbAdaPoolLayerTest::getTestCaseName);
