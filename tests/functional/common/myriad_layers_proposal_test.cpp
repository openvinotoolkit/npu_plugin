...
protected:
 ...
 public:
    void InferProposalLayer() {
        ...
        _inputsInfo["rpn_cls_prob_reshape"]->setInputPrecision(precision);
        _inputsInfo["rpn_cls_prob_reshape"]->setLayout(NCHW);
        _inputsInfo["rpn_bbox_pred"]->setInputPrecision(precision);
        _inputsInfo["rpn_bbox_pred"]->setLayout(NCHW);
        _inputsInfo["im_info"]->setInputPrecision(precision);
        ...
        std::string inputTensor1Binary = get_data_path() + cls_prob_file;
        ASSERT_TRUE(fromBinaryFile(inputTensor1Binary, rpn_cls_prob_reshape));

        // rpn_bbox_pred
        std::string inputTensor2Binary = get_data_path() + bbox_pred_file;
    ...
    void compareOutputSampleToRef(std::vector<float> & gt_values, const float error_threshold) {
        ...
        SizeVector out_sampling_dims = outputBlob->dims();
        out_sampling_dims[1] = OUTPUT_SAMPLING_NUM;

        // Generate GT
        _refBlob = make_blob_with_precision(outputBlob->precision(), outputBlob->layout(), out_sampling_dims);
        _refBlob->allocate();
        ...
        Blob::Ptr outputSample = make_blob_with_precision(outputBlob->precision(), outputBlob->layout(), out_sampling_dims);
        ...
        Compare(outputSample, _refBlob, error_threshold);
    }
