...
auto refMax = [](const float a, const float b)noexcept {
    return std::max(a, b);
};

auto refMul = [](const float a, const float b)noexcept {
    return a * b;
};

auto refSum = [](const float a, const float b)noexcept {
    return a + b;
};

auto refSub = [](const float a, const float b) noexcept {
    return a - b;
};

auto refDiv = [](const float a, const float b) noexcept {
    return a / b;
};

auto refMin = [](const float a, const float b) noexcept {
    return std::min(a, b);
};

auto refSqDiff = [](const float a, const float b) noexcept {
    return (a - b) * (a - b);
};

auto refPow = [](const float a, const float b) noexcept {
    return powf(a, b);
};

auto refFloorMod = [](const float a, const float b) noexcept {
    return a - b * floorf(a / b);
};

auto refEqual = [](const float a, const float b) noexcept {
    return a == b ? 1.f : 0.f;
};

auto refNotEqual = [](const float a, const float b) noexcept {
    return a != b ? 1.f : 0.f;
};

auto refGreater = [](const float a, const float b) noexcept {
    return a > b ? 1.f : 0.f;
};

auto refGreaterEqual = [](const float a, const float b) noexcept {
    return a >= b ? 1.f : 0.f;
};

auto refLess = [](const float a, const float b) noexcept {
    return a < b ? 1.f : 0.f;
};

auto refLessEqual = [](const float a, const float b) noexcept {
    return a <= b ? 1.f : 0.f;
};

auto refLogicalNot = [](const float a, const float b) noexcept {
    return (a == 0) ? 1.f : 0.f;
};

auto refLogicalAnd = [](const float a, const float b) noexcept {
    return (a != 0) && (b != 0) ? 1.f : 0.f;
};

auto refLogicalOr = [](const float a, const float b) noexcept {
    return (a != 0) || (b != 0) ? 1.f : 0.f;
};

auto refLogicalXor = [](const float a, const float b) noexcept {
    return int((a != 0) && !(b != 0)) + int(!(a != 0) && (b != 0)) ? 1.f : 0.f;
};

auto refMean = [](const float a, const float b) noexcept {
    return (a + b)/2.f;
};

typedef float (*kernel)(float a, float b);
...
void genRandomDataLogic(Blob::Ptr blob) {
    ...
    for (size_t indx = 0; indx < count; ++indx) {
        float val = rand();
        blobRawDataFp16[indx] = val <.5f ? 0x0000 : 0xFFFF;
    }
}
...
protected:
    template <typename Func>void RefEltwise(Func fun, std::vector<float> coeff)
    {
        ...
        uint32_t dst_coords[4];
        SizeVector refDims = convertDims(_refBlob->dims());
        SizeVector itrDims = convertDims(itr->second->dims());

        if (fun == s_kernels.at(ELTWISE_LOGICAL_NOT)) {
            for (int i = 0; i < _refBlob->size(); i++) {
                getCoord(i, refDims, dst_coords);

                for (int c = 0; c < refDims.size(); c++)
                    if (dst_coords[c] >= itrDims[c])
                        dst_coords[c] = 0;

                int src_i = getNum(dst_coords, itrDims);

                dstData[i] = PrecisionUtils::f32tof16(fun(PrecisionUtils::f16tof32(srcData[i]), 0.f));
            }
        } else {
            for (int i = 0; i < _refBlob->size(); i++) {
                getCoord(i, refDims, dst_coords);
        ...
        while(itr != _inputMap.end()) {
            ...
            uint16_t *dstData = _refBlob->buffer().as<uint16_t*>();
            itrDims = convertDims(itr->second->dims());
            ...
            for (int i = 0; i < _refBlob->size(); i++) {
                ...
                int src_i = getNum(dst_coords, itrDims);
                float val = fun(PrecisionUtils::f16tof32(dstData[i]), PrecisionUtils::f16tof32(srcData[src_i])*coeff[coeff_num]);
...
<net batch="1" name="VNect: Test" version="2">
    ...
    CNNNetReader reader;
    ASSERT_NO_THROW(reader.ReadNetwork(model.c_str(), model.length()));
    ...
    _refBlob = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, output->layout(), output->dims());
    _refBlob->allocate();
    ref_eltwise(inputBlob, inputBlob, _refBlob, refMul, std::vector<float>({1.0f, 1.0f}));

    Compare(_refBlob, output, 0.1f);
};
