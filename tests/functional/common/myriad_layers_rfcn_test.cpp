...
static void ref_soft_max(const Blob::Ptr& src, Blob::Ptr& dst, int axis) {
    ...
    dimy = src->dims()[1]; // H:1
    dimz = src->dims()[2]; // C:numCLASS
    dimx = src->dims()[3]; // N:numROI
...
static void refGlobalAvgPooling7x7Rfcn(const Blob::Ptr src,
    ...
    int32_t IW = src->dims()[0];   // 7
    int32_t IH = src->dims()[1];   // 7
    int32_t IC = src->dims()[2];   // numROI*numCLASS
    int32_t INUM = src->dims()[3]; // 1
    int32_t OW = dst->dims()[0];   // 1
    int32_t OH = dst->dims()[1];   // 1
    int32_t OC = dst->dims()[2];   // numCLASS
    int32_t ONUM = dst->dims()[3]; // numROI

    // Compare multiplied value since input shape might be 3D
...
public:
    ...
    void PrepareInputAndReference(const std::string& model_prior_network, const std::string& output_layer, StatusCode& st)
    {
        ...
        ASSERT_NO_THROW(netReader.ReadNetwork(model_prior_network.data(), model_prior_network.length()));
        ASSERT_TRUE(netReader.isParseSuccess());

        auto network_part = netReader.getNetwork();

        auto inputsInfo = network_part.getInputsInfo();
        inputsInfo["input0"]->setInputPrecision(Precision::FP16);
        inputsInfo["input0"]->setLayout(NCHW); // Input layout for PSROIPooling should be NCHW order, it's same as psroipooling test
        inputsInfo["input1"]->setInputPrecision(Precision::FP16);
        ...
        ASSERT_NO_THROW(st = myriadPluginPtr->LoadNetwork(exeNetwork, network_part, networkConfig, &_resp));
        ...
        // Allocate buffer
        input0_share = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, ANY, input0->dims());
        input0_share->allocate();
        input1_share = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, ANY, input1->dims());
    ...
    void RunNetwork(const std::string& model, const std::string& output_layer, StatusCode& st)
    {
        ...
        _inputsInfo["input0"]->setInputPrecision(Precision::FP16);
        ...
        ASSERT_NO_THROW(st = myriadPluginPtr->LoadNetwork(_exeNetwork, network, networkConfig, &_resp));
        ASSERT_EQ(StatusCode::OK, st) << _resp.msg;
...
TEST_F(myriadLayersRfcnTests_nightly, ReshapeRfcn)
{
    ...
    Compare(outputBlob, prior_network_output, 0.0f);
}
