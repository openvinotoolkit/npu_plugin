...
bool SortBBoxIndexPairDescend(const std::pair<NormalizedBBox, int>& bbox1,
                              const std::pair<NormalizedBBox, int>& bbox2) {
...
bool ProjectBBox_GetRatio(const NormalizedBBox& src_bbox, const NormalizedBBox& bbox, NormalizedBBox* proj_bbox,
                          float& ratio) {
...
void DecodeBBoxes(const std::vector<NormalizedBBox>& prior_bboxes, const std::vector<float>& prior_variances,
                  ...
                  const std::vector<NormalizedBBox>& bboxes, std::vector<NormalizedBBox>* decode_bboxes) {
    ...
    for (int i = 0; i < num_bboxes; ++i) {
        ...
        DecodeBBox(prior_bboxes[i], &prior_variances[i * 4], code_type, variance_encoded_in_target, bboxes[i],
                   &decode_bbox);
...
void DecodeBBoxesAll(const std::vector<LabelBBox>& all_loc_preds, const std::vector<NormalizedBBox>& prior_bboxes,
                     const std::vector<float>& prior_variances, const int num, const bool share_location,
                     const int num_loc_classes, const int background_label_id, const CodeType code_type,
                     const bool variance_encoded_in_target, std::vector<LabelBBox>* all_decode_bboxes) {
...
void GetLocPredictions(const Dtype* loc_data, const int num, const int num_preds_per_class, const int num_loc_classes,
    ...
    for (int i = 0; i < num; ++i) {
        ...
        if (share_location) {
            ...
            for (int c = 0; c < num_loc_classes; ++c) {
                label_bbox[c] = BBoxArrayWrapper<Dtype>(&bbox_data[(i * num_loc_classes + c) * num_preds_per_class],
                                                        num_preds_per_class);
            }
        }
    }
...
template void GetLocPredictions(const float* loc_data, const int num, const int num_preds_per_class,
                                ...
                                std::vector<LabelBBox>* loc_preds);
...
void ApplyNMSFast(const std::vector<NormalizedBBox>& bboxes, const std::vector<Dtype>& scores,
    ...
    ApplyNMSFast_impl<Dtype, std::vector<NormalizedBBox>>(bboxes, scores, score_threshold, nms_threshold, top_k,
                                                          indices);
}
...
template void ApplyNMSFast(const std::vector<NormalizedBBox>& bboxes, const std::vector<float>& scores,
                           const float score_threshold, const float nms_threshold, const int top_k,
                           std::vector<int>* indices);
