...
using ROIPoolingTestParams = std::tuple<Dims, roi_pooling_param, uint32_t, roi_pooling_method>;
...
public:
    ...
    void NetworkInit(std::map<std::string, std::string>* params,
                        const ROIPoolingParams& test_params,
                        const uint32_t num_rois,
                        const t_ROIPooling_method method)
    {
        std::string xml;
        genXML("ROIPooling", params, 0, 0, xml);
        ASSERT_NO_THROW(_net_reader.ReadNetwork(xml.data(), xml.length()));
        ASSERT_EQ(_net_reader.isParseSuccess(), true);
        setup(test_params, num_rois, method);
    }

    void setup(const ROIPoolingParams& params,
                 const uint32_t num_rois,
                 const t_ROIPooling_method method)
    {
        ...
        for (auto inputInfo : _inputsInfo) {
            inputInfo.second->setInputPrecision(Precision::FP16);
        }
        ...
        for (auto inpt : _inputsInfo)
        {
            ...
            InferenceEngine::Layout layout = inpt.second->getLayout();
...
TEST_P(myriadLayersTestsROIPooling_nightly, ROIPooling) {
    tensor_test_params dims_layer_in = std::tr1::get<0>(GetParam());
    ROIPoolingParams test_params = std::tr1::get<1>(GetParam());
    ...
    ASSERT_NO_FATAL_FAILURE(NetworkInit(&layer_params, test_params, num_rois, method));
    ...
    InferenceEngine::TBlob<ie_fp16>::Ptr _refBlob = make_shared_blob<ie_fp16, const SizeVector>(dst->precision(), dst->layout(), dst->dims());
    _refBlob->allocate();
