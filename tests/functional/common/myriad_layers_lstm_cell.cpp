...
TEST_P(myriadLayersTestsLSTMCell_nightly, LSTMCell) {
    ...
    /* reference version */
    auto refOut0 = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, Layout::NC, dims_output[0]);
    refOut0->allocate();
    auto refOut1 = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, Layout::NC, dims_output[1]);
    refOut1->allocate();
    auto gatesBlob = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, Layout::NC, {1, ngates * state_size});
    gatesBlob->allocate();
    // num_weights + num_bias
    auto weightsBlob0_repacked = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, Layout::NC, {1, ngates * state_size * input_size});
    weightsBlob0_repacked->allocate();
    auto weightsBlob1_repacked = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, Layout::NC, {1, ngates * state_size * state_size});
    ...
    Compare(outputBlob0, refOut0, ERROR_BOUND);
    Compare(outputBlob1, refOut1, ERROR_BOUND);
}
...
TEST_F(myriadLayersTests_nightly, LSTMCellSequenceNet) {
    ...
    ASSERT_NO_THROW(full_reader.ReadNetwork(tensorIteratorModel.c_str(), tensorIteratorModel.length()));
    ASSERT_TRUE(full_reader.isParseSuccess());
    ASSERT_NO_THROW(full_reader.SetWeights(weightsBlob_for_net));
    ...
    networkConfig[VPU_CONFIG_KEY(COMPUTE_LAYOUT)] = VPU_CONFIG_VALUE(NCHW);
    StatusCode st;
    ...
    matrix_copy_transpose(weights_inv1, weights1_repacked, ngates * state_size, state_size);
    /* reference version */
    auto refOut0 = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, Layout::NC, {seq_size * batch_size, state_size});
    refOut0->allocate();
    auto refOut1 = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, Layout::NC, {seq_size * batch_size, state_size});
    refOut1->allocate();
    auto gatesBlob = make_shared_blob<ie_fp16, const SizeVector>(Precision::FP16, Layout::NC, {1, ngates * state_size});
    gatesBlob->allocate();
    ie_fp16* h_dst = static_cast<ie_fp16*>(refOut0->buffer());
    ...
    Compare(output, refOut0, ERROR_BOUND);
}
