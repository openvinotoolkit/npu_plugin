...
class KmbUpstreamReduceStridedSliceSubGraphTest :
    ...
    void SetUp() override {
        ...
        for (auto idx = 0; idx < paramOuts.size(); idx++) {
            ...
            const auto weightsConvert = std::make_shared<ngraph::op::v0::Convert>(weightsConst, ngraph::element::Type_t::f32);

            std::vector<double> subtractData(filterShape[0], 0);
            const auto subtractConst =
                std::make_shared<ngraph::op::Constant>(ngraph::element::Type_t::f32, ngraph::Shape({filterShape[0], 1, 1, 1}), subtractData);
            ...
            const auto multiplyConst =
                std::make_shared<ngraph::op::Constant>(ngraph::element::Type_t::f32, ngraph::Shape({filterShape[0], 1, 1, 1}), multiplyData);
            const auto weightsMultiply = std::make_shared<ngraph::op::v1::Multiply>(weightsSubtract, multiplyConst);

            const auto inputLowConst = std::make_shared<ngraph::op::Constant>(ngraph::element::Type_t::f32, ngraph::Shape({1}),
                                                                     std::vector<double>({-1.0}));
            const auto inputHighConst = std::make_shared<ngraph::op::Constant>(ngraph::element::Type_t::f32, ngraph::Shape({1}),
                                                                        std::vector<double>({1.0}));
            const auto inputFq = std::make_shared<ngraph::op::v0::FakeQuantize>(paramOuts[idx], inputLowConst, inputHighConst,
            ...
            const auto actLowConst = std::make_shared<ngraph::op::Constant>(ngraph::element::Type_t::f32, ngraph::Shape({1}),
                                                                     std::vector<double>({0.0}));
            const auto actHighConst = std::make_shared<ngraph::op::Constant>(ngraph::element::Type_t::f32, ngraph::Shape({1}),
                                                                      std::vector<double>({4.0}));
            const auto activationFq =
                    std::make_shared<ngraph::op::v0::FakeQuantize>(relu, actLowConst, actHighConst, actLowConst, actHighConst, 256);
        ...
        const auto resultLowConst = std::make_shared<ngraph::op::Constant>(ngraph::element::Type_t::f32, ngraph::Shape({1}),
                                                                     std::vector<double>({0.0}));
        const auto resultHighConst = std::make_shared<ngraph::op::Constant>(ngraph::element::Type_t::f32, ngraph::Shape({1}),
                                                                      std::vector<double>({8.0}));

        const auto reluFq =
                std::make_shared<ngraph::op::v0::FakeQuantize>(relu, resultLowConst, resultHighConst, resultLowConst, resultHighConst, 256);
