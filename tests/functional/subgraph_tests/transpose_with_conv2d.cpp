...
class KmbTransposeWithConv2dTest :
    ...
    void SetUp() override {
        ...
        auto transposeArg0Const = std::make_shared<ngraph::op::Constant>(
                ngraph::element::Type_t::i64, ngraph::Shape{transposeArg0Weights.size()},
                transposeArg0Weights.data());

        auto transposeArg0Node = std::make_shared<ngraph::op::v1::Transpose>(
                paramOuts.at(0), transposeArg0Const->output(0));
...
INSTANTIATE_TEST_SUITE_P(conv2d_with_act, KmbTransposeWithConv2dTest,
