...
class VPUXConvPwlQuantizedSubGraphTest_VPU3700 :
    ...
    void SetUp() override {
        ...
        if (postOpType == PostOp::SIGMOID) {
            ...
            outputFq = ngraph::builder::makeFakeQuantize(postOp, ngraph::element::f32, outLevels, {}, {0.0}, {1.0},
                                                         {0.0}, {1.0});
        } else if (postOpType == PostOp::TANH) {
            const auto postOp = std::make_shared<ngraph::opset7::Tanh>(conv);
            outputFq = ngraph::builder::makeFakeQuantize(postOp, ngraph::element::f32, outLevels, {}, {-1.0}, {1.0},
                                                         {-1.0}, {1.0});
        } else if (postOpType == PostOp::PRELU) {
            const auto negativeSlope = ngraph::builder::makeConstant<float>(ngraph::element::f32, {1}, {0.1}, false);
            const auto postOp = std::make_shared<ngraph::op::v0::PRelu>(conv, negativeSlope);
            outputFq = ngraph::builder::makeFakeQuantize(postOp, ngraph::element::f32, outLevels, {}, {-128.0}, {127.0},
                                                         {-128.0}, {127.0});
        }
