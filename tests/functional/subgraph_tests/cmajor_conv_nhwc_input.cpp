...
typedef std::tuple<InferenceEngine::Precision, InferenceEngine::Precision, InferenceEngine::SizeVector, InferenceEngine::SizeVector,
                    ngraph::Strides, ngraph::Strides, std::pair<ngraph::CoordinateDiff, ngraph::CoordinateDiff>> KmbCMajorConvNHWCTestParams;
class KmbCMajorConvNHWCTest : public LayerTestsUtils::KmbLayerTestsCommon,
...
const std::vector<InferenceEngine::Precision> prec = {
...
const std::vector<InferenceEngine::SizeVector> weightShapes {
    {16, 3, 1, 1}
};

const std::vector<ngraph::Strides> strides {
    {1, 1}
};

const std::vector<ngraph::Strides> dilations {
    {1, 1}
};

const std::vector<std::pair<ngraph::CoordinateDiff, ngraph::CoordinateDiff>> pads {
    { {0, 0}, {0, 0} }
};
...
INSTANTIATE_TEST_CASE_P(smoke_Permute_NHWC_To_NCHW, KmbCMajorConvNHWCTest,
                        ::testing::Combine(::testing::ValuesIn(prec), ::testing::ValuesIn(prec), ::testing::ValuesIn(inputShapes),
                        ::testing::ValuesIn(weightShapes), ::testing::ValuesIn(strides), ::testing::ValuesIn(dilations),
