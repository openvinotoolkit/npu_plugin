...
class KmbStridedSliceSubGraphTest :
    ...
    void SetUp() override {
        ...
        const auto sliceBeginConst =
                ngraph::builder::makeConstant<int64_t>(ngraph::element::i64, {sliceBegin.size()}, sliceBegin, false);
        std::vector<int64_t> sliceEnd = test_params._end_data;
        const auto sliceEndConst =
                ngraph::builder::makeConstant<int64_t>(ngraph::element::i64, {sliceEnd.size()}, sliceEnd, false);
        std::vector<int64_t> sliceStrides = test_params._strides_data;
        const auto sliceStridesConst = ngraph::builder::makeConstant<int64_t>(
                ngraph::element::i64, {sliceStrides.size()}, sliceStrides, false);

        const auto stridedSlice = std::make_shared<ngraph::op::v1::StridedSlice>(
                paramOuts.at(0), sliceBeginConst, sliceEndConst, sliceStridesConst, test_params._begin_mask,
                test_params._end_mask, test_params._new_axis_mask, test_params._shrink_axis_mask,
                test_params._ellipsis_mask);
        const ngraph::ResultVector results{std::make_shared<ngraph::opset1::Result>(stridedSlice)};
