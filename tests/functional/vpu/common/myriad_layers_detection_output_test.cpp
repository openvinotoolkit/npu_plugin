...
void refDetectionOutput(const TBlob<float>::Ptr &locations,
    ...
    int _num_priors = prior->dims()[2] / 4;

    ASSERT_EQ(_num_priors * _num_loc_classes * 4, locations->dims()[1])
        << "Number of priors must match number of location predictions";
    ASSERT_EQ(_num_priors * NUM_CLASSES, confidence->dims()[1])
    ...
    const int num = locations->dims()[0];
    ...
    const int DETECTION_OUTPUT_SIZE = output->dims()[3];
...
public:
    ...
    void PrepareInput() {
        ...
        ASSERT_NO_THROW(netReader.ReadNetwork(PRIOR_BOX_CLUSTERED_MODEL.data(), PRIOR_BOX_CLUSTERED_MODEL.length()));
        ASSERT_TRUE(netReader.isParseSuccess());

        auto network = netReader.getNetwork();

        auto inputsInfo = network.getInputsInfo();
        inputsInfo["data1"]->setInputPrecision(Precision::FP16);
        inputsInfo["data2"]->setInputPrecision(Precision::FP16);
    ...
    void CalcRefOutput(const bool decrease_label_id) {
        auto locations = make_shared_blob<float, const SizeVector>(Precision::FP32, Layout::ANY, {1, NUM_LOC});
        locations->allocate();
        {
            float *dst = locations->data();
            std::memcpy(dst, gen_locations.data(), NUM_LOC * sizeof(float));
        }

        auto confidence = make_shared_blob<float, const SizeVector>(Precision::FP32, Layout::ANY, {1, NUM_CONF});
        confidence->allocate();
        {
            float *dst = confidence->data();
            std::memcpy(dst, gen_confidence.data(), NUM_CONF * sizeof(float));
        }

        auto prior = make_shared_blob<float, const SizeVector>(Precision::FP32, Layout::ANY, {1, 2, NUM_LOC});
        prior->allocate();
        {
        ...
        refOutput = make_shared_blob<float, const SizeVector>(Precision::FP32, Layout::ANY, {1, 1, NUM_OUT, 7});
        refOutput->allocate();
...
TEST_F(myriadDetectionOutputTests_nightly, NoConst) {
    ...
    CNNNetReader netReader;
    ASSERT_NO_THROW(netReader.ReadNetwork(DETECTION_OUTPUT_MODEL.data(), DETECTION_OUTPUT_MODEL.length()));
    ASSERT_TRUE(netReader.isParseSuccess());
    ...
    inputsInfo["data1"]->setInputPrecision(Precision::FP16);
    inputsInfo["data2"]->setInputPrecision(Precision::FP16);
    inputsInfo["locations"]->setInputPrecision(Precision::FP16);
    inputsInfo["confidence"]->setInputPrecision(Precision::FP16);
...
TEST_F(myriadDetectionOutputTests_nightly, MxNet) {
    ...
    TBlob<uint8_t>::Ptr weights(new TBlob<uint8_t>(Precision::U8, Layout::C, {priorOutput->byteSize()}, priorOutput->buffer().as<uint8_t *>()));

    CNNNetReader netReader;
    ASSERT_NO_THROW(netReader.ReadNetwork(DETECTION_OUTPUT_MODEL_MXNET.data(), DETECTION_OUTPUT_MODEL_MXNET.length()));
    ASSERT_NO_THROW(netReader.SetWeights(weights));
    ASSERT_TRUE(netReader.isParseSuccess());
...
TEST_F(myriadDetectionOutputTests_nightly, WithConst) {
    ...
    CNNNetReader netReader;
    ASSERT_NO_THROW(netReader.ReadNetwork(DETECTION_OUTPUT_MODEL_WITH_CONST.data(), DETECTION_OUTPUT_MODEL_WITH_CONST.length()));
    ASSERT_NO_THROW(netReader.SetWeights(weights));
    ASSERT_TRUE(netReader.isParseSuccess());
