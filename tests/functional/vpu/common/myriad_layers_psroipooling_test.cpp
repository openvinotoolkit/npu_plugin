...
public:
    ...
    void NetworkInit(std::map<std::string, std::string>* params,
                     ...
                     const uint32_t num_rois) {
        std::string xml;
        genXML("PSROIPooling", params, 0, 0, xml);
        ASSERT_NO_THROW(_net_reader.ReadNetwork(xml.data(), xml.length()));
        ASSERT_EQ(_net_reader.isParseSuccess(), true);
        setup(test_params, num_rois);
    }

    void setup(const PSROIPoolingParams& params,
               const uint32_t num_rois) {
        ...
        for (auto inputInfo: _inputsInfo)
        {
            InferenceEngine::SizeVector inputDims = inputInfo.second->getDims();
            InferenceEngine::Layout layout = inputInfo.second->getLayout();
...
TEST_P(myriadLayersTestsPSROIPooling_nightly, PSROIPooling) {
    ...
    PSROIPoolingParams test_params = std::tr1::get<1>(GetParam());
    ...
    ASSERT_NO_FATAL_FAILURE(NetworkInit(&layer_params, test_params, num_rois));
