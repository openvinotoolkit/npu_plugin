...
func.func @BuildSubgraphVFInput(%arg0: tensor<1x16x256x256x!qElemType0, {order = #NHWC}>, %arg1: tensor<1x16x256x256x!qElemType2, {order = #NHWC}>) -> (tensor<1x32x256x256x!qElemType0, {order = #NHWC}>) {
    ...
    %1 = VPU.QuantizeCast(%0) {dstElemType = !qElemType0} : tensor<1x32x256x256x!qElemType2, {order = #NHWC}> -> tensor<1x32x256x256x!qElemType0, {order = #NHWC}>
    %2 = VPU.VerticalFusion (%arg0 as %arg2: tensor<1x16x256x256x!qElemType0, {order = #NHWC}>, %cst_0 as %arg3: tensor<32x16x3x3x!qElemType1, {order = #NHWC}>, %cst_1 as %arg4: tensor<32x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 2, 1]} -> tensor<1x32x256x256x!qElemType0, {order = #NHWC}> {
      %4 = VPU.NCE.Convolution(%arg2, %arg3, %arg4) 
         {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, 
         pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, 
    ...
    %3 = VPU.VerticalFusion (%2 as %arg2: tensor<1x32x256x256x!qElemType0, {order = #NHWC}>, %1 as %arg3: tensor<1x32x256x256x!qElemType0, {order = #NHWC}>) attributes {tilingStrategy = [1, 1, 2, 1]} -> tensor<1x32x256x256x!qElemType0, {order = #NHWC}> {
    ...
    return %3 : tensor<1x32x256x256x!qElemType0, {order = #NHWC}>

    //CHECK: [[VERTICAL_FUSION0:%.+]] = VPU.VerticalFusion 
    //CHECK: [[CONV0:%.+]] = VPU.NCE.Convolution(%arg2, %arg3, %arg4) 
    //CHECK: VPU.Yield [[CONV0]] 

    //CHECK: [[QUANTCAST:%.+]] = VPU.QuantizeCast([[VERTICAL_FUSION0]]) {dstElemType = !qElemType0} 
