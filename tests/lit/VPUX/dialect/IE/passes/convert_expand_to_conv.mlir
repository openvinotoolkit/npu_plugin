...
// CHECK-LABEL: @SkipQuantizedExpand
func.func @SkipQuantizedExpand(%arg0: tensor<1x3x64x224x!qElemType, {order = #NHWC}>)
    ...
    // CHECK-SAME:      pads_begin = [0, 0, 0, 0],
    // CHECK-SAME:      pads_end = [0, 13, 0, 0]
    // CHECK-SAME:  } : tensor<1x3x64x224x!qElemType, {order = #NHWC}>
    // CHECK-SAME:      -> tensor<1x16x64x224x!qElemType, {order = #NHWC}>

    // CHECK:   return [[EXPAND]] : tensor<1x16x64x224x!qElemType, {order = #NHWC}>
}
...
// CHECK-LABEL: @SkipLargeExpand
func.func @SkipLargeExpand(%arg0: tensor<1x3x512x896xf16, {order = #NHWC}>)
    ...
    // CHECK:   return [[EXPAND]] : tensor<1x16x512x896xf16, {order = #NHWC}>
}
