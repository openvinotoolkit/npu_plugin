...
// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=VPUX37XX" --swizzling="enable-activation-swizzling=true enable-weights-swizzling=false" %s | FileCheck %s
...
func.func @SetSwizzlingForDpuToDpuBufferOnly(%in : memref<1x16x56x56xf16, #NHWC, @DDR>) -> memref<1x16x56x56xf16, #NHWC, @DDR> {
    ...
    // CHECK:   VPUIP.NCEClusterTask {activation_window_channel_length = 27 : i64, kernel_padding = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, kernel_size = [1, 1], kernel_strides = [1, 1], task_type = "CONV"}
    // CHECK:   input(%8 : memref<1x16x56x56xf16, #NHWC, @CMX_NN>)
    // CHECK:   weights(%7 : memref<16x16x1x1xf16, #NHWC, @CMX_NN>)
    // CHECK:   weight_table(%6 : memref<16x1x1x4xsi32, #NHWC, @CMX_NN>)
    // CHECK:   parent_input(%8 : memref<1x16x56x56xf16, #NHWC, @CMX_NN>)
    // CHECK:   parent_output(%1 : memref<1x16x56x56xf16, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingScheme<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN>)
    // CHECK:   outputs(%1 : memref<1x16x56x56xf16, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingScheme<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN>) -> memref<1x16x56x56xf16, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingScheme<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN>
}
