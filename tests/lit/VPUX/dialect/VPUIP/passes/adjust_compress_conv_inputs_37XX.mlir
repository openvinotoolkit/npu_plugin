...
// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=%arch%" --adjust-compress-conv-inputs %s | FileCheck %s
...
func.func @CompressConvWeightsLessThan4IC(%arg0: memref<1x16x224x224x!qElemType2, #NHWC, [@CMX_NN, 0]>) -> memref<1x64x112x112x!qElemType3, #NHWC, [@CMX_NN, 0]> {
    %cst = const.Declare memref<64x1x1x4xsi32> = dense<1> : tensor<64x1x1x4xsi32>
...
func.func @CompressConvActivations(%arg0: memref<1x4x224x224x!qElemType2, #NHWC>,
...
!InputBuffer_DDR = memref<1x4x224x224x!qElemType0, #NHWC>
!InputBuffer = memref<1x4x224x224x!qElemType0, #NHWC, [@CMX_NN, 0]>
!WeightsBuffer = memref<64x16x2x2x!qElemType1, #NHWC, [@CMX_NN, 0]>
!WeightsTableBuffer = memref<64x1x1x4xsi32, [@CMX_NN, 0]>
!OutputBuffer = memref<1x64x112x112x!qElemType2, #NHWC, [@CMX_NN, 0]>

// CHECK:       func.func @CompressTiledConvActivations(
...
func.func @CompressTiledConvActivations(%input_ddr: !InputBuffer_DDR,
...
// CHECK:       func.func @CompressConvSparseActivations([[INPUT_DDR:%.+]]: memref<1x4x224x224x!qElemType0, #NHWC>,
...
func.func @CompressConvSparseActivations(%input_ddr: memref<1x4x224x224x!qElemType0, #NHWC>,
...
!OutputSMDistributed = !VPUIP.DistributedBuffer<
...
!InputBuffer_DDR = memref<1x4x224x224x!qElemType0, #NHWC>
!InputSMBuffer_DDR = memref<1x4x224x224xi1, #NHWC>
!InputBuffer = memref<1x4x224x224x!qElemType0, #NHWC, [@CMX_NN, 0]>
!InputSMBuffer = memref<1x4x224x224xi1, #NHWC, [@CMX_NN, 0]>
!WeightsBuffer = memref<64x16x2x2x!qElemType1, #NHWC, [@CMX_NN, 0]>
!WeightsTableBuffer = memref<64x1x1x4xsi32, [@CMX_NN, 0]>
!OutputBuffer = memref<1x64x112x112x!qElemType2, #NHWC, [@CMX_NN, 0]>
!OutputSMBuffer = memref<1x64x112x112xi1, #NHWC, [@CMX_NN, 0]>

// CHECK:       func.func @CompressTiledConvSparseActivations(
// CHECK-SAME:      [[INPUT_DDR:%.+]]: memref<1x4x224x224x!qElemType0, #NHWC>,
// CHECK-SAME:      [[INPUT_SM_DDR:%.+]]: memref<1x4x224x224xi1, #NHWC>,
// CHECK-SAME:      [[WEIGHTS:%.+]]: memref<64x16x2x2x!qElemType1, #NHWC, [@CMX_NN, 0]>,
// CHECK-SAME:      [[WEIGHTS_TABLE:%.+]]: memref<64x1x1x4xsi32, [@CMX_NN, 0]>)
...
func.func @CompressTiledConvSparseActivations(%input_ddr: !InputBuffer_DDR,
...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
!qElemType0 = !quant.uniform<u8:f16, 6.8494566078261131E-4:127>
!qElemType2 = !quant.uniform<u8:f16, 0.0173492431640625:114>
!qElemType3 = !quant.uniform<u8:f16, 0.012699142156862745>
...
func.func @CompressConvWeightsSharedTable(%arg0: memref<1x16x224x224x!qElemType2, #NHWC, [@CMX_NN, 0]>) -> memref<1x64x112x112x!qElemType3, #NHWC, [@CMX_NN, 0]> {
    ...
    %cst_w1 = const.Declare memref<64x16x7x7x!qElemType0, #NHWC> = dense<1.0> :
        tensor<64x13x7x7xf16>, [#const.ConvertElemType<ui8>, #const.QuantCast<!qElemType0>,
        #const.Reorder<#NHWC>, #const.PadWithZero<[0, 0, 0, 0], [0, 3, 0, 0]>]
    %cst_w2 = const.Declare memref<64x64x1x1x!qElemType0, #NHWC> = dense<1.0> :
        tensor<64x64x1x1xf16>, [#const.ConvertElemType<ui8>, #const.QuantCast<!qElemType0>,
        #const.Reorder<#NHWC>, #const.PadWithZero<[0, 0, 0, 0], [0, 0, 0, 0]>]

    %weights1_cmx = memref.alloc() : memref<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0]>
    %weights1 = VPUIP.Copy
        inputs(%cst_w1 : memref<64x16x7x7x!qElemType0, #NHWC>)
        outputs(%weights1_cmx : memref<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0]>)
         -> memref<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0]>
    ...
    %conv1 = VPUIP.NCEClusterTask {
      ...
      input(%arg0 : memref<1x16x224x224x!qElemType2, #NHWC, [@CMX_NN, 0]>)
      weights(%weights1 : memref<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0]>)
    ...
    %weights2_cmx = memref.alloc() : memref<64x64x1x1x!qElemType0, #NHWC, [@CMX_NN, 0]>
    %weights2 = VPUIP.Copy
        inputs(%cst_w2 : memref<64x64x1x1x!qElemType0, #NHWC>)
        outputs(%weights2_cmx : memref<64x64x1x1x!qElemType0, #NHWC, [@CMX_NN, 0]>)
         -> memref<64x64x1x1x!qElemType0, #NHWC, [@CMX_NN, 0]>
    ...
    %conv2 = VPUIP.NCEClusterTask {
      ...
      weights(%weights2 : memref<64x64x1x1x!qElemType0, #NHWC, [@CMX_NN, 0]>)
...
func.func @CompressTiledConvWeightsSharedTable(%arg0: memref<1x16x224x224x!qElemType2, #NHWC, [@CMX_NN, 0]>) -> memref<1x64x112x112x!qElemType3, #NHWC, [@CMX_NN, 0]> {
    ...
    %weights1_cmx = VPURT.AllocDistributed -> !VPUIP.DistributedBuffer<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0],
        {mode = "DUPLICATED", num_clusters = 2 : i64}>
    %weights1 = VPUIP.NCEClusterTiling
        inputs(%cst_w1 as %arg2: memref<64x16x7x7x!qElemType0, #NHWC>)
        outputs(%weights1_cmx as %arg3: memref<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0]>)
        -> !VPUIP.DistributedBuffer<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0],
        {mode = "DUPLICATED", num_clusters = 2 : i64}> {
            %0 = VPUIP.Copy
                inputs(%arg2 : memref<64x16x7x7x!qElemType0, #NHWC>)
                outputs(%arg3 : memref<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0]>)
                -> memref<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0]>
    }
    ...
    %weights_table1 = VPUIP.NCEClusterTiling
        ...
        outputs(%wt1_cmx as %arg3: memref<64x1x1x4xsi32, [@CMX_NN, 0]>) -> !VPUIP.DistributedBuffer<64x1x1x4xsi32, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
    ...
    %conv1 = VPUIP.NCEClusterTiling
        inputs(%arg0 as %arg2: memref<1x16x224x224x!qElemType2, #NHWC, [@CMX_NN, 0]>,
            %weights1 as %arg3: memref<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0]>,
        ...
        -> memref<1x64x112x112x!qElemType3, #NHWC, [@CMX_NN, 0]> {
            ...
            weights(%arg3 : memref<64x16x7x7x!qElemType0, #NHWC, [@CMX_NN, 0]>)
    ...
    //CHECK:        [[WT1_CMX:%.*]] = VPURT.AllocDistributed -> !VPUIP.DistributedBuffer<64x1x1x4xsi32, affine_map<(d0, d1, d2, d3)
    //CHECK-SAME:           -> (d0, d1, d2, d3)>, [@CMX_NN, 0], {mode = "DUPLICATED", num_clusters = 2 : i64}>
    ...
    //CHECK-SAME:           -> !VPUIP.DistributedBuffer<64x1x1x4xsi32, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>
    ...
    //CHECK-SAME:           -> memref<64x1x1x4xsi32, [@CMX_NN, 0]>
    //CHECK:        [[CONV1:%.*]] = VPUIP.NCEClusterTiling inputs(%arg0 as %arg1: memref<1x16x224x224x!qElemType0, #NHWC, [@CMX_NN, 0]>
    //CHECK-SAME:           [[WT1]] as %arg3: memref<64x1x1x4xsi32, [@CMX_NN, 0]>)
    //CHECK-SAME:           -> memref<1x64x112x112x!qElemType1, #NHWC, [@CMX_NN, 0]> {
    //CHECK:        [[CONV1_TASK:%.*]] = VPUIP.NCEClusterTask {
    //CHECK-SAME:           input(%arg1 : memref<1x16x224x224x!qElemType0, #NHWC, [@CMX_NN, 0]>)
