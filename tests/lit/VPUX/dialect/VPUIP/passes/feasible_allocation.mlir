...
func @main(%in: memref<1x16x4x4xf16, #NHWC, [@CMX_NN, 0]>, %out: memref<1x16x4x4xf16, #NHWC>) -> memref<1x16x4x4xf16, #NHWC> {
    ...
    %t0, %f0 = async.execute -> !async.value<memref<1x16x4x4xf16, #NHWC, [@CMX_NN, 0]>>
        ...
        %0 = VPUIP.NCEClusterTask {
            ...
            variants :
            {
                DPUTask { end = [16, 4, 4], mpe_mode = "VECTOR_FP16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0] }
            }
            PPE : {
            }
...
func @main(%arg0: memref<1x16x4x4xf16, #NHWC, [@CMX_NN, 0]>, %arg1: memref<1x16x4x4xf16, #NHWC>, %arg2: memref<1x16x4x4xf16, #NHWC>)
    ...
    %cst = const.Declare memref<1x16x4x4xf16, #NHWC, [@CMX_NN, 0]> = #const.Content<dense<1.000000e+00> : tensor<1x16x4x4xf16>, [#const.Reorder<#NHWC>]>
    %wt = const.Declare memref<16x1x1x4xsi32, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<16x1x1x4xsi32>>
    %act_win = const.Declare memref<1x1x1x16xui8, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<1x1x1x16xui8>>
