...
func.func @MoveSubviewUp(%arg0: memref<1x64x4x4xf16, #NHWC>,
    ...
    %se_table = VPUIP.StorageElementTable { dataElemType = f16, dataShape=[1, 64, 4, 4], seDepth = 1 : i64, seSize = 64 : i64, seAttr = #VPU.SEInterpolate<mode = "BILINEAR", nearest_mode = "FLOOR", coordinate_transformation_mode = "ASYMMETRIC", scale = [1., 1., 2., 2.], offsets = [0, 0, 0, 0], sizes = [1, 64, 9, 9]> } -> memref<1x1x9x9xi32, #NHWC>

    // Group all buffers
    %sparse = VPUIP.GroupSparseBuffer(%arg0, %cst_sm, %se_table)  {seAttr =  #VPU.SEInterpolate<mode = "BILINEAR", nearest_mode = "FLOOR", coordinate_transformation_mode = "ASYMMETRIC", scale = [1., 1., 2., 2.], offsets = [0, 0, 0, 0], sizes = [1, 64, 9, 9]>}
...
func.func @MoveSubviewUpCheckReinfer(%arg0: memref<1x64x4x4xf16, #NHWC>,
    ...
    %1, %2, %3 = VPUIP.UngroupSparseBuffer(%sparse_subview) {result_segment_sizes = dense<[1, 1, 1]> : vector<3xi32>} -> memref<1x64x3x4xf16, {order = #NHWC, strides = [1024, 1, 256, 64]}>, memref<1x64x5x9xi1, {order = #NHWC, strides = [5184, 1, 576, 64]}>, memref<1x1x5x9xi32, #NHWC>
    ...
    // CHEK-NOT:    VPUIP.GroupSparseBuffer
    // CHEK-NOT:    VPUIP.UngroupSparseBuffer
...
func.func @MoveMultipleSubviewUp(
    ...
    // CHECK-SAME:              seAttr = #VPU.SEInterpolate<mode = "BILINEAR", nearest_mode = "FLOOR", coordinate_transformation_mode = "ASYMMETRIC", scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00], offsets = [0, 0, 1, 0], sizes = [1, 64, 4, 9]>, seDepth = 1 : i64, seSize = 64 : i64} -> memref<1x1x4x9xi32, #NHWC>
    // CHECK:           [[SET_OP_1:%.*]] = VPUIP.StorageElementTable {dataElemType = f16, dataShape = [1, 64, 3, 4],
    // CHECK-SAME:              seAttr = #VPU.SEInterpolate<mode = "BILINEAR", nearest_mode = "FLOOR", coordinate_transformation_mode = "ASYMMETRIC", scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00], offsets = [0, 0, 0, 0], sizes = [1, 64, 5, 9]>, seDepth = 1 : i64, seSize = 64 : i64} -> memref<1x1x5x9xi32, #NHWC>
    ...
    // CHECK-SAME:              {seAttr = #VPU.SEInterpolate<mode = "BILINEAR", nearest_mode = "FLOOR", coordinate_transformation_mode = "ASYMMETRIC", scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00],
    // CHECK-SAME:                        offsets = [0, 0, 0, 0], sizes = [1, 64, 5, 9]>}
    // CHECK-SAME:              -> !VPUIP.SparseBuffer<data=memref<1x64x3x4xf16, {order = #NHWC, strides = [1024, 1, 256, 64]}>,
    // CHECK-SAME:                      sparsity_map=memref<1x64x5x9xi1, #NHWC>,
    // CHECK-SAME:                      storage_element_table=memref<1x1x5x9xi32, #NHWC>,
    // CHECK-SAME:                      #VPU.SEInterpolate<mode = "BILINEAR", nearest_mode = "FLOOR", coordinate_transformation_mode = "ASYMMETRIC", scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00],
    // CHECK-SAME:                      offsets = [0, 0, 0, 0], sizes = [1, 64, 5, 9]>>
    ...
    // CHECK-SAME:                      storage_element_table=memref<1x1x4x9xi32, #NHWC>, #VPU.SEInterpolate<mode = "BILINEAR", nearest_mode = "FLOOR", coordinate_transformation_mode = "ASYMMETRIC", scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00],
    // CHECK-SAME:                      offsets = [0, 0, 1, 0], sizes = [1, 64, 4, 9]>>
    ...
    // CHECK:           return [[COPY_1]], [[COPY_0]] : !VPUIP.SparseBuffer<data=memref<1x64x3x4xf16, #NHWC>, sparsity_map=memref<1x64x5x9xi1, #NHWC>, storage_element_table=memref<1x1x5x9xi32, #NHWC>, #VPU.SEInterpolate<mode = "BILINEAR", nearest_mode = "FLOOR", coordinate_transformation_mode = "ASYMMETRIC", scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00], offsets = [0, 0, 0, 0], sizes = [1, 64, 5, 9]>>,
    // CHECK-SAME:                                      !VPUIP.SparseBuffer<data=memref<1x64x2x4xf16, #NHWC>, sparsity_map=memref<1x64x4x9xi1, #NHWC>, storage_element_table=memref<1x1x4x9xi32, #NHWC>, #VPU.SEInterpolate<mode = "BILINEAR", nearest_mode = "FLOOR", coordinate_transformation_mode = "ASYMMETRIC", scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00], offsets = [0, 0, 1, 0], sizes = [1, 64, 4, 9]>>
}
