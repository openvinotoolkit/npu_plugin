...
func @LegalizeCopy(
    ...
    // Cut the second tile:
    // CHECK: [[VAR3:%.*]] = VPUIP.SubView %arg0 [0, 32, 0, 0] [1, 32, 512, 512] :
    // CHECK-SAME:      memref<1x64x512x512xf16>
    // CHECK-SAME:   to memref<1x32x512x512xf16, {order = #NCHW, strides = [16777216, 262144, 512, 1]}>
    // CHECK: [[VAR4:%.*]] = VPUIP.SubView %arg1 [0, 32, 0, 0] [1, 32, 512, 512] :
    // CHECK-SAME:      memref<1x64x512x512xf16>
    // CHECK-SAME:   to memref<1x32x512x512xf16, {order = #NCHW, strides = [16777216, 262144, 512, 1]}>
...
func @DoNotLegalizeCopy(
    ...
    %0 = VPUIP.Copy inputs(%arg0 : memref<1x32x512x512xf16, #NCHW>)
