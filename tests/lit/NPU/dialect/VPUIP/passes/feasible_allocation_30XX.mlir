...
func.func @main(%in: memref<1x16x96x96xf16, #NHWC>, %out: memref<1x16x96x96xf16, #NHWC>) -> memref<1x16x96x96xf16, #NHWC> {
    ...
    %t_in, %r_in = async.execute -> !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 0 : i64} {
        %0 = VPUIP.NNDMA inputs(%in : memref<1x16x96x96xf16, #NHWC>) outputs(%buf_in : memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>) -> memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>
    ...
    %t0, %r0 = async.execute -> !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 1 : i64} {
        %0 = VPUIP.NNDMA inputs(%cst0 : memref<1x16x96x96xf16, #NHWC>) outputs(%buf0 : memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>) -> memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>
        async.yield %0 : memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>
    }

    %t3, %r3 = async.execute [%t_in] (%r_in as %0 : !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 2 : i64} {
    ...
    %t1, %r1 = async.execute [%t3, %t0] (%r3 as %0 : !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>>, %r0 as %1 : !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 3 : i64} {
    ...
    %t5, %r5 = async.execute [%t_in, %t1] (%r_in as %0 : !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>>, %r1 as %1 : !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 4 : i64} {
        %2 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
            }
    ...
    %t6, %r6 = async.execute [%t5] (%r5 as %0 : !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %1 = VPUIP.NNDMA inputs(%0 : memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>) outputs(%out : memref<1x16x96x96xf16, #NHWC>) -> memref<1x16x96x96xf16, #NHWC>
    ...
    // CHECK:         IE.MemoryResource 884736 bytes of @CMX_NN
    ...
    // CHECK-NEXT:       VPUIP.NNDMA {port = 0 : i64} inputs(%arg0 : memref<1x16x96x96xf16, #NHWC>) outputs([[BUF0:%.*]] :
    ...
    // CHECK-SAME:      ([[R5]] as [[ARG2:%.*]]: !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>>,
    // CHECK:       VPUIP.NCEClusterTask
    // CHECK-SAME:      task_type = #VPUIP.nce_task_type<ELTWISE>
...
func.func @main(%in: memref<1x16x96x96xf16, #NHWC>, %out: memref<1x16x96x96xf16, #NHWC>) -> memref<1x16x96x96xf16, #NHWC> {
    ...
    %t0, %r0 = async.execute -> !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 0 : i64} {
        %0 = VPUIP.NNDMA inputs(%in : memref<1x16x96x96xf16, #NHWC>) outputs(%buf0 : memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>) -> memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>
        async.yield %0 : memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>
    }
    ...
    %t1, %r1 = async.execute -> !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 1 : i64} {
        %0 = VPUIP.NNDMA inputs(%cst0 : memref<1x16x96x96xf16, #NHWC>) outputs(%buf3 : memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>) -> memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>
        async.yield %0 : memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>
    }
    ...
    %t5, %r5 = async.execute [%t4] (%r4 as %arg0 : !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x96x96xf16, #NHWC>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 5 : i64} {
        %0 = VPUIP.NNDMA inputs(%arg0 : memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>) outputs(%out : memref<1x16x96x96xf16, #NHWC>) -> memref<1x16x96x96xf16, #NHWC>
    ...
    // CHECK-NEXT       VPUIP.NNDMA inputs([[BUF2]] : memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>)
    // CHECK:       [[T2:%.+]], [[R2:%.+]] = async.execute
    // CHECK-NEXT       VPUIP.NNDMA
    ...
    // CHECK-NEXT       VPUIP.NNDMA inputs(%arg2 : memref<1x16x96x96xf16, #NHWC, @DDR>)
    // CHECK-SAME       -> memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>
...
func.func @main(%in: memref<1x16x64x64xf16, #NHWC>, %out: memref<1x16x64x64xf16, #NHWC>) -> memref<1x16x64x64xf16, #NHWC> {
    ...
    %t_dma_in, %r_dma_in = async.execute -> !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 0 : i64} {
        %0 = VPUIP.NNDMA inputs(%in : memref<1x16x64x64xf16, #NHWC>) outputs(%buf0 : memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>) -> memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>
    ...
    %t0, %r0 = async.execute [%t_dma_in] (%r_dma_in as %arg0 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 1 : i64} {
        ...
        %1 = VPUIP.NCEClusterTask {
            ...
            variants :
            {
                DPUTask { outEnd = [16, 64, 64], mpe_mode = #VPU.mpe_mode<VECTOR_FP16>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, outStart = [0, 0, 0] }
            }
            PPE : {
            }
    ...
    %t1, %r1 = async.execute [%t0] (%r0 as %arg0 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 2 : i64} {
    ...
    %t2, %r2 = async.execute -> !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 3 : i64} {
        %0 = VPUIP.NNDMA inputs(%cst0 : memref<1x16x64x64xf16, #NHWC>) outputs(%buf1 : memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>) -> memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>
        async.yield %0 : memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>
    }

    %t3, %r3 = async.execute [%t1] (%r1 as %arg0 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 4 : i64} {
        %0 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 27 : i64,
                kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
                kernel_size = [1, 1],
    ...
    %t4, %r4 = async.execute [%t3, %t2] (%r3 as %arg0 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>, %r2 as %arg1 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 5 : i64} {
        %0 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
            }
    ...
    %t5, %r5 = async.execute [%t1, %t4] (%r1 as %arg0 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>, %r4 as %arg1 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 6 : i64} {
        %0 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
            }
    ...
    %t6, %r6 = async.execute [%t5] (%r5 as %arg0 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 7 : i64} {
        %0 = VPUIP.SubView %buf_master [0, 0, 0, 0][1, 16, 64, 64] : memref<1x32x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]> to memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>
        %1 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
    ...
    %t7, %r7 = async.execute [%t6] (%r0 as %arg0 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>, %r6 as %arg1 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 8 : i64} {
        %0 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
            }
    ...
    %t_tma_out, %r_dma_out = async.execute [%t7] (%r7 as %arg0 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>)
        ...
        %0 = VPUIP.NNDMA inputs(%arg0 : memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>) outputs(%out : memref<1x16x64x64xf16, #NHWC>) -> memref<1x16x64x64xf16, #NHWC>
    ...
    // CHECK:       builtin.module @UsedMemory 
    // CHECK:         IE.MemoryResource 786432 bytes of @CMX_NN
    ...
    // CHECK:       [[T7:%.+]], [[R7:%.+]] = async.execute
    // CHECK-SAME:      ([[R3]] as [[ARG0:%.*]]: !async.value<memref<1x32x64x64xf16, #NHWC, @DDR>>
    // CHECK:       VPUIP.NNDMA
    // CHECK-SAME:      spillId
...
func.func @main(%in: memref<1x16x85x85xf16, #NHWC>, %out: memref<1x16x85x85xf16, #NHWC>) -> memref<1x16x85x85xf16, #NHWC> {
    ...
    %t_in, %r_in = async.execute -> !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 0 : i64} {
        %0 = VPUIP.NNDMA inputs(%in : memref<1x16x85x85xf16, #NHWC>) outputs(%buf_in : memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>) -> memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>
    ...
    %t0, %r0 = async.execute -> !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 1 : i64} {
        %0 = VPUIP.NNDMA inputs(%cst0 : memref<1x16x85x85xf16, #NHWC>) outputs(%buf0 : memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>) -> memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>
        async.yield %0 : memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>
    }

    %t1, %r1 = async.execute [%t_in] (%r_in as %0 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 2 : i64} {
        %1 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 27 : i64,
                kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
                kernel_size = [1, 1],
            ...
            variants :
            {
                DPUTask { outEnd = [16, 85, 85], mpe_mode = #VPU.mpe_mode<VECTOR_FP16>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, outStart = [0, 0, 0] }
            }
            PPE : {
            }
    ...
    %t2, %r2 = async.execute [%t0, %t1] (%r0 as %0 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>, %r1 as %1 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 3 : i64} {
        %2 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
            }
    ...
    %t3, %r3 = async.execute [%t_in, %t2] (%r_in as %0 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>, %r2 as %1 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 4 : i64} {
        %2 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
            }
    ...
    %t4, %r4 = async.execute [%t3] -> !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 5 : i64} {
        %0 = VPUIP.NNDMA inputs(%cst0 : memref<1x16x85x85xf16, #NHWC>) outputs(%buf4 : memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>) -> memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>
        async.yield %0 : memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>
    }
    ...
    %t5, %r5 = async.execute [%t3, %t4] (%r3 as %0 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>, %r4 as %1 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 6 : i64} {
        %2 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
            }
    ...
    %t6, %r6 = async.execute [%t_in, %t5] (%r_in as %0 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>, %r5 as %1 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 7 : i64} {
        %2 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
            }
    ...
    %t7, %r7 = async.execute [%t6] (%r6 as %0 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %1 = VPUIP.NNDMA inputs(%0 : memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>) outputs(%out : memref<1x16x85x85xf16, #NHWC>) -> memref<1x16x85x85xf16, #NHWC>
    ...
    // CHECK:       builtin.module @UsedMemory 
    // CHECK:         IE.MemoryResource 693696 bytes of @CMX_NN
    ...
    // CHECK:       [[T0:%.+]], [[R0:%.+]] = async.execute ->
    // CHECK-NEXT:       VPUIP.NNDMA {port = 0 : i64} inputs(%arg0 : memref<1x16x85x85xf16, #NHWC>) outputs([[BUF_TO_SPILL:%.*]] :
...
func.func @main(%in: memref<1x16x112x112xf16, #NHWC>, %out0: memref<1x16x112x112xf16, #NHWC>, %out1: memref<1x16x112x112xf16, #NHWC>) -> (memref<1x16x112x112xf16, #NHWC>, memref<1x16x112x112xf16, #NHWC>) {
    ...
    %t0, %f0 = async.execute -> !async.value<memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 0 : i64} {
        %0 = VPUIP.NNDMA inputs(%in : memref<1x16x112x112xf16, #NHWC>) outputs(%buf0 : memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>) -> memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>
    ...
    %t1, %f1 = async.execute (%f0 as %arg0 : !async.value<memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 1 : i64} {
        %0 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 27 : i64,
                kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
                kernel_size = [1, 1],
    ...
    %t2, %f2 = async.execute (%f1 as %arg0 : !async.value<memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %0 = VPUIP.NNDMA inputs(%arg0 : memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>) outputs(%out0 : memref<1x16x112x112xf16, #NHWC>) -> memref<1x16x112x112xf16, #NHWC>
    ...
    %t3, %f3 = async.execute (%f0 as %arg0 : !async.value<memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 3 : i64} {
        %0 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 27 : i64,
                kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
                kernel_size = [1, 1],
    ...
    %t4, %f4 = async.execute (%f3 as %arg0 : !async.value<memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %0 = VPUIP.NNDMA inputs(%arg0 : memref<1x16x112x112xf16, #NHWC, [@CMX_NN, 0]>) outputs(%out1 : memref<1x16x112x112xf16, #NHWC>) -> memref<1x16x112x112xf16, #NHWC>
        async.yield %0 : memref<1x16x112x112xf16, #NHWC>
    }
    ...
    // CHECK:       builtin.module @UsedMemory 
    // CHECK:         IE.MemoryResource 802816 bytes of @CMX_NN
    
    // CHECK:       [[BUF0:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <0>
    // CHECK:       [[BUF1:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <401408>
    // CHECK:       [[BUF2:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <401408>
...
func.func @main(%in: memref<1x32x60x60xf16, #NHWC>, %out: memref<1x32x60x60xf16, #NHWC>) -> memref<1x32x60x60xf16, #NHWC> {
    ...
    %t_in, %r_in = async.execute -> !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 0 : i64} {
        %0 = VPUIP.NNDMA inputs(%in : memref<1x32x60x60xf16, #NHWC>) outputs(%buf_in : memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>) -> memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>
    ...
    %t3, %r3 = async.execute [%t_in] (%r_in as %0 : !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 1 : i64} {
        %1 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 27 : i64,
                kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
                kernel_size = [1, 1],
    ...
    %t0, %r0 = async.execute -> !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 2 : i64} {
        %0 = VPUIP.NNDMA inputs(%cst0 : memref<1x32x60x60xf16, #NHWC>) outputs(%buf1 : memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>) -> memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>
        async.yield %0 : memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>
    }
    ...
    %t1, %r1 = async.execute [%t3, %t0] (%r3 as %0 : !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>>, %r0 as %1 : !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 3 : i64} {
        %2 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
            }
    ...
    %t5, %r5 = async.execute [%t_in, %t1] (%r_in as %0 : !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>>, %r1 as %1 : !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 1 : i64, "async-deps-index" = 4 : i64} {
        %2 = VPUIP.NCEClusterTask {
                activation_window_channel_length = 0 : i64,
                task_type = #VPUIP.nce_task_type<ELTWISE>
            }
    ...
    %t6, %r6 = async.execute [%t5] (%r5 as %0 : !async.value<memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %1 = VPUIP.NNDMA inputs(%0 : memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>) outputs(%out : memref<1x32x60x60xf16, #NHWC>) -> memref<1x32x60x60xf16, #NHWC>
    ...
    // CHECK:       builtin.module @UsedMemory 
    // CHECK:         IE.MemoryResource 691200 bytes of @CMX_NN
    
    // CHECK:       [[BUF0:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <0>
    // CHECK:       [[BUF1:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <230400>
    // CHECK:       [[BUF2:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <460800>
    // CHECK:       [[BUF3:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <0>
    // CHECK:       [[BUF4:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <460800>
    // CHECK:       [[BUF_SPILL_WRITE:%.*]] = memref.alloc() : memref<1x32x60x60xf16, #NHWC, @DDR>
    // CHECK:       [[BUF_SPILL_READ:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <230400> -> memref<1x32x60x60xf16, #NHWC, [@CMX_NN, 0]>
...
func.func @main(%input: !BufMemrefDDR) -> !BufMemrefDDR {
    ...
    %t5, %r5 = async.execute [%t_in, %t1] (%r_in as %async_arg0 : !async.value<!BufDistributed>, %r1 as %async_arg1 : !async.value<!BufDistributed>) 
            -> !async.value<!BufDistributed> attributes {VPUIP.executor = @DPU, VPUIP.num_units = 4 : i64, "async-deps-index" = 7 : i64} {
    ...
    // CHECK:       builtin.module @UsedMemory 
    // CHECK:         IE.MemoryResource 787008 bytes of @CMX_NN
    ...
    // CHECK:       [[BUF0:%.*]] = VPURT.DeclareBuffer <CMX_NN> <0> -> !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    // CHECK:       [[BUF1:%.*]] = VPURT.DeclareBuffer <CMX_NN> <262144> -> !VPUIP.DistributedBuffer<32x1x1x4xsi32, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    // CHECK:       [[BUF2:%.*]] = VPURT.DeclareBuffer <CMX_NN> <262656> -> !VPUIP.DistributedBuffer<1x1x1x32xui8, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    // CHECK:       [[BUF3:%.*]] = VPURT.DeclareBuffer <CMX_NN> <262720> -> !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    ...
    // CHECK:       [[BUF5:%.*]] = VPURT.DeclareBuffer <CMX_NN> <0> -> !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    ...
    // CHECK:       [[BUF_SPILL_WRITE:%.*]] = memref.alloc() : memref<1x32x64x64xf16, #NHWC, @DDR>
    // CHECK:       [[BUF_SPILL_READ:%.*]] = VPURT.DeclareBuffer <CMX_NN> <262144> -> !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    ...
    // CHECK:       [[T6:%.*]], [[R6:%.*]] = async.execute
    // CHECK-SAME:    [[T1]], [[T2]], [[T3]], [[T5]], [[T4]]]
    // CHECK-SAME:    ([[R3]] as [[ARG1]]: !async.value<!VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>>
    // CHECK-SAME:     [[R5]] as [[ARG2]]: !async.value<!VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>>)
...
func.func @main(%in: memref<1x32x16x16xf16, #NHWC>, %out: memref<1x128x4x4xf16, #NHWC>) -> memref<1x128x4x4xf16, #NHWC> {
    ...
    // CHECK:           VPUIP.NNDMA
    // CHECK:       {VPUIP.executor = @DMA_NN, VPUIP.executorIdx = [0], "async-deps-index" = 1 : i64, cycleBegin = 683 : i64, cycleCost = 2172 : i64, cycleEnd = 2855 : i64}
    // CHECK:           VPUIP.NNDMA
    // CHECK:       {VPUIP.executor = @DMA_NN, VPUIP.executorIdx = [0], "async-deps-index" = 2 : i64, cycleBegin = 2855 : i64, cycleCost = 22 : i64, cycleEnd = 2877 : i64}
    // CHECK:           VPUIP.NNDMA

    // CHECK:       {VPUIP.executor = @DPU, "async-deps-index" = 5 : i64, cycleBegin = 2877 : i64, cycleCost = 734 : i64, cycleEnd = 3611 : i64}
    // CHECK:           VPUIP.NCEClusterTask

    // Prefetched Copy executing in parallel cycles to NCEClusterTask
    
    // CHECK:       {VPUIP.executor = @DMA_NN, VPUIP.executorIdx = [0], "async-deps-index" = 3 : i64, cycleBegin = 2877 : i64, cycleCost = 6144 : i64, cycleEnd = 9021 : i64}
    // CHECK:           VPUIP.NNDMA
    // CHECK:       {VPUIP.executor = @DMA_NN, VPUIP.executorIdx = [0], "async-deps-index" = 4 : i64, cycleBegin = 9021 : i64, cycleCost = 61 : i64, cycleEnd = 9082 : i64}
    // CHECK:           VPUIP.NNDMA

    // CHECK:       {VPUIP.executor = @DPU, "async-deps-index" = 6 : i64, cycleBegin = 9082 : i64, cycleCost = 686 : i64, cycleEnd = 9768 : i64}
    // CHECK:           VPUIP.NCEClusterTask
...
func.func @main(%arg0: memref<1x16x16x3xui8, @DDR>, %arg1: memref<1x16x16x3xf32, @DDR>) -> memref<1x16x16x3xf32, @DDR> {
    ...
    %token, %results = async.execute -> !async.value<memref<1x16x16x3xui8, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, "async-deps-index" = 0 : i64, cycleCost = 27 : i64} {
      %7 = VPUIP.NNDMA inputs(%arg0 : memref<1x16x16x3xui8, @DDR>) outputs(%0 : memref<1x16x16x3xui8, [@CMX_NN, 0]>) -> memref<1x16x16x3xui8, [@CMX_NN, 0]>
    ...
    %token_0, %results_1 = async.execute [%token] (%results as %arg2: !async.value<memref<1x16x16x3xui8, [@CMX_NN, 0]>>) -> !async.value<memref<1x3x16x16xui8, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, "async-deps-index" = 1 : i64, cycleCost = 27 : i64} {
      %7 = VPUIP.PermuteDMA {mem_perm = #NWCH} inputs(%arg2 : memref<1x16x16x3xui8, [@CMX_NN, 0]>) outputs(%1 : memref<1x3x16x16xui8, [@CMX_NN, 0]>) -> memref<1x3x16x16xui8, [@CMX_NN, 0]>
    ...
    %token_2, %results_3 = async.execute [%token_0] (%results_1 as %arg2: !async.value<memref<1x3x16x16xui8, [@CMX_NN, 0]>>) -> !async.value<memref<1x3x16x16xui8, @DDR>> attributes {VPUIP.executor = @DMA_NN, "async-deps-index" = 2 : i64, cycleCost = 27 : i64} {
      %7 = VPUIP.NNDMA inputs(%arg2 : memref<1x3x16x16xui8, [@CMX_NN, 0]>) outputs(%2 : memref<1x3x16x16xui8, @DDR>) -> memref<1x3x16x16xui8, @DDR>
    ...
    %token_6, %results_7 = async.execute [%token_4] (%results_5 as %arg2: !async.value<memref<1x3x16x16xf16, @DDR>>) -> !async.value<memref<1x3x16x16xf16, @DDR>> attributes {VPUIP.executor = @SHAVE_UPA, "async-deps-index" = 4 : i64, cycleCost = 2 : i64} {
      %7 = VPUIP.EltwiseUPA {task_type = #VPU.eltwise_type<POWER>} inputs(%arg2 : memref<1x3x16x16xf16, @DDR>, %cst : memref<1x1x1x1xf16>) outputs(%4 : memref<1x3x16x16xf16, @DDR>) -> memref<1x3x16x16xf16, @DDR>
      async.yield %7 : memref<1x3x16x16xf16, @DDR>
    }
