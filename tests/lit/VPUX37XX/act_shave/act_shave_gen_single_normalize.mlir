...
IE.CNNNetwork
    ...
    outputsInfo : {
        IE.DataInfo "normalize" : tensor<1x512x64x64xf16>
    }
...
module @VPU.SW {
    ...
    func private @builtin_normalize(%input : memref<*xf16>, %output : memref<*xf16>,
    %eps : f32,
    %across_spatial: i64,
    %channel_shared: i64
    )
        attributes {
            VPU.kernel_code = "normalize_fp16.cpp",
            VPU.kernel_entry = "normalize_fp16"
        }
...
func @main(%1: memref<1x512x64x64xf16>, %2: memref<1x512x64x64xf16>) -> memref<1x512x64x64xf16> {

    %in_tile0_cmx  = VPURT.DeclareBuffer "CMX_NN" [0] <0> -> memref<1x512x64x64xf16, [@CMX_NN, 0]>
    %out_tile0_cmx = VPURT.DeclareBuffer "CMX_NN" [0] <2048> -> memref<1x512x64x64xf16, [@CMX_NN, 0]>

    %b0 = VPURT.ConfigureBarrier<0> -> !VPURT.Barrier
    %b1 = VPURT.ConfigureBarrier<1> -> !VPURT.Barrier

    VPURT.Task updates(%b0 : !VPURT.Barrier) {
        VPUIP.NNDMA inputs(%1 : memref<1x512x64x64xf16>) outputs(%in_tile0_cmx : memref<1x512x64x64xf16, [@CMX_NN, 0]>) -> memref<1x512x64x64xf16, [@CMX_NN, 0]>
    }

    // Genetic Kernel information for the scheduler.
    VPURT.Task waits(%b0  : !VPURT.Barrier) updates(%b1  : !VPURT.Barrier) {
        VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>}
                    @VPU.SW::@builtin_normalize            // The reference to the Kernel function.
                    inputs(%in_tile0_cmx as %arg0: memref<1x512x64x64xf16, [@CMX_NN, 0]>)     // Inputs/outputs buffers for generic operation interface
                    outputs(%out_tile0_cmx as %arg1: memref<1x512x64x64xf16, [@CMX_NN, 0]>)   // and their mapping to inner region.
                    on tile 0                           // The tile index to execute on.

        -> memref<1x512x64x64xf16, [@CMX_NN, 0]> {

                // The arguments mapping, the order must match the kernel parameter structure.
                VPUIP.SW.Kernel.run {attrs=[1, 1, 0.00001]} (%arg0, %arg1)
                    : memref<1x512x64x64xf16, [@CMX_NN, 0]>
                    , memref<1x512x64x64xf16, [@CMX_NN, 0]>
        }
    }
    ...
    VPURT.Task waits(%b1 : !VPURT.Barrier) {
        %0 = VPUIP.NNDMA inputs(%out_tile0_cmx : memref<1x512x64x64xf16, [@CMX_NN, 0]>) outputs(%2 : memref<1x512x64x64xf16>) -> memref<1x512x64x64xf16>
    }
...
// CHECK:                  },
// CHECK:                  dataSection: {
// CHECK:                    name: "builtin_normalize_invo",
// CHECK:                    locale: "GFEmbeddedKernel",
// CHECK:                  },
