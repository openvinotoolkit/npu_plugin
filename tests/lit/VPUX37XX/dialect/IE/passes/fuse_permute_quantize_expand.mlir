...
//
// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=VPUX37XX" --fuse-permute-quantize-expand  %s | FileCheck %s
...
func @fusePermuteQuantizeExpand(%arg0: tensor<1x3x8x16xf16>) -> tensor<1x1x8x16xf16> {
  ...
  %4 = IE.Add(%3, %3) {auto_broadcast = "NONE_OR_EXPLICIT"} : tensor<1x16x8x16x!quant.uniform<u8:f16, 5.000000e-01>, {order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}>, tensor<1x16x8x16x!quant.uniform<u8:f16, 5.000000e-01>, {order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}> -> tensor<1x16x8x16xf16, {order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}>
...
// CHECK: [[CST:%.*]] = const.Declare tensor<16x4x1x1x!qElemType0, {order = #NHWC}> =
...
// CHECK: [[VAL3:%.+]] = IE.Add([[VAL2]], [[VAL2]]) {auto_broadcast = "NONE_OR_EXPLICIT"} : tensor<1x16x8x16x!qElemType2, {order = #NHWC}>, tensor<1x16x8x16x!qElemType2, {order = #NHWC}> -> tensor<1x16x8x16xf16, {order = #NHWC}>
...
func @FusePermuteQuantizeExpandTogheterRewrite(%arg0: tensor<1x3x8x16xf16>) -> tensor<1x1x8x16xf16> {
  ...
  %2 = IE.Add(%1, %1) {auto_broadcast = "NONE_OR_EXPLICIT"} : tensor<1x16x8x16xf16, {order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}>, tensor<1x16x8x16xf16, {order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}> -> tensor<1x16x8x16x!quant.uniform<u8:f16, 2.000000e+00>, {order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}>
  ...
  %6 = IE.Add(%5, %5) {auto_broadcast = "NONE_OR_EXPLICIT"} : tensor<1x16x8x16x!quant.uniform<u8:f16, 5.000000e-01>, {order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}>, tensor<1x16x8x16x!quant.uniform<u8:f16, 5.000000e-01>, {order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}> -> tensor<1x16x8x16xf16, {order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}>
...
// CHECK-SAME:          [[INPUT:%arg[0-9]]]: tensor<1x3x8x16xf16>
// CHECK: [[CST:%.*]] = const.Declare tensor<16x16x1x1x!qElemType0, {order = #NHWC}> =
...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
!qElemType0 = type !quant.uniform<u8:f16, 0.0021560968137254903>
!qElemType1 = type !quant.uniform<u8:f16, 0.0010780484068627452>

func @FuseQuantizeCastExpandIntoPermuteQuantizeQuantizeCastRewrite(%arg0: tensor<1x1x1x64xf16>) -> tensor<1x16x1x64xf16, {order = #NHWC}> {
  ...
  %3 = IE.Add(%2, %2) {auto_broadcast = "NONE_OR_EXPLICIT"} : tensor<1x16x1x64x!qElemType1, {order = #NHWC}>, tensor<1x16x1x64x!qElemType1, {order = #NHWC}> -> tensor<1x16x1x64xf16, {order = #NHWC}>
...
// CHECK: [[VAL2:%.+]] = IE.Add([[VAL1]], [[VAL1]]) {auto_broadcast = "NONE_OR_EXPLICIT"} : tensor<1x16x1x64x!qElemType1, {order = #NHWC}>, tensor<1x16x1x64x!qElemType1, {order = #NHWC}> -> tensor<1x16x1x64xf16, {order = #NHWC}>
...
func @PreserveQuantCast(%arg0: tensor<1x3x62x62xf16>) -> tensor<1x4x62x62x!qElemType1, {order = #NHWC}> {
  %0 = IE.Expand(%arg0) {
      pads_begin = [0, 0, 0, 0],
      pads_end = [0, 1, 0, 0]
...
func @FusePermuteQuantizeExpandTogheterRewriteFp32(%arg0: tensor<1x3x62x62xf32>) -> tensor<1x4x62x62x!qElemType1, {order = #NHWC}> {
    %10 = IE.Convert(%arg0) {
    dstElemType = f16
...
func @FusePermuteQuantizeExpandTogheterRewriteFp32WithReshape(%arg0: tensor<3x62x62xf32>) -> tensor<1x4x62x62x!qElemType1, {order = #NHWC}> {
  %10 = IE.Convert(%arg0) {
    dstElemType = f16
