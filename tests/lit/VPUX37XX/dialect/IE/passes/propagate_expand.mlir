...
func.func @PropagateExpandFullCase(%arg0: tensor<1x3x512x512xf16>, %arg1: tensor<1x32x256x256x!qElemType0, {order = #NHWC}>) -> (tensor<1x16x128x128x!qElemType0, {order = #NHWC}>, tensor<1x4x512x512x!qElemType1, {order = #NHWC}>) {
    ...
    //CHECK:    [[CST_0:%.*]] = const.Declare tensor<64x4x4x4x!qElemType2, {order = #NHWC}>
    //CHECK:    [[CST_1:%.*]] = const.Declare tensor<16x64x3x3x!qElemType3, {order = #NHWC}> = dense<1.000000e+00>
    //CHECK-SAME: tensor<16x48x3x3xf16>, [#const.ConvertElemType<ui8>, #const.QuantCast<!qElemType3>, #const.Reorder<#NHWC>, #const.Reshape<[16, 3, 48, 3]>, #const.PadWithZero<[0, 0, 0, 0], [0, 1, 0, 0]>, #const.Reshape<[16, 64, 3, 3]>]
    //CHECK:    [[CST_2:%.*]] = const.Declare tensor<16x32x3x3x!qElemType3, {order = #NHWC}> = dense<1.000000e+00> : tensor<16x32x3x3xf16>, [#const.ConvertElemType<ui8>, #const.QuantCast<!qElemType3>, #const.Reorder<#NHWC>]
    ...
    //CHECK:    [[S2D:%.*]] = IE.Convolution([[EXPAND]], [[CST_0]]) {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [4, 4]} : tensor<1x4x512x512x!qElemType0, {order = #NHWC}>, tensor<64x4x4x4x!qElemType2, {order = #NHWC}> -> tensor<1x64x128x128x!qElemType0, {order = #NHWC}>
    //CHECK:    [[CONV_0:%.*]] = IE.Convolution([[S2D]], [[CST_1]]) {dilations = [1, 1], pads_begin = [1, 1], pads_end = [1, 1], post_op = {attrs = {negative_slope = 0.30000001192092896 : f64}, name = "IE.LeakyRelu"}, strides = [1, 1]} : tensor<1x64x128x128x!qElemType0, {order = #NHWC}>, tensor<16x64x3x3x!qElemType3, {order = #NHWC}> -> tensor<1x16x128x128x!qElemType0, {order = #NHWC}>
    //CHECK:    [[CONV_1:%.*]] = IE.Convolution(%arg1, [[CST_2]]) {dilations = [1, 1], pads_begin = [1, 1], pads_end = [1, 1], post_op = {attrs = {negative_slope = 0.30000001192092896 : f64}, name = "IE.LeakyRelu"}, strides = [1, 1]} : tensor<1x32x256x256x!qElemType0, {order = #NHWC}>, tensor<16x32x3x3x!qElemType3, {order = #NHWC}> -> tensor<1x16x256x256x!qElemType0, {order = #NHWC}>
    //CHECK:    [[D2S:%.*]] = IE.DepthToSpace([[CONV_1]]) {block_size = 2 : i64, mode = #IE.depth_to_space_mode<BLOCKS_FIRST>, padded_channels = {input = 4 : i64, output = 1 : i64}} : tensor<1x16x256x256x!qElemType0, {order = #NHWC}> -> tensor<1x4x512x512x!qElemType0, {order = #NHWC}>
