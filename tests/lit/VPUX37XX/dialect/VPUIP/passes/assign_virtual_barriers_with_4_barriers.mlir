// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=VPUX37XX" --assign-virtual-barriers="num-barriers=4 num-slots-per-barrier=256" %s | FileCheck %s
...
func @ParallelGraph(%arg0: memref<1x16x32x32xf16, #NHWC>, %arg1: memref<1x16x32x32xf16>) -> memref<1x16x32x32xf16> {
    ...
    VPURT.Task waits(%bar4: !VPURT.Barrier) updates(%bar5: !VPURT.Barrier) attributes {cycleBegin = 6 : i64, cycleEnd = 7 : i64} {
         VPUIP.NNDMA
    ...
    // CHECK:       VPURT.Task waits([[BAR0]], [[BAR2]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR3]] : !VPURT.Barrier)
    ...
    // CHECK:       VPURT.Task waits([[BAR7]] : !VPURT.Barrier) updates([[BAR9]] : !VPURT.Barrier)
    // CHECK:       VPUIP.NNDMA

    // CHECK:       VPURT.Task waits([[BAR9]] : !VPURT.Barrier) updates([[BAR10]] : !VPURT.Barrier)
    // CHECK:       VPUIP.NCEClusterTask

    // CHECK:       VPURT.Task waits([[BAR8]] : !VPURT.Barrier) updates([[BAR10]] : !VPURT.Barrier)
    // CHECK:       VPUIP.NNDMA

    // CHECK:       VPURT.Task waits([[BAR10]] : !VPURT.Barrier) updates([[BAR5]] : !VPURT.Barrier)
    // CHECK:       VPUIP.NNDMA
