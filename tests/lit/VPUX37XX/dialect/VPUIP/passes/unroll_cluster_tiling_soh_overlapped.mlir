...
func.func @UnrollNCEWithInconsistentPadAndKernelSize(%input: !Input_DDR, %output: !Output_DDR) -> !Output_DDR {
    ...
    //CHECK:    [[ACTWIN1_CMX:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <34378> -> memref<1x1x1x16xui8, [@CMX_NN, 0]>
    //CHECK:    [[ACTWIN2_CMX:%.*]] = VPURT.DeclareBuffer <CMX_NN> [1] <34378> -> memref<1x1x1x16xui8, [@CMX_NN, 1]>
    //CHECK:    [[ACTWIN_CMX_COPY:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0, 1] <34378> -> !VPUIP.DistributedBuffer<1x1x1x16xui8, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>
    ...
    //CHECK:          VPUIP.NNDMA {port = 0 : i64}
    //CHECK-SAME:       inputs([[ACTWIN_CST]] : memref<1x1x1x16xui8>)
    //CHECK-SAME:       outputs([[ACTWIN_CMX_COPY]] : !VPUIP.DistributedBuffer<1x1x1x16xui8, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>)
    //CHECK:        }
