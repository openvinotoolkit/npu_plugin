...
// CHECK-LABEL: @SwizzleConstant
func @SwizzleConstant() -> memref<256x1x1x1xsi32, [@CMX_NN, 0]> {
    %weight_table_const = const.Declare memref<256x1x1x1xsi32> = #const.Content<dense<1> : tensor<256x1x1x1xsi32>>
    %weights_const = const.Declare memref<64x16x3x3xf16, #NHWC> = #const.Content<dense<1.0> : tensor<64x16x3x3xf16>, [#const.Reorder<#NHWC>]>
    ...
    %5 = VPUIP.NCEClusterTask {kernel_padding = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, kernel_size = [3, 3], kernel_strides = [2, 2], task_type = "CONV"} 
    ...
    // CHECK: [[WT_CONST:%.*]] = const.Declare memref<256x1x1x1xsi32> = #const.Content<dense<1> : tensor<256x1x1x1xsi32>, [#const.SwizzleConstant<5 : i64, 3 : i64>]>
    // CHECK: [[W_CONST:%.*]] = const.Declare memref<64x16x3x3xf16, #NHWC> = #const.Content<dense<1.000000e+00> : tensor<64x16x3x3xf16>, [#const.Reorder<#NHWC>, #const.SwizzleConstant<5 : i64, 3 : i64>]>
...
// CHECK-LABEL: @SwizzleConstantWithNCEClusterTiling
func @SwizzleConstantWithNCEClusterTiling(%arg0: !Input_DDR) -> !Output_DDR {

    %buf_in = VPURT.DeclareBuffer "CMX_NN" <0> -> !InputDistributed
    %buf_W = VPURT.DeclareBuffer "CMX_NN" <16384> {swizzlingKey = 5 : i64} -> !WeightsDistributed
    %buf_WT = VPURT.DeclareBuffer "CMX_NN" <86016> {swizzlingKey = 5 : i64} -> !WeightsTableDistributed
    ...
    // CHECK:       [[WEIGHT_BUF:%.*]] = VPURT.DeclareBuffer "CMX_NN" <[[WEIGHTS_ADDR:[^>]+]]> {swizzlingKey = 5 : i64} -> !VPUIP.DistributedBuffer
    // CHECK:       [[WEIGHT_TABLE_BUF:%.*]] = VPURT.DeclareBuffer "CMX_NN" <86016> {swizzlingKey = 5 : i64} -> !VPUIP.DistributedBuffer
    ...
    // CHECK:       [[CONST_W:%.*]] = const.Declare memref<64x32x3x3xf16, #NHWC, @DDR> = #const.Content<dense<1.000000e+00> : tensor<64x32x3x3xf16>, [#const.Reorder<#NHWC>, #const.SwizzleConstant<5 : i64, 3 : i64>]>
    // CHECK:       [[CONST_WT:%.*]] = const.Declare memref<64x1x1x4xsi32, #NHWC, @DDR> = #const.Content<dense<1> : tensor<64x1x1x4xsi32>, [#const.Reorder<#NHWC>, #const.SwizzleConstant<5 : i64, 3 : i64>]>
