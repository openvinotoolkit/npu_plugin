...
func @WithProfiling(%arg0: memref<1x512xf16>, %arg1: memref<1x512xf16>) -> memref<1x512xf16> {
    %t1, %f1 = async.execute -> !async.value<memref<1x512xf16>>
        attributes { VPUIP.executor = @DMA_NN, VPUIP.num_units = 1, cycleBegin = 0 : i64, cycleCost = 45 : i64, cycleEnd = 45 : i64 } {
    ...
    // CHECK:       VPURT.Task
    // CHECK-SAME:      cycleBegin = 1 : i64, cycleCost = 43 : i64, cycleEnd = 44 : i64
    // CHECK-NEXT:  [[NNDMA2:%.*]] = VPUIP.NNDMA
    // CHECK-SAME:      inputs([[VAR0]] : memref<1x512xf16, @DDR>)
    // CHECK-SAME:      outputs(%arg1 : memref<1x512xf16>)

    // CHECK:       VPURT.Task
    // CHECK-SAME:      cycleBegin = 44 : i64, cycleCost = 1 : i64, cycleEnd = 45 : i64
