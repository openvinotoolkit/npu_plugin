...
func @UnrollNCENonContiguousOutput(%input: !Input_DDR, %output: !Output_DDR) -> !Output_DDR {
         ...
         %0 = VPUIP.NCEClusterTiling
               ...
               %1 = VPUIP.NCEClusterTask {
                         ...
                         outputs(%arg3 : !OutputStub_CMX)
                             -> !OutputStub_CMX variants :  {
                            DPUTask {
                                start = [0, 0, 0], end = [15, 31, 15],
                                mpe_mode = "VECTOR_FP16",
                                pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64},
                                cluster_id = 0 : i64
                            }
                            DPUTask {
                                start = [0, 0, 16], end = [15, 31, 31],
                                mpe_mode = "VECTOR_FP16",
                                pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64},
                                cluster_id = 1 : i64
                            }
    ...
    //CHECK-SAME:       #const.Content<dense<1> : tensor<32x1x1x4xsi32>, [#const.SubView<[0, 0, 0, 0], [8, 1, 1, 4]>]>
    //CHECK:        [[WEIGHTS_TABLE2_CST:%.*]] = const.Declare memref<8x1x1x4xsi32> =
    //CHECK-SAME:       #const.Content<dense<1> : tensor<32x1x1x4xsi32>, [#const.SubView<[8, 0, 0, 0], [8, 1, 1, 4]>]>

    //CHECK:        [[WEIGHTS1_CST:%.*]] = const.Declare memref<8x16x1x1xf16, #NHWC> =
    //CHECK-SAME:       #const.Content<dense<1.000000e+00> : tensor<32x16x1x1xf16>, [#const.Reorder<#NHWC>, #const.SubView<[0, 0, 0, 0], [8, 16, 1, 1]>]>
    //CHECK:        [[WEIGHTS2_CST:%.*]] = const.Declare memref<8x16x1x1xf16, #NHWC> =
    //CHECK-SAME:       #const.Content<dense<1.000000e+00> : tensor<32x16x1x1xf16>, [#const.Reorder<#NHWC>, #const.SubView<[8, 0, 0, 0], [8, 16, 1, 1]>]>
    ...
    //CHECK-SAME:       variants :  {
    //CHECK:                DPUTask {cluster_id = 0 : i64, end = [15, 31, 15], mpe_mode = "VECTOR_FP16",
    //CHECK-SAME:               pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0]}
    //CHECK:          } PPE :  {
    //CHECK:          }
    //CHECK:        }
    ...
    //CHECK-SAME:       variants :  {
    //CHECK:                DPUTask {cluster_id = 1 : i64, end = [15, 31, 31], mpe_mode = "VECTOR_FP16",
    //CHECK-SAME:               pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 16]}
    //CHECK:          } PPE :  {
    //CHECK:          }
    //CHECK:        }
