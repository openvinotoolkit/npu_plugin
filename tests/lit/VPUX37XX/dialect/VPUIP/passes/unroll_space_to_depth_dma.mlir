...
//
// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=VPUX37XX" --unroll-space-to-depth-dma  %s | FileCheck %s
...
func @UnrollSegmentedClusterSpaceToDepthDMA() -> !OutputDistributed {
    %bar0 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    %bar1 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    ...
    VPURT.Task waits(%bar0 : !VPURT.Barrier) updates(%bar1 : !VPURT.Barrier) attributes {cycleBegin = 990 : i64, cycleEnd = 1758 : i64, isTrailingSWLayer = false} {
      VPUIP.NCEClusterTiling inputs(%0 as %arg0: memref<1x3x4x6x!qElemType, #NHWC, [@CMX_NN, 0]>) outputs(%1 as %arg1: memref<1x12x2x3x!qElemType, #NHWC, @CMX_NN>) -> !OutputDistributed {
        VPUIP.SpaceToDepthDMA {block_size = 2 : i64, mode = "BLOCKS_FIRST", port = 0 : i64} 
    ...
    //CHECK:      VPUIP.SpaceToDepthDMA {block_size = 2 : i64, dma_descriptor = {dstPlaneStride = 6 : i64, dstStride = 12 : i64, dstWidth = 6 : i64, len = 18 : i64, numPlanes = 2 : i64, srcPlaneStride = 18 : i64, srcStride = 36 : i64, srcWidth = 18 : i64}, mode = "BLOCKS_FIRST", port = 0 : i64}
