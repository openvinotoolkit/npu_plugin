...
func.func @UnrollExpandDMAWithSEGMENTED() -> !OutputDistributed {
    ...
    VPURT.Task updates(%bar0 : !VPURT.Barrier) {
        %0 = VPUIP.NCEClusterTiling inputs(%input as %arg0: memref<1x3x240x320xf16, #NHWC>)
                outputs(%output as %arg1: memref<1x4x240x320xf16, #NHWC, @CMX_NN>) -> !OutputDistributed {
            VPUIP.ExpandDMA {pads_begin = [0, 0, 0, 0], pads_end = [0, 1, 0, 0], port = 0 : i64}
                    inputs(%arg0 : memref<1x3x240x320xf16, #NHWC>)
                    outputs(%arg1 : memref<1x4x240x320xf16, #NHWC, @CMX_NN>) -> memref<1x4x240x320xf16, #NHWC, @CMX_NN>
        }
    }
    ...
    //CHECK:        VPUIP.ExpandDMA {dma_descriptor = {dstPlaneStride = 0 : i64, dstStride = 8 : i64, dstWidth = 6 : i64, len = 230400 : i64, numPlanes = 1 : i64, srcPlaneStride = 0 : i64, srcStride = 230400 : i64, srcWidth = 230400 : i64}, pads_begin = [0, 0, 0, 0], pads_end = [0, 1, 0, 0], port = 0 : i64}
    ...
    //CHECK:    VPURT.Task updates([[BARRIER]] : !VPURT.Barrier) attributes {isTrailingSWLayer = false} {
    //CHECK:        VPUIP.ExpandDMA {dma_descriptor = {dstPlaneStride = 0 : i64, dstStride = 8 : i64, dstWidth = 6 : i64, len = 230400 : i64, numPlanes = 1 : i64, srcPlaneStride = 0 : i64, srcStride = 230400 : i64, srcWidth = 230400 : i64}, pads_begin = [0, 0, 0, 0], pads_end = [0, 1, 0, 0], port = 1 : i64}
...
func.func @UnrollExpandDMAWithDUPLICATED() -> !OutputDistributed {
    ...
    VPURT.Task updates(%bar0 : !VPURT.Barrier) {
        %0 = VPUIP.NCEClusterTiling inputs(%input as %arg0: memref<1x4420x1x2xf16, #NHWC>)
                outputs(%output as %arg1: memref<1x4432x1x2xf16, #NHWC, @CMX_NN>) -> !OutputDistributed {
            VPUIP.ExpandDMA {pads_begin = [0, 0, 0, 0], pads_end = [0, 12, 0, 0], port = 0 : i64}
                    inputs(%arg0 : memref<1x4420x1x2xf16, #NHWC>)
                    outputs(%arg1 : memref<1x4432x1x2xf16, #NHWC, @CMX_NN>) -> memref<1x4432x1x2xf16, #NHWC, @CMX_NN>
        }
    }
    ...
    //CHECK:    VPURT.Task updates([[BARRIER]] : !VPURT.Barrier) attributes {isTrailingSWLayer = false} {
    //CHECK:        VPUIP.ExpandDMA {dma_descriptor = {dstPlaneStride = 0 : i64, dstStride = 8864 : i64, dstWidth = 8840 : i64, len = 17680 : i64, numPlanes = 1 : i64, srcPlaneStride = 0 : i64, srcStride = 17680 : i64, srcWidth = 17680 : i64}, pads_begin = [0, 0, 0, 0], pads_end = [0, 12, 0, 0], port = 0 : i64}
...
func.func @UnrollExpandDMAWithOVERLAPPED() -> !OutputDistributed {
    ...
    //CHECK:    [[INPUT1:%.*]] = VPURT.DeclareBuffer "NetworkInput" [0] <230400> -> memref<1x3x120x320xf16, #NHWC, @DDR>
    //CHECK:    [[OUTPUT:%.*]] = VPURT.DeclareBuffer "CMX_NN" <0> -> !VPUIP.DistributedBuffer<1x4x240x320xf16, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], kernel = [3, 3], pads = {bottom = 1 : i64, left = 0 : i64, right = 1 : i64, top = 0 : i64}, strides = [2, 2], num_clusters = 2 : i64}>

    //CHECK:    [[OUTPUT0:%.*]] = VPURT.DeclareBuffer "CMX_NN" [0] <0> -> memref<1x4x121x320xf16, #NHWC, [@CMX_NN, 0]>
    //CHECK:    [[OUTPUT1:%.*]] = VPURT.DeclareBuffer "CMX_NN" [1] <0> -> memref<1x4x120x320xf16, #NHWC, [@CMX_NN, 1]>
    //CHECK:    VPURT.Task updates([[BARRIER]] : !VPURT.Barrier) attributes {isTrailingSWLayer = false} {
    //CHECK:        VPUIP.ExpandDMA {dma_descriptor = {dstPlaneStride = 0 : i64, dstStride = 8 : i64, dstWidth = 6 : i64, len = 232320 : i64, numPlanes = 1 : i64, srcPlaneStride = 0 : i64, srcStride = 232320 : i64, srcWidth = 232320 : i64}, pads_begin = [0, 0, 0, 0], pads_end = [0, 1, 0, 0], port = 0 : i64}
    ...
    //CHECK:    return [[OUTPUT]] : !VPUIP.DistributedBuffer<1x4x240x320xf16, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], kernel = [3, 3], pads = {bottom = 1 : i64, left = 0 : i64, right = 1 : i64, top = 0 : i64}, strides = [2, 2], num_clusters = 2 : i64}>
}
...
func.func @UnrollExpandDMAWithLargeSizeAndDiffWithExpandAxis() -> memref<1x32x720x1280x!qElemType, #NWCH, @DDR> {
    ...
    //CHECK:    VPURT.Task updates([[BARRIER]] : !VPURT.Barrier) attributes {isTrailingSWLayer = false} {
    //CHECK:        VPUIP.ExpandDMA {dma_descriptor = {dstPlaneStride = 0 : i64, dstStride = 23040 : i64, dstWidth = 14400 : i64, len = 16776000 : i64, numPlanes = 1 : i64, srcPlaneStride = 0 : i64, srcStride = 16776000 : i64, srcWidth = 16776000 : i64}, pads_begin = [0, 0, 0, 0], pads_end = [0, 12, 0, 0], port = 0 : i64}
    ...
    //CHECK:    VPURT.Task updates([[BARRIER]] : !VPURT.Barrier) attributes {isTrailingSWLayer = false} {
    //CHECK:        VPUIP.ExpandDMA {dma_descriptor = {dstPlaneStride = 0 : i64, dstStride = 23040 : i64, dstWidth = 14400 : i64, len = 1656000 : i64, numPlanes = 1 : i64, srcPlaneStride = 0 : i64, srcStride = 1656000 : i64, srcWidth = 1656000 : i64}, pads_begin = [0, 0, 0, 0], pads_end = [0, 12, 0, 0], port = 1 : i64}
...
func.func @UnrollExpandDMAWithLargeSizeAndSameWithExpandAxis() -> memref<1x32x720x1280x!qElemType, #NCHW, @DDR> {
    ...
    //CHECK:    VPURT.Task updates([[BARRIER]] : !VPURT.Barrier) attributes {isTrailingSWLayer = false} {
    //CHECK:        VPUIP.ExpandDMA {dma_descriptor = {dstPlaneStride = 0 : i64, dstStride = 29491200 : i64, dstWidth = 16588800 : i64, len = 16588800 : i64, numPlanes = 1 : i64, srcPlaneStride = 0 : i64, srcStride = 16588800 : i64, srcWidth = 16588800 : i64}, pads_begin = [0, 0, 0, 0], pads_end = [0, 12, 0, 0], port = 0 : i64}
    ...
    //CHECK:    VPURT.Task updates([[BARRIER]] : !VPURT.Barrier) attributes {isTrailingSWLayer = false} {
    //CHECK:        VPUIP.ExpandDMA {dma_descriptor = {dstPlaneStride = 0 : i64, dstStride = 29491200 : i64, dstWidth = 1843200 : i64, len = 1843200 : i64, numPlanes = 1 : i64, srcPlaneStride = 0 : i64, srcStride = 1843200 : i64, srcWidth = 1843200 : i64}, pads_begin = [0, 0, 0, 0], pads_end = [0, 12, 0, 0], port = 1 : i64}
