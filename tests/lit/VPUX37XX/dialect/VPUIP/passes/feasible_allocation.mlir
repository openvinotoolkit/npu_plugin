...
func @main(%in: memref<1x32x96x96xf16, #NHWC>, %out: memref<1x32x96x96xf16, #NHWC>) -> memref<1x32x96x96xf16, #NHWC> {
    %cst0 = const.Declare memref<1x32x96x96xf16, #NHWC> = #const.Content<dense<2.0> : tensor<1x32x96x96xf16>, [#const.Reorder<#NHWC>]>
    ...
    %t3, %r3 = async.execute [%t_in] (%r_in as %0 : !async.value<memref<1x32x96x96xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %1 = VPUIP.NCEClusterTask {
            ...
            variants :
            {
                DPUTask { end = [32, 96, 96], mpe_mode = "VECTOR_FP16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0] }
            }
            PPE : {
            }
...
func @main(%in: memref<1x32x64x64xf16, #NHWC>, %out: memref<1x32x64x64xf16, #NHWC>) -> memref<1x32x64x64xf16, #NHWC> {
    %cst0 = const.Declare memref<1x32x64x64xf16, #NHWC> = #const.Content<dense<2.0> : tensor<1x32x64x64xf16>, [#const.Reorder<#NHWC>]>
    %wt = const.Declare memref<16x1x1x4xsi32, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<16x1x1x4xsi32>>
    %act_win = const.Declare memref<1x1x1x16xui8, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<1x1x1x16xui8>>
    ...
    %t0, %r0 = async.execute [%t_dma_in] (%r_dma_in as %arg0 : !async.value<memref<1x32x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>)
        ...
        %1 = VPUIP.NCEClusterTask {
            ...
            variants :
            {
                DPUTask { end = [32, 64, 64], mpe_mode = "VECTOR_FP16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0] }
            }
            PPE : {
            }
...
func @main(%in: memref<1x48x75x75xf16, #NHWC>, %out: memref<1x48x75x75xf16, #NHWC>) -> memref<1x48x75x75xf16, #NHWC> {
    %cst0 = const.Declare memref<1x48x75x75xf16, #NHWC> = #const.Content<dense<2.0> : tensor<1x48x75x75xf16>, [#const.Reorder<#NHWC>]>
    %wt = const.Declare memref<48x1x1x4xsi32, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<48x1x1x4xsi32>>
    %act_win = const.Declare memref<1x1x1x48xui8, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<1x1x1x48xui8>>
    ...
    %t1, %r1 = async.execute [%t_in] (%r_in as %0 : !async.value<memref<1x48x75x75xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %1 = VPUIP.NCEClusterTask {
            ...
            variants :
            {
                DPUTask { end = [48, 75, 75], mpe_mode = "VECTOR_FP16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0] }
            }
            PPE : {
            }
...
func @main(%in: memref<1x32x112x112xf16, #NHWC>, %out0: memref<1x32x112x112xf16, #NHWC>, %out1: memref<1x32x112x112xf16, #NHWC>) -> (memref<1x32x112x112xf16, #NHWC>, memref<1x32x112x112xf16, #NHWC>) {
    ...
    %t1, %f1 = async.execute (%f0 as %arg0 : !async.value<memref<1x32x112x112xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %0 = VPUIP.NCEClusterTask {
            ...
            variants :
            {
                DPUTask { end = [32, 112, 112], mpe_mode = "VECTOR_FP16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0] }
            }
            PPE : {
            }
        async.yield %0 : memref<1x32x112x112xf16, #NHWC, [@CMX_NN, 0]>
...
func @main(%in: memref<1x80x60x60xf16, #NHWC>, %out: memref<1x80x60x60xf16, #NHWC>) -> memref<1x80x60x60xf16, #NHWC> {
    %cst0 = const.Declare memref<1x80x60x60xf16, #NHWC> = #const.Content<dense<2.0> : tensor<1x80x60x60xf16>, [#const.Reorder<#NHWC>]>
    ...
    %t3, %r3 = async.execute [%t_in] (%r_in as %0 : !async.value<memref<1x80x60x60xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %1 = VPUIP.NCEClusterTask {
            ...
            variants :
            {
                DPUTask { end = [80, 60, 60], mpe_mode = "VECTOR_FP16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0] }
            }
            PPE : {
            }
...
func @main(%input: !BufMemrefDDR) -> !BufMemrefDDR {
    %cst0 = const.Declare memref<1x64x64x64xf16> = #const.Content<dense<2.0> : tensor<1x64x64x64xf16>>
    %cst1 = const.Declare !WtMemrefCMX = #const.Content<dense<1> : tensor<64x1x1x4xsi32>>
    %cst2 = const.Declare !ActWinMemrefCMX = #const.Content<dense<1> : tensor<1x1x1x64xui8>>
    ...
    // CHECK:       [[BUF1:%.*]] = VPURT.DeclareBuffer "CMX_NN" <1048576> -> !VPUIP.DistributedBuffer<64x1x1x4xsi32, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    // CHECK:       [[BUF2:%.*]] = VPURT.DeclareBuffer "CMX_NN" <1049600> -> !VPUIP.DistributedBuffer<1x1x1x64xui8, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    ...
    // CHECK:       [[BUF4:%.*]] = VPURT.DeclareBuffer "CMX_NN" <524288> -> !VPUIP.DistributedBuffer<1x64x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
