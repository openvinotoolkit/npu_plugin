...
func @DontMergeACTAndDMA(%arg0: memref<16xf16>, %arg1: memref<16xf16>, %arg2: memref<16xf16>)
    ...
    %t0, %f0 = async.execute
        ...
        %0 = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>}
            @VPU.SW::@builtin_TanhOp inputs(%arg0 : memref<16xf16>) outputs(%buf0 : memref<16xf16>) on tile 0 -> memref<16xf16>  {
            ^bb0(%arg3: memref<16xf16>, %arg4: memref<16xf16>):
    ...
    %t1, %f1 = async.execute [%t0] (%f0 as %0: !async.value<memref<16xf16>>)
        ...
        %1 = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>}
            @VPU.SW::@builtin_Sigmoid inputs(%0 : memref<16xf16>) outputs(%buf1 : memref<16xf16>) on tile 0 -> memref<16xf16>  {
            ^bb0(%arg3: memref<16xf16>, %arg4: memref<16xf16>):
                VPUIP.SW.Kernel.run {attrs = [0]}(%arg3, %arg4) : memref<16xf16>, memref<16xf16>
            }
    ...
    %t2, %f2 = async.execute [%t0] (%f0 as %0: !async.value<memref<16xf16>>)
        ...
        %2 = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>}
            @VPU.SW::@builtin_Sigmoid inputs(%0 : memref<16xf16>) outputs(%buf2 : memref<16xf16>) on tile 0 -> memref<16xf16>  {
            ^bb0(%arg3: memref<16xf16>, %arg4: memref<16xf16>):
                VPUIP.SW.Kernel.run {attrs = [0]}(%arg3, %arg4) : memref<16xf16>, memref<16xf16>
            }
    ...
    // CHECK:           [[VAR0:%.*]] = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>} @VPU.SW::@builtin_TanhOp inputs(%arg0 : memref<16xf16>) outputs([[BUF0]] : memref<16xf16>)
    // CHECK:           async.yield [[VAR0]]

    // CHECK:       [[T1:%.+]], [[F1:%.+]] = async.execute
    // CHECK-SAME:          [[T0]]
    // CHECK-SAME:          [[F0]] as [[VAR0_0:%.*]]: !async.value<memref<16xf16>>
    // CHECK:           [[VAR1:%.*]] = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>} @VPU.SW::@builtin_Sigmoid inputs([[VAR0_0]] : memref<16xf16>) outputs([[BUF1]] : memref<16xf16>)
    // CHECK:           async.yield [[VAR1]]

    // CHECK:       [[T2:%.+]], [[F2:%.+]] = async.execute
    // CHECK-SAME:          [[T0]]
    // CHECK-SAME:          [[F0]] as [[VAR0_1:%.*]]: !async.value<memref<16xf16>>
    // CHECK:           [[VAR2:%.*]] = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>} @VPU.SW::@builtin_Sigmoid inputs([[VAR0_1]] : memref<16xf16>) outputs([[BUF2]] : memref<16xf16>)
...
func @MergeDMAs(%arg0: memref<16xf16>, %arg1: memref<16xf16>, %arg2: memref<16xf16>)
    ...
    %t0, %f0 = async.execute
            ...
            attributes {VPUIP.executor = @SHAVE_ACT} {
        %0 = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>}
            @VPU.SW::@builtin_TanhOp inputs(%arg0 : memref<16xf16>) outputs(%buf : memref<16xf16>) on tile 0 -> memref<16xf16>  {
            ^bb0(%arg3: memref<16xf16>, %arg4: memref<16xf16>):
                VPUIP.SW.Kernel.run {attrs = [0]}(%arg3, %arg4) : memref<16xf16>, memref<16xf16>
            }
        async.yield %0 : memref<16xf16>
    ...
    // CHECK:       [[T0:%.+]], [[F0:%.+]] = async.execute
    // CHECK:           [[VAR0:%.*]] = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>} @VPU.SW::@builtin_TanhOp inputs(%arg0 : memref<16xf16>) outputs([[BUF]] : memref<16xf16>)
    // CHECK:           async.yield [[VAR0]]
...
func @TaskWithExclusiveUsers(%arg0: memref<16xf16>, %arg1: memref<16xf16>, %arg2: memref<16xf16>)
    ...
    %t0, %f0 = async.execute
            ...
            attributes {VPUIP.executor = @SHAVE_ACT} {
        %0 = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>}
            @VPU.SW::@builtin_Sigmoid inputs(%arg0 : memref<16xf16>) outputs(%buf : memref<16xf16>) on tile 0 -> memref<16xf16>  {
            ^bb0(%arg3: memref<16xf16>, %arg4: memref<16xf16>):
                VPUIP.SW.Kernel.run {attrs = [0]}(%arg3, %arg4) : memref<16xf16>, memref<16xf16>
            }
        async.yield %0 : memref<16xf16>
    ...
    %t1, %f1 = async.execute
            ...
            attributes {VPUIP.executor = @SHAVE_ACT} {
        %1 = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>}
            @VPU.SW::@builtin_Sigmoid inputs(%arg0 : memref<16xf16>) outputs(%arg1 : memref<16xf16>) on tile 0 -> memref<16xf16>  {
            ^bb0(%arg3: memref<16xf16>, %arg4: memref<16xf16>):
                VPUIP.SW.Kernel.run {attrs = [0]}(%arg3, %arg4) : memref<16xf16>, memref<16xf16>
            }
        async.yield %1 : memref<16xf16>
    ...
    %t2, %f2 = async.execute [%t0] (%f0 as %0: !async.value<memref<16xf16>>)
            ...
            attributes {VPUIP.executor = @SHAVE_ACT} {
        %2 = VPUIP.SW.Kernel {result_segment_sizes = dense<[1, 0]> : vector<2xi32>}
            @VPU.SW::@builtin_Sigmoid inputs(%0 : memref<16xf16>) outputs(%arg2 : memref<16xf16>) on tile 0 -> memref<16xf16>  {
            ^bb0(%arg3: memref<16xf16>, %arg4: memref<16xf16>):
                VPUIP.SW.Kernel.run {attrs = [0]}(%arg3, %arg4) : memref<16xf16>, memref<16xf16>
            }
        async.yield %2 : memref<16xf16>
