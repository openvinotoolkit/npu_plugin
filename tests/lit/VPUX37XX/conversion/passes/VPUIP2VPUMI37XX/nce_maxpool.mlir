...
//CHECK: %[[VAL0:.*]] = VPUMI37XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 3 : ui8}<0, -1> -> !VPURegMapped.Index<0:0:0>
//CHECK: %[[VAL1:.*]] = VPUMI37XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8}<1, -1> -> !VPURegMapped.Index<0:0:1>
...
//CHECK: VPUMI37XX.NNDMA {{.*}} inputs(%arg0 : memref<1x64x16x16xf16, #NHWC, @DDR>) outputs(%[[VAL2]] : [[TYPE2]]) updates(%[[VAL0]] : !VPURegMapped.Index<0:0:0>)

//CHECK-NOT: VPURT.Task
//CHECK: VPUMI37XX.NNDMA {{.*}} inputs(%[[VALcst]] : [[TYPE_CST]]) outputs(%[[VAL7]] : [[TYPE7]])
