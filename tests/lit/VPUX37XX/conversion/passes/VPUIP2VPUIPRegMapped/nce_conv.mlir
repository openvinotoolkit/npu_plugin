...
func private @multiple_clusters_dpu_soh_f16_f16_f16(%arg0: memref<1x32x32x32xf16, #NHWC, @DDR>, %arg1: memref<1x64x16x32xf16, #NHWC, @DDR>, %arg2: memref<1x64x16x32xf16, #NHWC, @DDR>) -> (memref<1x64x16x32xf16, #NHWC, @DDR>, memref<1x64x16x32xf16, #NHWC, @DDR>) {
  ...
  VPURT.Task waits(%0 : !VPURT.Barrier) updates(%1 : !VPURT.Barrier) attributes {isTrailingSWLayer = false} {
    %18 = VPUIP.NCEClusterTask {is_segmented, kernel_padding = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, kernel_size = [1, 1], kernel_strides = [1, 1], task_type = "CONV"} input(%12 : memref<1x32x16x32xf16, #NHWC, [@CMX_NN, 1]>) weights(%6 : memref<64x32x1x1xf16, #NHWC, [@CMX_NN, 1]>) weight_table(%14 : memref<64x1x1x4xsi32, #NHWC, [@CMX_NN, 1]>) parent_input(%10 : !VPUIP.DistributedBuffer<1x32x32x32xf16, {order = #NHWC, strides = [32768, 1, 1024, 32]}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, alignment = [1, 1, 1, 1]}>) parent_output(%7 : !VPUIP.DistributedBuffer<1x64x32x32xf16, {order = #NHWC, strides = [65536, 1, 2048, 64]}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) outputs(%9 : memref<1x64x16x32xf16, #NHWC, [@CMX_NN, 1]>) -> memref<1x64x16x32xf16, #NHWC, [@CMX_NN, 1]> variants : {
      DPUTask {outEnd = [31, 31, 63], mpe_mode = "CUBOID_16x16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, outStart = [0, 16, 0]}
    } PPE : {
    }
  }
...
//CHECK-NEXT: VPUIPRegMapped.NNDMA {{.*}} inputs(%[[CST]] : [[TYPE_CST]]) outputs(%[[VAL16]], %[[VAL17]] : [[TYPE16]], [[TYPE17]])
//CHECK-NOT: VPURT.Task
//CHECK-NEXT: VPUIPRegMapped.NNDMA {{.*}} inputs(%[[VAL2]] : [[TYPE2]]) outputs(%[[VAL11]] : [[TYPE11]])
//CHECK-NOT: VPURT.Task
//CHECK-NEXT: VPUIPRegMapped.NNDMA {{.*}} inputs(%[[VAL3]] : [[TYPE3]]) outputs(%[[VAL12]] : [[TYPE12]])
//CHECK-NOT: VPURT.Task
//CHECK-NEXT: %[[VAL21:.*]] = VPURT.DeclareBuffer "CMX_NN" [0] <102400> -> [[TYPE21:.*]]
//CHECK-NEXT: %[[VAL22:.*]] = VPURT.DeclareBuffer "CMX_NN" [1] <102400> -> [[TYPE22:.*]]
//CHECK-NEXT: VPUIPRegMapped.NNDMA {{.*}} inputs(%[[CST_0]] : [[TYPE_CST0]]) outputs(%[[VAL21]], %[[VAL22]] : [[TYPE21]], [[TYPE22]])
//CHECK-NOT: VPURT.Task
//CHECK-NEXT: VPUIPRegMapped.DPUInvariant {{.*}} is_segmented
...
module @mainModule {
  IE.CNNNetwork entryPoint : @multiple_clusters_dpu_sok_f16_f16_f16 inputsInfo : {
    DataInfo "input_0" : tensor<1x32x16x16xf16, {order = #NHWC}>
  } outputsInfo : {
    DataInfo "output_0" : tensor<1x64x16x16xf16, {order = #NHWC}>
    DataInfo "output_1" : tensor<1x64x16x16xf16, {order = #NHWC}>
  }

  func private @multiple_clusters_dpu_sok_f16_f16_f16(%arg0: memref<1x32x16x16xf16, #NHWC, @DDR>, %arg1: memref<1x64x16x16xf16, #NHWC, @DDR>, %arg2: memref<1x64x16x16xf16, #NHWC, @DDR>) -> (memref<1x64x16x16xf16, #NHWC, @DDR>, memref<1x64x16x16xf16, #NHWC, @DDR>) {
    %0 = VPURT.ConfigureBarrier<0> -> !VPURT.Barrier
    ...
    VPURT.Task waits(%0 : !VPURT.Barrier) updates(%12 : !VPURT.Barrier) attributes {isTrailingSWLayer = false} {
      %15 = VPUIP.NCEClusterTask {kernel_padding = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, kernel_size = [1, 1], kernel_strides = [1, 1], task_type = "CONV"} input(%9 : memref<1x32x16x16xf16, #NHWC, [@CMX_NN, 1]>) weights(%3 : memref<32x32x1x1xf16, #NHWC, [@CMX_NN, 1]>) weight_table(%11 : memref<32x1x1x4xsi32, #NHWC, [@CMX_NN, 1]>) parent_input(%7 : !VPUIP.DistributedBuffer<1x32x16x16xf16, {order = #NHWC, strides = [8192, 1, 512, 32]}, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>) parent_output(%4 : !VPUIP.DistributedBuffer<1x64x16x16xf16, {order = #NHWC, strides = [16384, 1, 1024, 64]}, @CMX_NN, {mode = "DUPLICATED|SEGMENTED", num_tiles = [1, 2, 1, 1], num_clusters = 2 : i64, alignment = [1, 16, 1, 1]}>) outputs(%6 : !VPUIP.DistributedBuffer<1x32x16x16xf16, {order = #NHWC, strides = [16384, 1, 1024, 64]}, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>) -> !VPUIP.DistributedBuffer<1x32x16x16xf16, {order = #NHWC, strides = [16384, 1, 1024, 64]}, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}> variants : {
        DPUTask {outEnd = [15, 15, 63], mpe_mode = "CUBOID_16x16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, outStart = [0, 0, 32]}
      } PPE : {
      }
    }
...
//CHECK-NOT: VPURT.Task
//CHECK-NEXT: VPUIPRegMapped.NNDMA {{.*}} inputs(%[[VALCST]] : [[TYPECST]]) outputs(%[[VAL2]] : [[TYPE2]])
//CHECK-NOT: VPURT.Task
//CHECK-NEXT: VPUIPRegMapped.NNDMA {{.*}} inputs(%[[VALCST0]] : [[TYPECST0]]) outputs(%[[VAL3]] : [[TYPE3]])
//CHECK-NOT: VPURT.Task
//CHECK-NEXT: VPUIPRegMapped.NNDMA {{.*}} inputs(%[[VALCST1]] : [[TYPECST1]]) outputs(%[[VAL10]] : [[TYPE10]])
//CHECK-NOT: VPURT.Task
//CHECK-NEXT: VPUIPRegMapped.NNDMA {{.*}} inputs(%[[VALCST2]] : [[TYPECST2]]) outputs(%[[VAL11]] : [[TYPE11]])
//CHECK-NEXT: %[[VAL19:.*]] = VPURT.DeclareBuffer "CMX_NN" [0] <34816> -> [[TYPE19:.*]]
//CHECK-NEXT: %[[VAL20:.*]] = VPURT.DeclareBuffer "CMX_NN" [1] <34816> -> [[TYPE20:.*]]
//CHECK-NOT: VPURT.Task
//CHECK-NEXT: VPUIPRegMapped.NNDMA {{.*}} inputs(%arg0 : memref<1x32x16x16xf16, #NHWC, @DDR>) outputs(%[[VAL19]], %[[VAL20]] : [[TYPE19]], [[TYPE20]])
...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
module @mainModule {
  func private @sparse_conv(%arg0: memref<1x128x32x32xf16, #NHWC, @DDR>, %arg1: memref<1x32x32x32xf16, #NHWC, @DDR>) -> memref<1x32x32x32xf16, #NHWC, @DDR> {
  ...
  IE.CNNNetwork entryPoint : @sparse_conv inputsInfo : {
    DataInfo "input_0" : tensor<1x128x32x32xf16, {order = #NHWC}>
  } outputsInfo : {
    DataInfo "output_0" : tensor<1x32x32x32xf16, {order = #NHWC}>
  }
}
...
//CHECK: VPUIPRegMapped.NNDMA {{.*}} inputs(%[[VALCST]] : memref<32x1x1x128xi1, @DDR>) outputs(%[[VALSPS]] : memref<32x1x1x128xi1, [@CMX_NN, 0]>)
//CHECK: VPUIPRegMapped.DPUInvariant
...
module @mainModule {
  ...
  IE.CNNNetwork entryPoint : @se_table_dpu_f16_f16_f16 inputsInfo : {
    ...
    DataInfo "output_0" : tensor<1x32x16x16xf16, {order = #NHWC}>
  }

  func private @se_table_dpu_f16_f16_f16(%arg0: memref<1x32x16x16xf16, #NHWC, @DDR>, %arg1: memref<1x32x16x16xf16, #NHWC, @DDR>) -> memref<1x32x16x16xf16, #NHWC, @DDR> {
...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
module @mainModule {
  func @ComputNCESeSizes(%arg0: memref<1x16x32x32xf16, #NHWC, [@CMX_NN, 0]>, %arg1: memref<1x16x32x32xi1, #NHWC, [@CMX_NN, 0]>,
      ...
      VPURT.Task attributes {isTrailingSWLayer = false} {
          %2:2 = VPUIP.NCEClusterTask {kernel_padding = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, kernel_size = [1, 1], kernel_strides = [1, 1], out_channel_offset = 0 : i64, task_type = "CONV", is_superdense, input_se_size = 16 : i64, output_se_size = 32 : i64}
          ...
          -> memref<1x32x32x32xf16, #NHWC, [@CMX_NN, 0]>, memref<1x32x32x32xi1, #NHWC, [@CMX_NN, 0]> variants : {
              DPUTask {cluster_id = 0 : i64, mpe_mode = "VECTOR_FP16", outEnd = [31, 31, 31], outStart = [0, 0, 0], pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}}
          } PPE : {
          }
      }
