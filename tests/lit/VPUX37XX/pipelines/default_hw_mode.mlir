...
module @Convolution {
    ...
    func.func @main(%arg: tensor<1x3x62x62xf32>) -> tensor<1x48x60x60xf32> {
        ...
        // CHECK-SAME:      [[weight_0:%.*]] : memref<48x16x3x3xf16, {compressionScheme = #VPUIP.CompressionScheme<axis = 0 : i64, numElems = dense<27> : tensor<48xi64>, alignment = 16 : i64>, order = #NHWC}, [@CMX_NN, 0]>)
        ...
        // CHECK-SAME:      [[parent_output_0:%.*]] : !VPUIP.DistributedBuffer<1x48x60x60xf16, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>)
        ...
        // CHECK-SAME:      [[weight_1:%.*]] : memref<48x16x3x3xf16, {compressionScheme = #VPUIP.CompressionScheme<axis = 0 : i64, numElems = dense<27> : tensor<48xi64>, alignment = 16 : i64>, order = #NHWC}, [@CMX_NN, 1]>)
        ...
        // CHECK-SAME:      [[parent_output_1:%.*]] : !VPUIP.DistributedBuffer<1x48x60x60xf16, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>)
        // CHECK-SAME:      [[output_1:%.*]] : memref<1x48x30x60xf16, #NHWC, [@CMX_NN, 1]>)
        // CHECK:               DPUTask
  }
}
...
module @ScaleShiftSubgraph {
    ...
    IE.CNNNetwork entryPoint : @main
    inputsInfo : {
        DataInfo "input" : tensor<1x256x20x20xf32, {order = #NCHW}>
    } outputsInfo : {
        DataInfo "output" : tensor<1x512x20x20xf32, {order = #NCHW}>
    }
    ...
    func.func @main(%arg: tensor<1x256x20x20xf32>) -> tensor<1x512x20x20xf32> {
        ...
        %1 = IE.Concat(%0, %0) {per_axis = {axis = 1 : i64}} : tensor<1x256x20x20xf32>, tensor<1x256x20x20xf32> -> tensor<1x512x20x20xf32>
        ...
        // CHECK:       VPURT.Task waits([[barrier_0:%.*]] : !VPURT.Barrier) updates([[barrier_1:%.*]] : !VPURT.Barrier)
        // CHECK:       VPUIP.NCEClusterTask {activation_window_channel_length = 4 : i64, is_segmented, kernel_padding = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, kernel_size = [1, 1], kernel_strides = [1, 1], task_type = "DWCONV"}
