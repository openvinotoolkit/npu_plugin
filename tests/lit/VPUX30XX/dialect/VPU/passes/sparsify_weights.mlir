...
func.func @DoNotSparsifyPaddedConv(%arg0: tensor<1x16x16x16xf16, {order = #NHWC}>, %arg1: tensor<16x1x1x4xsi32>) -> tensor<1x16x16x16xf16, {order = #NHWC}> {
    ...
    // CHECK-SAME:    : tensor<16x15x1x1xf16>, [#const.Reorder<#NHWC>, #const.PadWithZero<[0, 0, 0, 0], [0, 1, 0, 0]>]
...
func.func @SparsifyPaddedConv(%arg0: tensor<1x32x16x16xf16, {order = #NHWC}>, %arg1: tensor<16x1x1x4xsi32>) -> tensor<1x16x16x16xf16, {order = #NHWC}> {
    ...
    // CHECK-DAG:       [[weights:%.+]] = const.Declare tensor<16x32x1x1xf16, {order = #NHWC}> = dense<1.000000e+00>
    // CHECK-SAME:    : tensor<16x15x1x1xf16>, [#const.Reorder<#NHWC>, #const.PadWithZero<[0, 0, 0, 0], [0, 17, 0, 0]>, #const.Sparsify<false, dense<15> : tensor<16xi64>>]
    // CHECK-DAG:       [[sparsity_map:%.+]] = const.Declare tensor<16x1x1x128xi1> = dense<1.000000e+00>
    // CHECK-SAME:    : tensor<16x15x1x1xf16>, [#const.Reorder<#NHWC>, #const.PadWithZero<[0, 0, 0, 0], [0, 17, 0, 0]>, #const.GetSparsityMap]
