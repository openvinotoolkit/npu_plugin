...
func @OptimizeMultiClusteringSubgraphAlignment(%arg0: tensor<1x80x22x22xf16, {order = #NHWC}>) -> tensor<1x64x22x22xf16, {order = #NHWC}> {
    %cst_0 = const.Declare tensor<80x1x1x4xsi32> = #const.Content<dense<10> : tensor<80x1x1x4xsi32>>
    %cst_1 = const.Declare tensor<80x80x3x3xf16, {order = #NHWC}> = #const.Content<dense<1.000000e+00> : tensor<80x80x3x3xf16>, [#const.Reorder<#NHWC>]>
    ...
    %cst_2 = const.Declare tensor<64x1x1x4xsi32> = #const.Content<dense<10> : tensor<64x1x1x4xsi32>>
    %cst_3 = const.Declare tensor<64x80x3x3xf16, {order = #NHWC}> = #const.Content<dense<1.000000e+00> : tensor<64x80x3x3xf16>, [#const.Reorder<#NHWC>]>
    ...
    //CHECK:        [[WEIGHTSTABLE_0:%.*]] = const.Declare tensor<80x1x1x4xsi32> = #const.Content<dense<10> : tensor<80x1x1x4xsi32>>
    //CHECK:        [[WEIGHTS_0:%.*]] = const.Declare tensor<80x80x3x3xf16, {order = #NHWC}> = #const.Content<dense<1.000000e+00> : tensor<80x80x3x3xf16>, [#const.Reorder<#NHWC>]>

    //CHECK:        [[INPUT_CMX_0:%.*]] = VPU.NCE.ClusterTiling (%arg0 as %arg1: tensor<1x80x22x22xf16, {order = #NHWC}>)
    //CHECK-SAME:   -> !VPU.DistributedTensor<1x80x22x22xf16, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 4, 1], num_clusters = 4 : i64, alignment = [1, 1, 2, 1]}> {
    ...
    //CHECK:        [[WEIGHTSTABLE_1:%.*]] = const.Declare tensor<64x1x1x4xsi32> = #const.Content<dense<10> : tensor<64x1x1x4xsi32>>
    //CHECK:        [[WEIGHTS_1:%.*]] = const.Declare tensor<64x80x3x3xf16, {order = #NHWC}> = #const.Content<dense<1.000000e+00> : tensor<64x80x3x3xf16>, [#const.Reorder<#NHWC>]>
