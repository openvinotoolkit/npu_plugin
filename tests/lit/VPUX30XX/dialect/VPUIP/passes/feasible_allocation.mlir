...
func @main(%in: memref<1x16x96x96xf16, #NHWC>, %out: memref<1x16x96x96xf16, #NHWC>) -> memref<1x16x96x96xf16, #NHWC> {
    %cst0 = const.Declare memref<1x16x96x96xf16, #NHWC> = #const.Content<dense<2.0> : tensor<1x16x96x96xf16>, [#const.Reorder<#NHWC>]>
    %wt = const.Declare memref<16x1x1x4xsi32, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<16x1x1x4xsi32>>
    %act_win = const.Declare memref<1x1x1x16xui8, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<1x1x1x16xui8>>
    ...
    %t3, %r3 = async.execute [%t_in] (%r_in as %0 : !async.value<memref<1x16x96x96xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %1 = VPUIP.NCEClusterTask {
            ...
            variants :
            {
                DPUTask { end = [16, 96, 96], mpe_mode = "VECTOR_FP16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0] }
            }
            PPE : {
            }
...
func @main(%in: memref<1x16x64x64xf16, #NHWC>, %out: memref<1x16x64x64xf16, #NHWC>) -> memref<1x16x64x64xf16, #NHWC> {
    %cst0 = const.Declare memref<1x16x64x64xf16, #NHWC> = #const.Content<dense<2.0> : tensor<1x16x64x64xf16>, [#const.Reorder<#NHWC>]>
    %wt = const.Declare memref<16x1x1x4xsi32, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<16x1x1x4xsi32>>
    %act_win = const.Declare memref<1x1x1x16xui8, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<1x1x1x16xui8>>
    ...
    %t0, %r0 = async.execute [%t_dma_in] (%r_dma_in as %arg0 : !async.value<memref<1x16x64x64xf16, {order = #NHWC, strides = #strides}, [@CMX_NN, 0]>>)
        ...
        %1 = VPUIP.NCEClusterTask {
            ...
            variants :
            {
                DPUTask { end = [16, 64, 64], mpe_mode = "VECTOR_FP16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0] }
            }
            PPE : {
            }
    ...
    // CHECK:       VPUIP.Copy inputs([[ARG0]] : memref<1x32x64x64xf16, #NHWC, @DDR>) outputs([[BUF_SPILL_READ]] : memref<1x32x64x64xf16, {order = #NHWC, strides = [131072, 1, 2048, 32]}, [@CMX_NN, 0]>)
...
func @main(%in: memref<1x16x85x85xf16, #NHWC>, %out: memref<1x16x85x85xf16, #NHWC>) -> memref<1x16x85x85xf16, #NHWC> {
    %cst0 = const.Declare memref<1x16x85x85xf16, #NHWC> = #const.Content<dense<2.0> : tensor<1x16x85x85xf16>, [#const.Reorder<#NHWC>]>
    %wt = const.Declare memref<16x1x1x4xsi32, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<16x1x1x4xsi32>>
    %act_win = const.Declare memref<1x1x1x16xui8, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<1x1x1x16xui8>>
    ...
    %t1, %r1 = async.execute [%t_in] (%r_in as %0 : !async.value<memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>>)
        ...
        %1 = VPUIP.NCEClusterTask {
            ...
            variants :
            {
                DPUTask { end = [16, 85, 85], mpe_mode = "VECTOR_FP16", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0] }
            }
            PPE : {
            }
    ...
    // CHECK:       VPUIP.Copy inputs([[ARG0]] : memref<1x16x85x85xf16, #NHWC, @DDR>) outputs([[BUF_SPILL_READ0]] : memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>)
    ...
    // CHECK:       VPUIP.Copy inputs([[ARG2]] : memref<1x16x85x85xf16, #NHWC, @DDR>) outputs([[BUF_SPILL_READ1]] : memref<1x16x85x85xf16, #NHWC, [@CMX_NN, 0]>)
...
func @main(%in: memref<1x16x112x112xf16, #NHWC>, %out0: memref<1x16x112x112xf16, #NHWC>, %out1: memref<1x16x112x112xf16, #NHWC>) -> (memref<1x16x112x112xf16, #NHWC>, memref<1x16x112x112xf16, #NHWC>) {
    %wt = const.Declare memref<16x1x1x4xsi32, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<16x1x1x4xsi32>>
    %act_win = const.Declare memref<1x1x1x16xui8, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<1x1x1x16xui8>>
...
func @main(%in: memref<1x32x60x60xf16, #NHWC>, %out: memref<1x32x60x60xf16, #NHWC>) -> memref<1x32x60x60xf16, #NHWC> {
    %cst0 = const.Declare memref<1x32x60x60xf16, #NHWC> = #const.Content<dense<2.0> : tensor<1x32x60x60xf16>, [#const.Reorder<#NHWC>]>
    %wt = const.Declare memref<32x1x1x4xsi32, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<32x1x1x4xsi32>>
    %act_win = const.Declare memref<1x1x1x32xui8, [@CMX_NN, 0]> = #const.Content<dense<1> : tensor<1x1x1x32xui8>>
...
!BufMemrefDDR = type memref<1x32x64x64xf16, #NHWC, @DDR>
!BufMemrefCMX = type memref<1x32x64x64xf16, #NHWC, [@CMX_NN, 0]>

!WtMemrefDDR = type memref<32x1x1x4xsi32, @DDR>
!WtMemrefCMX = type memref<32x1x1x4xsi32, [@CMX_NN, 0]>

!ActWinMemrefDDR = type memref<1x1x1x32xui8, @DDR>
!ActWinMemrefCMX = type memref<1x1x1x32xui8, [@CMX_NN, 0]>
...
func @main(%input: !BufMemrefDDR) -> !BufMemrefDDR {
    %cst0 = const.Declare memref<1x32x64x64xf16> = #const.Content<dense<2.0> : tensor<1x32x64x64xf16>>
    %cst1 = const.Declare !WtMemrefCMX = #const.Content<dense<1> : tensor<32x1x1x4xsi32>>
    %cst2 = const.Declare !ActWinMemrefCMX = #const.Content<dense<1> : tensor<1x1x1x32xui8>>
    ...
    // CHECK:       [[CST0:%.*]] = const.Declare memref<1x32x64x64xf16>
    // CHECK:       [[CST1:%.*]] = const.Declare memref<32x1x1x4xsi32, [@CMX_NN, 0]>
    // CHECK:       [[CST2:%.*]] = const.Declare memref<1x1x1x32xui8, [@CMX_NN, 0]>
    ...
    // CHECK:       [[BUF1:%.*]] = VPURT.DeclareBuffer "CMX_NN" <524288> -> !VPUIP.DistributedBuffer<32x1x1x4xsi32, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    // CHECK:       [[BUF2:%.*]] = VPURT.DeclareBuffer "CMX_NN" <524800> -> !VPUIP.DistributedBuffer<1x1x1x32xui8, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    ...
    // CHECK:       [[BUF4:%.*]] = VPURT.DeclareBuffer "CMX_NN" <262144> -> !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    // CHECK:       [[BUF5:%.*]] = VPURT.DeclareBuffer "CMX_NN" <524288> -> !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
    // CHECK:       [[BUF6:%.*]] = VPURT.DeclareBuffer "CMX_NN" <262144> -> !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>
...
func @main(%in: memref<1x32x16x16xf16>, %out: memref<1x128x4x4xf16>) -> memref<1x128x4x4xf16> {
    ...
    %cst_4 = const.Declare memref<64x1x1x4xsi32> = #const.Content<dense<2> : tensor<64x1x1x4xsi32>>
    %cst_10 = const.Declare memref<64x32x3x3xf16, #NHWC> = #const.Content<dense<2.0> : tensor<64x32x3x3xf16>, [#const.Reorder<#NHWC>]>
    %cst_3 = const.Declare memref<128x1x1x4xsi32> = #const.Content<dense<2> : tensor<128x1x1x4xsi32>>
    %cst_11 = const.Declare memref<128x64x3x3xf16, #NHWC> = #const.Content<dense<2.0> : tensor<128x64x3x3xf16>, [#const.Reorder<#NHWC>]>
    ...
    %token_36, %results_37 = async.execute [%token_30, %token_32, %token_34] (%results_31 as %arg2: 
      ...
      %32 = VPUIP.NCEClusterTask 
        ...
        DPUTask {end = [3, 3, 63], mpe_mode = "MATRIX", pad = {bottom = 0 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 0, 0]}
        DPUTask {end = [7, 3, 63], mpe_mode = "MATRIX", pad = {bottom = 0 : i64, left = 0 : i64, right = 1 : i64, top = 0 : i64}, start = [4, 0, 0]}
        DPUTask {end = [3, 7, 63], mpe_mode = "MATRIX", pad = {bottom = 1 : i64, left = 0 : i64, right = 0 : i64, top = 0 : i64}, start = [0, 4, 0]}
        DPUTask {end = [7, 7, 63], mpe_mode = "MATRIX", pad = {bottom = 1 : i64, left = 0 : i64, right = 1 : i64, top = 0 : i64}, start = [4, 4, 0]}
      } PPE :  {
      }
    ...
    %token_42, %results_43 = async.execute [%token_36, %token_38, %token_40] (%results_37 as %arg2: 
      ...
      %32 = VPUIP.NCEClusterTask 
        ...
        DPUTask {end = [3, 3, 31], mpe_mode = "MATRIX", pad = {bottom = 1 : i64, left = 0 : i64, right = 1 : i64, top = 0 : i64}, start = [0, 0, 0]}
        DPUTask {end = [3, 3, 63], mpe_mode = "MATRIX", pad = {bottom = 1 : i64, left = 0 : i64, right = 1 : i64, top = 0 : i64}, start = [0, 0, 32]}
        DPUTask {end = [3, 3, 95], mpe_mode = "MATRIX", pad = {bottom = 1 : i64, left = 0 : i64, right = 1 : i64, top = 0 : i64}, start = [0, 0, 64]}
        DPUTask {end = [3, 3, 127], mpe_mode = "MATRIX", pad = {bottom = 1 : i64, left = 0 : i64, right = 1 : i64, top = 0 : i64}, start = [0, 0, 96]}
      } PPE :  {
      }
    ...
    %token_43, %result_44 = async.execute [%token_42] (%results_43 as %0 : !async.value<memref<1x128x4x4xf16, #NHWC, [@CMX_NN, 0]>>)
            -> !async.value<memref<1x128x4x4xf16>> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 7 : i64, "cycleCost" = 171 : i64} {
    ...
    // CHECK:       {VPUIP.executor = @DMA_NN, "async-deps-index" = 0 : i64, cycleBegin = 0 : i64, cycleCost = 683 : i64, cycleEnd = 683 : i64}
    // CHECK:           VPUIP.Copy
    // CHECK:       {VPUIP.executor = @DMA_NN, "async-deps-index" = 2 : i64, cycleBegin = 683 : i64, cycleCost = 22 : i64, cycleEnd = 705 : i64}
    // CHECK:           VPUIP.Copy
    // CHECK:       {VPUIP.executor = @DMA_NN, "async-deps-index" = 1 : i64, cycleBegin = 705 : i64, cycleCost = 2172 : i64, cycleEnd = 2877 : i64}
    // CHECK:           VPUIP.Copy
    ...
    // CHECK:       {VPUIP.executor = @DMA_NN, "async-deps-index" = 4 : i64, cycleBegin = 2877 : i64, cycleCost = 6144 : i64, cycleEnd = 9021 : i64}
    // CHECK:           VPUIP.Copy
    // CHECK:       {VPUIP.executor = @DMA_NN, "async-deps-index" = 5 : i64, cycleBegin = 9021 : i64, cycleCost = 61 : i64, cycleEnd = 9082 : i64}
    // CHECK:           VPUIP.Copy

    // CHECK:       {VPUIP.executor = @NCE, "async-deps-index" = 6 : i64, cycleBegin = 9082 : i64, cycleCost = 686 : i64, cycleEnd = 9768 : i64}
    // CHECK:           VPUIP.NCEClusterTask
    // CHECK:       {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 7 : i64, cycleBegin = 9768 : i64, cycleCost = 171 : i64, cycleEnd = 9939 : i64}
    // CHECK:           VPUIP.Copy
}
