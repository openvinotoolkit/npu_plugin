...
// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=VPUX30XX" --adjust-scale-shift-for-dw-conv %s | FileCheck %s
...
func.func @NotAdjustScaleShiftToEnableCMajorWithUPALayer(%arg0: tensor<17x3x256x256xf16>) -> tensor<17x32x256x256xf16> {
    ...
    %1 = IE.ScaleShift(%0, %weights_0, %bias_0) {operand_segment_sizes = dense<1> : vector<3xi32>} : tensor<17x3x512x512xf16>, tensor<1x3x1x1xf16>, tensor<1x3x1x1xf16> -> tensor<17x3x512x512xf16>
...
func.func @AdjustScaleShiftWithNCELayer(%arg0: tensor<17x32x512x512xf16>) -> tensor<17x32x256x256xf16> {
    ...
    // CHECK-DAG:       [[BIAS_1:%.*]] = const.Declare tensor<1x51x1x1xf16> = dense<7.843020e-03> : tensor<1x3x1x1xf16>, [#const.Reshape<[1, 3, 1, 1]>, #const.Broadcast<0 : i64, 17 : i64>, #const.Reshape<[1, 51, 1, 1]>]
    // CHECK-DAG:       [[WEIGHTS_1:%.*]] = const.Declare tensor<1x51x1x1xf16> = dense<-1.000000e+00> : tensor<1x3x1x1xf16>, [#const.Reshape<[1, 3, 1, 1]>, #const.Broadcast<0 : i64, 17 : i64>, #const.Reshape<[1, 51, 1, 1]>]
    ...
    // CHECK:           [[SCALESHIFT:%.*]] = IE.ScaleShift([[RESHAPE_1]], [[WEIGHTS_1]], [[BIAS_1]]) {operand_segment_sizes = dense<1> : vector<3xi32>} : 
