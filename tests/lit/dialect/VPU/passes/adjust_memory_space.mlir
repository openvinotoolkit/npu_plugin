...
func @DepthConvNCEtoCMX(%arg0: tensor<1x16x40x80xf16, {order = #NHWC}>) -> tensor<1x16x37x73xf16, {order = #NHWC}> {
    ...
    %weights_table = const.Declare tensor<16x1x1x4xsi32, {order = #NHWC}> = #const.Content<dense<1> : tensor<16x1x1x4xsi32>, [#const.Reorder<#NHWC>]>
    %activation_window = const.Declare tensor<16x1x1x16xui8, {order = #NHWC}> = #const.Content<dense<1> : tensor<16x1x1x16xui8>, [#const.Reorder<#NHWC>]>
    ...
    // CHECK:       [[WEIGHTS_TABLE_DDR:%.+]] = const.Declare tensor<16x1x1x4xsi32, {order = #NHWC}>
    // CHECK:       [[ACTIVATION_WINDOW_DDR:%.+]] = const.Declare tensor<16x1x1x16xui8, {order = #NHWC}>
...
func @MaxPoolNCEtoCMX(%arg0: tensor<1x16x1x4xf16, {order = #NHWC}>) -> tensor<1x16x1x4xf16, {order = #NHWC}> {
    ...
    %weights_table = const.Declare tensor<16x1x1x16xui8, {order = #NHWC}> = #const.Content<dense<1> : tensor<16x1x1x16xui8>, [#const.Reorder<#NHWC>]>
    ...
    // CHECK:       [[WEIGHTS_TABLE_DDR:%.+]] = const.Declare tensor<16x1x1x16xui8, {order = #NHWC}>
