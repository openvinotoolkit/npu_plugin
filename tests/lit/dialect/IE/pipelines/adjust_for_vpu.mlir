...
func @AdjustForVPU(%arg0: tensor<1x16x64xf16>) -> tensor<1x1x64xf16> {
    ...
    // CHECK: [[VAL0:%.*]] = IE.Reshape(%arg0) {shape_value = [1, 16, 1, 64]} : tensor<1x16x64xf16> -> tensor<1x16x1x64xf16>
    ...
    // CHECK: [[VAL2:%.*]] = IE.Reshape([[VAL1]]) {shape_value = [1, 1, 64]} : tensor<1x1x1x64xf16> -> tensor<1x1x64xf16>
