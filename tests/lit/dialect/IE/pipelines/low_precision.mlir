...
func @QuantizedConv(%input: tensor<1x3x62x62xf32>) -> tensor<1x4x60x60xf32> {
    ...
    // CHECK-DAG:   [[WEIGHTS_FQ:%.*]] = IE.Constant tensor<4x3x3x3xf32> = dense<0.000000e+00> : tensor<4x3x3x3xf32>
    // CHECK-DAG:   [[INPUT_MIN:%.*]] = IE.Constant tensor<f32> = dense<0.000000e+00> : tensor<f32>
    // CHECK-DAG:   [[INPUT_MAX:%.*]] = IE.Constant tensor<f32> = dense<2.550000e+02> : tensor<f32>
