// RUN: vpux-opt --split-input-file --set-compile-params="vpu-arch=KMB compilation-mode=DefaultHW" --expand-activation-channels --canonicalize %s | FileCheck %s
...
// CHECK-SAME:        [[INPUT:%arg[0-9]]]: tensor<1x3x30x30x!qElemType0>

// CHECK: [[EXPAND_OUT:%.+]] = IE.Expand([[INPUT]]) {pads_begin = [0, 0, 0, 0], pads_end = [0, 13, 0, 0]} :
// CHECK-SAME: tensor<1x3x30x30x!qElemType0> -> tensor<1x16x30x30x!qElemType0>

// CHECK: [[MAXPOOL_OUT:%.+]] = IE.MaxPool([[EXPAND_OUT]]) {kernel_size = [5, 5], pads_begin = [2, 0], pads_end = [2, 0], rounding_type = "FLOOR", strides = [2, 2]} :
// CHECK-SAME: tensor<1x16x30x30x!qElemType0>
// CHECK-SAME: -> tensor<1x16x15x13x!qElemType2>

// CHECK: [[SLICE_OUT:%.+]] = IE.Slice [[MAXPOOL_OUT]] [0, 0, 0, 0] [1, 3, 15, 13] :
// CHECK-SAME: tensor<1x16x15x13x!qElemType2>
// CHECK-SAME: to tensor<1x3x15x13x!qElemType1>

// CHECK: return [[SLICE_OUT]] : tensor<1x3x15x13x!qElemType1>
...
// CHECK-SAME: tensor<1x16x30x30x!qElemType0>,
// CHECK-SAME: tensor<16x16x3x3x!qElemType3>
// CHECK-SAME: -> tensor<1x16x28x28x!qElemType4>

// CHECK: [[SLICE_OUT:%.+]] = IE.Slice [[CONV_OUT]] [0, 0, 0, 0] [1, 5, 28, 28] :
// CHECK-SAME: tensor<1x16x28x28x!qElemType4>
// CHECK-SAME: to tensor<1x5x28x28x!qElemType2>

// CHECK: return [[SLICE_OUT]] : tensor<1x5x28x28x!qElemType2>
...
func @ExpandEltwiseAndChannelsSameInputs(%arg0: tensor<1x3x30x25xf16>) -> tensor<1x3x30x25xf16> {
    %0 = IE.And(%arg0, %arg0) {auto_broadcast = "NUMPY"} : tensor<1x3x30x25xf16>, tensor<1x3x30x25xf16> -> tensor<1x3x30x25xf16>
    return %0 : tensor<1x3x30x25xf16>

    // CHECK:       [[EXPAND_INPUT:%.*]] = IE.Expand(%arg0) {pads_begin = [0, 0, 0, 0], pads_end = [0, 13, 0, 0]} : tensor<1x3x30x25xf16> -> tensor<1x16x30x25xf16>
    // CHECK:       [[ELTWISE_AND:%.*]] = IE.And([[EXPAND_INPUT]], [[EXPAND_INPUT]]) {auto_broadcast = "NUMPY"} : tensor<1x16x30x25xf16>, tensor<1x16x30x25xf16> -> tensor<1x16x30x25xf16>
    // CHECK:       [[OUT:%.*]] = IE.Slice [[ELTWISE_AND]] [0, 0, 0, 0] [1, 3, 30, 25] : tensor<1x16x30x25xf16> to tensor<1x3x30x25xf16>
...
// CHECK-SAME:        [[INPUT:%arg[0-9]]]: tensor<1x3x30x30x!qElemType0>,
// CHECK-SAME:        [[FILTER:%arg[0-9]]]: tensor<3x1x3x3x!qElemType1>

// CHECK: [[EXPAND_OUT:%.+]] = IE.Expand([[INPUT]]) {pads_begin = [0, 0, 0, 0], pads_end = [0, 13, 0, 0]} : tensor<1x3x30x30x!qElemType0> -> tensor<1x16x30x30x!qElemType0>
// CHECK: [[PAD_OUT:%.+]] = IE.Expand([[FILTER]]) {pads_begin = [0, 0, 0, 0], pads_end = [13, 0, 0, 0]} : tensor<3x1x3x3x!qElemType1> -> tensor<16x1x3x3x!qElemType3>
...
// CHECK-SAME: tensor<1x16x30x30x!qElemType0>,
// CHECK-SAME: tensor<16x1x3x3x!qElemType3>
// CHECK-SAME: -> tensor<1x16x15x15x!qElemType4>

// CHECK: [[SLICE_OUT:%.+]] = IE.Slice [[CONV_OUT]] [0, 0, 0, 0] [1, 3, 15, 15] :
// CHECK-SAME: tensor<1x16x15x15x!qElemType4>
// CHECK-SAME: to tensor<1x3x15x15x!qElemType2>

// CHECK: return [[SLICE_OUT]] : tensor<1x3x15x15x!qElemType2>

// -----

func @ExpandQuantGroupConvolutionChannelsConst(%input: tensor<1x3x30x30xf16>) -> tensor<1x3x15x15xf16> {
    %filter = const.Declare tensor<3x1x3x3xf16> = #const.Content<dense<1.0> : tensor<3x1x3x3xf16>>
    %1 = IE.GroupConvolution(%input, %filter) { dilations = [1, 1], groups = 3, pads_begin = [0, 0], pads_end = [1, 1], strides = [2, 2] }
        : tensor<1x3x30x30xf16>, tensor<3x1x3x3xf16> -> tensor<1x3x15x15xf16>
    return %1 : tensor<1x3x15x15xf16>
}

// CHECK-LABEL: func @ExpandQuantGroupConvolutionChannelsConst
// CHECK-SAME:        [[INPUT:%arg[0-9]]]: tensor<1x3x30x30xf16>

// CHECK: [[EXPAND_FILTER:%.*]] = const.Declare tensor<16x1x3x3xf16> = #const.Content<dense<1.000000e+00> : tensor<3x1x3x3xf16>, [#const.PadWithZero<[0, 0, 0, 0], [13, 0, 0, 0]>]>
...
// CHECK-SAME: tensor<16x1x3x3xf16>
// CHECK-SAME: -> tensor<1x16x15x15xf16>
