...
func @FuseQuantParamsIntoConv(%arg0: tensor<1x3x16x16xf16>) -> tensor<1x3x14x14xf16> {
  ...
  %4 = IE.Convolution(%2, %3) {dilations = [1 : i32, 1 : i32], pads_begin = [0 : i32, 0 : i32], pads_end = [0 : i32, 0 : i32], strides = [1 : i32, 1 : i32]} : tensor<1x3x16x16xf16>, tensor<3x3x3x3xf16> -> tensor<1x3x14x14xf16>
...
func @FuseQuantParamsIntoPool(%arg0: tensor<1x3x16x16xf16>) -> tensor<1x3x16x16xf16> {
  ...
  %3 = IE.MaxPool(%2) {kernel_size = [1 : i32, 1 : i32], pads_begin = [0 : i32, 0 : i32], pads_end = [0 : i32, 0 : i32], rounding_type = "FLOOR", strides = [1 : i32, 1 : i32]} : tensor<1x3x16x16xf16> -> tensor<1x3x16x16xf16>
  ...
  //CHECK: [[VAL1:%.*]] = IE.MaxPool([[VAL0]]) {kernel_size = [1 : i32, 1 : i32], pads_begin = [0 : i32, 0 : i32], pads_end = [0 : i32, 0 : i32], rounding_type = "FLOOR", strides = [1 : i32, 1 : i32]} : tensor<1x3x16x16x!quant.uniform<u8:f16, 0.57450980392156858>> -> tensor<1x3x16x16x!quant.uniform<u8:f16, 0.57450980392156858>>
