...
func @ParsePrintClusterTiling(%arg0: memref<1x32x16x16xf16, #NHWC, @CMX_NN>) -> memref<1x64x14x14xf16, #NHWC, @CMX_NN> {
    ...
    %t1, %r1 = async.execute
                -> !async.value<memref<1x64x14x14xf16, #NHWC, @CMX_NN>>
                    attributes {IERT.executor = @NCE, IERT.num_units = 1 : i64, "async-deps-index" = 0 : i64} {
    ...
    //CHECK:        %token, %results = async.execute -> !async.value<memref<1x64x14x14xf16, #NHWC, @CMX_NN>> attributes {IERT.executor = @NCE, IERT.num_units = 1 : i64, "async-deps-index" = 0 : i64} {
...
func @ParsePrintDistributedBuffer(%input: !Input_DDR) -> !Output_DDR {
    ...
    //CHECK-SAME:                           {kernel = [3, 3], mode = "overlapped", num_tiles = [1, 1, 4, 1],
    ...
    //CHECK-SAME:               -> !VPUIP.DistributedBuffer<1x32x16x16xf16, #NHWC, @CMX_NN, {kernel = [3, 3], mode = "overlapped", num_tiles = [1, 1, 4, 1], pads = {bottom = 1 : i64, left = 1 : i64, right = 1 : i64, top = 1 : i64}}> {
    ...
    //CHECK:        %token_2 = async.execute [%token, %token_0, %token_1] attributes {IERT.executor = @NCE, IERT.num_units = 4 : i64, "async-deps-index" = 3 : i64} {
    //CHECK:              %5 = VPUIP.NCEClusterTiling inputs(
    //CHECK-SAME:                   [[INPUT_CMX]] as %arg1: memref<1x32x16x16xf16, #NHWC, @CMX_NN>,
    ...
    //CHECK:        %token_3 = async.execute attributes {IERT.executor = @DMA_NN, IERT.num_units = 1 : i64, "async-deps-index" = 0 : i64} {
    ...
    //CHECK:                %6 = IERT.Copy {out_mem_space = @DDR} inputs(%arg1 : memref<1x64x16x16xf16, #NHWC, @CMX_NN>)
