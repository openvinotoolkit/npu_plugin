...
func @ParallelGraph(%arg0: memref<1x16x32x32xf16, #NHWC>, %arg1: memref<1x16x32x32xf16>) -> memref<1x16x32x32xf16> {
    ...
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR0]] : !VPURT.Barrier) updates([[BAR1]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR1]] : !VPURT.Barrier) updates([[BAR2]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR2]] : !VPURT.Barrier) updates([[BAR3]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR3]] : !VPURT.Barrier) updates([[BAR4]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR4]] : !VPURT.Barrier) updates([[BAR5]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR5]] : !VPURT.Barrier) updates([[BAR6]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR6]] : !VPURT.Barrier) updates([[BAR7]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR7]] : !VPURT.Barrier) updates([[BAR8]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR8]] : !VPURT.Barrier) updates([[BAR9]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR9]] : !VPURT.Barrier) updates([[BAR10]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR10]] : !VPURT.Barrier) updates([[BAR11]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR11]] : !VPURT.Barrier) updates([[BAR12]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR12]] : !VPURT.Barrier) updates([[BAR13]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR13]] : !VPURT.Barrier) updates([[BAR14]] : !VPURT.Barrier)
    // CHECK:       VPURT.Task
    // CHECK-SAME:       waits([[BAR14]] : !VPURT.Barrier)
}
