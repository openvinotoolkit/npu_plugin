...
module @Conv2dTest attributes {VPUIP.arch = "VPU3700", VPUIP.compilationMode = "ReferenceHW"}  {
...
// CHECK: %6 = IERT.Constant memref<16x1x1x4xsi32> = dense{{.*}} : tensor<16x1x1x4xsi32>
// CHECK: %7 = memref.alloc() : memref<16x1x1x4xsi32, "CMX_NN">
// CHECK: %8 = IERT.Copy inputs(%6 : memref<16x1x1x4xsi32>) outputs(%7 : memref<16x1x1x4xsi32, "CMX_NN">) -> memref<16x1x1x4xsi32, "CMX_NN">
...
module @MaxPoolTest attributes {VPUIP.arch = "VPU3700", VPUIP.compilationMode = "ReferenceHW"}  {
  VPUIP.Graph options : "NONE" version : {contextStr = "VPUX Compiler", hash = "", majorV = 3 : i32, minorV = 11 : i32, patchV = 0 : i32}
  IERT.RunTimeResources availableMemory :  {
    IERT.MemoryResource 1073741824 bytes
    IERT.MemoryResource 31457280 bytes of "DDR" {VPUIP.bandwidth = 8 : i64, VPUIP.derateFactor = 6.000000e-01 : f64}
  ...
  IE.CNNNetwork entryPoint : @main inputsInfo :  {
    IE.DataInfo "Parameter_0" : memref<1x16x1x4xf16>
  } outputsInfo :  {
    IE.DataInfo "MaxPool_1" : memref<1x16x1x4xf16>
  }
  func @main(%arg0: memref<1x16x1x4xf16>, %arg1: memref<1x16x1x4xf16>) -> memref<1x16x1x4xf16> {
    %0 = memref.alloc() : memref<1x16x1x4xf16>
    %1 = IERT.MaxPool {kernel_size = [1 : i32, 1 : i32], pads_begin = [0 : i32, 0 : i32], pads_end = [0 : i32, 0 : i32], strides = [1 : i32, 1 : i32]} inputs(%arg0 : memref<1x16x1x4xf16>) outputs(%0 : memref<1x16x1x4xf16>) -> memref<1x16x1x4xf16>
...
// CHECK: %0 = memref.alloc() : memref<1x16x1x4xf16>
// CHECK: %1 = memref.alloc() : memref<1x16x1x4xf16, #NHWC>
// CHECK: %2 = IERT.Reorder inputs(%arg0 : memref<1x16x1x4xf16>) outputs(%1 : memref<1x16x1x4xf16, #NHWC>) -> memref<1x16x1x4xf16, #NHWC>
// CHECK: %3 = IERT.Constant memref<16x1x1x16xui8> = dense{{.*}} : tensor<16x1x1x16xui8>
// CHECK: %4 = memref.alloc() : memref<16x1x1x16xui8, "CMX_NN">
// CHECK: %5 = IERT.Copy inputs(%3 : memref<16x1x1x16xui8>) outputs(%4 : memref<16x1x1x16xui8, "CMX_NN">) -> memref<16x1x1x16xui8, "CMX_NN">
// CHECK: %6 = IERT.Constant memref<16x1x1x4xsi32> = dense{{.*}} : tensor<16x1x1x4xsi32>
// CHECK: %7 = memref.alloc() : memref<16x1x1x4xsi32, "CMX_NN">
// CHECK: %8 = IERT.Copy inputs(%6 : memref<16x1x1x4xsi32>) outputs(%7 : memref<16x1x1x4xsi32, "CMX_NN">) -> memref<16x1x1x4xsi32, "CMX_NN">
// CHECK: %9 = memref.alloc() : memref<1x16x1x4xf16, #NHWC, "CMX_NN">
// CHECK: %10 = IERT.Copy inputs(%2 : memref<1x16x1x4xf16, #NHWC>) outputs(%9 : memref<1x16x1x4xf16, #NHWC, "CMX_NN">) -> memref<1x16x1x4xf16, #NHWC, "CMX_NN">
// CHECK: %11 = memref.alloc() : memref<1x16x1x4xf16, #NHWC, "CMX_NN">
// CHECK: %12 = VPUIP.NCEClusterTask {activation_window_channel_length = 4 : i32, fixed_ppe_task = "NOOP", kernel_padding = [0 : i32, 0 : i32, 0 : i32, 0 : i32], kernel_size = [1 : i32, 1 : i32], strides = [1 : i32, 1 : i32], task_type = "MAXPOOL"} inputs(%10 : memref<1x16x1x4xf16, #NHWC, "CMX_NN">, %8 : memref<16x1x1x4xsi32, "CMX_NN">), %5 : memref<16x1x1x16xui8, "CMX_NN">) parent_input(%10 : memref<1x16x1x4xf16, #NHWC, "CMX_NN">) parent_output(%11 : memref<1x16x1x4xf16, #NHWC, "CMX_NN">) outputs(%11 : memref<1x16x1x4xf16, #NHWC, "CMX_NN">) variants :  {
// CHECK:   VPUIP.DPUTask {end = [3 : i32, 0 : i32, 15 : i32], mpe_mode = "VECTOR_FP16", pads_begin = [0 : i32, 0 : i32], pads_end = [0 : i32, 0 : i32], start = [0 : i32, 0 : i32, 0 : i32]}
// CHECK: } -> memref<1x16x1x4xf16, #NHWC, "CMX_NN">
// CHECK: %13 = memref.alloc() : memref<1x16x1x4xf16, #NHWC>
// CHECK: %14 = IERT.Copy inputs(%12 : memref<1x16x1x4xf16, #NHWC, "CMX_NN">) outputs(%13 : memref<1x16x1x4xf16, #NHWC>) -> memref<1x16x1x4xf16, #NHWC>
// CHECK: %15 = IERT.Reorder inputs(%14 : memref<1x16x1x4xf16, #NHWC>) outputs(%0 : memref<1x16x1x4xf16>) -> memref<1x16x1x4xf16>
// CHECK: %16 = IERT.Copy inputs(%15 : memref<1x16x1x4xf16>) outputs(%arg1 : memref<1x16x1x4xf16>) -> memref<1x16x1x4xf16>
// CHECK: return %16 : memref<1x16x1x4xf16>
