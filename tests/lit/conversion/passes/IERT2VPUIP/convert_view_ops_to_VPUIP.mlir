...
func @Reshape(%arg0: memref<1x512xf16>, %arg1: memref<1x512xf16>) -> memref<1x512xf16> {
    ...
    %1 = VPURT.DeclareBuffer "VPU_DDR_Heap" [0] <0> -> memref<1x512x1x1xf16, "DDR">
    ...
    // CHECK:       [[VAR0:%.*]] = VPURT.DeclareBuffer "ProgrammableInput" [0] <0> -> memref<1x512x1x1xf16>
    ...
    // CHECK:       [[VAR3:%.*]] = VPURT.DeclareBuffer "VPU_DDR_Heap" [0] <0> -> memref<1x512xf16, "DDR">
...
func @SubView(%arg0: memref<4x4xf16>, %arg1: memref<4x4xf16>) -> memref<4x4xf16> {
    ...
    // CHECK:       [[VAR0:%.*]] = VPURT.DeclareBuffer "ProgrammableInput" [0] <0> -> memref<2x4xf16>
    // CHECK:       [[VAR1:%.*]] = VPURT.DeclareBuffer "ProgrammableOutput" [0] <0> -> memref<2x4xf16>
    // CHECK:       [[VAR2:%.*]] = VPUIP.NNDMA
    // CHECK-SAME:      inputs([[VAR0]] : memref<2x4xf16>)
    // CHECK-SAME:      outputs([[VAR1]] : memref<2x4xf16>) -> memref<2x4xf16>

    // CHECK:       [[VAR3:%.*]] = VPURT.DeclareBuffer "ProgrammableInput" [0] <16> -> memref<2x4xf16>
    // CHECK:       [[VAR4:%.*]] = VPURT.DeclareBuffer "ProgrammableOutput" [0] <16> -> memref<2x4xf16>
...
func @Broadcasting(%arg0: memref<256xf16>) -> memref<256xf16> {
    %0 = VPURT.DeclareBuffer "VPU_CMX_NN" [0, 1, 2, 3] <0> -> memref<512xf16, "CMX_NN">
    ...
    // CHECK:       [[VAR1:%.*]] = VPURT.DeclareBuffer "VPU_CMX_NN" [0, 1, 2, 3] <0> -> memref<256xf16, "CMX_NN">
    // CHECK:       [[VAR2:%.*]] = VPUIP.NNDMA
    // CHECK-SAME:      inputs(%arg0 : memref<256xf16>)
    // CHECK-SAME:      outputs([[VAR1]] : memref<256xf16, "CMX_NN">) -> memref<256xf16, "CMX_NN">

    // CHECK:       [[VAR3:%.*]] = VPURT.DeclareBuffer "VPU_CMX_NN" [0, 1, 2, 3] <512> -> memref<256xf16, "CMX_NN">
    // CHECK:       [[VAR4:%.*]] = VPUIP.NNDMA
    // CHECK-SAME:      inputs(%arg0 : memref<256xf16>)
...
func @WithAsyncRegions(%arg0: memref<1x1x1x512xf32>, %arg1: memref<1x1x1x512xf32>) -> memref<1x1x1x512xf32> {
    %t0, %f0 = async.execute -> !async.value<memref<1x1x1x512xf16, "DDR">> {
        %0 = VPURT.DeclareBuffer "VPU_DDR_Heap" [0] <0> -> memref<1x1x1x512xf16, "DDR">
    ...
    %t1, %f1 = async.execute [%t0] (%f0 as %1: !async.value<memref<1x1x1x512xf16, "DDR">>) -> !async.value<memref<1x512xf16, "DDR">> {
        ...
        %3 = VPURT.DeclareBuffer "VPU_DDR_Heap" [0] <1024> -> memref<1x512xf16, "DDR">
    ...
    // CHECK:           [[VAR0:%.*]] = VPURT.DeclareBuffer "VPU_DDR_Heap" [0] <0> -> memref<1x1x1x512xf16, "DDR">
    ...
    // CHECK:           [[VAR2:%.*]] = VPURT.DeclareBuffer "VPU_DDR_Heap" [0] <0> -> memref<1x512xf16, "DDR">
    // CHECK:           [[VAR3:%.*]] = VPURT.DeclareBuffer "VPU_DDR_Heap" [0] <1024> -> memref<1x512xf16, "DDR">
    ...
    // CHECK:           [[VAR5:%.*]] = VPURT.DeclareBuffer "VPU_DDR_Heap" [0] <1024> -> memref<1x1x1x512xf16, "DDR">
...
func @PermuteCast(%arg0: memref<1x12x16x16xf16, #NHWC>, %arg1: memref<1x16x16x12xf16>) -> memref<1x16x16x12xf16> {
    ...
    %1 = VPURT.DeclareBuffer "VPU_DDR_Heap" [0] <2000> -> memref<1x16x16x12xf16, "DDR">
    ...
    //CHECK:        [[VAR0:%.*]] = VPURT.DeclareBuffer "ProgrammableInput" [0] <0> -> memref<1x16x16x12xf16>
    //CHECK:        [[VAR1:%.*]] = VPURT.DeclareBuffer "VPU_DDR_Heap" [0] <2000> -> memref<1x16x16x12xf16, "DDR">
