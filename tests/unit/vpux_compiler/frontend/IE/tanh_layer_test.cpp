...
TEST(MLIR_IE_FrontEndTest, TanhLayer) {
    std::shared_ptr<ngraph::Function> f;
    {
        auto param1 = std::make_shared<ngraph::opset1::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 64, 64});
        auto tanh = std::make_shared<ngraph::opset1::Tanh>(param1);
        tanh->set_friendly_name("Tanh");
        auto result = std::make_shared<ngraph::op::Result>(tanh);
