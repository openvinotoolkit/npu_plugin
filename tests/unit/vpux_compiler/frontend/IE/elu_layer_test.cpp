...
TEST(MLIR_IE_FrontEndTest, EluLayer) {
    std::shared_ptr<ngraph::Function> f;
    {
        auto param1 = std::make_shared<ngraph::opset1::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 64, 64});
        auto elu = std::make_shared<ngraph::opset1::Elu>(param1, 1.0);
        elu->set_friendly_name("Elu");
        auto result = std::make_shared<ngraph::op::Result>(elu);
