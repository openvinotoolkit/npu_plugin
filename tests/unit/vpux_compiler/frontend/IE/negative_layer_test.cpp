...
TEST(MLIR_IE_FrontEndTest, NegativeLayer) {
    std::shared_ptr<ngraph::Function> f;
    {
        auto param1 = std::make_shared<ngraph::opset1::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 64, 64});
        auto negative = std::make_shared<ngraph::opset1::Negative>(param1);
        negative->set_friendly_name("Negative");
        auto result = std::make_shared<ngraph::op::Result>(negative);
