...
TEST(MLIR_IE_FrontEndTest, GRNLayer) {
    std::shared_ptr<ngraph::Function> f;
    {
        auto param1 = std::make_shared<ngraph::opset1::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 64, 64});
        auto grn = std::make_shared<ngraph::opset1::GRN>(param1, 1.0);
        grn->set_friendly_name("GRN");
        auto result = std::make_shared<ngraph::op::Result>(grn);
