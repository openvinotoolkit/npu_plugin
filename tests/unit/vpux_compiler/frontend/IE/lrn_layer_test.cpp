...
TEST(MLIR_IE_FrontEndTest, LRNLayer) {
    std::shared_ptr<ngraph::Function> f;
    {
        auto param1 = std::make_shared<ngraph::opset1::Parameter>(ngraph::element::f32, ngraph::Shape{1, 6, 64, 64});
        auto axisConstant = ngraph::op::Constant::create(ngraph::element::Type_t::i64, {2}, {1, 2});
        auto lrn = std::make_shared<ngraph::opset1::LRN>(param1, axisConstant, 0.0001, 0.75, 1, 5);
        lrn->set_friendly_name("LRN");
        auto result = std::make_shared<ngraph::op::Result>(lrn);
