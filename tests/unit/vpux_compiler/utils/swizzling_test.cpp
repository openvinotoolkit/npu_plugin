...
class SwizzlingTest_VPUX40XX :
    public TestWithParam<std::tuple<uint32_t, uint32_t>>
{};

class SwizzlingTest_VPUX37XX :
    public TestWithParam<std::tuple<uint32_t, uint32_t>>
{};
...
bool swizzlingTest(uint32_t elements, uint32_t swizzleKey, VPU::ArchKind archKind) {
    ...
    bufferTransform.swizzle<T>(inputVector, swizzledArrayRef);
...
TEST_P(SwizzlingTest_VPUX40XX, swizzlingTest_VPUX40XX) {
    ...
    bool result = false;
    EXPECT_TRUE(result = swizzlingTest<uint32_t>(elements, swizzlingKey, VPU::ArchKind::VPUX40XX));
}
