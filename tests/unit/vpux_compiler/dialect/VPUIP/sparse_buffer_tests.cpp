...
TEST(MLIR_NDTypeInterface, SparseBufferType_Weights) {
    ...
    const auto sparseBufferType = VPUIP::SparseBufferType::get(data, sparsityMap);
    ...
    const SmallVector<Bit> sparsityMapStrides({144_Bit, 9_Bit, 3_Bit, 1_Bit});
    ...
    EXPECT_EQ(sparseChangedStrides.getSparsityMap().cast<NDTypeInterface>().getStrides(), StridesRef(sparsityMapStrides));
    ...
    const SmallVector<Bit> tileSparsityMapStrides({72_Bit, 9_Bit, 3_Bit, 1_Bit});
    ...
    EXPECT_EQ(sparseTiledType.getSparsityMap().cast<NDTypeInterface>().getStrides(), StridesRef(tileSparsityMapStrides));
    ...
    EXPECT_EQ(sparseViewTile.getSparsityMap().cast<NDTypeInterface>().getStrides(), StridesRef(tileSparsityMapStrides));
...
TEST(MLIR_NDTypeInterface, SparseBufferType_Activation) {
    ...
    const auto data = mlir::MemRefType::get(shape.raw(), mlir::Float16Type::get(&ctx), layout, memSpace);
    const auto sparsityMap = mlir::MemRefType::get(shape.raw(), mlir::IntegerType::get(&ctx, 1), layout, memSpace);
    const auto storageElementTable = mlir::MemRefType::get(seTableShape.raw(), mlir::IntegerType::get(&ctx, 32), layout, memSpace);
    ...
    EXPECT_EQ(sparseChangedStrides.getData().cast<NDTypeInterface>().getStrides(), StridesRef(newStrides));
    EXPECT_EQ(sparseChangedStrides.getSparsityMap().cast<NDTypeInterface>().getStrides(), StridesRef(sparsityMapStrides));
    EXPECT_EQ(sparseChangedStrides.getStorageElementTable().cast<NDTypeInterface>().getStrides(), StridesRef(seTableStrides));
    ...
    EXPECT_EQ(sparseTiledType.getStorageElementTable().cast<NDTypeInterface>().getStrides(), StridesRef(tileSETableStrides));
