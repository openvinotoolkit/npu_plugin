...
TEST(DeviceInfo, CreateFromString) {
    ...
    EXPECT_NO_THROW(std::ignore = DeviceInfo("VPUX3720"));
    EXPECT_NO_THROW(std::ignore = DeviceInfo("vpux30xx"));
}
...
TEST(DeviceInfo, CreateFromString_WrongFormat) {
    ...
    EXPECT_ANY_THROW(std::ignore = DeviceInfo("VPUX4X1X"));
}
...
TEST(DeviceInfo, Compare) {
    ...
    EXPECT_NE(DeviceInfo("vpux30xx"), DeviceInfo::VPUX37XX);
}

TEST(DeviceInfo, Print) {
    EXPECT_EQ(printToString("{0}", DeviceInfo::VPUX30XX), "VPUX30XX");
}
