...
std::vector<uint8_t> fillRemoteBlobWithRandomValues(ie::RemoteBlob::Ptr blob, const size_t offset, const size_t size) {
    ...
    std::generate(blobData + offset, blobData + offset + size, []() {
...
// Every ROI blob has its own InferenceEngine::ROI data which keep information about ROI frame geometry (offset from
// parent and sizes) When we are using cascade ROI, ROI offsets are calculated according to the superposition principle
TEST_F(VPUXRemoteBlobUnitTests, cascadeROIBlobCorrect) {
...

    {
        ...
        const ie::RemoteBlob::Ptr remoteROIBlobPtr =
                std::static_pointer_cast<ie::RemoteBlob>(remoteBlobPtr->createROI(roi1Begin, roi1End));

        {
            ...
            checkROIFrameData.erase(checkROIFrameData.cbegin(),
                                    checkROIFrameData.cbegin() + (fullFrameByteSize - ROIFrameByteSize));
            const auto ROIFrameData =
                    readRemoteBlob(remoteROI2BlobPtr, fullFrameByteSize - ROIFrameByteSize, ROIFrameByteSize);
...
TEST_F(VPUXRemoteBlobUnitTests, constructorWithCorrectParamsNoThrow) {
    ASSERT_NO_THROW(
            vpux::VPUXRemoteBlob blob(_tensorDesc, _remoteContext, _dummyDevice->getAllocator(), _dummyBlobParams));
}
...
TEST_F(VPUXRemoteBlobUnitTests, getContextAllocatedBlobReturnSameAsOnInit) {
    const auto remoteBlobPtr = std::make_shared<vpux::VPUXRemoteBlob>(_tensorDesc, _remoteContext,
                                                                      _dummyDevice->getAllocator(), _dummyBlobParams);
