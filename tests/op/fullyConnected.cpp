...
TEST(ops, fullyConnected)
{
    ...
    auto input = om.input({8, 8, 16}, mv::DTypeType::Float16, mv::Order(mv::Order::getColMajorID(3)));
    std::vector<double> weightsData = mv::utils::generateSequence<double>(input->getShape().totalSize() * 100u);
    auto weights1 = om.constant(weightsData, {input->getShape().totalSize(), 100}, mv::DTypeType::Float16, mv::Order(mv::Order::getColMajorID(3)));
