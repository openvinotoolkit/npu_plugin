...
TEST(ops, conv2D)
{
    ...
    ASSERT_EQ(convOp->getAttr("stride").getContent<mv::std::array<unsigned short, 2>>().e0, 4);
    ASSERT_EQ(convOp->getAttr("stride").getContent<mv::std::array<unsigned short, 2>>().e1, 4);
    ASSERT_EQ(convOp->getAttr("padding").getContent<mv::std::array<unsigned short, 4>>().e0, 1);
    ASSERT_EQ(convOp->getAttr("padding").getContent<mv::std::array<unsigned short, 4>>().e1, 1);
    ASSERT_EQ(convOp->getAttr("padding").getContent<mv::std::array<unsigned short, 4>>().e2, 1);
    ASSERT_EQ(convOp->getAttr("padding").getContent<mv::std::array<unsigned short, 4>>().e3, 1);
