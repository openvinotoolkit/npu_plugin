...
TEST (graph_critical_path, test1)
{
    ...
    std::map<graph_char_string::node_list_iterator, unsigned, NodeIteratorComp> costs;
    costs[n1] = 1;
    costs[n2] = 3;
    costs[n3] = 100;
    costs[n4] = 5;
    ...
    auto result2 = mv::critical_path<std::string, std::string, NodeIteratorComp, EdgeIteratorComp>(g, n4, n5, costs);
    ASSERT_EQ(*result2[0], "n4_n3");
    ASSERT_EQ(*result2[1], "n3_n5");

    std::cout << std::endl;

    auto result1 = mv::critical_path<std::string, std::string, NodeIteratorComp, EdgeIteratorComp>(g, n1, n5, costs);
