...
ComputationBase::ComputationBase(Priv* priv): m_priv(priv) {}

void ComputationBase::warmUp() {
    m_priv->m_c.apply(
        m_priv->m_v_in, m_priv->m_v_out, cv::compile_args(InferenceEngine::gapi::preproc::sipp::kernels()));
}

void ComputationBase::apply() { m_priv->m_c.apply(m_priv->m_v_in, m_priv->m_v_out); }

namespace {
cv::gapi::own::Mat to_own(test::Mat mat) { return {mat.rows, mat.cols, mat.type, mat.data, mat.step}; }
...
ResizeComputation::ResizeComputation(test::Mat inMat, test::Mat outMat, int interp)
    : ComputationBase(new Priv {buildResizeComputation(outMat, interp), {to_own(inMat)}, {to_own(outMat)}}),
...
void ResizeComputation::apply() { m_resizePriv->m_cc(cv::gin(m_priv->m_v_in[0]), cv::gout(m_priv->m_v_out[0])); }
...
NV12toRGBComputation::NV12toRGBComputation(test::Mat inMat_y, test::Mat inMat_uv, test::Mat outMat)
    : ComputationBase(new Priv {buildNV12toRGBComputation(), to_own({inMat_y, inMat_uv}), {to_own(outMat)}}) {}
