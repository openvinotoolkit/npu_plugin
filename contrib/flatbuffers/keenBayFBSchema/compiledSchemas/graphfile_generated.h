...
template<> struct SpecificTaskTraits<DPUTask> {
  static const SpecificTask enum_value = SpecificTask_DPUTask;
};
...
struct Task FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  const DPUTask *task_as_DPUTask() const {
    return task_type() == SpecificTask_DPUTask ? static_cast<const DPUTask *>(task()) : nullptr;
  }
...
template<> inline const DPUTask *Task::task_as<DPUTask>() const {
  return task_as_DPUTask();
}
...
inline bool VerifySpecificTask(flatbuffers::Verifier &verifier, const void *obj, SpecificTask type) {
  switch (type) {
    ...
    case SpecificTask_DPUTask: {
      auto ptr = reinterpret_cast<const DPUTask *>(obj);
      return verifier.VerifyTable(ptr);
    }
