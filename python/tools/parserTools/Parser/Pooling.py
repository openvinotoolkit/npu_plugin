...
class Pooling(Layer):
    ...
    def convert2DwConv(self):
        ...
        layer = ConvolutionDepthWise2D(self.name.stringifyOriginalName(), [], [])
        ...
        if layer.getInputTensors()[0].isQuantized():
            weights_data = np.ones((1, self.getOutputTensors()[0].shape[1], self.kernelHeight, self.kernelWidth)).astype(np.uint8)
            weights_range = [(1, 1)] * len(weights_data.shape)
        else:
            weights_data = np.ones((1, self.getOutputTensors()[0].shape[1], self.kernelHeight, self.kernelWidth)).astype(np.uint8)/(self.kernelHeight * self.kernelWidth)

        layer.weights = PopulatedTensor(weights_data)
        layer.weights.name = MangledName(OriginalName(self.name.stringifyOriginalName() + "_weights"))

        if layer.getInputTensors()[0].isQuantized():
            scale = np.array([1/(self.kernelHeight * self.kernelWidth)]).astype(np.float32)
            zero_point = np.array([0]).astype(np.int32)
            tensor_range = [(128, 128)] * len(weights_data.shape)
            layer.weights.setQuantizationParameters(QuantizationParameters(scale,zero_point, weights_data.dtype, weights_range))
