...
class Permute(Layer):
    ...
    def load_order_from_caffe(self, caffe_order):
        ...
        if len(caffe_order) != 4:
            throw_error(ErrorTable.StageDetailsNotSupported, "Support only for 3 axes minus batch axis")

        # Permutation of the batch axis is not supported in MvTensor.
        if caffe_order[0] != 0:
            throw_error(ErrorTable.StageDetailsNotSupported, "Permutation of the batch axis is not supported")
