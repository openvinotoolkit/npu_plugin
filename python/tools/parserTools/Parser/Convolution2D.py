...
from parserTools.Tensor import UnpopulatedTensor, PopulatedTensor
import parserTools.Models.Layouts as Layouts
from parserTools.TensorFormat import TensorFormat
from parserTools.Models.EnumDeclarations import PadStyle
from parserTools.Parser.Upsampling import Upsampling
from parserTools.Parser.crop import Crop
...
class Convolution2DUniversal(Layer):
    ...
    def setBias(self, data):
        ...
        self.bias.name = MangledName(OriginalName(self.name.stringifyOriginalName() + "_bias"))
    ...
    def setWeights(self, data):
        ...
        self.weights.name = MangledName(OriginalName(self.name.stringifyOriginalName() + "_weights"))
    ...
    def setQuantizationParameters(self, weight_q, bias_q = None):
        ...
        tensor_range = [(weight_q.Min(idx), weight_q.Max(idx)) for idx in range(weight_q.MinLength())]
        self.weights.setQuantizationParameters(QuantizationParameters(weight_q.ScaleAsNumpy(), weight_q.ZeroPointAsNumpy(), self.weights.dtype, tensor_range))

        if self.biasEnabled():
            tensor_range = [(bias_q.Min(idx), bias_q.Max(idx)) for idx in range(bias_q.MinLength())]
            self.bias.setQuantizationParameters(QuantizationParameters(bias_q.ScaleAsNumpy(), bias_q.ZeroPointAsNumpy(), self.bias.dtype, tensor_range))
...
class Deconvolution(Convolution2DUniversal):
    ...
    def convert2Conv(self):
        ...
        if (self.strideHeight == 1 and self.strideWidth == 1 and
            ...
            upsampled_tensor_name = MangledName(
                OriginalName(self.getInputTensorNames()[0].stringifyOriginalName()))
...
class Convolution2D(Convolution2DUniversal):
    ...
    def genGroupConvs(self):
        ...
        group_shape = list(map(lambda a, b: a // b, self.getInputTensors()[0].getShape(), group_filter))
        group_channels = group_shape[1]

        out_ch = self.getOutputTensors()[0].getShape()[1]
        output_group_shape = list(map(lambda a, b: a // b, self.getOutputTensors()[0].getShape(), group_filter))
        ...
        for group in range(self.getGroupSize()):
            ...
            group_N_weights = self.weights.data[self.weights.data.shape[0] // self.getGroupSize() * group:
                                self.weights.data.shape[0] // self.getGroupSize() * (group + 1), ]

            # Our slice of Input...
            group_N_input = UnpopulatedTensor(group_shape)
            group_N_input.setName(MangledName(OriginalName(self.getInputTensors()[0].name.stringifyOriginalName())))
            

            # And Output...
            outputSlice = UnpopulatedTensor((output_group_shape))
            outputSlice.setName(MangledName(OriginalName(self.getOutputTensors()[0].name.stringifyOriginalName())))
            ...
            if self.biasEnabled():
                ...
                l.setBias(bias_data[group * (out_ch // self.getGroupSize()) : (group + 1) * (out_ch // self.getGroupSize())])  # Correct?
            ...
            c.setOffset(np.array([self.getInputTensors()[0].shape[1] // self.getGroupSize() * group, 0, 0]))
            c.setImplicit()
...
class ConvolutionDepthWise2D(Convolution2DUniversal):
    ...
    def convert2GroupConv(self, scheduler):
        ...
        group_conv = Convolution2D('group_' + self.name.stringifyOriginalName(), [], [])
        ...
        new_weights = np.zeros((self.weights.data.shape[0], self.groupSize*self.weights.data.shape[1]//new_group_size, self.kernelHeight, self.kernelWidth))
        ...
        for idx, group in enumerate(range(self.groupSize)):
            ...
            new_weights[stepk*idx:stepk*(idx+1),  stepc*(idx%max_c):stepc*((idx%max_c)+1), ...] = group_N_weights
