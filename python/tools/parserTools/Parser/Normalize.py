...
class Normalize(Layer):
    def __init__(self, *args):
        ...
        self.formatPool = [(tfIV, tfIV), (tfPL, tfPL), (tfPL, tfIV), (tfIV, tfPL),
                           (tfCM, tfCM)]

    def load_parameters(self, normalize_parameters):
        if(normalize_parameters.across_spatial is not False):
            throw_error(ErrorTable.StageDetailsNotSupported, "Only Values of False accepted for Across Spatial, Normalization")
        self.across_spatial = normalize_parameters.across_spatial

        if(normalize_parameters.channel_shared is not False):
            throw_error(ErrorTable.StageDetailsNotSupported, "Only Values of False accepted for Channel Shared, Normalization")
