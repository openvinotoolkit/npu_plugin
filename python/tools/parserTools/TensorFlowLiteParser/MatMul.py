...
from parserTools.TensorFlowLiteParser.tflite.FullyConnectedOptions import FullyConnectedOptions
...
def load(obj, op_type, tensors, buffers):
    ...
    if len(weight_data.shape) == 2:
        weight_data = np.expand_dims(np.expand_dims(np.transpose(weight_data, (1, 0)), -1), 0)
    else:
