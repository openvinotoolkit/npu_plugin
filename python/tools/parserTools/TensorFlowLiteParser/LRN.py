...
import numpy as np
from parserTools.Parser.LRN import LRN
from parserTools.TensorFlowLiteParser.tflite.BuiltinOperator import BuiltinOperator
from parserTools.TensorFlowLiteParser.tflite.LocalResponseNormalizationOptions import LocalResponseNormalizationOptions
...
def load(obj, op_type, tensors, buffers):
    ...
    x = LRN(out.Name(), [activation.Name()], [out.Name()])

    options = LocalResponseNormalizationOptions()
    ...
    x.loadBeta(options.Beta())
    x.loadK(options.Bias())
    x.loadSquareKernelSize(options.Radius())
