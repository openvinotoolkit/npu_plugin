...
from parserTools.TensorFlowLiteParser.tflite.TensorType import TensorType
from parserTools.TensorFlowLiteParser.tflite.Padding import Padding
from parserTools.TensorFlowLiteParser.tflite.ActivationFunctionType import ActivationFunctionType as actType
from parserTools.Parser.ReLU import ReLU
from parserTools.Parser.tan_h import TanH
...
def getActivation(options, out):
    if options.FusedActivationFunction() == actType.RELU:
        act = ReLU(out.Name().decode("utf-8")+"/ReLU", [out.Name()], [out.Name()])
    elif options.FusedActivationFunction() == actType.TANH:
        act = TanH(out.Name().decode("utf-8")+"/TanH", [out.Name()], [out.Name()])
    elif options.FusedActivationFunction() == actType.NONE:
        return None
    else:
        raise ValueError("Unsupported activation function {}".format(options.FusedActivationFunction()))
...
def getPadding(in_dim, kernel_dim, stride_dim, padding_type):
    def same_padding(in_dim, kernel_dim, stride_dim):
        ...
        pad = [(int(pad[idx]//2), int(pad[idx] - pad[idx]//2)) for idx in range(len(pad))]
        return pad
...
def findTensorValue(tensor, buffers):
    ...
    data = np.reshape(data, [tensor.Shape(idx) for idx in range(tensor.ShapeLength())])
