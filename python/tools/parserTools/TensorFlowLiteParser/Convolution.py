...
from parserTools.Parser.Convolution2D import (Convolution2D,
    ConvolutionDepthWise2D, Deconvolution)
from parserTools.TensorFlowLiteParser.tflite.BuiltinOperator import BuiltinOperator

from parserTools.TensorFlowLiteParser.tflite.Conv2DOptions import Conv2DOptions
from parserTools.TensorFlowLiteParser.tflite.TransposeConvOptions import TransposeConvOptions
from parserTools.TensorFlowLiteParser.tflite.DepthwiseConv2DOptions import DepthwiseConv2DOptions
...
def load(obj, op_type, tensors, buffers):
    ...
    if op_type == BuiltinOperator.CONV_2D:
        ...
        x = ConvolutionDepthWise2D(out.Name(), [activation.Name()], [out.Name()])
    ...
    padding = getPadding([activation.Shape(idx) for idx in range(1,3)], kernelSize,
        (stride_h, stride_w), conv.Padding())
