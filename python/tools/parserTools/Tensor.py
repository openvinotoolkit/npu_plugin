...
class Tensor():
    ...
    def setSparsity(self, sparsity):
        def calulate_sparsity_map(data):
            # Calculate the sparsity map
            cmp_tensor = np.zeros(data.shape).astype(np.uint8)
            zero_point_tensor = np.stack([self.getSparsityZeroPoint(k)*np.ones(data.shape[1:]) for k in range(data.shape[0])]).astype(data.dtype)
            cmp_tensor[data!=zero_point_tensor] = 1
            sparsity_map = np.squeeze(np.array(np.split(cmp_tensor, math.ceil(self.shape[3] / 8), axis=3)))
            sparsity_map = np.einsum('bka,a->kb',sparsity_map, np.array([1<<n for n in range(8)]))

            # Reshape and pad
            padded_cxy = ((sparsity_map.shape[1]+15)//16)*16
            padded_sparsity_map = np.zeros(shape=[sparsity_map.shape[0], 1, 1, padded_cxy]).astype(np.uint8)
            for k in range(sparsity_map.shape[0]):
                padded_sparsity_map[k, 0, 0, :sparsity_map.shape[1]] = sparsity_map[k, :]
        ...
        if sparsity:
            ...
            self.se = UnpopulatedTensor(shape=[self.shape[0], 1, self.shape[2], self.shape[3]])
            self.se.setDatatype(np.int32)
            self.se.setLayout(self.getLayout())
            self.se.name = MangledName(OriginalName(self.name.stringifyOriginalName()+"_se"))
            # Effectively calculate this from data
            if hasattr(self, 'data'):
                self.sparsity = PopulatedTensor(calulate_sparsity_map(self.data))
            else:
            ...
            self.sparsity.name = MangledName(OriginalName(self.name.stringifyOriginalName()+"_sm"))
    ...
    def isQuantized(self):
        return hasattr(self, "quantization") and (not self.dtype in [np.float16, np.float32])
    ...
    def resolve(self, resolve_subtensors = False):
        rt = ResolvedTensor(self, resolve_subtensors=resolve_subtensors)
        return rt
    ...
    # NB: the sparsity map should be padded to 16 Bytes over the dimension with bigger strides (top left)
    def flatnonzero(self, data):
        ...
        if not hasattr(self, "compressed_data"):
            ...
            for k in range(data.shape[0]):
                compressed_kernel = list(data[k, ...].flatten()[data[k, ...].flatten() != self.getSparsityZeroPoint(k)])
                data_size = len(compressed_kernel)*np.dtype(data.dtype).itemsize
                self.compressed_data.append(compressed_kernel + (round_up(data_size, 16) - data_size) * [self.getSparsityZeroPoint(k)])

            self.address_offset = [len(part)* np.dtype(data.dtype).itemsize for part in self.compressed_data]
            self.compressed_data = sum(self.compressed_data, [])

        return self.compressed_data, self.address_offset

    # Count the number of elements that are not equal of the zeroPoint (for each output channel)
    ...
    def setAddress(self, address):
        ...
        if self.sparse:
            ...
            if not hasattr(self, "data"):
                ...
                self.se.address = address + (self.size - self.se.size - self.sparsity.size)
                self.sparsity.address = address + (self.size - self.sparsity.size)
    ...
    def splitAcrossClusters(self, wl_list, split_over_h = False, multicast = False):
        def __gen_tile(wl, cluster):
            if len(wl.rect_lst) != 1:
                raise ValueError("More than one rectangle ({}) in the workload.. impossible to generate a subtensor".format(len(wl.rect_lst)))
            rr = wl.rect_lst[0]

            if split_over_h:
                subtensor = UnpopulatedTensor([self.shape[0], self.shape[1], rr.height, rr.width])
                subtensor.offset = (0, 0, rr.bl[0], rr.bl[1])
            else:
                subtensor = UnpopulatedTensor([rr.height, rr.width, self.shape[2], self.shape[3]])
            ...
            subtensor.name = MangledName(OriginalName(self.name.stringifyOriginalName()))
            subtensor.setSparsity(self.sparse)
            if not split_over_h or multicast:
                subtensor.place(self, subtensor.offset)
            return subtensor

        self.wl_list = wl_list
        self.subtensors = [__gen_tile(wl, idx) for idx, wl in enumerate(wl_list)]
...
class PopulatedTensor(Tensor):
    ...
    def place(self, largerTensor, topCornerInLargerTensor):
        # assert(isinstance(largerTensor, UnpopulatedTensor)) # not hold for split over K
        super().place(largerTensor, topCornerInLargerTensor)
    ...
    def splitAcrossClusters(self, wl_list, split_over_h = False):
        def __gen_tile(wl, cluster):
            ...
            splitted_data = self.data if split_over_h else self.data[rr.bl[0]:rr.bl[0]+rr.height, rr.bl[1]:rr.bl[1]+rr.width, :, :]
...
class ResolvedTensor():
    ...
    """
    def __init__(self, tensor, resolve_subtensors=False):
        ...
        self.__alternate_strides = [elem_size] +  [tensor.getStrideInBytes(axis)* tensor.shape[axis] for axis in range(len(tensor.shape))]
        ...
        for st in tensor.subtensors:
            if resolve_subtensors and (st.enclosure == None):
                st.place(tensor, st.offset)
            self.__subtensors.append(ResolvedTensor(st))
        ...
        if isinstance(tensor, PopulatedTensor):
            ...
            if self.__sparse:
                # Compress the data
                compress_data, self.__compressed_kernel_sizes = tensor.flatnonzero(self.__data)
                self.__data = np.array(compress_data).astype(tensor.dtype)
        else:
            if hasattr(self.original_tensor, "placedTensors") and len(self.original_tensor.placedTensors) > 0:
                # A Populated Tensor was placed inside an Unpopulated Tensor
                # Note: Placement should be ensuring that populated tensors will not overlap.
                self.__opaque = True
                for p in tensor.placedTensors:
                    if (type(p) == UnpopulatedTensor):
                        self.__opaque = False
                        self.__data = np.zeros(tensor.shape).transpose(self.__layout)
                    else:
                        d = p.data
                        offset = p.getAbsolutePosition()
                        self.__data = np.zeros(tensor.shape).transpose(self.__layout).astype(p.getDatatype())
                        self.__data[offset[0]:d.shape[0], offset[1]:d.shape[1], offset[2]:d.shape[2], offset[3]:d.shape[3]] = d
        ...
        self.__address = tensor.address + self.__local_offset * np.dtype(self.__dtype).itemsize
    ...
    def can_flatten(self):
        ...
        stride_size = self.__strides[max_stride_index] * self.__dimensions[max_stride_index]
    ...
    def __getitem__(self, key):
        return self.__subtensors[key] if key < len(self.__subtensors) and not self.__broadcast else self
