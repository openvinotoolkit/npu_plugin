    ...
    if (${from} MATCHES "(http:)|(https:)|(ftp:)")
      ...
      if (${aria2c} STREQUAL "aria2c-NOTFOUND")
        if (NOT ${WGET_FOUND})
          ...
          execute_process(COMMAND ${WGET_EXECUTABLE} "--no-cache" "--proxy=off" "${from}" "-O" "${to}"
            TIMEOUT 2000
            RESULT_VARIABLE status_code)
        endif()
      else()
        message(STATUS "${aria2c} --no-proxy nnt-srv01.inn.intel.com,*.*.*.* -d ${download_dir} ${from}")
        execute_process(COMMAND "${aria2c}" "--no-proxy" "nnt-srv01.inn.intel.com,*.*.*.*" "-s10" "-x10" "--dir=${download_dir}" "${from}"
            TIMEOUT 2000
            RESULT_VARIABLE status_code)
      endif()
