...
static int getDeviceName(int index, deviceDesc_t* deviceDesc , int pid, XLinkProtocol_t protocol)
{
    if (index < 0) {
...
#if (!defined(_WIN32) && !defined(_WIN64))
        uint16_t  bcdusb = -1;
        usbBootError_t rc = usb_find_device_with_bcd(index, deviceDesc->name, XLINK_MAX_NAME_SIZE, 0, vid, pid, &bcdusb);
#else
        usbBootError_t rc = usb_find_device(index, deviceDesc->name, XLINK_MAX_NAME_SIZE, 0, vid, pid);
#endif
    ...
    if(protocol == X_LINK_ANY_PROTOCOL ||
       protocol == X_LINK_PCIE) {
        ...
        // #-18686
        return pcie_find_device_port(0, deviceDesc->name, XLINK_MAX_NAME_SIZE);
    }

    return X_LINK_PLATFORM_DEVICE_NOT_FOUND;
}

int XLinkPlatformGetDeviceName(int index, deviceDesc_t* deviceDesc)
{
    return getDeviceName(index, deviceDesc, AUTO_PID, X_LINK_ANY_PROTOCOL);
}

int XLinkPlatformGetDeviceNameExtended(int index, deviceDesc_t* deviceDesc, int pid, XLinkProtocol_t protocol)
{
    return getDeviceName(index, deviceDesc, pid, protocol);
}

int XLinkPlatformBootRemote(deviceDesc_t* deviceDesc, const char* binaryPath)
{
    if(deviceDesc->protocol == X_LINK_PCIE) {
        mvLog(MVLOG_INFO, "Unable to load firmware to PCIe device");
        return 0;
    }
    ...
    // Boot it
    rc = usb_boot(deviceDesc->name, tx_buf, filesize);
    free(tx_buf);
    if(rc)
    {
        return rc;
