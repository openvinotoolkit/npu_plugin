...
static void resetAll()
{
    ...
    while (timeInSeconds() < waittm) {
        rc = XLinkGetDeviceName(index, &deviceDesc, AUTO_PID, X_LINK_USB_VSC);
        if (rc != X_LINK_SUCCESS)
            break; //no more devices found

        if (strlen(getProductName(deviceDesc.name)) == 1 &&
            deviceDesc.protocol != X_LINK_PCIE) { //name doesn't have product number
            ...
            handler->devicePath = (char*)deviceDesc.name;
    ...
    if (stalled_count) {
        ...
        while ((after_reset_count < bootrom_count + stalled_count) &&
            ...
            while (timeInSeconds() < waittm) {
                XLinkError_t rc = XLinkGetDeviceName(index, &deviceDesc, AUTO_PID, X_LINK_USB_VSC);
                if (rc != X_LINK_SUCCESS)
                break; //no more devices found

                if (strlen(getProductName(deviceDesc.name)) > 1 &&
                    deviceDesc.protocol != X_LINK_PCIE) { //name has product number
                    after_reset_count++;
                }
                index++;
            }
...
static XLinkProtocol_t convertProtocol(
...
ncStatus_t getFirmwarePath(char* mv_cmd_file_path, const char* dev_addr) {
    ...
    if (strstr(productName, "ma2480")) {
        snprintf(mv_cmd_file_name, 40, "MvNCAPI%s.mvcmd", "-ma2x8x");
        useUniversalFirmware = 1;
    } else {
        snprintf(mv_cmd_file_name, 40, "MvNCAPI%s.mvcmd", productName);
    }
...
ncStatus_t ncDeviceOpen(struct ncDeviceHandle_t **deviceHandlePtr,
        ncDevicePlatform_t platform, ncDeviceProtocol_t protocol,
        int watchdogInterval, const char* customFirmwareDirectory) {
    ...
    XLinkProtocol_t x_link_protocol = convertProtocol(protocol);
    ...
    while ((rc != X_LINK_SUCCESS) && (timeInSeconds() < waittm)) {
        rc = XLinkGetDeviceName(0, &deviceDesc, pid, x_link_protocol);
    }
    ...
    if (rc != X_LINK_SUCCESS) {
        ...
        if (platform == ANY_PLATFORM) {
            mvLog(MVLOG_ERROR, "Failed to find a device, rc: %s", XLinkErrorToStr(rc));
        } else {
            /**
             ...
             */
            mvLog(MVLOG_WARN, "Failed to find %s device, rc: %s", platformToStr(platform), XLinkErrorToStr(rc));
        }
    ...
    if (dH && d) {
        dH->private_data = d;
        d->protocol = deviceDesc.protocol;
        d->dev_addr = strdup(deviceDesc.name);
    ...
    if (handler->protocol != X_LINK_PCIE) {
        ...
        deviceDesc_t beforeBootDevices[MAX_DEVICES] = {{0}};
        int n = 0;
        for (; n < MAX_DEVICES; ++n) {
            if (XLinkGetDeviceName(n, &beforeBootDevices[n], AUTO_PID, x_link_protocol))
                break;
        }
        ...
        while ((timeInSeconds() < waittm) && !deviceBooted) {
            ...
            for (; dev_indx < MAX_DEVICES; ++dev_indx) {
                rc = XLinkGetDeviceName(dev_indx, &booted_device, AUTO_PID, x_link_protocol);
    ...
    } else {    //handler->protocol == X_LINK_PCIE
...
ncStatus_t ncDeviceLoadFirmware(const ncDevicePlatform_t devicePlatform, const char* customFirmwareDir) {
    ...
    deviceDesc_t deviceDesc = {0};
    rc = XLinkGetDeviceName(0, &deviceDesc, getPidBy(devicePlatform), X_LINK_USB_VSC);
    if (rc) {
...
ncStatus_t ncDeviceClose(struct ncDeviceHandle_t **deviceHandlePtr) {
    ...
    if (!wasConnectedToBooted) {
        ...
        while (timeInSeconds() < waittm) {
            ...
            int n = 0;
            while (XLinkGetDeviceName(n++, &device_desc, AUTO_PID, d->protocol) == X_LINK_SUCCESS) {
                                        ...
                                        void *data, unsigned int *dataLength)
{
    ...
    case NC_RO_DEVICE_PLATFORM:
        if (d->dev_attr.fw_version[1] == 0x2480){
            ...
            *(ncDevicePlatform_t *) data = ANY_PLATFORM;
        }
