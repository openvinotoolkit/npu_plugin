...
//system/nn/blob/3600/inc/software_generated.h
...
static inline void write(uint32_t adr, uint32_t val) {
    *(uint32_t *)(adr) = val;
    return;
}
                ...
                )
{
    ...
    write(0x2e1100D0, (uint32_t)tile_size);
    write(0x2e1100D4, (uint32_t)UNROLL_SIZE);
...
void hwc_postOps_3D_core(t_HWCPostOps3DParams *params)
{
    ...
    write(0x2e1100C4, (uint32_t)input);
    write(0x2e1100C8, (uint32_t)output);
    write(0x2e1100CC, (uint32_t)weights);
...
#else
        UNUSED(transfer_out_size);
        UNUSED(output_stride);
        UNUSED(output);
#endif
    }
}
