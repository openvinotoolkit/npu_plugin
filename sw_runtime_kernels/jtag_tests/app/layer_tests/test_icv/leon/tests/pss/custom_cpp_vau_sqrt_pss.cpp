...
#include "sk.vau_sqrt_fp16.3720xx.text.xdat"

#include "pss/param_vau_sqrt.h"

namespace ICV_TESTS_NAMESPACE(ICV_TESTS_PASTE2(ICV_TEST_SUITE_NAME, Sqrt_pss)) {
    static constexpr std::initializer_list<SingleTest> sqrt_test_list{
            {{1, 1, 8}, {1, 1, 8}, orderZYX, FPE("vau_sqrt_fp16.3720xx.elf"), {sw_params::Location::NN_CMX}},
            {{1, 2, 8}, {1, 2, 8}, orderZYX, FPE("vau_sqrt_fp16.3720xx.elf"), {sw_params::Location::NN_CMX}},
            {{1, 1, 1000}, {1, 1, 1000}, orderZYX, FPE("vau_sqrt_fp16.3720xx.elf"), {sw_params::Location::NN_CMX}},
    };
    ...
    protected:
        ...
        void initData() override {
            ...
            m_vauSqrtParams = reinterpret_cast<sw_params::VauSqrtParams*>(paramContainer);
            *m_vauSqrtParams = sw_params::VauSqrtParams();
            m_params.paramData = reinterpret_cast<uint32_t*>(paramContainer);
            m_params.paramDataLen = sizeof(sw_params::VauSqrtParams);
            m_requiredTensorLocation = static_cast<sw_params::Location>(test->customLayerParams.layerParams[0]);
            m_params.baseParamData = sw_params::ToBaseKernelParams(m_vauSqrtParams);

            m_params.kernel = reinterpret_cast<uint64_t>(sk_vau_sqrt_fp16_3720xx_text);
        }
    ...
    private:
        ...
        sw_params::VauSqrtParams* m_vauSqrtParams;
    };
