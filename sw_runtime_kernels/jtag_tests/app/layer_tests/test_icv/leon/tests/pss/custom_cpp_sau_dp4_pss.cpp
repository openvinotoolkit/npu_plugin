...
#include "sk.sau_dp4.3720xx.text.xdat"

#include "pss/param_sau_dp4.h"

namespace ICV_TESTS_NAMESPACE(ICV_TESTS_PASTE2(ICV_TEST_SUITE_NAME, SauDp4_pss)) {
    static constexpr std::initializer_list<SingleTest> dp4_test_list{
            {{16, 1, 1}, {16, 1, 1}, orderZYX, FPE("sau_dp4.3720xx.elf"), {sw_params::Location::NN_CMX}},
            {{32, 1, 1}, {32, 1, 1}, orderZYX, FPE("sau_dp4.3720xx.elf"), {sw_params::Location::NN_CMX}},
            {{1000, 1, 1}, {1000, 1, 1}, orderZYX, FPE("sau_dp4.3720xx.elf"), {sw_params::Location::NN_CMX}},
    };

    class CustomCppSauDp4Test : public CustomCppTests<fp16> {
    public:
        explicit CustomCppSauDp4Test(): m_testsLoop(dp4_test_list, "test") {
        }
        virtual ~CustomCppSauDp4Test() {
        }

    protected:
        const char* suiteName() const override {
            return "CustomCppSauDp4Testpss";
        }
        ...
        void initData() override {
            ...
            m_sauDp4Params = reinterpret_cast<sw_params::SauDp4Params*>(paramContainer);
            *m_sauDp4Params = sw_params::SauDp4Params();
            m_params.paramData = reinterpret_cast<uint32_t*>(paramContainer);
            m_params.paramDataLen = sizeof(sw_params::SauDp4Params);
            m_requiredTensorLocation = static_cast<sw_params::Location>(test->customLayerParams.layerParams[0]);
            m_params.baseParamData = sw_params::ToBaseKernelParams(m_sauDp4Params);

            m_params.kernel = reinterpret_cast<uint64_t>(sk_sau_dp4_3720xx_text);
        }
    ...
    private:
        ...
        sw_params::SauDp4Params* m_sauDp4Params;
    };

    ICV_TESTS_REGISTER_SUITE(CustomCppSauDp4Test)
}  // namespace )
