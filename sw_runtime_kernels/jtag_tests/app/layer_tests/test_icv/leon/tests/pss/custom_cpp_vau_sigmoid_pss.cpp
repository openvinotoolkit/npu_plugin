...
#include "sk.vau_sigm_fp16.3720xx.text.xdat"

#include "pss/param_vau_sigm.h"

namespace ICV_TESTS_NAMESPACE(ICV_TESTS_PASTE2(ICV_TEST_SUITE_NAME, Sigmoid_pss)) {
    static constexpr std::initializer_list<SingleTest> sigmoid_test_list{
            {{1, 1, 8}, {1, 1, 8}, orderZYX, FPE("vau_sigm_fp16.3720xx.elf"), {sw_params::Location::NN_CMX}},
            {{1, 2, 8}, {1, 2, 8}, orderZYX, FPE("vau_sigm_fp16.3720xx.elf"), {sw_params::Location::NN_CMX}},
            {{1, 1, 1000}, {1, 1, 1000}, orderZYX, FPE("vau_sigm_fp16.3720xx.elf"), {sw_params::Location::NN_CMX}},
    };
    ...
    protected:
        const char* suiteName() const override {
            return "CustomCppSigmoidTestpss";
        }
        ...
        void initData() override {
            ...
            m_vauSigmParams = reinterpret_cast<sw_params::VauSigmParams*>(paramContainer);
            *m_vauSigmParams = sw_params::VauSigmParams();
            m_params.paramData = reinterpret_cast<uint32_t*>(paramContainer);
            m_params.paramDataLen = sizeof(sw_params::VauSigmParams);
            m_requiredTensorLocation = static_cast<sw_params::Location>(test->customLayerParams.layerParams[0]);
            m_params.baseParamData = sw_params::ToBaseKernelParams(m_vauSigmParams);

            m_params.kernel = reinterpret_cast<uint64_t>(sk_vau_sigm_fp16_3720xx_text);
        }
    ...
    private:
        ...
        sw_params::VauSigmParams* m_vauSigmParams;
    };
