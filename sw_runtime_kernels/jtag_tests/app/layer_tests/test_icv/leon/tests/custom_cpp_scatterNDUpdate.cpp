...
protected:
    const char* suiteName() const override {
        static const std::string name = std::string{ICV_TESTS_STRINGIFY(ICV_TEST_SUITE_NAME)} + "_" +
                                        TypeNameTrait<SrcType>::name() + "_" + TypeNameTrait<int32_t>::name();
        return name.c_str();
    }
    ...
    void initData() override {
        ...
        *m_scatterNDUpdateParams = sw_params::ScatterNDUpdateParams();
        ...
        m_requiredTensorLocation = static_cast<sw_params::Location>(sw_params::Location::NN_CMX);
