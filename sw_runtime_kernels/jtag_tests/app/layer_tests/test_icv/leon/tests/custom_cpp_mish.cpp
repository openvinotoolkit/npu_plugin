...
namespace ICV_TESTS_NAMESPACE(ICV_TESTS_PASTE2(ICV_TEST_SUITE_NAME, Mish)) {
    static constexpr std::initializer_list<SingleTest> mish_test_list {
            {{1, 1, 10}, {1, 1, 10}, orderZYX, FPE("mish_fp16.elf"), {sw_params::Location::NN_CMX}},
            {{1, 20, 2}, {1, 20, 2}, orderZYX, FPE("mish_fp16.elf"), {sw_params::Location::NN_CMX}},
            {{1000, 1, 1}, {1000, 1, 1}, orderZYX, FPE("mish_fp16.elf"), {sw_params::Location::NN_CMX}}
                        };
...
#ifdef CONFIG_TARGET_SOC_3720
            m_params.kernel = reinterpret_cast<uint64_t>(sk_mish_fp16_3720xx_text);
#else
            m_params.kernel = reinterpret_cast<uint64_t>(PREAMBLE_FUNC(mish_fp16));
#endif
        }
