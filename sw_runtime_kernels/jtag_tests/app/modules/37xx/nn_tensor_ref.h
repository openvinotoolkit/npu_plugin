...
/// FIXME: This is a forced legacy port from KMB. It should probably be factored out
...
struct TensorRefNDData {
    ...
    common_runtime::RelativeAddress dataAddr;
...
static inline bool fillTensorRefNDData(TensorRefNDData &dstTensor, int32_t dims[], int32_t strides[], DataType dType,
                                       NDOrder ndOrder, int32_t bitsPP, uint8_t *addr = nullptr) {
    bool success = false;
    NDDims perm = orderNDToPermutation(ndOrder, success);
    int prevStride = 1;
    for (int i = 0; i < perm.ndims(); i++) {
        if (strides[i] < prevStride)
            return false;
...
public:
    TensorRef() = default;
    TensorRef(TensorRefNDData data)
        : TensorRefNDData(data){};
    ...
    bool setByStrides(DataType dataType, const uint32_t newDims[], const uint32_t newStrides[],
                      const uint64_t newStridesBits[], int dimensionality, NDOrder baseLineFullOrder = FULL_ND_NHWC);
