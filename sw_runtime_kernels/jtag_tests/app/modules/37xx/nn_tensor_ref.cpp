...
unsigned int __attribute__((section(".nncmx0.shared.data"), aligned(64))) actShaveData[SHAVE_LIB_DATA_SIZE];
...
bool isNDDataLayoutFit(NDOrder ndOrder, const nn::TensorRefNDData &tensor) {
    uint32_t lDims[MAX_ND_DIMS];
    uint32_t lStrides[MAX_ND_DIMS];
    bool success = false;
    auto tensorPerm = subspace::orderNDToPermutation(tensor.ndOrder, success);
    if (!success || tensor.ndims != tensorPerm.ndims())
        return false;
...
bool TensorRef::setByStrides(DataType dataType, const uint32_t newDims[], const uint32_t newStrides[],
                             const uint64_t newStridesBits[], int dimensionality, NDOrder baseLineOrder) {
    bool success = false;
...
sw_params::MemRefData TensorRef::toMemRefData(sw_params::Location loc, bool doCopy) const {
    ...
    if (loc == sw_params::Location::NN_CMX) {
        ...
        if (bytesToAllocate <= SHAVE_LIB_DATA_SIZE - actShaveDataReserved) {
            ...
            if (doCopy) {
                nn::cache::invalidate(reinterpret_cast<uint8_t*>(this->addr), usedBytes);
                memcpy(reinterpret_cast<uint8_t*>(ret.dataAddr), reinterpret_cast<uint8_t*>(this->addr), usedBytes);
                nn::cache::flush(reinterpret_cast<uint8_t*>(ret.dataAddr), usedBytes);
            }
        } else {
...
void resetReservedAllocation(){
    memset(actShaveData, 0, SHAVE_LIB_DATA_SIZE);
