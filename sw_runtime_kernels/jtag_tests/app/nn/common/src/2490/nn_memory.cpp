...
#ifdef __leon_rt__
#include <ddrMemManager.h>

memory::shared_t memory::shared;

void *nn::memory::shared_alloc(size_t alignment, size_t size)
{
    if (alignment < NN_CACHE_LINE_LENGTH)
        alignment = NN_CACHE_LINE_LENGTH;
    ...
    enum
    {
        DdrMemMgrAlignment = 64
    };

    // DdrMemMgr does not support custom alignment
    if (alignment != DdrMemMgrAlignment)
    {
        nnLog(MVLOG_ERROR, "Cannot honor custom memory alignment of %u bytes when using DdrMemMgr!", alignment);
        return nullptr;
    }

    rtems_status_code sc = DdrMemMgrAlloc(size, &p);
    if (sc != RTEMS_SUCCESSFUL)
        nnLog(MVLOG_ERROR, "Failed to allocate %u bytes of DDR!", size);
    else if (reinterpret_cast<unsigned int>(p) % DdrMemMgrAlignment)
        nnLog(MVLOG_WARN, "DdrMemMgrAlloc does not honor %u byte alignment! p = %p", DdrMemMgrAlloc, p);
...
void nn::memory::shared_free(void *p)
{
    if (p)
    {
