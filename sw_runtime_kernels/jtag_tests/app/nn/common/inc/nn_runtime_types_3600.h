...
namespace nn
{
	namespace inference_runtime
	{
        ...
        enum class OutputTensorDType : uint8_t
        {
             ...
             LOG   = 0x07,
             BIN   = 0x08,
             FP32  = 0x04, //The FP32 is same as I32 since NCE_DPU_ODU_CFG enums are based on dtype size
    ...
    {
        typedef void (*nnKernelEntry)(void *);

        enum class ActWLType : uint8_t
        {
            KERNEL          = 0x00,
            MANAGE          = 0x01,
            PERF_REPORT     = 0x02,
...
#ifdef NN_ENABLE_CONTEXT_DEBUGGING
        enum class ActDebug : uint8_t
        {
            DEBUG_ACK                   = 0x00,
            DEBUG_FIFO_CONFIG_GLOBAL    = 0x01,
            DEBUG_FIFO_WRITE            = 0x02,
            DEBUG_FIFO_WRONG_READ       = 0x03,
            DEBUG_FIFO_WRONG_WRITE      = 0x04,
            DEBUG_VALID_BARRIER         = 0x05,
            DEBUG_INVALID_BARRIER_P     = 0x06,
            DEBUG_INVALID_BARRIER_C     = 0x07,
            DEBUG_CACHE_FLUSH           = 0x08,
            DEBUG_CACHE_INVALIDATE      = 0x09,
            DEBUG_ACK_WAIT              = 0x0A,
            DEBUG_FIFO_CLEAR            = 0x0B,
        };
#endif
        extern "C"
        struct ActWorkload
        {
            ActWLType type_ {ActWLType::UNKNOWN};
            nnKernelEntry kernelEntry_ {nullptr};
            void* kernelArg_ {nullptr};

#ifdef NN_ENABLE_CONTEXT_DEBUGGING
            ActDebug dbg_type_ {ActDebug::DEBUG_ACK};
