...
struct md_parser_t {
  ...
  // Return the number of global arguments a kernel has
  //
  uint32_t get_num_global_args(const md_kernel_descriptor_t *kernel) const {
    uint32_t out = 0;
    for (uint32_t i = 0; i < kernel->arg_count; ++i) {
      const md_kernel_argument_t *arg = get_argument(kernel->arg_index + i);
      out += arg->addr_space == md_addr_space_global;
    }
    return out;
  }
  ...
  // Return true if an expression can be evaluated
  //
  // - expression may not be evaluatable when using ManualDMA.
  bool can_evaluate(const md_expr_t *expr) const {
    return expr &&
           expr->node_first != md_invalid_index &&
           expr->node_count != 0;
  }

  // Evaluate an arbitrary expression
  //
  // - user should call `can_evaluate()` first to check if an expression can be
  //   evaluated.
  // - will return ~0u when it cant be evaluated
  uint32_t evaluate_expr(const md_expr_t *expr,
