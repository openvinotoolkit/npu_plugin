...
//    GroupParams<WD> groupParams;
    nn::shave_lib::CustomCppOpIDs customCppOp;
...
//        loadMemFromFileSimple(const_cast<char*>(m_currentTest->kernelName), 0, m_elfBuffer);
        ...
        m_params = {
            0xFFFFFFFF,
            m_elfBuffer,
            0, // FIXME: temporary without the code from blob   MAX_BINARY_SIZE,    //VcsHooks doesn't provide file length. We can either calculate ELF size based on headers or hardcode it.
            reinterpret_cast<const uint32_t*>(&(m_currentTest->customLayerParams.layerParams)),
            MAX_LOCAL_PARAMS,
            m_currentTest->customLayerParams.customCppOp
