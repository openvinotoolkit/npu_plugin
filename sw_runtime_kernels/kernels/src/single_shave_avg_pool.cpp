...
void single_shave_avg_pool(const struct AvgPoolParams* p) {
    ...
    if(p->input.dimsOrder == 0x4321){ //NCHW
         C = iDims[2];
        iH = iDims[1]; oH = oDims[1];
        iW = iDims[0]; oW = oDims[0];
        elStride  = 1;
        iChStride = iW * iH;
        oChStride = oW * oH;
        iLnStride = iW;
        ...
        iH = iDims[2]; oH = oDims[2];
        iW = iDims[1]; oW = oDims[1];
         C = iDims[0];
        elStride  = C;
        iChStride = 1;
        oChStride = 1;
   ...
   //for all lines
    ...
    {
       if(iy+kernel[0]>iH) break; //no V-pad

       //current line
       ox = 0;
       for(ix=0; ix<iLnStride; ix+=strides[1]*elStride, ox++)
       {
          if(ix+kernel[1]*elStride > iLnStride) break; //no H-pad
          ...
          for(uint32_t ch=0; ch<C; ch++)
          {
            ...
            for(ky=0; ky<kernel[0]; ky++){
             ...
             {
               avg += in[(iy+ky)*iLnStride + ix + (kx*elStride) + ch*iChStride];
             }
            }
            avg = avg * norm;
            out[oy*oLnStride + ch*oChStride + ox] = avg;
          }
       }
   }
}
