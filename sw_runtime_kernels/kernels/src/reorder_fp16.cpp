...
void reorder_fp16(uint32_t lParamsAddr) {
    const ReorderParams* lParams = (const ReorderParams*)lParamsAddr;

    const uint8_t* inData = (const uint8_t*)(lParams->input.dataAddr); // 0x1F000000
    uint8_t* outData = (uint8_t*)(lParams->output.dataAddr); // 0x1F004000
