...
void selu_fp16(uint32_t lParamsAddr) {
    const SeluParams* lParams = (const SeluParams*)lParamsAddr;
    ...
    const SeluParams* layerParams = reinterpret_cast<const SeluParams*>(lParams);
    const half alpha = layerParams->alpha;
    const half lambda = layerParams->lambda;
...
#pragma clang loop unroll_count(8)
    for (i = 0; i < numVectors; i++) {
        ...
        p_act_out_v[i] = lambda * (max + alpha * (vOut - one));
    }
    ...
    for (i = numVectors * VECTOR_SIZE; i < nElements; i++) {
        ...
        p_act_out_s[i] = lambda * (max + alpha * (out - 1.0f));
    }
}
}
}  // namespace shave_lib
