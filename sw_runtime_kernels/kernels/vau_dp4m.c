...
#include <vau_dp4m_args.h>

void vau_dp4m(vau_dp4m_args *args) {
    ...
    uchar16 *p_data2 = (uchar16 *)(local_input + (args->tensor_size / 2));

    int4 *p_out = (int4 *)(args->output + args->local_output_offset);
    const uint32_t elements = args->tensor_size / 16;
    for (uint32_t e = 0; e < elements; e++) {
        *p_out++ = __builtin_shave_vau_dp4m_v16i8_rr(*p_data1++, *p_data2++);
    }
}
