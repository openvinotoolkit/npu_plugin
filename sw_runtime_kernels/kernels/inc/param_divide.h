...
struct __attribute__((packed)) DivideParams {
    struct MemRefData input1;
...
inline struct BaseKernelParams ToBaseKernelParams(struct DivideParams* params) {
    struct BaseKernelParams result;
    result.numInputs = 2;
    result.numOutputs = 1;
#ifdef __cplusplus
    result.inputsOffset = reinterpret_cast<uint8_t*>(&(params->input1)) - reinterpret_cast<uint8_t*>(params);
