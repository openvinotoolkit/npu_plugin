...
${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy -O binary --only-section=.text ${KERNEL_DIR}/prebuild/sigmoid_fp16_3010xx.elf ${KERNEL_DIR}/prebuild/act_shave_bin/sk.sigmoid_fp16.3010xx.text
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.sigmoid_fp16.3010xx.text failed exit $?\n'; exit $?; fi
${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy -O binary --only-section=.arg.data ${KERNEL_DIR}/prebuild/sigmoid_fp16_3010xx.elf ${KERNEL_DIR}/prebuild/act_shave_bin/sk.sigmoid_fp16.3010xx.data
