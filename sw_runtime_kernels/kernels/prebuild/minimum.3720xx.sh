...
rm -f "${KERNEL_DIR}/prebuild/minimum_${cpu}.o" "${KERNEL_DIR}/prebuild/minimum_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.minimum.${cpu}.text" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.minimum.${cpu}.data"
...
if [ $? -ne 0 ]; then echo $'\nLinking of minimum_${cpu}.elf failed exit $?\n'; exit $?; fi
