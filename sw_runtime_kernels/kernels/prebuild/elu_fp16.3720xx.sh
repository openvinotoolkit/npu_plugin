...
if [ $? -ne 0 ]; then echo $'\nLinking of elu_fp16_${cpu}.elf failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.text "${KERNEL_DIR}/prebuild/elu_fp16_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.elu_fp16.${cpu}.text"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.elu_fp16.${cpu}.text failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.arg.data "${KERNEL_DIR}/prebuild/elu_fp16_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.elu_fp16.${cpu}.data"
...
xxd -i sk.elu_fp16.${cpu}.text ../sk.elu_fp16.${cpu}.text.xdat
if [ $? -ne 0 ]; then echo $'\nGenerating includable binary of text segment failed $?\n'; cd -; exit $?; fi
xxd -i sk.elu_fp16.${cpu}.data ../sk.elu_fp16.${cpu}.data.xdat
