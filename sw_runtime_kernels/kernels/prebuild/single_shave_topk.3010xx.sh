...
rm -f "${KERNEL_DIR}/prebuild/single_shave_topk_${cpu}.o" "${KERNEL_DIR}/prebuild/mvSubspaces_${cpu}.o" "${KERNEL_DIR}/prebuild/single_shave_topk_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.single_shave_topk.${cpu}.text" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.single_shave_topk.${cpu}.data"

"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/bin/moviCompile" -mcpu=${cpu} ${optimization} \
 -c "${KERNEL_DIR}/single_shave_topk.cpp" -o "${KERNEL_DIR}/prebuild/single_shave_topk_${cpu}.o" \
...
obj_files="${KERNEL_DIR}/prebuild/single_shave_topk_${cpu}.o"
...
obj_files="${KERNEL_DIR}/prebuild/single_shave_topk_${cpu}.o ${KERNEL_DIR}/prebuild/mvSubspaces_${cpu}.o"
fi
...
-zmax-page-size=16 \
 ...
 --output "${KERNEL_DIR}/prebuild/single_shave_topk_${cpu}.elf"

if [ $? -ne 0 ]; then echo $'\nLinking of single_shave_topk_3010.elf failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.text "${KERNEL_DIR}/prebuild/single_shave_topk_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.single_shave_topk.3010xx.text"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.single_shave_topk.${cpu}.text failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.arg.data "${KERNEL_DIR}/prebuild/single_shave_topk_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.single_shave_topk.3010xx.data"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.single_shave_topk.${cpu}.data failed exit $?\n'; exit $?; fi
...
rm "${KERNEL_DIR}/prebuild/single_shave_topk_${cpu}.o" "${KERNEL_DIR}/prebuild/mvSubspaces_${cpu}.o"
printf "\n \"${KERNEL_DIR}/prebuild/act_shave_bin/sk.single_shave_topk.${cpu}.text\"\n \"${KERNEL_DIR}/prebuild/act_shave_bin/sk.single_shave_topk.${cpu}.data\"\nhave been created successfully\n"
exit $?
