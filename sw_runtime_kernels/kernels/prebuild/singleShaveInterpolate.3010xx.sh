...
if [ $? -ne 0 ]; then echo $'\nLinking of singleShaveInterpolate_3010.elf failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.text "${KERNEL_DIR}/prebuild/singleShaveInterpolate_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.singleShaveInterpolate.3010xx.text"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.singleShaveInterpolate.${cpu}.text failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.arg.data "${KERNEL_DIR}/prebuild/singleShaveInterpolate_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.singleShaveInterpolate.3010xx.data"
...
xxd -i sk.singleShaveInterpolate.3010xx.text ../sk.singleShaveInterpolate.3010xx.text.xdat
#xxd -i sk.singleShaveInterpolate.3010xx.text sk.singleShaveInterpolate.3010xx.text.xdat
if [ $? -ne 0 ]; then echo $'\nGenerating includable binary of text segment failed $?\n'; cd -; exit $?; fi
xxd -i sk.singleShaveInterpolate.3010xx.data ../sk.singleShaveInterpolate.3010xx.data.xdat
