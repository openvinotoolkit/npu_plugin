...
obj_files="${KERNEL_DIR}/prebuild/single_shave_scatterNDUpdate_${cpu}.o"
...
-entry singleShaveScatterNDUpdate \
--gc-sections \
--strip-debug \
--discard-all \
-zmax-page-size=16 \
 ...
 --output "${KERNEL_DIR}/prebuild/singleShaveScatterNDUpdate_${cpu}.elf"

if [ $? -ne 0 ]; then echo $'\nLinking of singleShaveScatterNDUpdate_3010.elf failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.text "${KERNEL_DIR}/prebuild/singleShaveScatterNDUpdate_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.singleShaveScatterNDUpdate.3010xx.text"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.singleShaveScatterNDUpdate.${cpu}.text failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.arg.data "${KERNEL_DIR}/prebuild/singleShaveScatterNDUpdate_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.singleShaveScatterNDUpdate.3010xx.data"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.singleShaveScatterNDUpdate.${cpu}.data failed exit $?\n'; exit $?; fi

cd ${KERNEL_DIR}/prebuild/act_shave_bin
if [ $? -ne 0 ]; then echo $'\nCan not cd to \"$${KERNEL_DIR}/prebuildact_shave_bin\"\n'; exit $?; fi
xxd -i sk.singleShaveScatterNDUpdate.3010xx.text ../sk.singleShaveScatterNDUpdate.3010xx.text.xdat
#xxd -i sk.singleShaveScatterNDUpdate.3010xx.text sk.singleShaveScatterNDUpdate.3010xx.text.xdat
if [ $? -ne 0 ]; then echo $'\nGenerating includable binary of text segment failed $?\n'; cd -; exit $?; fi
xxd -i sk.singleShaveScatterNDUpdate.3010xx.data ../sk.singleShaveScatterNDUpdate.3010xx.data.xdat
