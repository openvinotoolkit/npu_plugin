...
rm -f "${KERNEL_DIR}/prebuild/maximum_${cpu}.o" "${KERNEL_DIR}/prebuild/maximum_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.maximum.3010xx.text" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.maximum.3010xx.data"
...
if [ $? -ne 0 ]; then echo $'\nLinking of maximum_3010.elf failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.text "${KERNEL_DIR}/prebuild/maximum_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.maximum.3010xx.text"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.maximum.${cpu}.text failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.arg.data "${KERNEL_DIR}/prebuild/maximum_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.maximum.3010xx.data"
...
xxd -i sk.maximum.3010xx.text ../sk.maximum.3010xx.text.xdat
if [ $? -ne 0 ]; then echo $'\nGenerating includable binary of text segment failed $?\n'; cd -; exit $?; fi
xxd -i sk.maximum.3010xx.data ../sk.maximum.3010xx.data.xdat
if [ $? -ne 0 ]; then echo $'\nGenerating includable binary of data segment failed $?\n'; cd -; exit $?; fi
cd -

rm "${KERNEL_DIR}/prebuild/maximum_${cpu}.o"
printf "\n \"${KERNEL_DIR}/prebuild/act_shave_bin/sk.maximum.3010xx.text\"\n \"${KERNEL_DIR}/prebuild/act_shave_bin/sk.maximum.3010xx.data\"\nhave been created successfully\n"
exit $?
