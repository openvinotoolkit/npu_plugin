...
rm -f "${KERNEL_DIR}/prebuild/divide_fp16_${cpu}.o" "${KERNEL_DIR}/prebuild/divide_fp16_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.divide_fp16.${cpu}.text" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.divide_fp16.${cpu}.data"

"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/bin/moviCompile" -mcpu=${cpu} ${optimization} \
 -c "${KERNEL_DIR}/divide_fp16.cpp" -o "${KERNEL_DIR}/prebuild/divide_fp16_${cpu}.o" \
...
obj_files="${KERNEL_DIR}/prebuild/divide_fp16_${cpu}.o"
...
-zmax-page-size=16 \
 ...
 --output "${KERNEL_DIR}/prebuild/divide_fp16_${cpu}.elf"

if [ $? -ne 0 ]; then echo $'\nLinking of divide_fp16_3010.elf failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.text "${KERNEL_DIR}/prebuild/divide_fp16_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.divide_fp16.3010xx.text"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.divide_fp16.3010xx.text failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.arg.data "${KERNEL_DIR}/prebuild/divide_fp16_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.divide_fp16.3010xx.data"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.divide_fp16.3010xx.data failed exit $?\n'; exit $?; fi

cd ${KERNEL_DIR}/prebuild/act_shave_bin
if [ $? -ne 0 ]; then echo $'\nCan not cd to \"$${KERNEL_DIR}/prebuild/act_shave_bin\"\n'; exit $?; fi
xxd -i sk.divide_fp16.3010xx.text ../sk.divide_fp16.3010xx.text.xdat
if [ $? -ne 0 ]; then echo $'\nGenerating includable binary of text segment failed $?\n'; cd -; exit $?; fi
xxd -i sk.divide_fp16.3010xx.data ../sk.divide_fp16.3010xx.data.xdat
if [ $? -ne 0 ]; then echo $'\nGenerating includable binary of data segment failed $?\n'; cd -; exit $?; fi
cd -

rm "${KERNEL_DIR}/prebuild/divide_fp16_${cpu}.o"
printf "\n \"${KERNEL_DIR}/prebuild/act_shave_bin/sk.divide_fp16.3010xx.text\"\n \"${KERNEL_DIR}/prebuild/act_shave_bin/sk.divide_fp16.3010xx.data\"\nhave been created successfully\n"
exit $?
