...
rm -f "${KERNEL_DIR}/prebuild/${kernel}_${cpu}.o" "${KERNEL_DIR}/prebuild/${cpu}/${kernel}_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/${cpu}/sk.${kernel}.${cpu}.text" "${KERNEL_DIR}/prebuild/act_shave_bin/${cpu}/sk.${kernel}.${cpu}.data"
...
"${KERNEL_DIR}/prebuild/${kernel}_${cpu}.o" \
 ...
 --output "${KERNEL_DIR}/prebuild/${cpu}/${kernel}_${cpu}.elf"

if [ $? -ne 0 ]; then echo $'\nLinking of ${kernel}_3010.elf failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.text "${KERNEL_DIR}/prebuild/${cpu}/${kernel}_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/${cpu}/sk.${kernel}.${cpu}.text"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.${kernel}.${cpu}.text failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.arg.data "${KERNEL_DIR}/prebuild/${cpu}/${kernel}_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/${cpu}/sk.${kernel}.${cpu}.data"
...
rm "${KERNEL_DIR}/prebuild/${kernel}_${cpu}.o" #"${KERNEL_DIR}/prebuild/${cpu}/${kernel}_${cpu}.elf"
printf "\n \"${KERNEL_DIR}/prebuild/act_shave_bin/${cpu}/sk.${kernel}.${cpu}.text\"\n \"${KERNEL_DIR}/prebuild/act_shave_bin/${cpu}/sk.${kernel}.${cpu}.data\"\nhave been created successfully\n"
exit $?
