...
rm -f ${KERNEL_DIR}/prebuild/elu_fp16_3010xx.o ${KERNEL_DIR}/prebuild/mvSubspaces_3010xx.o ${KERNEL_DIR}/prebuild/dma_shave_nn_3010xx.o ${KERNEL_DIR}/prebuild/elu_fp16_3010xx.elf ${KERNEL_DIR}/prebuild/act_shave_bin/sk.elu.3010xx.text ${KERNEL_DIR}/prebuild/act_shave_bin/sk.elu.3010xx.data

${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/bin/moviCompile -mcpu=3010xx -O3 \
 -c ${KERNEL_DIR}/elu_fp16.cpp -o ${KERNEL_DIR}/prebuild/elu_fp16_3010xx.o \
...
--discard-all \
-zmax-page-size=16 \
${KERNEL_DIR}/prebuild/elu_fp16_3010xx.o ${KERNEL_DIR}/prebuild/mvSubspaces_3010xx.o ${KERNEL_DIR}/prebuild/dma_shave_nn_3010xx.o\
 ...
 --output ${KERNEL_DIR}/prebuild/elu_fp16_3010xx.elf

if [ $? -ne 0 ]; then echo $'\nLinking of elu_fp16_3010.elf failed exit $?\n'; exit $?; fi
${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy -O binary --only-section=.text ${KERNEL_DIR}/prebuild/elu_fp16_3010xx.elf ${KERNEL_DIR}/prebuild/act_shave_bin/sk.elu_fp16.3010xx.text
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.elu_fp16.3010xx.text failed exit $?\n'; exit $?; fi
${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy -O binary --only-section=.arg.data ${KERNEL_DIR}/prebuild/elu_fp16_3010xx.elf ${KERNEL_DIR}/prebuild/act_shave_bin/sk.elu_fp16.3010xx.data
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.elu_fp16.3010xx.data failed exit $?\n'; exit $?; fi
...
rm ${KERNEL_DIR}/prebuild/elu_fp16_3010xx.o ${KERNEL_DIR}/prebuild/mvSubspaces_3010xx.o ${KERNEL_DIR}/prebuild/dma_shave_nn_3010xx.o
