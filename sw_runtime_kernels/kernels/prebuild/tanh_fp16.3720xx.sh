...
rm -f ${KERNEL_DIR}/prebuild/tanh_fp16_${cpu}.o ${KERNEL_DIR}/prebuild/tanh_fp16_${cpu}.elf ${KERNEL_DIR}/prebuild/act_shave_bin/sk.tanh.${cpu}.text ${KERNEL_DIR}/prebuild/act_shave_bin/sk.tanh.${cpu}.data ${KERNEL_DIR}/prebuild/sk.tanh_fp16.${cpu}.text.xdat ${KERNEL_DIR}/prebuild/sk.tanh_fp16.${cpu}.data.xdat
...
if [ $? -ne 0 ]; then echo $'\nLinking of tanh_fp16_${cpu}.elf failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.text "${KERNEL_DIR}/prebuild/tanh_fp16_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.tanh_fp16.${cpu}.text"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.tanh_fp16.${cpu}.text failed exit $?\n'; exit $?; fi
"${MV_TOOLS_DIR}/${MV_TOOLS_VERSION}/linux64/sparc-myriad-rtems-6.3.0/bin/sparc-myriad-rtems-objcopy" -O binary --only-section=.arg.data "${KERNEL_DIR}/prebuild/tanh_fp16_${cpu}.elf" "${KERNEL_DIR}/prebuild/act_shave_bin/sk.tanh_fp16.${cpu}.data"
if [ $? -ne 0 ]; then echo $'\nExtracting of sk.tanh_fp16.${cpu}.data failed exit $?\n'; exit $?; fi

cd ${KERNEL_DIR}/prebuild/act_shave_bin
if [ $? -ne 0 ]; then echo $'\nCan not cd to \"$${KERNEL_DIR}/prebuild/act_shave_bin\"\n'; exit $?; fi
xxd -i sk.tanh_fp16.${cpu}.text ../sk.tanh_fp16.${cpu}.text.xdat
if [ $? -ne 0 ]; then echo $'\nGenerating includable binary of text segment failed $?\n'; cd -; exit $?; fi
xxd -i sk.tanh_fp16.${cpu}.data ../sk.tanh_fp16.${cpu}.data.xdat
if [ $? -ne 0 ]; then echo $'\nGenerating includable binary of data segment failed $?\n'; cd -; exit $?; fi
cd -

rm ${KERNEL_DIR}/prebuild/tanh_fp16_${cpu}.o
printf "\n \"${KERNEL_DIR}/prebuild/act_shave_bin/sk.tanh_fp16.${cpu}.text\"\n \"${KERNEL_DIR}/prebuild/act_shave_bin/sk.tanh_fp16.${cpu}.data\"\nhave been created successfully\n"
exit $?
